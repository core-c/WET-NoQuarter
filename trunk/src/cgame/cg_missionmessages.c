 ...
 *
 * desc			core: remake of the cg_popupmessages.c file
...
void CG_UpdateMMLists( void ) {
	...
	lastItem = NULL;
	while( listItem ) {
		int t = (CG_TimeForPopupMM( listItem->type ) + listItem->time + (cg_pmWaitTime.value*SECONDS_1) + (cg_pmFadeTime.value*SECONDS_1));
		if( cg.time > t ) {
...
void CG_AddMMItem( popupMessageType_t type, const char* message, qhandle_t shader, vec3_t color, int weapon )
{
	...
	{
		mmListItem_t *listItem = CG_FindFreeMMItem();
		char*			end;
		qboolean		hasShader;
...
void CG_DrawMMItems( void )
{
	...
	t = cg_mmWaitingList->time + CG_TimeForPopupMM( cg_mmWaitingList->type ) + (cg_pmFadeTime.value*SECONDS_1);
	if( cg.time > t ) {
	...
	for( i = 0; i < MAX_VISIBLE_ITEMS-1 && listItem; i++, listItem = listItem->next ) {
		...
		t = listItem->time + CG_TimeForPopupMM( listItem->type ) + (cg_pmWaitTime.value*SECONDS_1);
		if( cg.time > t ) {
			colourText[3] = colour[3] = 1 - ((cg.time - t) / (cg_pmFadeTime.value*SECONDS_1));
		}
		else {
