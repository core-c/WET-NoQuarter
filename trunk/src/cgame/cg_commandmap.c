...
// core: mapEntities[] only holds data for players & tanks
static mapEntityData_t mapEntities[MAX_GENTITIES];
static int mapEntityCount = 0;

// core: mapEntities2[] only holds data for non-players & non-tanks
static mapEntityData_t mapEntities2[MAX_GENTITIES];
static int mapEntityCount2 = 0;

static qboolean expanded = qfalse;

/// extern playerInfo_t pi;

static playerPath_t playerPaths[MAX_CLIENTS];

//--- core: show the path a player is moving..
void C_StorePlayerPath( mapEntityData_t* mEnt ) {
	if ( mEnt->type != ME_PLAYER || mEnt->data < 0 || mEnt->data >= MAX_CLIENTS ) {
		return;
	}
	{
		int	clientNum = mEnt->data;
		int	index = playerPaths[clientNum].count % MAX_PLAYERPATH;

		playerPaths[clientNum].count++;

		playerPaths[clientNum].current[index].position[0] = mEnt->x;
		playerPaths[clientNum].current[index].position[1] = mEnt->y;
		playerPaths[clientNum].current[index].position[2] = (cgs.ccLayers)? mEnt->z : 0.0f;
		playerPaths[clientNum].current[index].yaw = mEnt->yaw;
		playerPaths[clientNum].current[index].time = cg.time;
	}
}

//--- core: interpolate between current and previous position & yaw..
void C_InterpolatePlayerPath( mapEntityData_t* mEnt, float x, float y, float w, float h ) {
	if ( mEnt->data < 0 || mEnt->data >= MAX_CLIENTS || mEnt->type != ME_PLAYER || playerPaths[mEnt->data].count <= 1 ) {
		return;
	}
	{
		int			clientNum = mEnt->data;
		int			count = playerPaths[clientNum].count;
		int			index = (count - 1 + MAX_PLAYERPATH) % MAX_PLAYERPATH;
		int			previndex = (count - 2 + MAX_PLAYERPATH) % MAX_PLAYERPATH;
		float		factor;
		vec3_t		transformed, lasttransformed, v, v2;
		int			yaw, lastyaw, deltayaw;

		// get current position..
		transformed[0] = ((playerPaths[clientNum].current[index].position[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0]) * w;
		transformed[1] = ((playerPaths[clientNum].current[index].position[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1]) * h;
		transformed[2] = 0.0f;
		// get current yaw..
		yaw = playerPaths[clientNum].current[index].yaw;
		// get previous position
		lasttransformed[0] = ((playerPaths[clientNum].current[previndex].position[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0]) * w;
		lasttransformed[1] = ((playerPaths[clientNum].current[previndex].position[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1]) * h;
		lasttransformed[2] = 0.0f;
		// get previous yaw
		lastyaw = playerPaths[clientNum].current[previndex].yaw;

		// interpolate..
		factor = (float)(cg.time - playerPaths[clientNum].current[index].time) / (float)(playerPaths[clientNum].current[index].time - playerPaths[clientNum].current[previndex].time);
		factor = (factor<0.0f)? 0.0f : (factor>1.0f)? 1.0f : factor;
		VectorSubtract( transformed, lasttransformed, v );
		deltayaw = yaw - lastyaw;
		VectorMA( lasttransformed, factor, v, v2 );
		mEnt->transformed[0] = v2[0];
		mEnt->transformed[1] = v2[1];
		mEnt->yaw = lastyaw + deltayaw * factor;
	}
}

void C_ResetPlayerPath( int clientNum ) {
	if ( clientNum < 0 || clientNum >= MAX_CLIENTS ) {
		return;
	}
	{
		int i;
		// enforce no interpolation on next draw..
		playerPaths[clientNum].count = 0;
		for ( i=0; i<MAX_PLAYERPATH; ++i ) {
			playerPaths[clientNum].current[i].time = cg.time + 10000000;
		}
	}
}
//---
...
static qboolean CG_ScissorEntIsCulled( mapEntityData_t* mEnt, mapScissor_t *scissor, qboolean doClip ) {
	if( !scissor->circular ) {
		...
		// core: exchanged behavior of cg_drawCompass.integer values 1 & 2..
		if ( cg_drawCompass.integer == 2 && doClip ) {
			i=1.60f;
		}
		if( distSquared > Square( i * 0.5f * (scissor->br[0] - scissor->tl[0]) ) ) {
			return qtrue;
		}
	}
...
static qboolean CG_ScissorPointIsCulled( vec2_t vec, mapScissor_t *scissor, qboolean closeCull ) {
	if( !scissor->circular ) {
		...
		// core: exchanged behavior of cg_drawCompass.integer values 1 & 2..
		if ( cg_drawCompass.integer == 2 && !closeCull ) {
			i=1.60f;
		}
		if( distSquared > Square( i * 0.5f * (scissor->br[0] - scissor->tl[0]) ) ) {
			return qtrue;
...
void CG_TransformAutomapEntity( void ) {
	...
	for (i=0; i<mapEntityCount; i++) {
		...
		mEnt->automapTransformed[0] = (mEnt->x - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] * 100 * cg_automapZoom.value;
...
void CG_ParseMapEntity( int* mapEntityCount, int* offset, team_t team ) {
	...
	// core: Store the player position & yaw..
	C_StorePlayerPath( mEnt );
}

void CG_ParseMapEntity2( int* mapEntityCount2, int* offset, team_t team ) {
	mapEntityData_t* mEnt = &mapEntities2[(*mapEntityCount2)];
	char buffer[16];
...
void CG_ParseMapEntityInfo( int axis_number, int allied_number ) {
	...
	// core: this will be done in CG_DrawMapEntity() anyway..
	//CG_TransformAutomapEntity();
}
/*
=======================
core:
CG_ParseMapEntityInfo2
=======================
*/
void CG_ParseMapEntityInfo2( int axis_number, int allied_number ) {
	...
	for ( i = 0; i < axis_number; ++i ) {
		CG_ParseMapEntity2( &mapEntityCount2, &offset, TEAM_AXIS );
	}

	for ( i = 0; i < allied_number; ++i ) {
		CG_ParseMapEntity2( &mapEntityCount2, &offset, TEAM_ALLIES );
	}
}
...
static void CG_DrawGrid( float x, float y, float w, float h, mapScissor_t *scissor ) {
	...
	if( !gridInitDone ) {
		...
		while ( Max0Min0 / gridStep[0] < 7 ) gridStep[0] -= 50.f;
		while ( Min1Max1 / gridStep[1] < 7 ) gridStep[1] -= 50.f;
...
qboolean CG_DisguiseMapCheck( mapEntityData_t *mEnt ) {
	...
	if ( !(cgs.clientinfo[mEnt->data].skillBits[SK_COVERTOPS] & (1<<8)) )
		return qfalse;

	if ( !( cg_entities[mEnt->data].currentState.powerups & (1 << PW_OPS_DISGUISED) ) )
		return qfalse;

	if ( cgs.clientinfo[mEnt->data].skillBits[SK_COVERTOPS] & (1<<9) )
		return qtrue;
...
void CG_DrawMapEntity( mapEntityData_t *mEnt, float x, float y, float w, float h, int mEntFilter, mapScissor_t *scissor, qboolean interactive, snapshot_t* snap, int icon_size ) {
	...
	case ME_PLAYER:
		...
		} else if( ci->team == snap->ps.persistant[PERS_TEAM] || CG_DisguiseMapCheck(mEnt) ) {
			if( !scissor ) {
				// core: on the commandmap players should be seen even when they are out of our own PVS.. (grab the mEnt positions).
				// The icons are updated on intervals, so they will not move so smoothly, but at least we see all players everywhere now.
				mEnt->transformed[0] = ((mEnt->x - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0]) * w;
				mEnt->transformed[1] = ((mEnt->y - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1]) * h;
				// core: interpolate the player position & yaw.. to make it smooth again.
				C_InterpolatePlayerPath( mEnt, x, y, w, h );
			}
			else {
		...
		if( scissor && CG_ScissorEntIsCulled( mEnt, scissor, qfalse ) ) {
			return;
		}
		...
		if( scissor ) {
			icon_pos[0] = mEnt->automapTransformed[0] - scissor->tl[0] + x - ((float)icon_size * (scissor->zoomFactor / AUTOMAP_ZOOM));
			icon_pos[1] = mEnt->automapTransformed[1] - scissor->tl[1] + y - ((float)icon_size * (scissor->zoomFactor / AUTOMAP_ZOOM));
			string_pos[0] = string_pos[1] = 0;	// Jaybird - dunno if this is right, but cleaning up warnings
		}
		else {
		...
		if( mEnt->type == ME_PLAYER_REVIVE ) {
			...
			CG_DrawPic( icon_pos[0] + 3, icon_pos[1] + 3, icon_extends[0] - 3, icon_extends[1] - 3, cgs.media.medicIcon_cm);
			trap_R_SetColor( NULL );
		}
		else {
	...
	case ME_COMMANDMAP_MARKER:
		...
		// now check to see if the entity is within our clip region
		if( scissor && CG_ScissorEntIsCulled( mEnt, scissor, qtrue ) ) {
			return;
		}
...
#define CONST_ICON_EXPANDED_SIZE 48.f
		...
		// core: hovering over the icon while in limbo..
		if( interactive && !expanded && BG_RectContainsPoint( x + mEnt->transformed[0] - (CONST_ICON_NORMAL_SIZE*0.5f), y + mEnt->transformed[1] - (CONST_ICON_NORMAL_SIZE*0.5f), CONST_ICON_NORMAL_SIZE, CONST_ICON_NORMAL_SIZE, cgDC.cursorx, cgDC.cursory )) {
			...
			if (pic) CG_DrawPic( icon_pos[0] - (icon_extends[0]*0.5f), icon_pos[1] - (icon_extends[1]*0.5f), icon_extends[0], icon_extends[1], pic );
		...
		/*
		// core: highlighting the selected objective icon in limbo.
		// this part will never be executed because ->yaw is never sent..
	...
	case ME_LANDMINE:
		...
		// core: position the icons on radar at the landmine-locations, and not at the spotting-player-location..
		if( scissor ) {
...
void CG_DrawMap( float x, float y, float w, float h, int mEntFilter, mapScissor_t *scissor, qboolean interactive, float alpha, qboolean borderblend ) {
	...
	if ( scissor ) {
		...
		if ( scissor->br[0] >= scissor->tl[0] ) {
			float sc_x, sc_y, sc_w, sc_h, s0, s1, t0, t1;
		...
		CG_DrawPic( x, y, w, h, cgs.media.blackmask ); // jaquboss, etpro icons support
		if( cgs.ccLayers ) {
...
#define SPAWN_SIZEUPTIME SECONDS_1_F
	for ( i = 1; i < cg.spawnCount; ++i ) {
		...
		if( scissor && CG_ScissorPointIsCulled( point, scissor, qfalse ) ) {
			continue;
		}
...
void CG_DrawMortarMarker( int px, int py, int pw, int ph, qboolean draw, mapScissor_t *scissor, int expand ) {
	...
	if( COM_BitCheck( cg.snap->ps.weapons, WP_MORTAR_SET ) || COM_BitCheck( cg.snap->ps.weapons, WP_MORTAR2_SET )) {
		int i;
		static vec4_t colour = { 1.f, 1.f, 1.f, 1.f };
		int fadeTime;

		for( i = 0; i < cgs.maxclients; ++i ) {
			vec3_t point;
			fadeTime = cg.time - (cg.artilleryRequestTime[i] + SECONDS_25);

			if( fadeTime < SECONDS_5) {
				if( fadeTime > 0 ) {
					colour[3] = 1.f - (fadeTime/SECONDS_5_F);
				}
...
mapEntityData_t* CG_ScanForCommandCentreEntity( void ) {
	vec_t	rangeSquared = 1000000;// Square(1000);
	int		ent = 0;
	mapEntityData_t* mEnt = &mapEntities[0];
	int i;
	float rngSquared;
...
qboolean CG_PlayerSelected( void ) {
	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		...
		if( cgs.clientinfo[clientNum].team == snap->ps.persistant[PERS_TEAM] ) {
			if( cgs.clientinfo[clientNum].ccSelected ) {
				return qtrue;
			}
		}
	}
