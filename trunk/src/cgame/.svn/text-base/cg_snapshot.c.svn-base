...
void CG_SetInitialSnapshot( snapshot_t *snap ) {
	...
	// IlDuca - reset the arty time and position to 0
	cg.artyTime = 0;
	cg.artyPosition[0] = 0;
	cg.artyPosition[1] = 0;
	cg.artyPosition[2] = 0;
	...
	// IRATA: update client XP for spectator frames so cg.xp is up to date from the beginning ...
	if( cg.snap->ps.clientNum == cg.clientNum ) { // sanity check should always be the case in CG_SetInitialSnapshot
...
static void CG_TransitionSnapshot( void ) {
	...
	if( cg.snap->ps.clientNum == cg.clientNum ) {
		...
		if( cg.xp < cXP /*cg.snap->ps.stats[STAT_XP]*/ ) {
			cg.xpChangeTime = cg.time;
		}
		cg.xp = cXP;
	}
...
static snapshot_t *CG_ReadNextSnapshot( void ) {
	qboolean		r;
	snapshot_t		*dest;
	static qboolean	snapsvalid = qfalse;
	static int		count = -1;
	...
	while ( cgs.processedSnapshotNum < cg.latestSnapshotNum ) {
		...
		if ( cg.snap && r && dest->serverTime == cg.snap->serverTime ) {
			// IRATA: added - thx to dumbledore !
			// see http://www.lucasforums.com/showthread.php?t=140104&page=3
			// "there seems to be a bug in interpolation on playback of demos recorded from a localhost
			// server (created through the create game ui option).
			// if you watch one of these demos, especially in lower timescale,
			// the motion seems jerky compared to normal remote-server demos. ..."
		...
		if ( r ) {
			...
			// (after a vid_restart/connection interrupted)
			if ( !snapsvalid ) {
				// flow of snaps changing from: invalid to valid..
				snapsvalid = qtrue;
				count = 0;
			} else {
				count++;
				if ( (cgs.csMethod_refresh == 1 && count > 100) ||
					(cgs.csMethod_requesting != 0 && cg.time > cgs.csMethod_requesting + SECONDS_15) )
				{
					// start a request for data on establishing a new connection ||
					// if it took some time, and the server did not fully reply, request again..
					C_CSMethodRequest();
				}
			}
		...
		// (after a vid_restart/connection interrupted)
		if ( snapsvalid ) {
			// flow of snaps changing from: valid to invalid..
			snapsvalid = qfalse;
			count = 0;
		} else {
			count++;
