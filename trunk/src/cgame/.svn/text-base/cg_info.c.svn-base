...
void CG_DrawInformation( qboolean forcerefresh ) {
	...
	nextcall = ms + 250;	// core: shorter interval for a smooth progressbar - IRATA: changed from 50 -> 250 will do it too
	...
	switch(key) {
		...
		case K_SPACE:
			if(!down) {
				...
				cgs.timescaleUpdate = cg.time + SECONDS_1;
			}
			return;
		case K_KP_DOWNARROW:
			if(!down) {
				...
				cgs.timescaleUpdate = cg.time + (int)(SECONDS_1_F * tscale);
			}
			return;
		case K_MWHEELDOWN:
		...
		case K_KP_LEFTARROW:
			if(!down && cg_timescale.value > 0.1f) {
				...
				cgs.timescaleUpdate = cg.time + (int)(SECONDS_1_F * cg_timescale.value - 0.1f);
			}
			return;
		case K_KP_UPARROW:
			if(!down) {
				trap_Cvar_Set("timescale", va("%f", cg_timescale.value + 1.0f));
				cgs.timescaleUpdate = cg.time + (int)(SECONDS_1_F * cg_timescale.value + 1.0f);
			}
			return;
		case K_MWHEELUP:
			if(!cgs.fKeyPressed[K_SHIFT]) {
...
void CG_GameStatsDraw()
{
	if(cgs.gamestats.show == SHOW_OFF) {
		...
		h = 2 + tSpacing + 2 +							// Header
			...
			tSpacing * ((gs->fHasStats) ? JP_EXTRA_STRINGS : 0) +
			...
			2
			+ tSpacing; // jet Pilot, fit extra stats
		...
		CG_DrawRect(GS_X, y, GS_W, h, 1, HUD_Border );					//borderColor);
		CG_FillRect(GS_X, y, GS_W, h,    HUD_Background );				//bgColor);
