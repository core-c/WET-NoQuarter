...
void CG_AdjustFrom640( float *x, float *y, float *w, float *h ) {
	...
	*w *= cgs.screenXScale;
	*h *= cgs.screenYScale;
	// core: adjust x-coordinate and width for possible widescreens..
...
void CG_DrawTinyStringColor( int x, int y, const char *s, vec4_t color ) {
	CG_DrawStringExt( x, y, s, color, qtrue, qfalse, SMALLCHAR_WIDTH -2, SMALLCHAR_HEIGHT -2, 0 );
}

void CG_DrawMicroStringColor( int x, int y, const char *s, vec4_t color ) {
	CG_DrawStringExt( x, y, s, color, qtrue, qfalse, MICROCHAR_WIDTH, MICROCHAR_HEIGHT, 0 );
}
