...
void CG_CheckLocalSounds( playerState_t *ps, playerState_t *ops ) {
	...
	if ( cgs.timelimit > 0 && cgs.gamestate == GS_PLAYING) {
		...
		if ( cgs.timelimit > 5 && !( cg.timelimitWarnings & 1 ) && (msec > (cgs.timelimit - 5) * 60 * SECONDS_1) &&
			( msec < (cgs.timelimit-5)*60*SECONDS_1 + SECONDS_1 ) ) {
		...
		if ( cgs.timelimit > 2 && !( cg.timelimitWarnings & 2 ) && (msec > (cgs.timelimit - 2) * 60 * SECONDS_1) &&
			( msec < (cgs.timelimit-2)*60*SECONDS_1 + SECONDS_1 ) ) {
		...
		if ( !( cg.timelimitWarnings & 4 ) && (msec > (cgs.timelimit) * 60 * SECONDS_1 - SECONDS_30) &&
			(msec < (cgs.timelimit) * 60 * SECONDS_1 - 29000 ) ) {
...
void CG_TransitionPlayerState( playerState_t *ps, playerState_t *ops ) {
	...
	if( ps->eFlags & EF_FIRING ) {
		...
		if( cg.weaponFireTime > SECOND_HALF && cg.weaponFireTime ) {
