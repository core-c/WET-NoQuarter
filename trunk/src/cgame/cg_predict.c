...
static void CG_InterpolatePlayerState( qboolean grabAngles ) {
	...
	snapshot_t		*prev = cg.snap, *next= cg.nextSnap;
...
static void CG_TouchTriggerPrediction( void ) {
	...
	for ( i = 0 ; i < cg_numTriggerEntities ; ++i ) {
		...
		if ( ent->eType == ET_ITEM && !spectator && (cg.predictedPlayerState.groundEntityNum == ENTITYNUM_WORLD) ) {
			// CG_TouchItem( cent ); // reminder for whatever CG_TouchItem only did return
			continue;
		}
...

	{
		...
		if( cg.time > cg.cameraShakeTime ) {
			...
			if ( cg_rtcwShake.integer ){
				...
				vec3_t vec;
				trace_t tr;
				VectorAdd(cg.predictedPlayerState.origin, tv(valx,valy,valz), vec);
				CG_Trace( &tr, cg.predictedPlayerState.origin, mins, maxs, vec, cg.predictedPlayerState.clientNum, MASK_SOLID);
				if ( !(tr.allsolid || tr.startsolid) ) {
					VectorCopy(tr.endpos, cg.predictedPlayerState.origin);
				}
			}
		}
	}
