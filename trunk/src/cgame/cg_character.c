...
static qboolean	CG_ParseGibModels( char *modelPath, bg_character_t *character ) {
	...
	for (i=0; i<MAX_GIB_MODELS; i++) {
		...
		if (!token[0] ) { // IRATA: case empty token
#ifdef _DEBUG
			CG_Printf( "CG_ParseGibModels empty token for character %s and gibModel[%i] - skipping trap_R_RegisterModel()\n", modelPath, i);
#endif
			break;
		}
...
#ifdef _DEBUG
		CG_Printf( "CG_ParseGibModels gibModel[%i] %s registered %s\n", i, token, modelPath);
#endif
	}
...
static qboolean	CG_ParseHudHeadConfig( const char *filename, animation_t* hha ) {
	...
	for ( i = 0 ; i < MAX_HD_ANIMATIONS ; i++ ) {
		...
		hha[i].frameLerp = SECONDS_1 / fps;
		hha[i].initialLerp = SECONDS_1 / fps;
...
static qboolean CG_RegisterAcc( const char *modelName, int *model, const char* skinname, qhandle_t* skin ) {
	...
	*model = trap_R_RegisterModel( modelName );
	...
	if( !*model ) {
		CG_Printf( S_COLOR_YELLOW "WARNING: failed to register accessory '%s'\n", modelName );
		return qfalse;
	}
	...
	*skin = trap_R_RegisterSkin( filename );
	...
	if (!*skin) {
		// uncomment this and you'll see some skin files are missing (...and probably not needed, skin is defined in model ?!)
		// it's a bit unlucky ... MAX_QPATH is reached in some cases
		// CG_Printf( S_COLOR_YELLOW "WARNING: failed to register skin accessory of '%s' referenced from '%s'->'%s'\n", modelName, skinname, filename );
		
		// IRATA: return also qfalse if skin isn't loaded ????
		// return qfalse;
	}
...
qboolean CG_RegisterCharacter( const char *characterFile, bg_character_t *character ) {
	...
	if( *characterDef.undressedCorpseModel ) {
		...
		COM_StripExtensionSafe( characterDef.undressedCorpseModel, buf, sizeof(buf) );
