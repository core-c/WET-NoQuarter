...
//	ps->sprintTime = (ci->sprintTime < 0) ? SECONDS_20 : (int)((float)ci->sprintTime / 100.0f * 20000.0f);
	...
	ps->ammo[WeaponTable[ps->weapon].ammoindex] = ci->ammo;
	ps->ammoclip[WeaponTable[ps->weapon].clipindex] = ci->ammoclip;
...
void CG_mvWindowOverlay(int pID, float b_x, float b_y, float b_w, float b_h, float s, int wState, qboolean fSelected)
{
	...
	if(ci->health > 0) {
		if(cent->currentState.eFlags & EF_ZOOMING) CG_mvZoomBinoc(b_x, b_y, b_w, b_h);
	}
