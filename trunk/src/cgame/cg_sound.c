 ...
 * name:		cg_sound.c
 *
 * desc:		Ridah - parsing and use of sound script files
 *				IRATA - we do register all sounds at start now
 *
 * NQQS:		IRATA - engine hashes 4096 total sounds - see MAX_SFX
 */
...
#define	MAX_SOUND_SCRIPTS		1024 // 4096 IRATA: decreased, vanilla/NQ + map is max 200
static	soundScript_t		soundScripts[MAX_SOUND_SCRIPTS];
int	numSoundScripts=0;

#define	MAX_SOUND_SCRIPT_SOUNDS	4096 // 8192 IRATA: decreased see NQQS -still too big?!
	...
	if( oldestSound ) {
		...
		if( !sound->streaming ) {
			if( !oldestSound->sounds[pos].sfxHandle ) {
				// tmp: remove on 1.3.0 release .. we shouldn't have these calls anymore
				CG_Printf(S_COLOR_YELLOW "Warning: trap_S_RegisterSound() call during game: %s\n", oldestSound->sounds[pos].filename);
		...
		CG_Error( S_COLOR_RED "Unable to locate a valid sound for soundScript: %s\n", sound->name );
	}
...
void CG_AddBufferedSoundScript( soundScript_t* sound ) {
	...
	// CG_Printf("Current sound size add %d\n", cg.numbufferedSoundScripts);
...
void CG_UpdateBufferedSoundScripts( void ) {
	...
	if( cg.time > cg.bufferedSoundScriptEndTime ) {
		...
		cg.numbufferedSoundScripts--;
		// CG_Printf("Current sound size update %d\n", cg.numbufferedSoundScripts);
		if( !cg.numbufferedSoundScripts ) {
			return;
		}
...
static void CG_SoundParseSounds ( char *filename, char *buffer ) {
	...
	// Jaybird - clean compiler warnings
	...
	while ( 1 ) {
		...
		if( !*token ) {
			if( inSound ) {
				CG_Error(S_COLOR_RED "No concluding '}' in sound %s, file %s\n", sound.name, filename );
			}
			CG_Printf("... %s loaded - total script sounds: %i\n", filename , numSoundScripts); //   numSoundScripts is = numSoundScriptSounds
			return;
		}
		...
		if ( !Q_stricmp( token, "{" ) ) {
			...
			if (wantSoundName) {
				CG_Error( S_COLOR_RED "'{' found but not expected, after %s, file %s\n", sound.name, filename );
			}
			inSound = qtrue;
			...
			if( numSoundScripts == MAX_SOUND_SCRIPT_SOUNDS ) {
				CG_Error( S_COLOR_RED "MAX_SOUND_SCRIPT_SOUNDS exceeded.\nReduce number of sound scripts.\n" );
			}
		...
		if ( !Q_stricmp( token, "}" ) ) {
			if (!inSound) {
				CG_Error( S_COLOR_RED "'}' unexpected after sound %s, file %s\n", sound.name, filename );
			}
			...
			if (numSoundScripts == MAX_SOUND_SCRIPTS) {
				CG_Error( S_COLOR_RED "MAX_SOUND_SCRIPTS exceeded.\nReduce number of sound scripts.\n" );
			}
		...
		if ( !inSound ) {
			...
			if (!wantSoundName) {
				CG_Error( S_COLOR_RED "'%s' unexpected after sound %s, file %s\n", token, sound.name, filename );
			}
			memset( &sound, 0, sizeof(sound) );
		...
		if ( !Q_stricmp( token, "sound" ) ) {
			...
			if( scriptSound->numsounds >= MAX_SOUNDSCRIPT_SOUNDS ) {
				CG_Error( S_COLOR_RED "Too many sounds for soundscript MAX_SOUNDSCRIPT_SOUNDS [%i] reached.\n" , scriptSound->numsounds);
			}
			...
			// IRATA: register all script sounds at start - add it to the engine cache gathering
			// we did not do this before 1.3.0 - there is no real 'precaching for these sfXHandles
			trap_S_RegisterSound(scriptSound->sounds[scriptSound->numsounds].filename, qfalse);
...
#define MAX_SOUND_FILES		16 // IRATA: decreased from 128 - common ET uses 3! (2 x team voice, map)
static void CG_SoundLoadSoundFiles( void ) {
	...
	if ( !numSounds ) {
		CG_Printf( S_COLOR_YELLOW "WARNING: no sound files found\n" );
		return;
	}
	...
	for ( i = 0; i < numSounds; ++i ) {
		...
		if( len <= 0 ) {
			if( i != (numSounds - 1) )
				CG_Error( S_COLOR_RED "Couldn't load %s", filename );
			continue;
		}
		if( len > sizeof(bigTextBuffer) ) {
			CG_Error( S_COLOR_RED "%s is too big, make it smaller (max = %i bytes)\n", filename, sizeof(bigTextBuffer) );
...
void CG_SoundInit( void ) {
	if( numSoundScripts ) {
		...
		// CG_Printf("...sound scripts: %i\n", numSoundScriptSounds );
...
/*
 * CG_PlayItemPickupSound
 * IRATA based on bg_misc.c/bg_itemlist gitem_t pickup_sound
 * Note: pickup_sound in bg_itemlist needs a value for EV_GLOBAL_ITEM_PICKUP events ... see first if
 */
void CG_PlayItemPickupSound(gitem_t	*item, int entityNum, qboolean checkSound) {
	qboolean run = qtrue;

	if(checkSound) { // for EV_GLOBAL_ITEM_PICKUP events sound is not always played ...
	...
	if(run) {
		switch (item->giType) {
			case IT_AMMO:
				trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupAmmo );
				break;
			case IT_HEALTH:
				switch (item->classnamehash) {
					case 227098: // "item_health_small"
					case 212921: // "item_health_wall"
						trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupHealthSmall);
						break;
					case 245923: // "item_health_turkey" . hot meal
						trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupHot);
						break;
					case 325011: // "item_health_breadandmeat"
						trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupCold);
					default:
						trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupHealth );
						break;
				}
				break;
			case IT_WEAPON:
				trap_S_StartSound (NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupWeapon );
				break;
			case IT_POWERUP:
				switch (item->classnamehash) {
					case 197306: // "item_flakJacket"
						trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupFlakjacket );
						break;
					case 228322: // "item_helmetshield"
					case 144483: // "item_helmet"
						trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupHelmet );
						break;
					case 323204:// Uniforms,
					case 289724:
					case 384978:
					case 337599:
					case 357664:
					...
					case 364279:
					case 384630:
						trap_S_StartSound(NULL, entityNum, CHAN_AUTO,	cgs.media.itemPickupUniform );
						break;
					default:
						break;
				}
