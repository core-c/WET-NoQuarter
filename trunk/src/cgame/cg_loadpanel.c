...
#define DRAWLOADSTRING
#ifdef DRAWLOADSTRING
...
const char* CG_LoadPanel_GameTypeName( gametype_t gt ) {
	switch( gt ) {
/*	IRATA: There is no game type support for real anymore
...
void CG_DrawConnectScreen( qboolean interactive, qboolean forcerefresh ) {
	...
	if( !DC ) {		// core: they check for DC.. it exists, but .glconfig values are invalid; Use cgs.glconfig values instead.
		return;
	}
	...
	if( !bg_loadscreeninited ) {
		...
		C_PanelButtonsSetup( loadpanelButtons, cgs.wideXoffset );	// core: convert to possible widescreen coordinates..
	...
	if( *buffer ) {
		...
		if( str && *str && (atoi( str ) & 1)) {
			x = 461.0f + cgs.wideXoffset;
			CG_DrawPic( x, y, 16, 16, bg_filter_ff );
		}

		str = Info_ValueForKey( buffer, "sv_punkbuster" );
		if( str && *str && atoi( str ) ) {
			x = 518.0f + cgs.wideXoffset;
			CG_DrawPic( x, y, 16, 16, bg_filter_pb );
		}

		str = Info_ValueForKey( buffer, "g_heavyWeaponRestriction" ); // unused!   // core: not entirely unused, because they can check other servers in the serverlist for g_heavyWeaponRestriction.
		if( str && *str && atoi( str ) != 100 ) {
			x = 546.0f + cgs.wideXoffset;
			CG_DrawPic( x, y, 16, 16, bg_filter_hw );
		}

		str = Info_ValueForKey( buffer, "g_antilag" );
		if( str && *str && atoi( str ) ) {
			x = 575.0f + cgs.wideXoffset;
			CG_DrawPic( x, y, 16, 16, bg_filter_al );
		}

		str = Info_ValueForKey( buffer, "g_balancedteams" );
		if( str && *str && atoi( str ) ) {
			x = 604.0f + cgs.wideXoffset;
...
void CG_LoadPanel_RenderCampaignNameText( panel_button_t* button ) {
	...
	if( cgs.gametype == GT_WOLF_CAMPAIGN ) {
		...
		const char* cs = DC->nameForCampaign();
		if( !cs ) {
			return;
		}
