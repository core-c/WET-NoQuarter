...
void CG_InitPMGraphics( void )
{
	...
	// cs: new shaders for these?
	cgs.media.pmImages[PM_WEAPON]			= trap_R_RegisterShaderNoMip( "sprites/voiceChat" );
	cgs.media.pmImages[PM_DOGTAGS]			= trap_R_RegisterShaderNoMip( "sprites/voiceChat" );
...
void CG_DrawPMItemsBig( void ) {
	...
	trap_R_SetColor( colourText );
	// draw
	CG_DrawPic( Ccg_WideX(640) - 106, y, 48, 48, cg_pmWaitingListBig->shader );
		...
		case PM_MINES:
			...
			if ( !lastSoundTime || (cg.time > lastSoundTime) ) {
				...
				if( cgs.clientinfo[cg.clientNum].team != cent->currentState.effect2Time ) {
					...
					lastSoundTime = cg.time + SECONDS_10;
				}
			}
			break;
		case PM_OBJECTIVE:
