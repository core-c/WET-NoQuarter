...
# Arkox : Get Arch (i386 for intel mac or ppc for É ppc mac !)
ARCH=`arch`
SO="${1%.mp.$ARCH.so}"
echo "Packing $1.mp.$ARCH.so..."
if [ ! -f ${SO}.mp.$ARCH.so ] ; then
  echo "Missing ${SO}.mp.$ARCH.so"
  exit
fi
...
cp ../${SO}.mp.$ARCH.so ${SO}_mac.bundle/Contents/MacOS/${SO}_mac
