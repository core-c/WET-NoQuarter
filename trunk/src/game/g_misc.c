...
void SP_info_null( gentity_t *self ) {
	...
	self->nextthink = level.time + (SERVER_FRAMETIME_DELTA*2);
}
...
void SP_dlight(gentity_t *ent) {
	...
	if(!dlightstarttime)						// sync up all the dlights
		dlightstarttime = level.time + SERVER_FRAMETIME_DELTA;
...
void mg42_fire( gentity_t *other ) {
	...
	// IRATA: Fire_Lead/Fire_Lead_Ext set these flags at start of function
...
void mg42_track (gentity_t *self, gentity_t *other) {
	...
	if (other->active) {
		...
		self->s.apos.trDuration = SERVER_FRAMETIME;
...
void mg42_think (gentity_t *self) {
	...
	if( self->mg42weapHeat >= MAX_MG42_HEAT ) {
		...
		if( self->flameQuotaTime < level.time ) {
			self->flameQuotaTime = level.time + SECONDS_2;
		}
	}
	...
	if( self->mg42weapHeat ) {
		self->mg42weapHeat -= (300.f * SERVER_FRAMETIME_DELTA * 0.001);	//%	-= (300.f * 50 * 0.001);
	...
	if( self->backupWeaponTime ) {
		self->backupWeaponTime -= SERVER_FRAMETIME;
...
void mg42_spawn (gentity_t *ent) {
	...
	if ( !doWar ) {
		...
		gun->nextthink =		level.time + SERVER_FRAMETIME_DELTA;
		gun->timestamp =		level.time + SECONDS_1;
...
void SP_misc_landmine( gentity_t *ent ) {
	...
	ent->nextthink = level.time + SERVER_FRAMETIME_DELTA * 5;
...
void G_InitTempTraceIgnoreEnts( void ) {
	...
	level.numTempTraceIgnoreEnts = 0;
}

void G_ResetTempTraceIgnoreEnts( void ) {
	int i;

	for( i = 0; i < level.numTempTraceIgnoreEnts; i++ ) {
		level.tempTraceIgnoreEnts[i]->r.linked = qtrue;
	}
	level.numTempTraceIgnoreEnts = 0;
}
...
void G_TempTraceIgnoreEntity( gentity_t* ent ) {
	...
	level.tempTraceIgnoreEnts[ level.numTempTraceIgnoreEnts++ ] = ent;
	ent->r.linked = qfalse;
}
