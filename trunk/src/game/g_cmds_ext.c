...
unsigned int cCommands = sizeof(aCommandInfo) / sizeof(aCommandInfo[0]) - 1;
...
qboolean G_commandCheck(gentity_t *ent, char *cmd, qboolean fDoAnytime)
{
	...
	for(i=0; i<cCommands; i++) {
		pCR = &aCommandInfo[i];
		if ( pCR->pCommand != NULL && !Q_stricmp(cmd, pCR->pszCommandName) ) {
			if ( pCR->fAnytime == fDoAnytime ) {
...
void G_commands_cmd(gentity_t *ent, unsigned int dwCommand, qboolean fValue)
{
	...
	CP("print \"^5Available OSP Game-Commands:\n----------------------------\n\"");
	for(i=0; i<rows; i++) {
	...
	if(ent->client->sess.referee) G_refPause_cmd(ent, fPause);
		...
		if(fPause) {
			if(0 == teamInfo[tteam].timeouts) {
				...
				G_spawnPrintf(DP_PAUSEINFO, level.time + SECONDS_15, NULL);
...
void G_players_cmd(gentity_t *ent, unsigned int dwCommand, qboolean fValue)
{
	...
	if(g_gamestate.integer == GS_PLAYING) {
		if(ent) {
			CP("print \"^3 ID^1 : ^3Player                    Nudge  Rate  MaxPkts  Snaps\n\"");
			CP("print \"^1-----------------------------------------------------------^7\n\"");
		}
		else {
	...
	if(ent) {
		CP(va("print \"^3%2d^7 total players\n\"", cnt));
	}
	else {
		G_Printf("%2d total players\n", cnt);
	}
	...
	for(iWeap=WS_KNIFE; iWeap<WS_MAX; iWeap++) {
		...
		for(i=0; i<cClients; i++) {
			...
			if(((doTop)?acc:(float)wBestAcc+0.999) >= ((doTop)?wBestAcc:acc)) {
				Q_strcat(z, sizeof(z), va(" %d %d %d %d %d %d", iWeap, aClients[i],
				...
				break;	// only display the player with the topscore, and not all the rest with this same weapon..
						// After some minutes of playing, the list will get too large anyway, and texts at the bottom get truncated.
			}
		}
	}
...
void G_weaponRankings_cmd(gentity_t *ent, unsigned int dwCommand, qboolean state)
{
	...
	if((iWeap = atoi(z)) == 0 || iWeap < WS_KNIFE || iWeap >= WS_MAX) {
		for(iWeap=WS_FOOTKICK; iWeap>=WS_KNIFE; iWeap--) {
