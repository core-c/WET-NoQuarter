...
void G_BounceMissile( gentity_t *ent, trace_t *trace ) {
	...
	if ( ent->s.eFlags & EF_BOUNCE_HALF ) {
		...
		if( trace->plane.normal[2] > 0.2 && VectorLengthSquared( relativeDelta ) < 1600 ) {
...
//----(SA)	make the world the owner of the dynamite, so the player can shoot it after it stops moving
			switch (ent->s.weapon) {
				...
				case WP_SATCHEL:
				case WP_SMOKE_BOMB:
				case WP_SMOKE_MARKER: //forty - canister kicking
...
//----(SA)	end
			...
			if( ent->s.weapon == WP_M7 || ent->s.weapon == WP_GPG40 ) {
				...
				ent->nextthink = level.time + (750 - (level.time + SECONDS_4 - ent->nextthink));
			}
			return;
		}
	}
...
void M_think (gentity_t *ent) {
	...
	tent->s.time = SECONDS_3;
	tent->s.time2 = SERVER_FRAMETIME_DELTA;
	tent->s.density = 0;
	if (ent->s.density == 1) {
		tent->s.angles2[0] = 16;
	}
...
void G_ExplodeMissile( gentity_t *ent ) {
	vec3_t		dir;
	vec3_t		origin;
	int			etype = ent->s.eType;

	if( ent->s.weapon == WP_SMOKE_MARKER ) {
		// cs: only subtract from the actual smoke marker, and not the bombs that they decided to call smoke markers ...
		if (ent->active) {
			if (!ent->damage) {
			...
			// smoke-markers do no damage (when blocking an airstrike)..
			ent->splashDamage = 0;
		}
	}
	...
	if( ent->accuracy == 1 ) {
		...
		trap_LinkEntity( ent );
		return; // NOTE: In this case cams & ents are not removed ?! accuracy == 3 is flamer only ?
	}
	else {
	...
	if (etype == ET_MISSILE){ //|| etype == ET_BOMB) {
		...
		switch (ent->s.weapon) {
			case WP_SATCHEL:
				// core: remove the landmine from the list..
				RemoveFromEntityArray( &g_satchels, ent );
				break;

			case WP_ARTY:
				// core: remove the arty from the list..
				RemoveFromEntityArray( &g_arties, ent );
				break;

			case WP_PANZERFAUST:
				// core: remove the missilecam..
				if ( jp_insanity.integer & JP_INSANITY_MISSILECAM ) {
					FreeCamera( ent->r.singleClient );
				}
				// core: remove the panzerfaust from the list..
				RemoveFromEntityArray( &g_panzerfausts, ent );
				break;
			...
			case WP_BAZOOKA:
				...
				// core: remove the bazooka from the list..
				RemoveFromEntityArray( &g_bazookas, ent );
				break;

			case WP_GPG40:
			case WP_M7:
				// core: remove the riflenadecam..
			...
			case WP_MORTAR_SET:
			case WP_MORTAR2_SET:
				// core: remove the mortarcam..
				if ( jp_insanity.integer & JP_INSANITY_MORTARCAM ) {
					FreeCamera( ent->r.singleClient );
				}
				// core: remove the mortar from the list..
				RemoveFromEntityArray( &g_mortars, ent );
				break;

			case WP_SMOKE_MARKER:
				if( ent->methodOfDeath == MOD_AIRSTRIKE ) {
					// core: remove the airstrike from the list..
					RemoveFromEntityArray( &g_airstrikes, ent );
				}
				break;
			...
			case WP_LANDMINE:
				{
					...
					// core: remove the landmine from the list..
					RemoveFromEntityArray( &g_landmines, ent );
				}
				break;
			...
			case WP_DYNAMITE:
				if ( ent->etpro_misc_1 & 1 ) { // do some scoring
					...
					vec3_t		mins, maxs;
					int			i,num,touch[MAX_GENTITIES_IN_A_BOX];
					gentity_t	*hit;
					...
					num = trap_EntitiesInBox( mins, maxs, touch, MAX_GENTITIES_IN_A_BOX );
					...
					for( i = 0; i < num; ++i ) {
						...
						if ( ((hit->spawnflags & AXIS_OBJECTIVE) && (ent->s.teamNum == TEAM_ALLIES)) || ((hit->spawnflags & ALLIED_OBJECTIVE) && (ent->s.teamNum == TEAM_AXIS)) ) {
							...
							{
								int msgType = 1; // destroyed
								if ( hit->spawnflags & 8 ) msgType = 0; //damaged (tank)
								G_AnnounceMessageForDestruction( ent->parent, hit, msgType, ent->s.weapon );
							}
							// core: award stats
							if (ent->parent && ent->parent->client) {
								ent->parent->client->sess.missionair++;
							...
							hit->nextthink = level.time + SERVER_FRAMETIME_DELTA;
#ifdef OMNIBOTS
							G_Script_ScriptEvent( hit, SE_DESTROYED, "" );
#endif
						}
					}
				}
		...
		switch (ent->s.weapon) {
			...
			case WP_SATCHEL:
			case WP_BAZOOKA:
			// IRATA: mortar should do the shakey shakey too
			case WP_MORTAR:
			case WP_MORTAR2:
			case WP_MORTAR_SET:
			case WP_MORTAR2_SET:
...
#define	MISSILE_DOWN_XP 5

/*
==============
G_MissileDownXPAward
==============
*/
void G_MissileDownXPAward( gentity_t *attacker, meansOfDeath_t mod ) {
	...
	switch( mod ) {
		...
		case MOD_SHOTGUN:
		case MOD_MP34:
			G_AddSkillPoints( attacker, SK_LIGHT_WEAPONS, MISSILE_DOWN_XP );
			G_DEBUG_ADD_SKILL_POINTS( attacker, SK_LIGHT_WEAPONS, MISSILE_DOWN_XP, "Light weapon missile shot" );
			break;
		...
		case MOD_JOHNSON_SCOPE:
			G_AddSkillPoints( attacker, SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, MISSILE_DOWN_XP );
			G_DEBUG_ADD_SKILL_POINTS( attacker, SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, MISSILE_DOWN_XP, "Scoped weapon missile shot" );
			break;
		...
		case MOD_BROWNING:
		case MOD_MG42:
			G_AddSkillPoints( attacker, SK_HEAVY_WEAPONS, MISSILE_DOWN_XP );
			G_DEBUG_ADD_SKILL_POINTS( attacker, SK_HEAVY_WEAPONS, MISSILE_DOWN_XP, "Heavy weapon missile shot" );
			break;
...
void G_RunMissile( gentity_t *ent ) {
	...
	if( IS_MORTAR_WEAPON_SET(ent->s.weapon) && ent->count2 == 1 ) {
		if( ent->r.currentOrigin[2] > origin[2] && origin[2] - BG_GetGroundHeightAtPoint(origin) < 512 ) {
			...
			VectorMA( origin, 8.0f, impactpos, impactpos );
...
void G_PredictBounceMissile( gentity_t *ent, trajectory_t *pos, trace_t *trace, int time ) {
	...
	dot = -2 * DotProduct( velocity, trace->plane.normal );
	VectorMA( velocity, dot, trace->plane.normal, pos->trDelta );
	...
	if ( ent->s.eFlags & EF_BOUNCE_HALF ) {
		...
		if ( trace->plane.normal[2] > 0.2 && VectorLengthSquared( pos->trDelta ) < 1600 ) {
...
int G_PredictMissile( gentity_t *ent, int duration, vec3_t endPos, qboolean allowBounce ) {
	...
	for (time = level.time + SERVER_FRAMETIME_DELTA; time < level.time + duration; time+=SERVER_FRAMETIME_DELTA) {
		...
		if ( tr.fraction != 1 ) {
			...
			if ( allowBounce && (ent->s.eFlags & ( EF_BOUNCE | EF_BOUNCE_HALF )) ) {
				G_PredictBounceMissile( ent, &pos, &tr, time - SERVER_FRAMETIME_DELTA + (int)((float)SERVER_FRAMETIME_DELTA*tr.fraction) );
				pos.trTime = time;
				continue;
			}
...
void G_BurnTarget( gentity_t *self, gentity_t *body, qboolean directhit ) {
	...
	if ( !body->client ) {
		if ( body->health > 0 )
            G_Damage( body, self->parent, self->parent, vec3_origin, self->r.currentOrigin, BG_Weapons[WP_FLAMETHROWER].damage, 0, MOD_FLAMETHROWER ); // jaquboss, was 2 using full 5 now
		return;
	}
	...
	if (body->flameQuotaTime && body->flameQuota > 0) {
		body->flameQuota -= (int)(((float)(level.time - body->flameQuotaTime)/SECONDS_1) * 2.5f);
	...
	if ( self->count && self->parent->client ) {
		G_addStats( body, self->parent, BG_Weapons[WP_FLAMETHROWER].damage, MOD_FLAMETHROWER );
		self->count = 0;
	}
}
...
void G_FlameDamage( gentity_t *self, gentity_t *ignoreent ) {
	gentity_t	*body;
	int			entityList[MAX_GENTITIES_IN_A_BOX];
	int			e, numListedEntities;
	float		radius = self->speed , boxradius = M_SQRT2 * radius; // radius * sqrt(2) for bounding box enlargement
	vec3_t		mins, maxs;

	mins[0] = self->r.currentOrigin[0] - boxradius;
	maxs[0] = self->r.currentOrigin[0] + boxradius;
	mins[1] = self->r.currentOrigin[1] - boxradius;
	maxs[1] = self->r.currentOrigin[1] + boxradius;
	mins[2] = self->r.currentOrigin[2] - boxradius;
	maxs[2] = self->r.currentOrigin[2] + boxradius;

	numListedEntities = trap_EntitiesInBox( mins, maxs, entityList, MAX_GENTITIES_IN_A_BOX );
...
void G_RunFlamechunk( gentity_t *ent ) {
 ...
 //@SV_FPS
	...
	if ( level.time - ent->timestamp > SERVER_FRAMETIME ) {
        speed -= (50.f/1000.f) * FLAME_FRICTION_PER_SEC * 20.f/((float)sv_fps.integer);
	...
	if ( tr.startsolid ) {
		...
		s = -2*dot;
		VectorMA( vel, s, tr.plane.normal, vel );
		VectorNormalize( vel );
	...
	if ( ent->flameQuotaTime <= level.time ) {
		ent->flameQuotaTime = level.time + SERVER_FRAMETIME_DELTA;
	...
	if ( ent->speed < FLAME_START_MAX_SIZE ) {
		ent->speed += 2000.f/((float)sv_fps.integer);
...
gentity_t *fire_flamechunk (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->classnamehash = FLAMECHUNK_HASH;

	bolt->timestamp = level.time;
	bolt->flameQuotaTime = level.time + SERVER_FRAMETIME;
...
// core: same as G_FadeItems( ent, MOD_LANDMINE)
void G_FadeLandmines(gentity_t* ent) {
	gentity_t*	e;
	int			i;

	for ( i=g_landmines.count-1; i>=0; --i ) {
		e = g_landmines.ent[i];
...
// core: same as G_FadeItems( ent, MOD_SATCHEL)
void G_FadeSatchels(gentity_t* ent) {
	gentity_t*	e;
	int			i;

	for ( i=g_satchels.count-1; i>=0; --i ) {
		e = g_satchels.ent[i];
...
// core: same as G_FadeItems( ent, MOD_MORTAR)
void G_FadeMortars(gentity_t* ent) {
	gentity_t*	e;
	int			i;

	for ( i=g_mortars.count-1; i>=0; --i ) {
		e = g_mortars.ent[i];
...
// core: same as G_FadeItems( ent, MOD_AIRSTRIKE)
void G_FadeAirstrikes(gentity_t* ent) {
	gentity_t*	e;
	int			i;

	for ( i=g_airstrikes.count-1; i>=0; --i ) {
		e = g_airstrikes.ent[i];
...
// core: same as G_FadeItems( ent, MOD_ARTY)
void G_FadeArties(gentity_t* ent) {
	gentity_t*	e;
	int			i;

	for ( i=g_arties.count-1; i>=0; --i ) {
		e = g_arties.ent[i];
...
// core: same as G_FadeItems( ent, MOD_PANZERFAUST)
void G_FadePanzerfausts(gentity_t* ent) {
	gentity_t*	e;
	int			i;

	for ( i=g_panzerfausts.count-1; i>=0; --i ) {
		e = g_panzerfausts.ent[i];
...
// core: same as G_FadeItems( ent, MOD_BAZOOKA)
void G_FadeBazookas(gentity_t* ent) {
	gentity_t*	e;
	int			i;

	for ( i=g_bazookas.count-1; i>=0; --i ) {
		e = g_bazookas.ent[i];
...
qboolean G_SweepForLandmines( vec3_t origin, float radius, int team ) {
	gentity_t *e;
	int i, num;
	int touch[MAX_GENTITIES_IN_A_BOX];
	vec3_t mins, maxs;
...
qboolean G_ExplodeSatchels(gentity_t* ent) {
	...
	for ( i=0; i<g_satchels.count; ++i ) {
		...
		if( VectorLengthSquared(dist) > 4000000) {
			continue;
		}
...
qboolean sEntWillTriggerMine(gentity_t *ent, gentity_t *mine)
{
	...
	if (ent->s.eType == ET_PLAYER && ent->client) {
		...
		if ( (VectorLengthSquared(dist) <= 4096.f) && (fabs(dist[2]) < 45.f) ) {
			return qtrue;
		}
	}
...
void G_LandmineThink( gentity_t *self ) {
	...
	cnt = trap_EntitiesInBox(mins, maxs, entityList, MAX_GENTITIES_IN_A_BOX);
...
void LandminePostThink( gentity_t *self ) {
	...
	if( level.time - self->missionLevel > SECONDS_5 ) {
		self->s.density = 0; // Gordon: time out the covert ops visibile thing, or we could get other clients being able to see mine later, etc
	}
...
gentity_t *fire_grenade (gentity_t *self, vec3_t start, vec3_t dir, int grenadeWPID) {
	...
	if(	self->client && self->client->ps.grenadeTimeLeft) {
		...
		bolt->nextthink = level.time + SECONDS_2_5;
	}

	switch (grenadeWPID) {
		case  WP_DYNAMITE:
			noExplode = qtrue;
			bolt->nextthink = level.time + SECONDS_15;
			bolt->think = DynaSink;
		...
		case WP_LANDMINE:
			...
			bolt->think = DynaSink;
			bolt->timestamp = level.time + 16500;
			// core: add the landmine to the list..
			AddToEntityArray( &g_landmines, bolt, qfalse );
			break;
		case WP_SATCHEL:
			noExplode = qtrue;
			bolt->nextthink = 0;
			...
			// core: add the satchel to the list..
			AddToEntityArray( &g_satchels, bolt, qfalse );
			break;
		case WP_MORTAR_SET:
		case WP_MORTAR2_SET:
			noExplode = qtrue;
			bolt->nextthink = 0;
			// core: add the mortar to the list..
			AddToEntityArray( &g_mortars, bolt, qfalse );
			break;
		default:
			break;
	}
	...
	bolt->damage 		= BG_Weapons[grenadeWPID].damage;
	bolt->splashDamage	= BG_Weapons[grenadeWPID].splashdamage;
    bolt->splashRadius	= BG_Weapons[grenadeWPID].splashdamage_radius;

	switch(grenadeWPID) {
		case WP_GPG40:
			bolt->classname				= "gpg40_grenade";
			bolt->classnamehash			= GPG40_GRENADE_HASH;
			bolt->methodOfDeath			= MOD_GPG40;
			bolt->splashMethodOfDeath	= MOD_GPG40;
			bolt->s.eFlags				= EF_BOUNCE_HALF | EF_BOUNCE;
			bolt->nextthink				= level.time + SECONDS_4;
			/*
			// core: assign a riflenadecam to the grenade..
			if(self->client) {
				if ( jp_insanity.integer & JP_INSANITY_RIFLENADECAM ) {
					gentity_t *cam = InitCamera( self->client->ps.clientNum );
					if ( cam ) {
						cam->activator = bolt;
			...
			*/
			if ( self->client ) // for rifle cam
				bolt->s.clientNum			= self->client->ps.clientNum;
			break;

		case WP_M7:
			bolt->classname				= "m7_grenade";
			bolt->classnamehash			= M7_GRENADE_HASH;
		...
		case WP_SMOKE_BOMB:
			...
			bolt->classnamehash			= SMOKE_BOMB_HASH;
			bolt->s.eFlags				= EF_BOUNCE_HALF | EF_BOUNCE;
		...
		case WP_GRENADE_LAUNCHER:
			...
			bolt->classnamehash			= GRENADE_HASH;
...
// JPW NERVE
		...
		case WP_MORTAR_SET:
			...
			bolt->classnamehash			= MORTAR_GRENADE_HASH;
		...
		case WP_LANDMINE:
			...
			bolt->classnamehash			= LANDMINE_HASH;
			bolt->damage				= 0;
		...
		case WP_SATCHEL:
			...
			bolt->classnamehash			= SATCHEL_CHARGE_HASH;
			bolt->damage				= 0;
		...
		case WP_DYNAMITE:
			...
			bolt->classnamehash			= DYNAMITE_HASH;
			bolt->damage				= 0;
...
void Destabilize_Missile( gentity_t *missile ) {
	...
	missile->s.pos.trTime = level.time - SERVER_FRAMETIME;
...
// Calculate the player's eyes origin, and store this origin in muzzle
	...
	// IRATA: this is actually next frame not 0,1 s
	missile->nextthink = level.time + (SECONDS_1/sv_fps.integer);
}
...
void Homing_Missile_Think( gentity_t *ent ) {
	...
	float		targetlength = 768, tentlength;
	int			i;
	...
	ent->nextthink = level.time + (int)(SECONDS_1/sv_fps.integer);

	if (!target) return;

	VectorMA(forward, 0.05f, targetdir, targetdir);
...
/*
=================
fire_rocket - used for panzer and bazooka

based on *fire_missile (gentity_t *self, vec3_t start, vec3_t dir, int damage, int radius, int speed, int mod, int size) {
=================
*/
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir, int rocketType) {
	gentity_t	*bolt = G_Spawn();
	int			mod = MOD_PANZERFAUST;

	if (rocketType == WP_BAZOOKA) {
		mod = MOD_BAZOOKA;
	}
	...
	bolt->classnamehash			= ROCKET_HASH;
	...
	if ((jp_insanity.integer & JP_INSANITY_GUIDED) && self->client) {
		...
		if (g_missileSpeed.integer > 1) {	// core: unwanted behavior if speed is 0
			VectorScale( dir, g_missileSpeed.integer, bolt->s.pos.trDelta );	// rocket speed
		}
		else {
	...
	bolt->r.ownerNum			= self->s.number;
	bolt->parent 				= self;
	bolt->damage 				= BG_Weapons[rocketType].damage; 	// JPW NERVE
	bolt->splashDamage			= BG_Weapons[rocketType].splashdamage;
	bolt->splashRadius			= BG_Weapons[rocketType].splashdamage_radius;
	...
	if ( self && self->client && !(jp_insanity.integer & JP_INSANITY_HOMING) && !(jp_insanity.integer & JP_INSANITY_GUIDED)) {
		...
		switch (g_missileGravity.integer) {
			case 0:
	...
	if (g_missileHealth.integer > 0) {
        ...
        VectorSet(bolt->r.mins, -10, -3, 0); // IRATA: why we don't use et default mins ? - 3,3,0
        VectorCopy(bolt->r.mins, bolt->r.absmin);
        VectorSet(bolt->r.maxs, 10, 3, 6); // 3,3,6
        VectorCopy(bolt->r.maxs, bolt->r.absmax);
	}
	...
	if(self->client) {
		bolt->s.teamNum = self->client->sess.sessionTeam;
		// core: assign a missilecam to the rocket..
		if ( jp_insanity.integer & JP_INSANITY_MISSILECAM ) {
			gentity_t *cam = InitCamera( self->client->ps.clientNum );
			if ( cam ) cam->activator = bolt;
		}
	}

	// core: add the rocket to the list..
	switch (mod) {
		case MOD_BAZOOKA:
			AddToEntityArray( &g_bazookas, bolt, qfalse );
			break;
		case MOD_PANZERFAUST:
			AddToEntityArray( &g_panzerfausts, bolt, qfalse );
			break;
		default:
			break;
	}
...
gentity_t *fire_flamebarrel (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->nextthink 			= level.time + SECONDS_3;
	bolt->think					= G_ExplodeMissile;
...
gentity_t *fire_mortar(gentity_t *self, vec3_t start, vec3_t dir) {
	...
	if(self->spawnflags) {
		gentity_t	*tent = G_TempEntity (self->s.pos.trBase, EV_MORTAREFX);
	...
	bolt->r.ownerNum			= self->s.number;
	bolt->parent 				= self;
	bolt->damage 				= BG_Weapons[WP_MORTAR_SET].damage;
	bolt->splashDamage 			= BG_Weapons[WP_MORTAR_SET].splashdamage;
	bolt->splashRadius 			= BG_Weapons[WP_MORTAR_SET].splashdamage_radius;
