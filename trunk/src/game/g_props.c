...
void touch_props_box_32(gentity_t *self, gentity_t *other, trace_t *trace) {
	if ( other->r.currentOrigin[2] > (self->r.currentOrigin[2]+10+15)) {
		...
		moveit (self, vectoyaw(v), (20 * ratio * SERVER_FRAMETIME_DELTA)*.001);
	}
}
...
void SP_props_box_32(gentity_t *self) {
	// self->s.modelindex = GAMEMODEL_WORLD_BOX_32; // not in path
...
void SP_props_box_48(gentity_t *self) {
	// self->s.modelindex = GAMEMODEL_WORLD_BOX_48; // not in path/registered
	self->s.modelindex = NULL_MODEL;
...
void SP_props_box_64(gentity_t *self) {
	// self->s.modelindex = GAMEMODEL_WORLD_BOX_64; // not in path/registered
	self->s.modelindex = NULL_MODEL;
...
void prop_smoke(gentity_t *ent) {
	...
	Psmoke->nextthink = level.time + SERVER_FRAMETIME_DELTA;
}
...
void PGUNsparks_use(gentity_t *ent, gentity_t *self, gentity_t *activator ) {
	gentity_t *tent = G_TempEntity (ent->r.currentOrigin, EV_GUNSPARKS);
...
//(SA) MOVE TO CLIENT!
	...
	ent->nextthink = level.time + SERVER_FRAMETIME_DELTA + ent->delay + (rand()%600);
}
...
void dust_angles_think (gentity_t *ent) {
	gentity_t *target = G_FindByTargetname( NULL, ent->target);
...
void InitProp( gentity_t *ent ) {
	...
	if (!Q_stricmp (ent->classname, "props_bench")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_BENCH; // not in path/registered
		ent->s.modelindex2 = NULL_MODEL;
	}
	else if (!Q_stricmp (ent->classname, "props_radio")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_RADIO; // not in path/registered
		ent->s.modelindex2 = NULL_MODEL;
	}
	else if (!Q_stricmp (ent->classname, "props_locker_tall")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_LOCKER;
		ent->s.modelindex2 = NULL_MODEL;
	}
	else if (!Q_stricmp (ent->classname, "props_flippy_table")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_WOODFLIP;
		ent->s.modelindex2 = NULL_MODEL;
	}
	else if (!Q_stricmp (ent->classname, "props_crate_32x64")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_CRATE_32X64; // not in path/registered
		ent->s.modelindex2 = NULL_MODEL;
	}
	else if (!Q_stricmp (ent->classname, "props_58x112tablew")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_TABLE_56X112;
		ent->s.modelindex2 = NULL_MODEL;
	}
	else if (!Q_stricmp (ent->classname, "props_castlebed")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_CASTLEBED; // not in path/registered
		ent->s.modelindex2 = NULL_MODEL;
	}
	else if (!Q_stricmp (ent->classname, "props_radioSEVEN")) {
		// ent->s.modelindex2 = GAMEMODEL_WORLD_RADIOS; // not in path/registered
		ent->s.modelindex2 = NULL_MODEL;
	}
...
void Just_Got_Thrown (gentity_t *self)
{
	...
	if (self->s.groundEntityNum == -1) {
		...
		// IRATA: sound not in path so removed for now
...
void Props_Activated (gentity_t *self)
{
	...
	if (owner->active == qfalse) {
		...
		prop->nextthink = level.time + SERVER_FRAMETIME_DELTA;
...
qboolean Prop_Touch (gentity_t *self, gentity_t *other, vec3_t v)
{
	if (!other->client) {
		...
		self->s.pos.trDuration = level.time + SERVER_FRAMETIME_DELTA;
		self->s.pos.trTime = level.time;
...
void Props_Chair_Touch (gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	if (level.time > self->random && has_moved) {
		...
		self->random = level.time + SECONDS_1 + (rand()%200);
	}
...
void Props_Chair_Animate (gentity_t *ent)
{
	...
	if (!Q_stricmp (ent->classname, "props_chair")) {
		if (ent->s.frame >= 27) {
			...
			G_UseTargets (ent, NULL);
			ent->think = G_FreeEntity;
			ent->nextthink = level.time + SECONDS_2;
			ent->s.time = level.time;
			ent->s.time2 = level.time + SECONDS_2;
			return;
		}
		else {
			ent->nextthink = level.time + SERVER_FRAMETIME;
	...
	if (ent->enemy) {
		...
		moveit (ent, vectoyaw(v), (ent->delay * ratio * SERVER_FRAMETIME_DELTA)*.001);
	}
}
...
void Spawn_Shard (gentity_t *ent, gentity_t *inflictor, int quantity, int type)
{
	...
	sfx->nextthink = level.time + SECONDS_1;
...
{
	// core: note, props only use 3 types of sounds from the obsolete shard_t enum
...
void Props_Chair_Die (gentity_t *ent, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
	...
	ent->health = 0; // changed for omni-bot 0.7
	ent->delay = damage;
	ent->takedamage = qfalse;
//	ent->enemy = inflictor;
...
void SP_Props_Chair (gentity_t *ent)
{
	...
	ent->s.modelindex = GAMEMODEL_WORLD_CHAIR_OFFICE;
	...
	// core: see comment in: void SP_crate_32 (gentity_t *self)
...
void SP_Props_ChairHiback (gentity_t *ent)
{
	...
	ent->s.modelindex = GAMEMODEL_WORLD_CHAIR_HIBACK;
...
void SP_Props_ChairSide (gentity_t *ent)
{
	...
	ent->s.modelindex = GAMEMODEL_WORLD_CHAIR_SIDECHAIR;
...
void SP_Props_ChairChat(gentity_t *ent) {
	// ent->s.modelindex = GAMEMODEL_WORLD_CHAIR_CHAT; // not in path
...
void SP_Props_ChairChatArm(gentity_t *ent) {
	// ent->s.modelindex = GAMEMODEL_WORLD_CHAIR_CHATARM; // not in path
	SP_Props_ChateauChair(ent);
}
...
void Use_Props_Shard_Generator (gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	int	type = ent->count;
	gentity_t *inflictor = G_FindByTargetname( NULL, ent->target );
...
void SP_Props_Desklamp (gentity_t *ent) {
	...
	// ent->s.modelindex = GAMEMODEL_WORLD_DESKLAMP; // not in path registered/registered
	ent->s.modelindex = NULL_MODEL;
...
void Props_Barrel_Animate (gentity_t *ent) {
	if (ent->s.frame == 14) {
		...
		//	G_UseTargets (ent, NULL);
			ent->think = G_FreeEntity;
			ent->nextthink = level.time + SECONDS_25;
			return;
		}
		else {
		//	G_UseTargets (ent, NULL);
...
void barrel_smoke (gentity_t *ent) {
	...
	tent->s.time = SECONDS_4;
	tent->s.time2 = SECONDS_1;
	tent->s.density = 0;
	tent->s.angles2[0] = 8;
	tent->s.angles2[1] = 64;
	tent->s.angles2[2] = 50;
...
void OilParticles_think (gentity_t *ent) {
	...
	if (owner && owner->takedamage && ent->count2 > level.time - SECONDS_5) {
		gentity_t *tent;
		ent->nextthink = level.time + SERVER_FRAMETIME;
...
return;
	...
	OilLeak->nextthink = level.time + SERVER_FRAMETIME_DELTA;
...
void OilSlick_remove (gentity_t *ent) {
	...
	remove->nextthink = level.time + SECONDS_1;
...
void SP_Props_Flamebarrel (gentity_t *ent) {
	...
	if (ent->spawnflags & 4) {
		// ent->s.modelindex = GAMEMODEL_WORLD_BARREL_C; // not registered in path
		ent->s.modelindex = NULL_MODEL;
	}
	else if (ent->spawnflags & 1){
		// ent->s.modelindex = GAMEMODEL_WORLD_BARREL_D; // not registered in path
		ent->s.modelindex = NULL_MODEL;
	}
	else {
		// ent->s.modelindex = GAMEMODEL_WORLD_BARREL_B; // not registered in path
		ent->s.modelindex = NULL_MODEL;
	}
...
void SP_crate_64 (gentity_t *self) {
	// self->s.modelindex = GAMEMODEL_WORLD_CRATE_64; model not in path
	self->s.modelindex = NULL_MODEL;
...
void SP_crate_32 (gentity_t *self) {
	// self->s.modelindex = GAMEMODEL_WORLD_CRATE_32; // not in path/registered
	self->s.modelindex = NULL_MODEL;
	...
	// core: sound-types for glass & wood are reversed in Radiant (using the old shard_t enum)..
	// Default sound should be 'wood', but for crates it's glass :S
	// I just overrule the 'type' property, and make it always 'wood'..
	// It makes no sense to do: if(!self->count) self->count = FXTYPE_WOOD, while FXTYPE_WOOD==0
	self->count = FXTYPE_WOOD;
...
void props_snowGenerator_think (gentity_t *ent) {
	if (!(ent->spawnflags & 1)) {
		...
		int			i;
		vec3_t		point;
		float high = ent->r.maxs[2] - ent->r.mins[2];
		float wide = ent->r.maxs[1] - ent->r.mins[1];
		float deep = ent->r.maxs[0] - ent->r.mins[0];
		...
		for (i=0; i<ent->count; ++i) {
			...
			tent->s.time = SECONDS_2; // life time
			tent->s.time2 = SECONDS_1; // alpha fade start
		}
...
void props_statue_animate (gentity_t *ent) {
	...
	if (((ent->delay * SECONDS_1) + ent->timestamp) > level.time) {
		ent->count = 0;
	}
	else if (ent->count == 5) {
		takeashot = qtrue;
...
void props_locker_use(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	ent->nextthink = level.time + SECONDS_1;	// rattle a sec
}
...
void props_locker_spawn_item(gentity_t *ent) {
	...
	if(!item) {	// empty
		...
		gentity_t *drop = LaunchItem( item, ent->r.currentOrigin, tv(0, 0, 20), ent->s.number, FL_DROPPED_ITEM );
...
//	trap_SetBrushModel (self, self->model);
	...
	// self->s.modelindex = GAMEMODEL_WORLD_FOOTLOCKER; // not in path/registered
	self->s.modelindex = NULL_MODEL;
	...
	if (G_SpawnString ("type", "wood", &type))  {
		...
		else if (!Q_stricmp(type,"fabric"))	self->key = FXTYPE_FABRIC;
	}
	else {
		self->key = 0;
	}
...
//			VectorCopy (angles, ent->r.currentAngles);	// (SA) wasn't working
	...
	if ((ent->timestamp + ent->duration) > level.time) {
		...
		ent->nextthink = ent->timestamp + SERVER_FRAMETIME;
	}
}
...
void props_flamethrower_use(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	if (ent->spawnflags & 2) {
 	...
 	if (ent->random) {
 		ent->timestamp = level.time + ( rand()%( (int) ent->random * SECONDS_1) );
	}
	else {
		ent->timestamp = level.time;
	}
