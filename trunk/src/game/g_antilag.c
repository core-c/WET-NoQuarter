...
void G_AdjustClientPositions( gentity_t* ent, int time, qboolean forward ) {
	...
	for( i = 0; i < level.numConnectedClients; i++, list++ ) {
		...
		if(forward) {
			...
			/*
			// core: i want the server to reconstruct the situation as how i saw it on my screen,
			// not how the server saw it at a specific time.
			// And i do not want to have to shoot behind someone to hit him.
			// The next line of code tries to improve the reconstruction..
			time -= (list->client->ps.ping >> 1);	// subtract half the opponent's ping
			*/
