...
static const char *gameNames[] = {
	"Single Player", 		// GT_SINGLE_PLAYER unused in g_vote.c
	"Cooperative",   		// GT_COOP unused in g_vote.c
	"Objective",	 		// GT_WOLF
	"Stopwatch",	 		// GT_WOLF_STOPWATCH
	"Campaign",		 		// GT_WOLF_CAMPAIGN
	"Last Man Standing", 	// GT_WOLF_LMS
...
static const vote_reference_t aVoteInfo[] = {
	...
	{ 0x1ff, "shuffleteamsxp", G_ShuffleTeams_v,"Shuffle Teams by XP (Restart)"," ^7\n  Randomly place players on each team, based on XP" },
...
void G_voteHelp(gentity_t *ent, qboolean fShowVote) {
	...
	if(fShowVote) CP("print \"Valid ^3callvote^7 commands are:\n^3----------------------------\n\"");
...
int G_Mute_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	...
	// Vote action (vote has passed)
		...
		if( level.clients[pid].sess.referee != RL_RCON ) {
			...
			ClientConfigStringChanged( &g_entities[pid] );
		}
		else {
		...
		if( g_gametype.integer == GT_WOLF_CAMPAIGN ) {
			...
			if( campaign->current + 1 < campaign->mapCount ) {
				...
				if (g_gamestate.integer == GS_PLAYING // don't do in intermission (check warmup/warmup-countdown
					...
					// IRATA: There is a flag for let the users decide
...
int G_Referee_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	...
	if(arg) {
		...
		if(g_entities[pid].r.svFlags & SVF_BOT ) {
			G_refPrintf(ent, "[lof]%s [lon] is a bot!", level.clients[pid].pers.netname);
			return(G_INVALID);
		}
...
int G_FriendlyFire_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	// Vote request (vote is being initiated)
	if(arg) {
		return(G_voteProcessOnOff(ent, arg, arg2, fRefereeCmd, G_FRIENDLYFIRE, vote_allow_friendlyfire.integer, dwVoteIndex));
	}
	// Vote action (vote has passed)
	else {
		AP(va("cpm \"^3Friendly Fire is: ^5%s\n\"", (G_FRIENDLYFIRE ? "DISABLED" : "ENABLED")));
		trap_SendConsoleCommand(EXEC_APPEND, va( "%sflag g_friendlyfire 1\n", (G_FRIENDLYFIRE ? "clear" : "set")));
	}
