...
pre parsing command params
====================
*/

//	PreParse_ScriptAction_Accum()
//
//	params:
//		data_int[0]		=	accum index			( for "accum 0 inc 1", the accum index is: "0" )
//		data_int[1]		=	command hash value	( for "accum 1 inc 1", the command is: "inc" )
//		data_int[2]		=	constant value <c>	( for "accum 2 inc 3", the constant value is: "3" )
//							Used in commands:	inc <c>
//												abort_if_less_than <c>
//												abort_if_greater_than <c>
//												abort_if_not_equal <c>
//												abort_if_equal <c>
//											*	bitset <c>
//											*	bitreset <c>
//											*	abort_if_bitset <c>
//											*	abort_if_not_bitset <c>
//												set <c>
//												random <c>
//												trigger_if_equal <c> <s> <t>
//												wait_while_equal <c>
//												dec <c>
//												mul <c>
//												div <c>
//						=	accum operand <a>
//												inc_accum <a>
//												abort_if_less_than_accum <a>
//												abort_if_greater_than_accum <a>
//												abort_if_not_equal_accum <a>
//												abort_if_equal_accum <a>
//												set_accum <a>
//												random_accum <a>
//												dec_accum <a>
//												mul_accum <a>
//												div_accum <a>
//						=	globalaccum operand <g>
//												inc_globalaccum <g>
//												abort_if_less_than_globalaccum <g>
//												abort_if_greater_than_globalaccum <g>
//												abort_if_not_equal_globalaccum <g>
//												abort_if_equal_globalaccum <g>
//												set_globalaccum <g>
//												random_globalaccum <g>
//												dec_globalaccum <g>
//												mul_globalaccum <g>
//												div_globalaccum <g>
//		data_int[3]		=	scriptname <s> hash
//												trigger_if_equal <c> <s> <t>
//		data_char[0]	=	scriptname <s>
//												trigger_if_equal <c> <s> <t>
//						=	targetname <t>
//												set_to_dynamitecount <t>
//		data_char[1]	=	targetname <t>
//												trigger_if_equal <c> <s> <t>
//
...
qboolean PreParse_ScriptAction_Accum( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	int		bufferIndex2;
	int		len;
	char	*Error_OutsideRange = va("%sG_ScriptAction_Accum(): accum buffer is outside range (0 - %i)\n", S_COLOR_RED, G_MAX_SCRIPT_ACCUM_BUFFERS-1);
	char	*Error_RequiresParam = NULL;

	memset( preparsed_params, 0, sizeof(g_script_preparsed_params_t) );
	...
	lastTokenHash = BG_StringHashValue_Lwr( lastToken );
	preparsed_params->data_int[1] = lastTokenHash;

	token = COM_ParseExt( &pString, qfalse );

	Error_RequiresParam = va("%sG_ScriptAction_Accum(): accum %s requires a parameter\n", S_COLOR_RED, lastToken);
	...
	switch( lastTokenHash ) {
		...
		case TRIGGER_IF_EQUAL_HASH:
			...
			token = COM_ParseExt( &pString, qfalse );
			if (!*token) {
				G_Error(S_COLOR_RED "G_ScriptAction_Accum(): accum must have a scriptname (and a trigger) identifier: %s\n", params );
			}

			len = strlen( token ) + 1;
			preparsed_params->data_char[0] = G_Alloc( len + 1 );
			Q_strncpyz( preparsed_params->data_char[0], token, len );
			preparsed_params->data_int[3] = BG_StringHashValue( token );
			...
			len = strlen( token ) + 1;
			preparsed_params->data_char[1] = G_Alloc( len + 1 );
			Q_strncpyz( preparsed_params->data_char[1], token, len );
			break;
		...
		case SET_TO_DYNAMITECOUNT_HASH:
			if( !*token ) {
				G_Error(S_COLOR_RED "G_ScriptAction_Accum(): accum %s requires a targetname parameter\n", lastToken );
			}
			len = strlen( token ) + 1;
			preparsed_params->data_char[0] = G_Alloc( len + 1 );
			Q_strncpyz( preparsed_params->data_char[0], token, len );
		...
		case SET_TO_PLAYINGCLIENTS_HASH:
			break;
...
//	PreParse_ScriptAction_GlobalAccum()
//
//	params:
//		See comment for PreParse_ScriptAction_Accum()..
//
qboolean PreParse_ScriptAction_GlobalAccum( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	int		bufferIndex2;
	int		len;
	char	*Error_OutsideRange = va("%sG_ScriptAction_GlobalAccum(): globalaccum buffer is outside range (0 - %i)\n", S_COLOR_RED, G_MAX_SCRIPT_ACCUM_BUFFERS-1);
	char	*Error_RequiresParam = NULL;
	...
	Error_RequiresParam = va("%sG_ScriptAction_GlobalAccum(): globalaccum %s requires a parameter\n", S_COLOR_RED, lastToken);
	...
	switch( lastTokenHash ) {
		...
		case TRIGGER_IF_EQUAL_HASH:
			...
			token = COM_ParseExt( &pString, qfalse );
			if (!*token) {
				G_Error(S_COLOR_RED "G_ScriptAction_GlobalAccum(): globalaccum must have a scriptname (and a trigger) identifier: %s\n", params );
			}
		...
		default:
			G_Error(S_COLOR_RED "G_ScriptAction_GlobalAccum(): globalaccum %s: unknown command\n", params );
			break;
	}
...
//	PreParse_ScriptAction_Trigger()
//
//	params:
//		data_int[0]		=	scriptname <s> lowercase hash
//		data_int[1]		=	scriptname <s> hash
//		data_char[0]	=	scriptname <s>
//		data_char[1]	=	targetname <t>
//
qboolean PreParse_ScriptAction_Trigger( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	if (!*name) {
		G_Error(S_COLOR_RED "G_ScriptAction_Trigger(): trigger must have a scriptname (and a trigger identifier): %s\n", params );
	}
	...
	len = strlen( name ) + 1;
	preparsed_params->data_char[0] = G_Alloc( len + 1 );
	Q_strncpyz( preparsed_params->data_char[0], name, len );
	preparsed_params->data_int[0] = BG_StringHashValue_Lwr( name );
	preparsed_params->data_int[1] = BG_StringHashValue( name );

	len = strlen( trigger ) + 1;
	preparsed_params->data_char[1] = G_Alloc( len + 1 );
	Q_strncpyz( preparsed_params->data_char[1], trigger, len );

	return qtrue;
}


//	PreParse_ScriptAction_SetState()
//
//		syntax: setstate <t> <e>
//
//	params:
//		data_int[0]		=	targetname <t> hash
//		data_int[1]		=	entity state <e>
//		data_char[0]	=	targetname <t>
//
qboolean PreParse_ScriptAction_SetState( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	Q_strncpyz( name, token, sizeof(name) );
	if( !*name ) {
		G_Error(S_COLOR_RED "G_ScriptAction_SetState(): setstate must have a targetname (and a state)\n" );
	}
	...
	preparsed_params->data_char[0] = G_Alloc( len + 1 );
	Q_strncpyz( preparsed_params->data_char[0], name, len );
	preparsed_params->data_int[0] = BG_StringHashValue( name );

	preparsed_params->data_int[1] = (int)entState;

	return qtrue;
}


//	PreParse_ScriptAction_ToggleSpeaker()
//
//		syntax: togglespeaker <t>
//
//	params:
//		data_int[0]		=	targetname <t> hash
//		//data_char[0]	=	targetname <t>
...
qboolean PreParse_ScriptAction_ToggleSpeaker( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	preparsed_params->data_int[0] = BG_StringHashValue( params );

	/*
	// core: storing the targetname of the speaker is not necessary,
	// because we only search by the hash-value..
	{
		int len;
		len = strlen( params ) + 1;
		preparsed_params->data_char[0] = G_Alloc( len + 1 );
		Q_strncpyz( preparsed_params->data_char[0], params, len );
	}
	*/
...
//	PreParse_ScriptAction_DisableSpeaker()
//
//		syntax: disablespeaker <t>
//
//	params:
//		data_int[0]		=	targetname <t> hash
//		//data_char[0]	=	targetname <t>
...
//	PreParse_ScriptAction_EnableSpeaker()
//
//		syntax: enablespeaker <t>
//
//	params:
//		data_int[0]		=	targetname <t> hash
//		//data_char[0]	=	targetname <t>
...
//	PreParse_ScriptAction_Wait()
//
//		syntax:	wait <c>
//				wait random <min> <max>
//
//	params:
//		data_int[0]		=	wait type		(value is 0 when using <c>, value is 1when using <min><max>)
//		data_int[1]		=	<c> or <min>
//		data_int[2]		=	<max>
//
qboolean PreParse_ScriptAction_Wait( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	if(!Q_stricmp( token, "random" )) {
		preparsed_params->data_int[0] = 1;				// random duration

		token = COM_ParseExt( &pString, qfalse );
		if (!*token) {
			G_Error(S_COLOR_RED "G_ScriptAction_Wait(): wait random must have a min duration\n" );
		}
		preparsed_params->data_int[1] = atoi(token);	// min

		token = COM_ParseExt( &pString, qfalse );
		if (!*token) {
			G_Error(S_COLOR_RED "G_ScriptAction_Wait(): wait random must have a max duration\n" );
		}
		preparsed_params->data_int[2] = atoi(token);	// max
	}
	else {
		preparsed_params->data_int[0] = 0;					// constant duration
		preparsed_params->data_int[1] = atoi(token);		// duration
	}
...
//	PreParse_ScriptAction_ShaderRemap()
//
//		syntax:	remapshader <original> <new>
//
//	params:
//		data_char[0]	=	<original>
//		data_char[1]	=	<new>
//
...
//	PreParse_ScriptAction_FollowSpline()
//
//		syntax:	followspline <d|accum <a>|globalaccum <g>> <t> <s> [w] [length <l>] [roll <a1> <a2>] [i] [o]
//
//	params:
//		data_int[0]		=	direction type		(value is 1 when using <a>, value is 2 when using <g>, value is 0 when using <d>)
//		data_int[1]		=	direction <d>, direction accum <a> or direction globalaccum <g>
//		data_int[2]		=	wait <w>
//		data_int[3]		=	roll start angle <a1>
//		data_int[4]		=	roll end angle <a2>
//		data_char[0]	=	targetname <t>
//		data_float[0]	=	speed <s>
//		data_float[1]	=	length value <l>
//
qboolean PreParse_ScriptAction_FollowSpline( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	if ( !Q_stricmp(token,"accum") ) {
		...
		preparsed_params->data_int[0] = 1;	// direction = accum x
		...
		preparsed_params->data_int[1] = globalaccum;	// globalaccum x
	}
	else {
		preparsed_params->data_int[0] = 0;				// direction = x
		preparsed_params->data_int[1] = atoi(token);	// x
	}
	...
	preparsed_params->data_float[0] = atof(token);

	//--- wait, roll, dampin, dampout
	preparsed_params->data_int[2] = (int)wait;
	preparsed_params->data_float[1] = length;
	preparsed_params->data_int[3] = (int)roll[0];	// start angle
	preparsed_params->data_int[4] = (int)roll[1];	// end angle
	while (token[0]) {
		token = COM_ParseExt( &pString, qfalse );
		if (token[0]) {
			if ( !Q_stricmp(token,"wait") ) {
				preparsed_params->data_int[2] = 1;	// wait = qtrue
			}
				...
				preparsed_params->data_float[1] = atof(token);	// length = x
			}
				...
				preparsed_params->data_int[3] = atoi(token);	// roll[0] = x

				token = COM_ParseExt( &pString, qfalse );
				if (!token[0]) {
					G_Error(S_COLOR_RED "G_ScriptAction_FollowSpline(): roll must have an end angle\n" );
				}
				preparsed_params->data_int[4] = atoi(token);	// roll[1] = x
			}
			else if ( !Q_stricmp(token,"dampin") ) {
				if ( preparsed_params->data_int[3] >= 0 ) {
					preparsed_params->data_int[3] += 1000;
				}
				else {
					preparsed_params->data_int[3] -= 1000;
				}
			}
			else if ( !Q_stricmp(token,"dampout") ) {
				if ( preparsed_params->data_int[3] >= 0 ) {
					preparsed_params->data_int[3] += 10000;
				}
				else {
					preparsed_params->data_int[3] -= 10000;
				}
...
//	PreParse_ScriptAction_GotoMarker()
//
//	syntax: gotomarker <t> <s> [a/d] [ttt] [w] [r <pt>]
//
//	params:
//		data_int[0]		=	trType [a/d]
//		data_int[1]		=	wait <w>
//		data_int[2]		=	turntotarget <ttt>
//		data_int[3]		=	relative <r>
//		data_char[0]	=	targetname <t>
//		data_char[1]	=	relative position targetname <pt>
//		data_float[0]	=	speed <s>
//
...
qboolean PreParse_ScriptAction_GotoMarker( gentity_t *ent, char *params, g_script_preparsed_params_t *preparsed_params )
{
	...
	// trType, wait
	preparsed_params->data_int[0] = TR_LINEAR_STOP;	// trType
	preparsed_params->data_int[1] = (int)wait;
	preparsed_params->data_int[2] = (int)turntotarget;
	preparsed_params->data_int[3] = 0;				// relative
	while (token[0]) {
		token = COM_ParseExt( &pString, qfalse );
		if (token[0]) {
			if (!Q_stricmp( token, "accel" )) {
				preparsed_params->data_int[0] = TR_ACCELERATE;
			}
			else if (!Q_stricmp( token, "deccel" )) {
				preparsed_params->data_int[0] = TR_DECCELERATE;
			}
			else if (!Q_stricmp( token, "wait" )) {
				preparsed_params->data_int[1] = 1;	// wait = qtrue;
			}
			else if (!Q_stricmp( token, "turntotarget" )) {
				preparsed_params->data_int[2] = 1;	// turntotarget = qtrue;
			}
			else if (!Q_stricmp( token, "relative" )) {
				preparsed_params->data_int[3] = 1;	// relative

				token = COM_ParseExt( &pString, qfalse );
				if (!token[0]) {
					G_Error(S_COLOR_RED "G_ScriptAction_GotoMarker(): \"relative\" parameter must have an targetname\n" );
				}
...
=================
core:
C_ScriptAction_Trigger

	This is a reworked version of G_ScriptAction_Trigger
	It's using pre-parsed data..
...
=================
*/
qboolean C_ScriptAction_Trigger( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	gentity_t					*trent;
	int							oldId;
	...
	preparsed_params = stack->items[ent->scriptStatus.scriptStackHead].parsed_params;
	if ( !preparsed_params ) {
		return qtrue;
	}

	switch( preparsed_params->data_int[0] ) {
		case SELF_HASH:
			trent = ent;
			oldId = trent->scriptStatus.scriptId;
			G_Script_ScriptEvent( trent, SE_TRIGGER, preparsed_params->data_char[1] );
		...
		case PLAYER_HASH:
			{
				...
				for ( i=0; i<level.numConnectedClients; ++i ) {
					G_Script_ScriptEvent( &g_entities[level.sortedClients[i]], SE_TRIGGER, preparsed_params->data_char[1] );
				}
			}
		...
		default:
			{
				...
				for (from=0; from<g_scripttriggers.count; ++from ) {
					...
					if ( trent->scriptnamehash != preparsed_params->data_int[1] ) continue;
					if ( Q_strncmp(trent->scriptName, preparsed_params->data_char[0], MAX_QPATH) ) continue;
					found = qtrue;
					if (!(trent->r.svFlags & SVF_BOT)) {
						oldId = trent->scriptStatus.scriptId;
						G_Script_ScriptEvent( trent, SE_TRIGGER, preparsed_params->data_char[1] );
				...
				if ( !found ) {
					trent = NULL;
					while ( (trent = G_FindByScriptnameFast(trent, preparsed_params->data_char[0], preparsed_params->data_int[1])) ) {
						found = qtrue;
						if (!(trent->r.svFlags & SVF_BOT)) {
							oldId = trent->scriptStatus.scriptId;
							G_Script_ScriptEvent( trent, SE_TRIGGER, preparsed_params->data_char[1] );
	...
	G_Printf( "G_ScriptAction_Trigger(): scriptname/trigger has unknown name: %s/%s\n", preparsed_params->data_char[0],preparsed_params->data_char[1]);
	return qtrue;
}
...
=================
core:
C_ScriptAction_Accum

	This is a reworked version of G_ScriptAction_Accum
	It's using pre-parsed data..
...
qboolean C_ScriptAction_Accum( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	int							accum, action;
	...
	accum = preparsed_params->data_int[0];
	action = preparsed_params->data_int[1];

	switch( action ) {
		case INC_HASH:
			ent->scriptAccumBuffer[accum] += preparsed_params->data_int[2];
			break;

		case ABORT_IF_LESS_THAN_HASH:
			if (ent->scriptAccumBuffer[accum] < preparsed_params->data_int[2]) goto abort_script;
			break;
		...
		case RANDOM_HASH:
			ent->scriptAccumBuffer[accum] = rand() % preparsed_params->data_int[2];
			break;

		case TRIGGER_IF_EQUAL_HASH:
			if (ent->scriptAccumBuffer[accum] == preparsed_params->data_int[2]) {
				qboolean	terminate = qfalse, found = qfalse;
				gentity_t	*trent = NULL;
				int			oldId;

				// for all entities/bots with this scriptName
				while ( (trent = G_FindByScriptnameFast(trent, preparsed_params->data_char[0], preparsed_params->data_int[3])) ) {
					found = qtrue;
					oldId = trent->scriptStatus.scriptId;
					G_Script_ScriptEvent( trent, SE_TRIGGER, preparsed_params->data_char[1] );
					// if the script changed, return false so we don't muck with it's variables
				...
				G_Printf(S_COLOR_YELLOW "C_ScriptAction_Accum(): accum scriptname/trigger has unknown name: %s/%s\n", preparsed_params->data_char[0],preparsed_params->data_char[1]);
				return qtrue;
			}
			break;
		...
		case SET_TO_DYNAMITECOUNT_HASH:
			{
				...
				target = G_FindByTargetname( NULL, preparsed_params->data_char[0] );
				if( !target ) {
					G_Error(S_COLOR_RED "C_ScriptAction_Accum(): accum could not find targetname \"%s\"\n", preparsed_params->data_char[0] );
				}

				// sigh, searching..
				for ( i=MAX_CLIENTS ; i< level.num_entities; ++i ){
					if ( !(g_entities[i].etpro_misc_1 & 1)) continue;
					if ( g_entities[i].etpro_misc_2 != target-g_entities) continue;
					num++;
				}

				ent->scriptAccumBuffer[accum] = num;
			}
			break;
		...
		case DIV_HASH:
			if (preparsed_params->data_int[2] != 0) ent->scriptAccumBuffer[accum] /= preparsed_params->data_int[2];
			break;

		// accum/accum
		case INC_ACCUM_HASH:
			ent->scriptAccumBuffer[accum] += ent->scriptAccumBuffer[preparsed_params->data_int[2]];
			break;

		case ABORT_IF_LESS_THAN_ACCUM_HASH:
			if (ent->scriptAccumBuffer[accum] < ent->scriptAccumBuffer[preparsed_params->data_int[2]]) goto abort_script;
			break;
		...
		case RANDOM_ACCUM_HASH:
			ent->scriptAccumBuffer[accum] = rand() % ent->scriptAccumBuffer[preparsed_params->data_int[2]];
			break;
		...
		case DIV_ACCUM_HASH:
			if ( ent->scriptAccumBuffer[preparsed_params->data_int[2]]!= 0 ) ent->scriptAccumBuffer[accum] /= ent->scriptAccumBuffer[preparsed_params->data_int[2]];
			break;

		case SET_TO_PLAYINGCLIENTS_HASH:
			ent->scriptAccumBuffer[accum] = level.numPlayingClients;
			break;

		// accum/globalaccum
		case INC_GLOBALACCUM_HASH:
			ent->scriptAccumBuffer[accum] += level.globalAccumBuffer[preparsed_params->data_int[2]];
			break;

		case ABORT_IF_LESS_THAN_GLOBALACCUM_HASH:
			if (ent->scriptAccumBuffer[accum] < level.globalAccumBuffer[preparsed_params->data_int[2]]) goto abort_script;
			break;
		...
		case RANDOM_GLOBALACCUM_HASH:
			ent->scriptAccumBuffer[accum] = rand() % level.globalAccumBuffer[preparsed_params->data_int[2]];
			break;
		...
		case DIV_GLOBALACCUM_HASH:
			if ( level.globalAccumBuffer[preparsed_params->data_int[2]] != 0 ) ent->scriptAccumBuffer[accum] /= level.globalAccumBuffer[preparsed_params->data_int[2]];
			break;
	}
	return qtrue;
...
=================
core:
C_ScriptAction_GlobalAccum

	This is a reworked version of G_ScriptAction_GlobalAccum
	It's using pre-parsed data..
...
=================
*/
qboolean C_ScriptAction_GlobalAccum( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	int							globalaccum, action;
	...
	globalaccum = preparsed_params->data_int[0];
	action = preparsed_params->data_int[1];

	switch( action ) {
		case INC_HASH:
			level.globalAccumBuffer[globalaccum] += preparsed_params->data_int[2];
			break;

		case ABORT_IF_LESS_THAN_HASH:
			if (level.globalAccumBuffer[globalaccum] < preparsed_params->data_int[2]) goto abort_script;
			break;
		...
		case RANDOM_HASH:
			level.globalAccumBuffer[globalaccum] = rand() % preparsed_params->data_int[2];
			break;

		case TRIGGER_IF_EQUAL_HASH:
			if (level.globalAccumBuffer[globalaccum] == preparsed_params->data_int[2]) {
				qboolean	terminate = qfalse, found = qfalse;
				gentity_t	*trent = NULL;
				int			oldId;
				...
				G_Printf(S_COLOR_YELLOW "C_ScriptAction_GlobalAccum(): globalaccum scriptname/trigger has unknown name: %s/%s\n", preparsed_params->data_char[0],preparsed_params->data_char[1]);
				return qtrue;
			}
			break;
		...
		case SET_TO_DYNAMITECOUNT_HASH:
			{
				...
				target = G_FindByTargetname( NULL, preparsed_params->data_char[0] );
				if( !target ) {
					G_Error(S_COLOR_RED "C_ScriptAction_GlobalAccum(): globalaccum could not find targetname \"%s\"\n", preparsed_params->data_char[0] );
				}
				...
				level.globalAccumBuffer[globalaccum] = num;
			}
			break;
		...
		case DIV_HASH:
			if (preparsed_params->data_int[2] != 0) level.globalAccumBuffer[globalaccum] /= preparsed_params->data_int[2];
			break;

		// accum/accum
		case INC_ACCUM_HASH:
			level.globalAccumBuffer[globalaccum] += ent->scriptAccumBuffer[preparsed_params->data_int[2]];
			break;

		case ABORT_IF_LESS_THAN_ACCUM_HASH:
			if (level.globalAccumBuffer[globalaccum] < ent->scriptAccumBuffer[preparsed_params->data_int[2]]) goto abort_script;
			break;
		...
		case RANDOM_ACCUM_HASH:
			level.globalAccumBuffer[globalaccum] = rand() % ent->scriptAccumBuffer[preparsed_params->data_int[2]];
			break;
		...
		case DIV_ACCUM_HASH:
			if ( ent->scriptAccumBuffer[preparsed_params->data_int[2]]!= 0 ) level.globalAccumBuffer[globalaccum] /= ent->scriptAccumBuffer[preparsed_params->data_int[2]];
			break;

		case SET_TO_PLAYINGCLIENTS_HASH:
			level.globalAccumBuffer[globalaccum] = level.numPlayingClients;
			break;

		// accum/globalaccum
		case INC_GLOBALACCUM_HASH:
			level.globalAccumBuffer[globalaccum] += level.globalAccumBuffer[preparsed_params->data_int[2]];
			break;

		case ABORT_IF_LESS_THAN_GLOBALACCUM_HASH:
			if (level.globalAccumBuffer[globalaccum] < level.globalAccumBuffer[preparsed_params->data_int[2]]) goto abort_script;
			break;
		...
		case RANDOM_GLOBALACCUM_HASH:
			level.globalAccumBuffer[globalaccum] = rand() % level.globalAccumBuffer[preparsed_params->data_int[2]];
			break;
		...
		case DIV_GLOBALACCUM_HASH:
			if ( level.globalAccumBuffer[preparsed_params->data_int[2]] != 0 ) level.globalAccumBuffer[globalaccum] /= level.globalAccumBuffer[preparsed_params->data_int[2]];
			break;
	}
	return qtrue;
...
=================
core:
C_ScriptAction_SetState

	This is a reworked version of G_ScriptAction_SetState
	It's using pre-parsed data..

	syntax: setstate <targetname> <state>

	Sets the visible state of an entity
=================
*/
qboolean C_ScriptAction_SetState( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	...
	// look for entities
	while( 1 ) {
		target = G_FindByTargetnameFast( target, preparsed_params->data_char[0], preparsed_params->data_int[0] );

		if( !target ) {
			if( !found ) {
				G_Printf(S_COLOR_YELLOW "Warning: setstate called and no entity found with targetname: %s\n", preparsed_params->data_char[0]);
			}
			break;
		}
		...
		G_SetEntState( target, (entState_t)preparsed_params->data_int[1] );
	}
...
=================
core:
C_ScriptAction_ToggleSpeaker

	This is a reworked version of G_ScriptAction_ToggleSpeaker
	It's using pre-parsed data..

	syntax: togglespeaker <targetname>

	Toggles a speaker on/off
=================
*/
qboolean C_ScriptAction_ToggleSpeaker( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	int							i;
	...
	for( i = 0; i < BG_NumScriptSpeakers(); ++i ) {
		...
		if( speaker->targetnamehash != preparsed_params->data_int[0] /*&& Q_stricmp(params, preparsed_params->data_char[0])*/ ) {
			continue;
		}
...
=================
core:
C_ScriptAction_DisableSpeaker

	This is a reworked version of G_ScriptAction_DisableSpeaker
	It's using pre-parsed data..
...
=================
*/
qboolean C_ScriptAction_DisableSpeaker( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	int							i;
...
=================
core:
C_ScriptAction_EnableSpeaker

	This is a reworked version of G_ScriptAction_EnableSpeaker
	It's using pre-parsed data..
...
=================
*/
qboolean C_ScriptAction_EnableSpeaker( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	int							i;
...
C_ScriptAction_Wait
	...
	This is a reworked version of G_ScriptAction_Wait
	It's using pre-parsed data..

	syntax:	wait <duration>
			wait random <min> <max>

	Makes the script wait for a specified duration.
=================
*/
qboolean C_ScriptAction_Wait( gentity_t *ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	...
	// Gordon: adding random wait ability
	if ( preparsed_params->data_int[0] ) {
		if(ent->scriptStatus.scriptStackChangeTime + preparsed_params->data_int[1] > level.time) {
			return qfalse;
		}

		if(ent->scriptStatus.scriptStackChangeTime + preparsed_params->data_int[2] < level.time) {
			return qtrue;
		}

		return !(rand() % (int)((preparsed_params->data_int[2] - preparsed_params->data_int[1]) * 0.02f));
	}

	// duration
	return (ent->scriptStatus.scriptStackChangeTime + preparsed_params->data_int[1] < level.time);
}
...
=================
core:
C_ScriptAction_ShaderRemap

	This is a reworked version of G_ScriptAction_ShaderRemap
	It's using pre-parsed data..

	syntax:	remapshader <oldshader> <newshader>

	Replace an existing shader with some other shader.
=================
*/
qboolean C_ScriptAction_ShaderRemap( gentity_t* ent, char *params )
{
	g_script_stack_t			*stack;
	g_script_preparsed_params_t	*preparsed_params;
	float						f;
	...
	f = level.time * 0.001;
	AddRemap(preparsed_params->data_char[0], preparsed_params->data_char[1], f);
	return qtrue;
}
...
===============
core:
C_ScriptAction_FollowSpline

	This is a reworked version of G_ScriptAction_FollowSpline
	It's using pre-parsed data..

	syntax: followspline <direction|accum <buffer_index>|globalaccum <buffer_index>> <targetname> <speed> [wait] [length <value>] [roll <start_angle> <end_angle>] [dampin] [dampout]

	NOTE: speed may be modified to round the duration to the next 50ms
	for smooth transitions
===============
*/
qboolean C_ScriptAction_FollowSpline( gentity_t* ent, char *params )
{
	...
	if (!params || ent->scriptStatus.scriptStackChangeTime < level.time) {			// we are waiting for it to reach destination
		...
		switch( preparsed_params->data_int[0] ) {
			case 1:	// accum x
				backward = (ent->scriptAccumBuffer[preparsed_params->data_int[1]] != 0)? qtrue : qfalse;
				break;
			case 2:	// globalaccum x
				backward = (level.globalAccumBuffer[preparsed_params->data_int[1]] != 0)? qtrue : qfalse;
				break;
			case 0:	// x
				backward = (qboolean)preparsed_params->data_int[1];
				break;
			default:
				backward = qfalse;
				break;
		...
		//--- targetname
		if( !(pSpline = BG_Find_Spline(preparsed_params->data_char[0])) ) {
			G_Error(S_COLOR_RED "G_ScriptAction_FollowSpline(): can't find spline with targetname = \"%s\"\n", preparsed_params->data_char[0] );
		}

		//--- speed
		speed = preparsed_params->data_float[0] * (float)g_moverScale.value;

		//--- wait, roll, dampin, dampout
		wait = (qboolean)preparsed_params->data_int[2];
		length = preparsed_params->data_float[1];
		roll[0] = (float)preparsed_params->data_int[3];
		roll[1] = (float)preparsed_params->data_int[4];
		...
		VectorClear( ent->s.pos.trDelta );
		ent->s.apos.trDuration = ent->s.pos.trDuration = SECONDS_1 * (pSpline->length / speed);

		if (!wait) {
			// round the duration to the next 50ms
			if (ent->s.pos.trDuration%SERVER_FRAMETIME) {
				float frac;
...
===============
core:
C_ScriptAction_GotoMarker

	This is a reworked version of G_ScriptAction_GotoMarker
	It's using pre-parsed data..
...
===============
*/
qboolean C_ScriptAction_GotoMarker( gentity_t *ent, char *params )
{
	if (params && (ent->scriptStatus.scriptFlags & SCFL_GOING_TO_MARKER)) {
		// we can't process a new movement until the last one has finished
		return qfalse;
	...
	if (!params || ent->scriptStatus.scriptStackChangeTime < level.time) {			// we are waiting for it to reach destination
		...
		float						speed, dist;
		vec3_t						vec;
		qboolean					wait=qfalse, turntotarget=qfalse, relative=qfalse;
		int							trType;
		vec3_t						diff;
		vec3_t						angles;
		...
		// targetname
		if ( (pPathCorner = BG_Find_PathCorner(preparsed_params->data_char[0])) ) {
		...
		// speed, trType, wait, turntotarget, relative
		speed = preparsed_params->data_float[0];
		trType = preparsed_params->data_int[0];
		wait = (qboolean)preparsed_params->data_int[1];
		turntotarget = (qboolean)preparsed_params->data_int[2];
		relative = (qboolean)preparsed_params->data_int[3];
		...
		if (relative) {
			...
			if ( (pPathCorner2 = BG_Find_PathCorner(preparsed_params->data_char[1])) ) {
				VectorCopy( pPathCorner2->origin, vec2 );
			}
			else if ((target2 = G_FindByTargetname(NULL, preparsed_params->data_char[1]) )) {
				VectorCopy( target2->r.currentOrigin, vec2 );
			}
			else {
				G_Error(S_COLOR_RED "Target for relative gotomarker not found: %s\n", preparsed_params->data_char[1] );
		...
		if (ent->s.eType == ET_MOVER) {
			...
			ent->s.pos.trDuration = SECONDS_1 * (dist / speed);
...
qboolean G_ScriptAction_FollowPath( gentity_t* ent, char *params )
{
	...
	if (!params || ent->scriptStatus.scriptStackChangeTime < level.time) {			// we are waiting for it to reach destination
		...
		ent->s.apos.trDuration = ent->s.pos.trDuration = SECONDS_1 * (dist / speed);
...
qboolean G_ScriptAction_SetChargeTimeFactor( gentity_t* ent, char *params )
{
	...
	else if( !Q_stricmp(playerclass, "fieldops") || !Q_stricmp(playerclass, "lieutenant") ) {	// FIXME: remove lieutenant from missionpack
...
qboolean G_ScriptAction_PlayAnim( gentity_t *ent, char *params )
{
	...
	if (token[0]) {
		if (!Q_stricmp(token, "looping")) {
			...
			if (!Q_stricmp(token, "untilreachmarker")) {
				if (level.time < ent->s.pos.trTime + ent->s.pos.trDuration) {
					endtime = level.time + SERVER_FRAMETIME_DELTA;
				}
				else {
					endtime = 0;
				}
				...
				ent->scriptStatus.scriptFlags |= SCFL_ANIMATING;
				endtime = level.time + SERVER_FRAMETIME_DELTA;		// we don't care when it ends, since we are going forever!
				forever = qtrue;
			}
			else {
		...
		if (!looping) {
			endtime = ent->scriptStatus.scriptStackChangeTime + ((endframe - startframe) * SERVER_FRAMETIME);
		}
	}
...
core:
	...
	If g_spawntimeMinPlayers is set, spawntimes can automatically be adjusted according to the number of players.
...
void CheckRespawnTimes( qboolean disabled )
{
	...
	if ( g_spawntimeMinPlayers.integer > 0 ) {
		if ( level.numPlayingClients > 0 && level.numPlayingClients < g_spawntimeMinPlayers.integer ) {
			float ratio = (float)(level.numPlayingClients) / (float)(g_spawntimeMinPlayers.integer);
			...
			if ( !g_userAxisRespawnTime.integer ) {
				...
				newRedTime += SECONDS_1 - (newRedTime % SECONDS_1);
				// spawntime must be at least 1 second..
				if ( newRedTime < SECONDS_1 ) newRedTime = SECONDS_1;
			...
			if ( !g_userAlliedRespawnTime.integer ) {
				...
				newBlueTime += SECONDS_1 - (newBlueTime % SECONDS_1);
				if ( newBlueTime < SECONDS_1 ) newBlueTime = SECONDS_1;
...
qboolean G_ScriptAction_AxisRespawntime( gentity_t *ent, char *params )
{
	...
	if ( g_userAxisRespawnTime.integer )
		trap_Cvar_Set( "g_redlimbotime", va( "%i", g_userAxisRespawnTime.integer * SECONDS_1 ) );
	else {
...
qboolean G_ScriptAction_AlliedRespawntime( gentity_t *ent, char *params )
{
	...
	if ( g_userAlliedRespawnTime.integer ) {
		trap_Cvar_Set( "g_bluelimbotime", va( "%i", g_userAlliedRespawnTime.integer * SECONDS_1 ) );
	}
	else {
...
qboolean G_ScriptAction_VoiceAnnounce( gentity_t *ent, char *params)
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error(S_COLOR_RED " G_ScriptAction_VoiceAnnounce(): team parameter required\n" );
	}
...
qboolean G_ScriptAction_TeamVoiceAnnounce( gentity_t *ent, char *params )
{
	...
	switch( atoi(token) ) {
		...
		case 1:
			...
			break;
		default:
			G_Error(S_COLOR_RED " G_ScriptAction_TeamVoiceAnnounce(): team parameter value can only be 0 or 1\n" );
			return qfalse; // cs: for compiler warning (even though it will never get here).
	}
...
qboolean G_ScriptAction_SetDamagable( gentity_t *ent, char *params )
{
	...
	// look for entities
	target = &g_entities[MAX_CLIENTS];
	hashName = BG_StringHashValue( name );
	while( (target = G_FindByTargetnameFast(target, name, hashName)) ) {
...
=================================================================================================
	core:
	The old functions, from before any pre-parse stuff was added.
	I left these in, along with the nq_scriptMode cvar,
	for checking what runs better: old or new code..
=================================================================================================
*/
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_Trigger( ent, params );
	} else {
		return G_ScriptAction_Trigger( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_Accum( ent, params );
	} else {
		return G_ScriptAction_Accum( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_GlobalAccum( ent, params );
	} else {
		return G_ScriptAction_GlobalAccum( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_SetState( ent, params );
	} else {
		return G_ScriptAction_SetState( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_ToggleSpeaker( ent, params );
	} else {
		return G_ScriptAction_ToggleSpeaker( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_DisableSpeaker( ent, params );
	} else {
		return G_ScriptAction_DisableSpeaker( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_EnableSpeaker( ent, params );
	} else {
		return G_ScriptAction_EnableSpeaker( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_Wait( ent, params );
	} else {
		return G_ScriptAction_Wait( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_ShaderRemap( ent, params );
	} else {
		return G_ScriptAction_ShaderRemap( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_FollowSpline( ent, params );
	} else {
		return G_ScriptAction_FollowSpline( ent, params );
	}
}
...
{
	if ( nq_scriptMode.integer & 1 ) {
		return C_ScriptAction_GotoMarker( ent, params );
	} else {
		return G_ScriptAction_GotoMarker( ent, params );
	}
}
/*
=================================================================================================
