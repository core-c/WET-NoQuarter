...
void Use_target_remove_powerups( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	if ( activator->client->ps.powerups[PW_REDFLAG] || activator->client->ps.powerups[PW_BLUEFLAG] ) {
		Team_ReturnFlag(&g_entities[activator->client->flagParent], activator); //core: the player himself returns the objective
	}
...
void target_laser_think (gentity_t *self)
{
	...
	VectorMA (self->s.origin, 2048.0f, self->movedir, end);
...
void target_laser_start (gentity_t *self)
{
	...
	if (self->target) {
		gentity_t * ent = G_FindByTargetname( NULL, self->target );
...
void SP_target_relay (gentity_t *self)
{
	...
	if(!(self->spawnflags & 32)) {	// !NO_LOCKED_NOISE
		...
		if ( G_SpawnString( "lockednoise", "0", &sound ) ) {
			...
			self->soundPos1 = GAMESOUND_MOOVERS_LOCK;
		}
	}
}
...
void G_KillEnts( const char* target, gentity_t* ignore, gentity_t* killer, meansOfDeath_t mod )
{
	...
	while ((targ = G_FindByTargetname( targ, target ))) {
		...
		targ->nextthink = level.time + SERVER_FRAMETIME_DELTA;
...
{
	gentity_t	*t = 0;
	int hash = BG_StringHashValue( ent->target );
...
void SP_target_smoke (gentity_t *ent)
{
	...
	if (!ent->delay) {
		ent->delay = SERVER_FRAMETIME_DELTA;
	}
	...
	if (!ent->s.time) {
		ent->s.time = SECONDS_5; // 5 seconds
	}

	ent->s.time2 = ent->duration;
	if (!ent->s.time2) {
		ent->s.time2 = SECONDS_2;
	}
	...
	if (ent->s.time < ent->s.time2) {
		ent->s.time = ent->s.time2 + SERVER_FRAMETIME_DELTA;
	}
...
void target_script_trigger_use (gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	if (ent->aiName) {
		...
		int hash = BG_StringHashValue( ent->aiName );
		gentity_t *trent = NULL;
		// for all entities/bots with this ainame
		trent = G_FindByScriptnameFast(trent, ent->aiName, hash);
		...
		if (trent) {
			...
			found = qtrue;
			// Play the script
			G_Script_ScriptEvent( trent, SE_TRIGGER, ent->target );
		}
	}
	...
	if (!found) {
		if ( ent->scriptName ) {
			G_Script_ScriptEvent( ent, SE_TRIGGER, ent->target );
		}
	}

	// core: This next call is a bit strange to me..
	// A target_script_trigger has a targetname + target assigned for the
	// entity trigger that it will activate.
	// That activation is already handled ^^above^^ with G_Script_ScriptEvent(ent,SE_TRIGGER,ent->target);
	// But now it also wants to fire the targets (for activator == other) ??
	// My tests report nothing is found for this call..
	// (which is what i'd expected, because a triggername is something different than a targetname)
	//
	//G_UseTargets ( ent, other);
}
