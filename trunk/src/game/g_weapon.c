...
void Weapon_Knife( gentity_t *ent ) {
	...
	// make sure damage != 0 to let admins disable knife damage entirely
	if (!BG_Weapons[ent->s.weapon].damage) {
		return;
	}
...
void MagicSink( gentity_t *self ) {
    ...
    self->nextthink = level.time + SECONDS_4;
...
gentity_t *fire_missile (gentity_t *self, vec3_t start, vec3_t dir, int damage, int radius, int speed, int mod, int size) {
	...
	if ((jp_insanity.integer & JP_INSANITY_GUIDED) && self->client) {
		bolt->nextthink			= level.time + SERVER_FRAMETIME_DELTA;
		...
		bolt->nextthink 		= level.time + SECONDS_20;					// push it out a little
	...
	bolt->s.pos.trTime = level.time - SERVER_FRAMETIME;		// move a bit on the very first frame
...
void Weapon_Medic_Ext(gentity_t *ent, vec3_t viewpos, vec3_t tosspos, vec3_t velocity) {
    ...
    ent2->nextthink = level.time + SECONDS_30;
...
void Weapon_MagicAmmo_Ext(gentity_t *ent, vec3_t viewpos, vec3_t tosspos, vec3_t velocity) {
	...
	VectorSet( maxs, radius, radius, 2*radius );
...
qboolean Weapon_Syringe(gentity_t *ent) {
	...
	// core: don't allow underwater stinging if it is not enabled..
	// Note: It seems better to invert the meaning of the bitflag (make it WPF_NO_UNDERWATER_SYRINGE)
	// because then less calls to trap_PointContents() are needed (assuming most people want underwater syringes, which is default up until 1.2.9)
	if ( !(g_weapons.integer & WPF_UNDERWATER_SYRINGE) ) {
		int contents = trap_PointContents( end, -1 );
		if ( contents & CONTENTS_WATER ) return qfalse;
	}
	...
	if ( (jp_insanity.integer & JP_INSANITY_SYRINGEGIBS) && (traceEnt->client->sess.sessionTeam != ent->client->sess.sessionTeam) ) {
		// visually gib the player..
		G_Damage( traceEnt, ent, ent, NULL, NULL, traceEnt->health + (-GIB_HEALTH) + 1, DAMAGE_NO_PROTECTION, MOD_SYRINGE );
		return qtrue;
	}
...
qboolean Weapon_Poison(gentity_t *ent) {
	...
	// make sure damage != 0 to let admins disable poison entirely
	...
	// #598 IRATA: don't do poison if match_warmupDamage doesn't allow it ...
	// 0-no damage, 1-Enemies only", 2-Everyone
	if ( level.intermissionQueued || g_gamestate.integer != GS_PLAYING) {
		if (match_warmupDamage.integer == 0) {
			return qfalse;
		}
		else if (match_warmupDamage.integer == 1 && OnSameTeam(ent, traceEnt)) {
			return qfalse;
		}
...
static void HandleEntsThatBlockConstructible( gentity_t *constructor, gentity_t *constructible, qboolean handleBlockingEnts, qboolean warnBlockingPlayers ) {
	...
	for( e = 0; e < listedEntities; e++ ) {
		...
		if( warnBlockingPlayers && check->s.eType == ET_PLAYER ) {
			// >= MIN_BLOCKINGWARNING_INTERVAL 5000
			if( (level.time - check->client->lastConstructibleBlockingWarnTime) >= SECONDS_5 ) {
		...
		for( e = 0; e < blockingEntities; e++ ) {
			...
			if( block->client || block->s.eType == ET_CORPSE ) {
				...
				if ( block->s.eType == ET_MISSILE && block->methodOfDeath == MOD_LANDMINE ) {
					// remove from map/radar..
					G_RemoveTeamMapData( block, qfalse, qtrue, qtrue );
				}
...
#define CONSTRUCT_POSTDECAY_TIME SECOND_HALF
	...
	if( constructible->s.eType == ET_CONSTRUCTIBLE &&
		...
		if( constructible->s.angles2[0] <= 0 ) {
			...
			if( ent->client->touchingTOI->chain && ent->client->touchingTOI->count2 ) {
				...
				G_RemoveTeamMapData( ent, qfalse, indicator->s.teamNum == TEAM_ALLIES, indicator->s.teamNum == TEAM_AXIS );	// inversed
			}
			...
			constructible->nextthink = level.time + SERVER_FRAMETIME_DELTA;

			G_PrintClientSpammyCenterPrint( ent-g_entities, "Constructing..." );
		}

		// forty - constructible xp sharing
		addhealth = (255.f/(constructible->constructibleStats.duration/(float)SERVER_FRAMETIME_DELTA));
		if(g_constructiblexpsharing.integer) {
			float xpperround = constructible->constructibleStats.constructxpbonus/(255.f/addhealth) + 0.01f;
		...
		// credits
		ent->client->sess.credits += CREDITS_OBJBONUS; // move to xp sharing block?
...
void Weapon_Engineer( gentity_t *ent ) {
	...
	vec3_t		mins;								// JPW NERVE
	int			touch[MAX_GENTITIES], scored = 0;	// JPW NERVE
	vec3_t		end;
	vec3_t		origin;
	...
	if( G_EmplacedGunIsRepairable( traceEnt, ent ) ) {
		...
		// core: don't allow underwater plier if it is not enabled..
		if ( !(g_weapons.integer & WPF_UNDERWATER_PLIERS) ) {
			int contents = trap_PointContents( end, -1 );
			if ( contents & CONTENTS_WATER ) return;
		}
		...
		if ( traceEnt->methodOfDeath == MOD_LANDMINE ) {
			...
			// cs: negative numbers = 'infinite' mines. this is needed so the check below does not incorrectly
			//		report that the team already has max mines placed.
			if ( teammaxLandmines < 0 ) {
				teammaxLandmines = 1023; // probably reasonable to make this much lower.
			}
...
// even if you're using all of yours :x
			...
			else if ( G_LandmineTeam(traceEnt) == ent->client->sess.sessionTeam && G_CountTeamLandmines(ent->client->sess.sessionTeam) >= teammaxLandmines) {
...
//bani - #471
				...
				if(G_LandmineUnarmed(traceEnt)) {
					...
					traceEnt->timestamp 	= level.time + SECONDS_1;
					traceEnt->health		= 0;
					traceEnt->s.teamNum		= ent->client->sess.sessionTeam;
					traceEnt->s.modelindex2 = 0;
					traceEnt->nextthink		= level.time + SECONDS_2;
...
evilbanigoto:
					...
					if ( traceEnt->health >= 250 ) {
						...
						// remove from map/radar..
						G_RemoveTeamMapData( traceEnt, qfalse, qtrue, qtrue );
			...
			if ( traceEnt->health >= 250 ) {
				...
				traceEnt->nextthink = level.time + SERVER_FRAMETIME_DELTA;
			...
			if ( traceEnt->s.teamNum >= 4 ) {
				...
				// ARM IT!
				traceEnt->nextthink = level.time + SECONDS_30;
				...
				for ( i=0 ; i<num ; ++i ) {
					...
					if ((hit->s.eType == ET_OID_TRIGGER)) {
						...
						if (!(hit->spawnflags & 128) && (((hit->spawnflags & AXIS_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_ALLIES)) ||
							...
							// credits
							ent->client->sess.credits += CREDITS_OBJBONUS;
...
//bani
					...
					// cs: moving team checks below so omnibot trigger is always fired
					...
					for ( i=0 ; i<num ; ++i ) {
						...
						if ((hit->s.eType == ET_OID_TRIGGER)) {
							...
							G_Script_ScriptEvent( hit->target_ent, SE_DEFUSED, "" );
...
//bani - reordered this check so its AFTER the primary obj check
					// Gordon - first see if the dynamite was planted near a constructable object that would have been destroyed
					{
						...
						for( e = 0; e < numListedEntities; ++e ) {
							...
							G_Script_ScriptEvent( hit, SE_DEFUSED, "" );
...
void G_AddAirstrikeToCounters( gentity_t* ent ) {
	if( ent->client->sess.sessionTeam == TEAM_AXIS ) {
		level.axisBombCounter += team_airstrikeTime.integer * SECONDS_1;
	}
	else {
		level.alliedBombCounter += team_airstrikeTime.integer * SECONDS_1;
	}
}
...
void G_AddArtyToCounters( gentity_t* ent ) {
	...
	if( ent->client->sess.sessionTeam == TEAM_AXIS ) {
		level.axisArtyCounter += team_artyTime.integer * SECONDS_1;
	}
	else {
		level.alliedArtyCounter +=  team_artyTime.integer * SECONDS_1;
	}
}
...
void weapon_checkAirStrikeThink1( gentity_t *ent ) {
	if( !weapon_checkAirStrike( ent ) ) {
		...
		ent->nextthink = level.time + SECONDS_1;
		return;
	}
...
void weapon_checkAirStrikeThink2( gentity_t *ent ) {
	...
	ent->nextthink = level.time + SECOND_HALF;
}
	...
	if( ent->s.teamNum == TEAM_AXIS || ent->s.teamNum == TEAM_ALLIES ) {
		if( level.numActiveAirstrikes[ent->s.teamNum-1] > 6 || !G_AvailableAirstrikes( ent->parent ) ) {
			...
			if( g_weapons.integer & WPF_TM_AIRSTRIKE_RESTORE_FULL ) {
				...
				jP_UnDoChargeCost(ent->parent, SK_FIELDOPS, 0.5f*BG_GetFromTable(chargeCost_Airstrike, ent->parent->client->sess.skillBits, SK_FIELDOPS));
			}
...
void weapon_callAirStrike( gentity_t *ent ) {
	static vec3_t	bbMins = { -8.f, -8.f, -8.f };
	static vec3_t	bbMaxs = { 8.f, 8.f, 8.f };
	int				i, j, touch[MAX_GENTITIES];
	vec3_t			bombaxis, lookaxis, pos, bomboffset, fallaxis, temp, dir, skypoint, origin;
	...
	if ( foBlock ) {
		...
		SnapVector( origin );
		VectorAdd( origin, bbMins, mins );
		VectorAdd( origin, bbMaxs, maxs );
		...
		if(g_debugBullets.integer >= 2) {
            ...
            VectorAdd(b1, bbMins, b1);
            VectorAdd(b2, bbMaxs, b2);
	...
	if ((tr.fraction < 1.0f) && (!(tr.surfaceFlags & SURF_NOIMPACT)) ) { //SURF_SKY)) ) { // JPW NERVE changed for trenchtoast foggie prollem
		G_HQSay( ent->parent, COLOR_YELLOW, "Pilot: ", "Aborting, can't see target." );
	...
	for( j = 0; j < ent->count; ++j ) {
		...
		for( i = 0; i < NUMBOMBS; ++i ) {
			bomb = G_Spawn();
			bomb->nextthink		= level.time + i * 100 + crandom() * 50 + SECONDS_1 + ( j * SECONDS_2 ); // 1000 for aircraft flyby, other term for tumble stagger
...
void artillerySpotterThink( gentity_t *ent ) {
	...
	for( i = 0; i < 7; ++i ) {
		...
		bomb->parent			= ent;
		bomb->s.teamNum			= ent->s.teamNum;
		bomb->nextthink			= level.time + SECONDS_1 + random() * 300;
...
EV_AIRSTRIKEMESSAGE and EV_ARTYMESSAGE

There is no reason to link those tents, we call G_TempEntityNotLinked ...

==================
*/
void G_GlobalClientEvent( int event, int param, int client ) {
	// gentity_t* tent 		= G_TempEntity( vec3_origin, event );
	gentity_t* tent 		= G_TempEntityNotLinked( event );
...
void Weapon_Artillery(gentity_t *ent) {
	...
	if ((trace.fraction < 1.0) && (!(trace.surfaceFlags & SURF_NOIMPACT)) )	{
		...
		if( g_weapons.integer & WPF_TM_AIRSTRIKE_RESTORE_FULL ) {
			...
			jP_UnDoChargeCost(ent, SK_FIELDOPS, 0.5f*BG_GetFromTable(chargeCost_Artillery, ent->client->sess.skillBits, SK_FIELDOPS));
		}
	...
	for( i = 0; i < ( (int)BG_GetFromTable( artilleryBombs, ent->client->sess.skillBits, SK_SIGNALS ) ); ++i ) {
		...
		if (i == 0) {
			bomb->nextthink			= level.time + SECONDS_5;
			...
			bomb->nextthink			= level.time + 8950 + SECONDS_2 * i + crandom() * 800;
			// Gordon: for explosion type
			bomb->accuracy			= 2;
			bomb->classname			= "air strike";
			bomb->classnamehash		= AIR_STRIKE_HASH;
...
// build arty falling sound effect in front of bomb drop
		...
		bomb2->damage			= 0;
		bomb2->nextthink		= level.time + SECONDS_2;
...
// 		case WP_SHOTGUN: // shotgun doesn't call Bullet_Fire
...
#ifdef NQ_TEST
	// core: TEST TEST TEST less spread
	if ( nq_test.integer & 8 ) {
		// half spread..
		Bullet_Endpos( ent, spread*0.5f, &end );
	}
	else {
#endif
	...
	Bullet_Fire_Extended(ent, ent, muzzleTrace, end, damage, distance_falloff, (rand() & 255), 0);
...
float G_DistanceDamageFalloff( float distance ) {
	...
	float scale= ( distance - 1500.f ) / 1000.f /*(2500.f - 1500.f)*/;
...
gentity_t *weapon_gpg40_fire(gentity_t *ent, int grenType) {
	static vec3_t	trBBmins = { -4.f, -4.f, 0.f };
	static vec3_t	trBBmaxs = { 4.f, 4.f, 6.f };
	...
	//bani - to prevent nade-through-teamdoor sploit
	trap_Trace( &tr, orig_viewpos, trBBmins, trBBmaxs, viewpos, ent->s.number, MASK_MISSILESHOT );
	if( tr.fraction < 1 ) { // oops, bad launch spot ) {
		VectorCopy( tr.endpos, tosspos );
		SnapVectorTowards( tosspos, orig_viewpos );
	}
	else {
		trap_Trace (&tr, viewpos, trBBmins, trBBmaxs, tosspos, ent->s.number, MASK_MISSILESHOT );
		if( tr.fraction < 1 ) { // oops, bad launch spot
			VectorCopy(tr.endpos, tosspos);
			SnapVectorTowards( tosspos, viewpos );
		}
	...
	if( (grenType == WP_GPG40 || grenType == WP_M7) ) { // not needed for real ... FireWeapon() calls weapon_gpg40_fire only for GPG40 + M7
		// IRATA: moved initcamera from fire_grenade() (see mortar cam code)
		if ( jp_insanity.integer & JP_INSANITY_RIFLENADECAM ) {
			gentity_t *cam = InitCamera( ent->client->ps.clientNum );
			if ( cam ) {
				cam->activator = m;
				// m->s.clientNum = ent->s.clientNum; // IRATA: fire_grenade sets it! (diff to old code, it's set before, not after initCamera ...)
			}
		}

		// core: when war-mode is active, give the player a new grenade for rifle
		if (( (nq_War.integer & (WARMODE_ENABLE | WARMODE_RIFLE)) == (WARMODE_ENABLE | WARMODE_RIFLE) )) {
...
gentity_t *weapon_mortar_fire( gentity_t *ent, int grenType ) {
	...
	trap_Trace (&tr, testPos, trBBmins, trBBmaxs, launchPos, ent->s.number, MASK_MISSILESHOT);
...
gentity_t *weapon_grenadelauncher_fire (gentity_t *ent, int grenType) {
	static vec3_t	trBBmins1 = { -12.f, -12.f, 0.f };
	static vec3_t	trBBmaxs1 = { 12.f, 12.f, 20.f };
	static vec3_t	trBBmins2 = { -16.f, -16.f, 0.f };
	static vec3_t	trBBmaxs2 = { 16.f, 16.f, 16.f };
	static vec3_t	trBBmins3 = { -4.f, -4.f, 0.f };
	static vec3_t	trBBmaxs3 = { 4.f, 4.f, 6.f };
	gentity_t		*m;
	trace_t			tr;
	vec3_t			viewpos;
	...
	if( grenType == WP_DYNAMITE || grenType == WP_SATCHEL ) {
		trap_Trace (&tr, viewpos, trBBmins1, trBBmaxs1, tosspos, ent->s.number, MASK_MISSILESHOT);
	}
	else if( grenType == WP_LANDMINE ) {
		trap_Trace (&tr, viewpos, trBBmins2, trBBmaxs2, tosspos, ent->s.number, MASK_MISSILESHOT);
	}
	else {
		trap_Trace (&tr, viewpos, trBBmins3, trBBmaxs3, tosspos, ent->s.number, MASK_MISSILESHOT);
	}
	...
	switch (grenType) {
		...
		case WP_SMOKE_MARKER:
			...
			if( ent->client->sess.skillBits[SK_FIELDOPS] & (1<<3) ) {
				...
				m->count = 1;
				m->nextthink = level.time + SECONDS_2_5;
...
void ThrowKnife( gentity_t *ent ) {
	...
	vec3_t		dir;
	// 'spread'
	float		r = crandom()*(BG_Weapons[ent->s.weapon].spread/1000);
	float 		u = crandom()*(BG_Weapons[ent->s.weapon].spread/1000);
	...
	knife->nextthink 			= level.time + SECONDS_20;
	...
	knife->s.pos.trTime 		= level.time - SERVER_FRAMETIME;	// move a bit on the very first frame
	...
	// rotation
	knife->s.apos.trTime = level.time - SERVER_FRAMETIME;
...
gentity_t *Weapon_FlamethrowerFire( gentity_t *ent ) {
	...
	ent->r.svFlags |= SVF_BROADCAST;
	ent->client->flametime = level.time + SECONDS_2_5;
	return traceEnt;
}
...
void CalcMuzzlePoints(gentity_t *ent, int weapon) {
	...
	if(IS_SCOPED_WEAPON(weapon)) {
		float pitchAmp, yawAmp, pitchMinAmp, yawMinAmp, phase;
		float spreadfrac = ent->client->currentAimSpreadScale;

		if( weapon == WP_FG42SCOPE || weapon == WP_JOHNSON_SCOPE ) {
		...
		phase = level.time / SECONDS_1_F * ZOOM_PITCH_FREQUENCY * M_PI * 2;
		viewang[PITCH]	+= ZOOM_PITCH_AMPLITUDE * sin( phase ) * (spreadfrac+pitchMinAmp);

		phase = level.time / SECONDS_1_F * ZOOM_YAW_FREQUENCY * M_PI * 2;
	...
	if ( ent->client && ent->client->ps.eFlags & EF_POISONED ) {
		float phase = level.time / SECONDS_1_F * 0.3f * M_PI; // time / 1000 * frequency * 2pi
...
void Kick_CheckTriggers (  gentity_t *ent, vec3_t start, vec3_t end ) {
	static vec3_t	trBBmins = { -6.f, -6.f, -6.f };
	static vec3_t	trBBmaxs = { 6.f, 6.f, 6.f };
	trace_t			tr;
	gentity_t		*traceEnt;

	trap_Trace( &tr, start, trBBmins, trBBmaxs, end, ent->s.number,  CONTENTS_TRIGGER);
...
void Kick_Fire( gentity_t *ent ) {
	static vec3_t	bbMins = { -6.f, -6.f, -6.f };
	static vec3_t	bbMaxs = { 6.f, 6.f, 6.f };
	trace_t			tr;
	vec3_t			end;
	gentity_t		*traceEnt;
	...
	// core: add a footkick "shot" to the weaponstats ...
	// IRATA: do it here to count any usage
	ent->client->sess.aWeaponStats[WS_FOOTKICK].atts++;
	...
	G_HistoricalTrace(ent, &tr, offset, bbMins, bbMaxs, end, ent->s.number, MASK_SHOT);
...
void G_createClusterNade( gentity_t *ent, int numNades ) {
	...
	for( it = 0; it < numNades; it++ ) {
		...
		nade->nextthink = level.time + SECOND_HALF + (rand()%SECONDS_1); // jet Pilot - randomize the timers on grenades a bit
	}
}
...
void C_CheckKickedDyna( gentity_t *player, gentity_t *dynaEnt ) {
	...
	vec3_t		origin;
	qboolean	wasAwarded		= (dynaEnt->etpro_misc_1 & 2);
	qboolean	wasKickedOut	= (dynaEnt->etpro_misc_1 & 4);
	qboolean	isArmed			= (dynaEnt->s.teamNum < 4);

	// a non-armed dynamite will never spawn any message..
	if ( !isArmed ) {
		return;
	}
			...
			if ( !(hit->spawnflags & 128) && isEnemyDyna ) {
				...
				if ( !wasAwarded ) {
					if ( isEnemyDyna && dynaEnt->parent && dynaEnt->parent->client ) {
						...
						// credits
						dynaEnt->parent->client->sess.credits += CREDITS_OBJBONUS;
					}
					// core: mark that missionair point is already given for this dynamite..
...
/*
#ifdef OMNIBOTS
	// notify omni-bot framework of defused dynamite
	if ( dynaEnt->etpro_misc_2 >= MAX_CLIENTS ) {
		gentity_t *toi = &g_entities[ dynaEnt->etpro_misc_2 ];
		if ( toi->numPlanted > 0 ) {
			toi->numPlanted -= 1;
		}
		// remove the goal..
		Bot_Util_SendTrigger(toi, NULL, va("Defused at %s.", toi->parent ? toi->parent->track : toi->track), "defused");
	}
#endif
*/

	dynaEnt->etpro_misc_2 = -1;
	dynaEnt->etpro_misc_1 &= ~1;	// not armed at an objective..
	...
	if ( wasAwarded && !wasKickedOut ) {
		...
		if ( player->client->sess.sessionTeam != dynaEnt->s.teamNum && !(dynaEnt->etpro_misc_1 & 4) ) {
			...
			// credits
			player->client->sess.credits += CREDITS_OBJBONUS;
		...
		// display a (priority) message for the one that kicked the dynamite out of range of the toi..
		if (!(g_entities[player-g_entities].r.svFlags & SVF_BOT)) trap_SendServerCommand(player-g_entities, "cp \"Dynamite kicked out of range\" 1");
	}
}
...
{
	static vec3_t	bbMins = { -32.f, -32.f, -24.f };
	static vec3_t	bbMaxs = { 32.f, 32.f, 8.f };
	gentity_t	*traceEnt;
	...
	for(i = 0; i < num; ++i) {
		...
		if(	traceEnt->s.weapon == WP_GRENADE_PINEAPPLE ||
			...
			traceEnt->s.pos.trTime = level.time - SERVER_FRAMETIME;		// move a bit on the very first frame
...
void Weapon_Shotgun( gentity_t *ent ) {
	...
	// IRATA: Using same way as used in Bullet_Fire() with one exception:
	// Bullet_Endpos is included during the historical trace
	G_HistoricalTraceBegin( ent );
