...
// core: a compiler directive for easy switching "do not re-use <1 second freed entities" check.
#define RELAXED_FREE_ENTS // #301
	...
	while ( (t = G_FindByTargetnameFast( t, ent->target, hash )) != NULL ) {
		if ( t == ent ) {
			...
			if ( t->use ) {
				...
				// is it a door?..   !Q_stricmp(t->classname,"func_door") || !Q_stricmp(t->classname,"func_door_rotating")
				if ( activator && ((t->classnamehash == FUNC_DOOR_HASH) || (t->classnamehash == FUNC_DOOR_ROTATING_HASH)) ) {
...
// with a chain-linked list.. init once at map/server start
void G_InitFreeEntitiesChain( void ) {
	...
	for ( i=MAX_CLIENTS; i<ENTITYNUM_MAX_NORMAL; i++ ) {
		...
		g_entities[i].next_free_entity = (i==ENTITYNUM_MAX_NORMAL-1)? NULL : &g_entities[i+1];
#ifdef RELAXED_FREE_ENTS
		g_entities[i].freetime = -10000;
#endif
	}

	g_entities[ENTITYNUM_NONE].prev_free_entity = NULL;
	g_entities[ENTITYNUM_NONE].next_free_entity = NULL;
	g_entities[ENTITYNUM_WORLD].prev_free_entity = NULL;
	g_entities[ENTITYNUM_WORLD].next_free_entity = NULL;

	first_free_entity = &g_entities[MAX_CLIENTS];
	last_free_entity = &g_entities[ENTITYNUM_MAX_NORMAL-1];
}
...
core: note that level.startTime == 0 during warmup.
=================
*/
gentity_t *G_Spawn( void ) {
	...
	for ( force=0; force<2; force++ ) {
		...
		while ( e ) {
			...
			if ( e->inuse
#ifdef RELAXED_FREE_ENTS
				|| (!force && level.time - e->freetime < SECONDS_1)
#endif
			) {
				// are we at the end of the chain?..
				if ( e == last_free_entity ) break;
				// pick the next one in the chain..
				e = e->next_free_entity;
				// no free entities to be found?..
				if ( !e ) break;
				// check the next one..
				continue;
			}

			// no more entities can be allocated, so bail out..
			if ( i >= ENTITYNUM_MAX_NORMAL ) break;
			...
			// reuse this slot, (or use the newly created one),
...
void G_FreeEntity( gentity_t *ed ) {
	...
	switch ( ed->s.eType ) {
		...
		case ET_CABINET_H:
		case ET_CABINET_A:
			RemoveFromEntityArray( &g_cabinets, ed );
			break;
	...
	if ( !first_free_entity && !last_free_entity ) {
		...
		if ( first_free_entity ) {
			if ( first_free_entity - g_entities < pn ) {
				// find the previous..
				while ( pn > MAX_CLIENTS + BODY_QUEUE_SIZE ) {	//players + corpses
					p = &g_entities[--pn];
					if ( !p->inuse ) {
						break;
					} else {
		...
		// check if a previous entity was found,
		...
		if ( p ) {
			...
			if (p->next_free_entity) p->next_free_entity->prev_free_entity = ed;
			...
			if ( last_free_entity ) {
				...
				while ( nn < ENTITYNUM_MAX_NORMAL-1 ) {
					n = &g_entities[++nn];
					if ( !n->inuse ) {
						break;
					}
...
Spawns an event entity that will be auto-removed and doesn't use origins.
It's not linked into the world. See SV_LinkEntity which is not called.

 * Its origin or angles don't change
 * Its bounds don't change
 * Its contents don't change (ent->s.solid)
 * Its brush model doesn't change (again, sets ent->s.solid)
...
void G_ClientSound( gentity_t *ent, int soundIndex ) {
	if( ent && ent->client ) {
		...
		te->s.teamNum = ( ent->client - level.clients ); // IRATA: does 'teamNum' make sense ?
		te->s.eventParm = soundIndex;
	}
}
...
void G_PrintClientSpammyCenterPrint(int entityNum, char* text) {
	...
	if(level.time - g_entities[entityNum].client->lastSpammyCentrePrintTime < SECONDS_1) {
		return;
	}
...
team_t G_GetTeamFromEntity( gentity_t *ent ) {
	switch( ent->s.eType ) {
		...
		case ET_GENERAL:	switch( ent->methodOfDeath ) {
							...
							break;
		case ET_MOVER:		if( ent->classnamehash == SCRIPT_MOVER_HASH /*&& !Q_stricmp(ent->classname, "script_mover")*/ ) {
								return ent->s.teamNum;
							}
							break;
