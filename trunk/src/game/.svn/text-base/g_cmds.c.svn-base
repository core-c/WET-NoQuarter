...
void Censor_Force_Suicide( gentity_t *ent) {
	...
	player_die(ent, ent, ent, (g_gamestate.integer == GS_PLAYING) ? 100000 : 135, MOD_CENSORED);
}
	...
	while (*textPos) {
		...
		while (word < dictionary->num_words && *textPos) {
			if(!Q_stricmpn(textPos, dictionary->words[word], strlen(dictionary->words[word]))) {
				...
				for (i = 0; i < strlen(dictionary->words[word]);++i) {
					*textPos++ =  '*';
				}
...
void G_CensorPenalize(gentity_t *ent) {
	...
	// core: if a player is already muted (by a referee or by shrubbot),
	// then increase the duration of the mute.
	if ( g_censorPenalty.integer & CNSRPNLTY_TMPMUTE ) {
		if ( !ent->client->sess.muted ) {
			ent->client->sess.muted = qtrue;
			ent->client->sess.auto_mute_time = level.time + g_censorMuteTime.integer*1000;	// .auto_mute_time is actually the time to unmute
			ent->client->sess.muted_by = MUTED_BY_CENSOR;
			ClientConfigStringChanged( ent );
			CPx(ent - g_entities, va("print \"^9You've been auto-muted for %d seconds for foul language.\"",g_censorMuteTime.integer ));
			AP(va("chat \"%s^9 has been auto-muted for %d seconds due to foul language.", ent->client->pers.netname, g_censorMuteTime.integer ));
		}
		else {
			// only increase the duration of the mute if it is no permanent mute..
			if ( ent->client->sess.auto_mute_time >= 0 ) {
				ent->client->sess.auto_mute_time += g_censorMuteTime.integer*1000;	// increase time to unmute
				CPx(ent - g_entities, va("print \"^9You've been auto-muted for another %d seconds for foul language.\"",g_censorMuteTime.integer ));
				AP(va("chat \"%s^9 has been auto-muted for another %d seconds due to foul language.", ent->client->pers.netname, g_censorMuteTime.integer ));
			}
		}
	}
...
void G_PrivateMessage(gentity_t *ent) {
	...
	if ( ent && !g_privateMessages.integer ) {
		// IRATA add client message
		SP(va("^9Private messages not active on this server\n"));
		return;
	}

	if ( ent ) trap_GetUserinfo( ent-g_entities, userinfo, sizeof( userinfo ));
	...
	if(!sent) {
		// cs: #549 - /m via rcon to unknown player crashes server
		if (!ent) {
			SP(va("player not found: %s\n", name));
		}
		else {
...
void G_PlaySound_Cmd(void) {
	...
	if(trap_Argc() > 2) {
		...
		if(!g_tyranny.integer) {
			G_Printf("playsound: g_tyranny must be enabled to play sounds to specific clients\n");
			return;
		}
...
/*	if(
	...
	Com_sprintf(entry,
		...
		totalXP,
		ping,
		(level.time - cl->pers.enterTime) / SECONDS_60,
...
void Cmd_DropWeapon_f(gentity_t *ent) {
	...
	if ( level.time - ent->client->dropWeaponTime < SECOND_HALF ) {// dont allow as often
		return;
	}
	...
	switch(ent->client->ps.weapon) {
		...
		case WP_JOHNSON_SCOPE:
			ent->client->ps.weapon = WP_JOHNSON;
			break;
	...
	{
		weapon_t primary = G_GetPrimaryWeaponForClient(ent->client);
		weapon_t secondary = G_GetSecondaryWeaponForClient( ent->client, primary ); // jaquboss modifed both because these are working, note that primary is used only as reference in secondary detection
...

	{
		...
		if ( ent->client->ps.weapon == primary ) {
			...
			if ( primary > WP_NONE && primary < WP_NUM_WEAPONS && primary != WP_FLAMETHROWER) { // don't drop flamethrower ammo
				...
				else if ( IS_PANZER_WEAPON(primary) ) { // bazooka/panzerfaust use only clip
...
void Cmd_GiveDisguise_f(gentity_t *ent, int startArg) {
	...
	if (!Q_stricmpn(cls, "m", 1)  || classNum == PC_MEDIC) {
		msg = "^9givedisguise : ^7Medic\n";
		classNum = 1;
	}
	if (!Q_stricmpn(cls, "e", 1) || classNum == PC_ENGINEER) {
		msg = "^9givedisguise : ^7Engineer\n";
		classNum = 2;
	}
	if (!Q_stricmpn(cls, "f", 1) || classNum == PC_FIELDOPS) {
		msg = "^9givedisguise : ^7Field Ops\n";
		classNum = 3;
	}
	if (!Q_stricmpn(cls, "c", 1) || classNum == PC_COVERTOPS) {
		msg = "^9givedisguise : ^7Covert Ops\n";
		classNum = 4;
	}
	if (!Q_stricmpn(cls, "s", 1) || classNum == PC_SOLDIER) {
...
==================
*/
qboolean ExecGive( gentity_t *ent, char *name, char *amt, char *amt2) {
	gitem_t		*it;
	int			i;
	qboolean	give_all;
	...
	if ( !Q_stricmpn(name, "class", 5) ) {
		if ( ent->client->sess.sessionTeam == TEAM_AXIS || ent->client->sess.sessionTeam == TEAM_ALLIES ) {
			if ( hasAmount ) {
				// is it a valid string?..
				if ( !Q_stricmpn(amt,"s",1) ) amount = PC_SOLDIER;
				else if ( !Q_stricmpn(amt,"m",1) ) amount = PC_MEDIC;
				else if ( !Q_stricmpn(amt,"e",1) ) amount = PC_ENGINEER;
				else if ( !Q_stricmpn(amt,"f",1) ) amount = PC_FIELDOPS;
				else if ( !Q_stricmpn(amt,"c",1) ) amount = PC_COVERTOPS;
				//
				if( amount >= PC_SOLDIER && amount <= PC_COVERTOPS ) {
					if ( !G_IsClassDisabled(ent, amount, qtrue) ) {
						ent->client->sess.latchPlayerType = amount;
						ClientConfigStringChanged( ent );
						if ( hasAmount2 && amount2 == 1 ) {
							// visually gib the player..
		...
		if( hasAmount ) {
			...
			for( i = SK_BATTLE_SENSE; i < SK_NUM_SKILLS; ++i ) {
				...
				if ( ent->client->sess.skill[i] < NUM_SKILL_LEVELS-1 ) {
					amount = skillLevels[i][ent->client->sess.skill[i] + 1] - floor(ent->client->sess.skillpoints[i]);
				}
	...
	if (give_all || !Q_stricmp(name, "weapons") ) {
		for( i = WP_KNIFE; i < WP_NUM_WEAPONS; ++i ) {
			if (i != WP_FOOTKICK) { // not the foot kick!
	...
	if (give_all || !Q_stricmpn(name, "ammo", 4) ) {
		if(amount) {
			if ( ent->client->ps.weapon && ent->client->ps.weapon != WP_SATCHEL && ent->client->ps.weapon != WP_SATCHEL_DET && ent->client->ps.weapon != WP_FOOTKICK ) {
			...
			for ( i = WP_KNIFE ; i < WP_NUM_WEAPONS ; ++i ) {
				if( COM_BitCheck( ent->client->ps.weapons, i ) && i != WP_SATCHEL && i != WP_SATCHEL_DET && i != WP_FOOTKICK) {
	...
	if ( !give_all ) {
		...
		it = BG_FindItem(name);
		if (!it) {
			it = BG_FindItemForClassName(name, 0);
			if (!it) {
				return result;
			}
		}
...
// IRATA: set to 3000 - quickly change spawnpoints
...
qboolean SetTeam( gentity_t *ent, const char *s, qboolean force, weapon_t w1, weapon_t w2, qboolean setweapons ) {
	...
	if( team != TEAM_SPECTATOR ) {
		...
		if(g_noTeamSwitching.integer && (team != ent->client->sess.sessionTeam && ent->client->sess.sessionTeam != TEAM_SPECTATOR ) && g_gamestate.integer == GS_PLAYING && !force ) {
			trap_SendServerCommand( clientNum, "cp \"^1You cannot switch during a match, please wait until the round ends.\n\"" );
			return qfalse;	// ignore the request
		}

		if ( !force && ((g_gametype.integer == GT_WOLF_LMS && g_lms_teamForceBalance.integer) || (g_gametype.integer != GT_WOLF_LMS && g_teamForceBalance.integer)) ) {
			...
			if ( team == TEAM_AXIS && counts[TEAM_AXIS] - counts[TEAM_ALLIES] >= 1 ) {
				CP("cp \"^1The Axis have too many players.\n\"");
				return qfalse; // ignore the request
			}
	...
	// core: only remove game objects from the world, if we were in team before..
	if ( oldTeam == TEAM_AXIS || oldTeam == TEAM_ALLIES ) {
	...
	G_UpdateCharacter( client );			// doesn't ClientBegin take care of this already?
	ClientUserinfoChanged( clientNum );		// core: ^^yes it does.. and the userinfo is updated as well.
	*/
	...
	*/
	...
	// IRATA: ClientBegin() calls ClientSpawn() which does G_UpdateSpawnCounts() already for non revive case
	// Let's test this and don't run this function twice in SetTeam() ...
	// G_UpdateSpawnCounts();
...
//				G_AddClientToFireteam( ent-g_entities, ft->joinOrder[0] );
	...
	if ( client->sess.sessionTeam == TEAM_AXIS || client->sess.sessionTeam == TEAM_ALLIES ) {
		ent->client->inactivityTime = level.time + ( g_inactivity.integer ? g_inactivity.integer : 60 ) * SECONDS_1;
		ent->client->inactivitySecondsLeft = (g_inactivity.integer)? g_inactivity.integer : 60;
	}
	else {
		ent->client->inactivityTime = level.time + ( g_spectatorInactivity.integer ? g_spectatorInactivity.integer : 60 ) * SECONDS_1;
...
qboolean G_IsClassDisabled( gentity_t* ent, int cls, qboolean quiet ) {
	...
	if ( wcount >= value ) {	// compare percentage of team
		isRestricted = qtrue;
	}
	if (isRestricted) {
		if (!quiet) {
...
int G_WeaponCount( gentity_t* ent, weapon_t weap ) {
	...
	for( i = 0; i < level.numConnectedClients; ++i ) {
		...
		if ( level.clients[j].sess.playerWeapon == weap || level.clients[j].sess.latchPlayerWeapon == weap ||
			level.clients[j].sess.playerWeapon == weapAlts[weap] || level.clients[j].sess.latchPlayerWeapon == weapAlts[weap] ) {
			++cnt;
		}
	}
	return cnt;
...
qboolean G_IsWeaponDisabled( gentity_t* ent, weapon_t weapon, qboolean quiet ) {
	...
	if ( weapon_restrictions[weapon].disabled ) {
		...
		if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
			CP(va("cp \"^1* ^3%s is disabled by admin. ^1*^7\" 1",BG_Weapons[weapon].name));
		}
		return qtrue;
	}
	...
	if ( weapon_restrictions[weapon].r_minplayers && level.numPlayingClients < weapon_restrictions[weapon].minplayers ) {
		// onscreen message..
		if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
			CP(va("cp \"^1* ^3%s restricted. Low player count. ^1*^7\" 1",BG_Weapons[weapon].name));
		}
		return qtrue;
	}
	...
	if ( weapon_restrictions[weapon].r_maxweap && wcount >= weapon_restrictions[weapon].maxweap ) {
		// onscreen message..
		if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
			CP(va("cp \"^1* ^3%s restricted. Maximum reached. ^1*^7\" 1",BG_Weapons[weapon].name));
		}
		return qtrue;
	}
	...
	if ( weapon_restrictions[weapon].r_percentage && wcount >= level.numPlayingClients * 0.01f * weapon_restrictions[weapon].percentage ) {
		// onscreen message..
		if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
			CP(va("cp \"^1* ^3%s exceeded.\nGet more players. ^1*^7\" 1",BG_Weapons[weapon].name));
		}
		return qtrue;
	}
...
core:
	G_FollowSame
	Try to follow the same client as last time (before getting killed)
=================
*/
qboolean G_FollowSame( gentity_t *ent ) {
	...
	// JPW NERVE -- couple extra checks for limbo mode
	if (ent->client->ps.pm_flags & PMF_LIMBO) {
		if (level.clients[clientnum].sess.sessionTeam != ent->client->sess.sessionTeam) return qfalse;
	}
...
char *G_ShortcutSanitize(char *text) {
	...
	Q_strncpyz(n, Q_StrReplace(n, "[i]", "(i)"), sizeof(n));
	return n;
}
...
char *G_Shortcuts(gentity_t *ent, char *text) {
	...
	char g[MAX_NAME_LENGTH]	= {"someone"};		// lastbye_client
	char i[MAX_NAME_LENGTH]	= {"someone"};		// lasthi_client
	...
	if (ent ) {
		...
		if ( level.lastconnected_client != -1 ) {
			client = &level.clients[level.lastconnected_client];
			if (client) {
				Q_strncpyz(x,
					G_ShortcutSanitize(client->pers.netname),
					sizeof(x));
		...
		if ( level.lastbye_client != -1 && level.time < level.lastbye_time ) {
			client = &level.clients[level.lastbye_client];
			if (client) {
				Q_strncpyz(g,
					G_ShortcutSanitize(client->pers.netname),
					sizeof(g));
		...
		if ( level.lasthi_client != -1 && level.time < level.lasthi_time ) {
			client = &level.clients[level.lasthi_client];
			if (client) {
				Q_strncpyz(i,
					G_ShortcutSanitize(client->pers.netname),
					sizeof(i));
		...
		if (  ent->client->sess.sessionTeam == TEAM_AXIS || ent->client->sess.sessionTeam == TEAM_ALLIES ) {
			...
			if ( weapon ){
				int clip = WeaponTable[ent->client->ps.weapon].clipindex;
				int ammo = WeaponTable[ent->client->ps.weapon].ammoindex;
...
void G_Voice( gentity_t *ent, gentity_t *target, int mode, const char *id, const char *customChat, qboolean voiceonly ) {
	...
	if( mode == SAY_BUDDY ) {
		...
		else if ( !(ent->r.svFlags & SVF_BOT) && !Q_stricmp(id,"Hi") && level.time > level.lasthi_time ) {
			level.lasthi_client = ent->client->ps.clientNum;
			level.lasthi_time = level.time + SECONDS_15;	// core: they can reply using the shortcut [i] within 15 seconds..
		}
		else if ( !(ent->r.svFlags & SVF_BOT) && !Q_stricmp(id,"Bye") && level.time > level.lastbye_time ) {
			level.lastbye_client = ent->client->ps.clientNum;
			level.lastbye_time = level.time + SECONDS_15;	// core: they can reply using the shortcut [g] within 15 seconds..
		}
...
qboolean Cmd_CallVote_f( gentity_t *ent, unsigned int dwCommand, qboolean fRefCommand ) {
	...
	//
	// core: NOTE! arguments seem to be tokenized before this function is called.
	...
	if( fRefCommand ) {
		...
		// jaquboss do not automatically vote yes in polls
...
qboolean G_UniformSteal(gentity_t *ent, gentity_t *traceEnt) {
	...
	if( BODY_VALUE(traceEnt) < 250 ) {
        BODY_VALUE(traceEnt) += SERVER_FRAMETIME_DELTA/sv_fps.integer;
		return qfalse;
	}
	...
	CPx( ent->s.number, va("cp \"Uniform of %s^7 has been stolen\" 1", ent->client->disguiseNetname) );
...
void G_ClassSteal_RemoveTools(gclient_t *client) {
	...
	// core: Let players explicitly switch to another weapon..
	// This should fix the issue of ending up as (for example) fieldops with a medkit still in hands.
...
qboolean G_ClassSteal(gentity_t *stealer, gentity_t *deadguy) {
	...
	if ( !(g_classChange.integer & CLASSCHANGE_ENABLE) ) return qfalse;
	...
	if ( BODY_VALUE(deadguy) < 250 ) {
		if(BODY_VALUE(deadguy) == 0) {
			...
			CPx( stealer->s.number, va( "cp \"Switching class to %s\" 1", BG_ClassnameForNumber(BODY_CLASS(deadguy)) ) );
		}
        BODY_VALUE(deadguy) += SERVER_FRAMETIME_DELTA/sv_fps.integer;
		return qtrue;
	}

	deadguy->nextthink = deadguy->timestamp + BODY_TIME;
	deadguy->activator = stealer;
	deadguy->s.time2 =	1;	// set pantsed flag..
	// sound effect
	...
	// core: keep the class on future spawnings?
	if ( g_classChange.integer & CLASSCHANGE_KEEPCLASS ) {
		stealer->client->sess.latchPlayerType = stealer->client->sess.playerType;
	}
	...
	ClientConfigStringChanged( stealer );
...
qboolean G_PushPlayer(gentity_t *ent, gentity_t *victim) {
	...
	if((level.time - ent->client->pmext.shoveTime) < SECOND_HALF)
		return qfalse;
...
static qboolean G_MedicActivate(gentity_t *ent, gentity_t *traceEnt) {
	...
	if ( traceEnt->health <= 0 ){
		...
		traceEnt->reviveTime = level.time + SECOND_HALF; // be long first time
	...
	ent->client->ps.pm_time = SECOND_HALF;
...
void Cmd_Activate2_f( gentity_t *ent ) {
	...
	if ( g_dropObj.integer ) {
		if ( ent->client->ps.powerups[PW_REDFLAG] || ent->client->ps.powerups[PW_BLUEFLAG] ) {
			if ( ent->client->pmext.objDrops < g_dropObj.integer ) {
				if(ent->client->ps.weapon != WP_KNIFE && ent->client->ps.weapon != WP_KNIFE_KABAR) {
					if ( level.time > ent->client->pmext.objDropTime ) {
						CP("cp \"You must switch to knife to drop objective\"");
						// core: prevent spamming this message..
						// Only display (and SEND!) it once every 6 seconds.
						ent->client->pmext.objDropTime = level.time + SECONDS_6;
					}
				}
				else {
					G_DropItems(ent);
				...
				// core: do not keep sending/spamming messages while they hold the "use"-key..
				// Display the warning message max. 3 times.
				if ( ent->client->pmext.objDrops <= g_dropObj.integer + 3 ) {
					CP( va("cp \"You can drop an objective only %i times\"",g_dropObj.integer) );
				}
				ent->client->pmext.objDrops++;
			}
		}
...
void G_UpdateSpawnCounts( void ) {
	gclient_t* client; // IRATA: moved up
...
void SetPlayerSpawn( gentity_t* ent, int spawn, qboolean update ) {
	...
	// IRATA/TODO: this is server printf !
...
void Cmd_IntermissionPlayerKillsDeaths_f ( gentity_t* ent ) {
	...
	for( i = 0; i < g_maxclients.integer; ++i ) {
		if( g_entities[i].inuse ) {
			Q_strcat( buffer, sizeof( buffer ), va( "%i %i %i ", level.clients[i].sess.kills, level.clients[i].sess.deaths, level.clients[i].sess.team_kills ) );
		}
		else {
...
void Cmd_Bet_f( gentity_t* ent ) {
	...
	// you must be on a team to start a bet..
	if ( ent->client->sess.sessionTeam != TEAM_AXIS && ent->client->sess.sessionTeam != TEAM_ALLIES ) {
		CP("print \"^dbet: ^9You are not on a team\n\"");
		return;
	}
	...
	// target must be on a team to start a bet..
	...
	if ( vic->r.svFlags & SVF_BOT ) {
		CP(va("print \"^dbet: ^7%s^9 is a bot and cannot bet\n\"",vic->client->pers.netname));
		return;
	}
#endif

	// the other must be on the opposite team..
	// this check eliminates the need to check for self-bets.
	if ( vic->client->sess.sessionTeam == ent->client->sess.sessionTeam ) {
		CP(va("print \"^dbet: ^7%s^9 is not on your opposing team\n\"",vic->client->pers.netname));
		return;
	}

	// check if target is ignoring us
	if(COM_BitCheck(vic->client->sess.ignoreClients,(ent-g_entities))) {
		CP(va("print \"^dbet: ^7%s ^9is ignoring you\n\"",vic->client->pers.netname));
		return;
	}

	// the other must not have a bet in progress, or even have a pending request..
	if ( vic->client->sess.currentBetTarget ) {
		CP(va("print \"^dbet: ^7%s^9 is already in another bet\n\"",vic->client->pers.netname));
		return;
	}
	...
	CP(va("print \"^dbet: ^9You requested to bet ^7%s ^9%i credits\n\"",vic->client->pers.netname,amount));
	//CPx(vic->client - level.clients, va("^dbet: ^9%s^9 wants to bet with you\n",ent->client->pers.netname));
	CPx(vic->client - level.clients, va("cp \"%s^9 wants to bet you %i credits\n\"",ent->client->pers.netname,amount));	// centerprint
}
...
void Cmd_Accept_f( gentity_t* ent ) {
	...
	if ( ent->client->sess.currentBetTarget ) {
		...
		if ( !vic->client ) {
			...
			SP(va("print \"^daccept: ^7%s^9 is no longer on a team\n\"",vic->client->pers.netname));
			return;
		}
	}
	else {
	...
	CP(va("print \"^daccept: ^9You accepted ^7%s^9's bet\n\"",vic->client->pers.netname));
...
void Cmd_Buy_f( gentity_t* ent ) {
	...
	if (!*buyingstr) {
		CP("print \"^dusage: ^2buy ^9[adrenaline|ammo|health|helmet|lunch]\n\"");
		CP("print \"  adrenaline: ^250 ^9credits\n\"");
		CP("print \"  ammo:      ^2100 ^9credits\n\"");
		CP("print \"  health:    ^2100 ^9credits\n\"");
		CP("print \"  helmet:     ^230 ^9credits\n\"");
		CP("print \"  lunch:      ^250 ^9credits\n\"");
		CP(va("print \"^9You currently have ^2%i ^9credits\n\"",ent->client->sess.credits));
		return;
	}

	// you must be on a team to be able to buy..
	if ( ent->client->sess.sessionTeam != TEAM_AXIS && ent->client->sess.sessionTeam != TEAM_ALLIES ) {
		CP("print \"^dbuy: ^9You must be on a team to buy something\n\"");
		return;
	}
	...
	// disallow /buy while in a fight if g_fear is set
	if ( g_fear.integer && (level.time - ent->client->lasthurt_time) < g_fear.integer ) {
		CP("print \"^dbuy: ^9You can't buy while fighting\n\"");
		return;
	}
	...
	if ( !Q_stricmp(buyingstr, "ammo") ) {
		...
		if ( ent->client->sess.credits < price ) {
			CP(va("print \"^dbuy: ^9You need at least ^2%i ^9credits to buy ammo\n\"",price));
			return;
		}
		AddMagicAmmo(ent, 10);
	...
	// buying health..
		...
		// see if the player has enough credits..
		if ( ent->client->sess.credits < price ) {
			CP(va("print \"^dbuy: ^9You need at least ^2%i ^9credits to buy health\n\"",price));
			return;
		}
		max = ent->client->ps.stats[STAT_MAX_HEALTH];
		if( ent->client->sess.playerType == PC_MEDIC && g_medics.integer & MEDIC_EXTRAHP ) {
			max *= 1.12f;
		}
		ent->health = max;
	...
	// buying lunch..
		...
		// see if the player has enough credits..
		if ( ent->client->sess.credits < price ) {
			CP(va("print \"^dbuy: ^9You need at least ^2%i ^9credits to buy lunch\n\"",price));
			return;
		}
		drop = Drop_Item(ent, &bg_itemlist[IL_HEALTH_HEALTH_BREADANDMEAT], 0.f, qfalse);
		if (drop) {
			drop->think = MagicSink;
			drop->timestamp = level.time + SECONDS_15;
		}
		CP("print \"^dbuy: ^9You bought yourself some lunch\n\"");
	}
	// buying adrenaline..
	else if ( !Q_stricmp(buyingstr, "adrenaline") ) {
		price = 50;
		// see if the player has enough credits..
		if ( ent->client->sess.credits < price ) {
			CP(va("print \"^dbuy: ^9You need at least ^2%i ^9credits to buy adrenaline\n\"",price));
			return;
		}
		// phishi: only allow if g_adrenaline bitflag is set for player's class
		if (( ent->client->sess.playerType == PC_SOLDIER && g_adrenaline.integer & ADF_SOLDIER ) ||
		( ent->client->sess.playerType == PC_MEDIC && g_adrenaline.integer & ADF_MEDIC ) ||
		( ent->client->sess.playerType == PC_ENGINEER && g_adrenaline.integer & ADF_ENGINEER ) ||
		( ent->client->sess.playerType == PC_FIELDOPS && g_adrenaline.integer & ADF_FIELDOPS ) ||
		( ent->client->sess.playerType == PC_COVERTOPS && g_adrenaline.integer & ADF_COVERTOPS )) {
			ent->client->ps.powerups[PW_ADRENALINE] = level.time + ADRENALINE_TIME;
			CP("print \"^dbuy: ^9You bought yourself some adrenaline\n\"");
		}
		else {
			CP("print \"^dbuy: ^9Adrenaline is not enabled for your player class\n\"");
			return;
		}
	}	
	// buying a helmet
	else if ( !Q_stricmp(buyingstr, "helmet") ) {
		price = 30;
		// see if the player has enough credits..
		if ( ent->client->sess.credits < price ) {
			CP(va("print \"^dbuy: ^9You need at least ^2%i ^9credits to buy a helmet\n\"",price));
			return;
		}
		if ( !(ent->client->ps.eFlags & EF_HEADSHOT) ) {
			CP("print \"^dbuy: ^9You can't buy a helmet if you still have one\n\"");
			return;
		}
		ent->client->ps.eFlags &= ~EF_HEADSHOT;
		CP("print \"^dbuy: ^9You bought yourself a helmet\n\"");
	}/*
	// buying a flak jacket
	else if ( !Q_stricmp(buyingstr, "flakjacket") ) {
		price = 30;
		// see if the player has enough credits..
		if ( ent->client->sess.credits < price ) {
			CP(va("print \"^dbuy: ^9You need at least ^2%i ^9credits to buy a flak jacket\n\"",price));
			return;
		}
		if ( ent->client->ps.powerups[PW_FLAKJACKET] ) {
			CP("print \"^dbuy: ^9You can't buy a flak jacket if you already have one\n\"");
			return;
		}
		ent->client->ps.powerups[PW_FLAKJACKET] = INT_MAX;
		CP("print \"^dbuy: ^9You bought yourself a flak jacket\n\"");
	}*/
	// invalid item..
	else {
	...
	// pay..
	ent->client->sess.credits -= price;

	// only once per life. cleared on spawn
	ent->client->sess.buyTime = level.time;
	// set the flag for displaying the buy sprite above their head
	ent->client->ps.eFlags |= EF_BUYING;
}
...
void ClientCommand( int clientNum ) {
	...
	switch( cmdHash ) {
		...
		case SGSTATS_HASH: // Player game stats
			G_statsPrint(ent, 2);
			return;

		case STSHOTS_HASH: // "Topshots" accuracy rankings
		...
		case GETGS_HASH: // core: received a request for csMethod-1-data..
			{
				int argc = trap_Argc();
		...
		case BET_HASH:
			if ( !(jp_insanity.integer & JP_INSANITY_ALLOWBETTING) ) {
				CP("print \"^1Betting is disabled on this server\n\"");
				return;
			}
			Cmd_Bet_f( ent );
			return;
		...
		case BUY_HASH:
			if ( !(jp_insanity.integer & JP_INSANITY_ALLOWBUYING) ) {
				CP("print \"^1Buying is disabled on this server\n\"");
				return;
			}
			Cmd_Buy_f( ent );
			return;
	...
	// check the rest of the (not-during-intermission) commands..
	switch( cmdHash ) {
		case FOLLOWNEXT_HASH:
			Cmd_FollowCycle_f (ent, 1, qfalse);
			return;

		case FOLLOWPREV_HASH:
			Cmd_FollowCycle_f (ent, -1, qfalse);
			return;
...
void C_SendAllNewConfigStrings( gentity_t *ent, qboolean wantModels, qboolean wantShaders, qboolean wantSkins, qboolean wantCharacters, qboolean wantSounds ) {
	...
	int		frametime = SECONDS_1/sv_fps.integer;
