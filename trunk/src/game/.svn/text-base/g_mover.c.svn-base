...
char *hintStrings[HINT_NUM_HINTS] = {
	...
	"HINT_NOEXIT",
	"HINT_PLYR_FRIEND",
	"HINT_PLYR_NEUTRAL", // IRATA: actually unused
	"HINT_PLYR_ENEMY",
	"HINT_PLYR_UNKNOWN",
...
//			return NULL;
		...
		vec3_t pos;
		VectorCopy( ent->s.pos.trBase, pos );
		pos[2] += 4;	// move up a bit - items on the ground otherwise don't always collide (especially on slopes)
...
#define	JITTER_MAX	(check->r.maxs[0]/2.0)
	...
	if (check->client) {
		...
		// core: and do it if the realism flag is enabled..
		if ( g_realism.integer & REALISM_MOVER_ROTATES_PLAYERS ||
			( !(g_realism.integer & REALISM_MOVER_ROTATES_PLAYERS) && (check->client->ps.eFlags & EF_PRONE || IS_MORTAR_WEAPON_SET(check->s.weapon)) ) )
		{
...
//core - referenced in G_MoverPush()
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	int			listedEntities = 0, moveEntities = 0;
	...
	*obstacle = NULL;
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		switch( check->s.eType ) {
			...
			case ET_PROP:
				if ( check->isProp ) continue;
				break;

			case ET_PLAYER:
				if ( !check->client ) continue;
				if ( (check->client->ps.eFlags & EF_TAGCONNECT) || (check->client->ps.pm_type == PM_NOCLIP) ) continue;
				break;

			case ET_CORPSE:
				// just check if it collides..
				break;

			case ET_ITEM:
				// core: skip CTF objectives, always collide with those (push them)..
				break;
			...
			default:
				if ( !check->physicsObject ) continue;
				break;
		}
	...
	for ( e = 0; e < moveEntities; e++ ) {
		...
		switch( check->s.eType ) {
			case ET_PLAYER:
				if ( !check->client ) break;
				if ( check->s.groundEntityNum != pusher->s.number ) {
					if ( check->client->ps.eFlags & (EF_DEAD | EF_PLAYDEAD | EF_PRONE | EF_PRONE_MOVING) ) {
						trap_LinkEntity( check );
						G_Damage( check, pusher, pusher, NULL, NULL, 9999, 0, MOD_CRUSH );
						moveList[e] = ENTITYNUM_NONE;	// prevent re-linking lateron
						continue;
					}
				}
				break;
			...
			case ET_ITEM:
				// core: check realism bitflag to see if items should be removed (except CTF objectives)
				if ( check->s.groundEntityNum == ENTITYNUM_WORLD ) {
					if ( check->item->giType != IT_TEAM && (g_realism.integer & REALISM_MOVER_GIBS_ITEMS) ) {
						trap_LinkEntity( check );
						G_FreeEntity( check );
						moveList[e] = ENTITYNUM_NONE;
						continue;
		...
		// the move was blocked by this entity
		...
		*obstacle = check;
		...
		for ( e = 0; e < moveEntities; e++ ) {
			if ( moveList[e] == ENTITYNUM_NONE ) continue;	// skip already removed entities..
			check = &g_entities[ moveList[e] ];
			trap_LinkEntity( check );
		}
		// movement failed
	...
	for ( e = 0; e < moveEntities; e++ ) {
		if ( moveList[e] == ENTITYNUM_NONE ) continue;
		check = &g_entities[ moveList[e] ];
		trap_LinkEntity( check );
	}
...
#ifdef PROFILER
	BG_ProfilerBlockStartTimer(PROFILER_G_RunMover, qfalse);
#endif
...
#ifdef PROFILER
		BG_ProfilerBlockStopTimer(PROFILER_G_RunMover);
#endif
		return;
	}
...
void SetMoverState( gentity_t *ent, moverState_t moverState, int time ) {
	...
	qboolean soft = (qboolean)(ent->flags & FL_SOFTACTIVATE);	//----(SA)	added
...
//----(SA)	numerous changes start here
		...
		case MOVER_1TO2ROTATE:	// opening
			...
			if(kicked) {
				f = 3500.0 / ent->gDuration;	// double speed when kicked open
				ent->s.apos.trDuration = ent->gDuration / 3.5;
			}
			else if(soft) {
...
void ReturnToPos2( gentity_t *ent ) {
	...
	// starting sound
	if ( ent->sound3to2 ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->sound3to2 );
}
...
void GotoPos3( gentity_t *ent ) {
	...
	// starting sound
	if ( ent->sound2to3 ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->sound2to3 );
}
// END JOSEPH
...
void ReturnToPos1Rotate( gentity_t *ent ) {
	...
	if ( ent->flags & FL_SOFTACTIVATE ) {
		if ( ent->soundSoftclose ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundSoftclose );
	}
	else {
		if ( ent->sound2to1 ) {
...
void Reached_BinaryMover( gentity_t *ent ) {
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		if ( ent->flags & FL_KICKACTIVATE ) {
			if ( ent->soundKickendo ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundKickendo );
		}
		else if ( ent->flags & FL_SOFTACTIVATE ) {
			if ( ent->soundSoftendo ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundSoftendo );
		}
		else
		...
		// play sound
		if ( ent->flags & FL_SOFTACTIVATE ) {
			if ( ent->soundSoftendc ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundSoftendc );
		}
		else
...
void Reached_TrinaryMover( gentity_t *ent ) {
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		// goto pos 3
		ent->think = GotoPos3;
		ent->nextthink = level.time + SECONDS_1; //FRAMETIME;
...
void Use_TrinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( IsBinaryMoverBlocked ( ent, other, activator) ) {
		MatchTeamReverseAngleOnSlaves( ent, MOVER_1TO2ROTATE, level.time + SERVER_FRAMETIME );
	...
	if ( ent->moverState == MOVER_POS1 ) {
		...
		MatchTeam( ent, MOVER_1TO2, level.time + SERVER_FRAMETIME );
	...
	if ( ent->moverState == MOVER_POS2 ) {
		...
		// start moving 50 msec later, becase if this was player
		// triggered, level.time hasn't been advanced yet
		MatchTeam( ent, MOVER_2TO3, level.time + SERVER_FRAMETIME );
...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( isblocked ) {
		...
		if(!nosound) {
			if ( ent->flags & FL_KICKACTIVATE ) {
				if ( ent->soundKickopen ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundKickopen );
			}
			else if ( ent->flags & FL_SOFTACTIVATE ) {
				if ( ent->soundSoftopen ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundSoftopen );
			}
			else
				if ( ent->sound1to2 ) G_AddEvent( ent, EV_GENERAL_SOUND, ent->sound1to2 );
		}
	...
	if ( ent->moverState == MOVER_POS1ROTATE ) {
		...
		// start moving 50 msec later, becase if this was player
		// triggered, level.time hasn't been advanced yet
		MatchTeam( ent, MOVER_1TO2ROTATE, level.time + SERVER_FRAMETIME );
	...
	if ( ent->moverState == MOVER_POS2 ) {
		if(ent->flags & FL_TOGGLE) {
			ent->nextthink = level.time + SERVER_FRAMETIME;
			return;
		}
	...
	if ( ent->moverState == MOVER_POS2ROTATE ) {
		if(ent->flags & FL_TOGGLE) {
			ent->nextthink = level.time + SERVER_FRAMETIME;	// do it *now* for toggles
		}
		else {
			ent->nextthink = level.time + ent->wait;
		}
...
void InitMover( gentity_t *ent ) {
	...
	ent->s.pos.trDuration = distance * SECONDS_1 / ent->speed;
	...
	if(ent->closespeed) {
		...
		ent->gDurationBack = distance * SECONDS_1 / ent->closespeed;
...
void Blocked_DoorRotate( gentity_t *ent, gentity_t *other ) {
	...
	if(other) {
		...
		if ( ent->flags & FL_KICKACTIVATE )
			G_Damage( other, ent, ent, NULL, NULL, 25, 0, MOD_CRUSH );
	}
...
qboolean findNonAIBrushTargeter(gentity_t *ent) {
	...
	while ((targeter = G_Find (targeter, FOFS(target), ent->targetname)) != NULL) {
		if ( Q_stricmp(targeter->classname, "func_invisible_user") )
			return qtrue;
	}
...
void DoorSetSounds(gentity_t *ent, int doortype, qboolean isRotating) {
	...
	ent->soundKickopen	= G_SoundIndex(va("sound/movers/doors/door%i_kicked.wav", doortype));	// opening noisy
	ent->soundKickendo	= G_SoundIndex(va("sound/movers/doors/door%i_kickedend.wav", doortype));	// open noisy
}
...
void G_TryDoor(gentity_t *ent, gentity_t *other, gentity_t *activator){
	qboolean walking = (qboolean)(ent->flags & FL_SOFTACTIVATE);
	qboolean kicking = (qboolean)(ent->flags & FL_KICKACTIVATE);
	...
	if ( (ent->s.apos.trType == TR_STATIONARY && ent->s.pos.trType == TR_STATIONARY) ) {
		if(ent->active == qfalse ) {
			...
			if (ent->teammaster && ent->team && ent != ent->teammaster) {
				...
				if(kicking)
					ent->teammaster->flags |= FL_KICKACTIVATE;		// lot of noise generated
				else if(walking)
				...
				if(kicking)
					ent->flags |= FL_KICKACTIVATE;		// lot of noise generated
				else if(walking)
					ent->flags |= FL_SOFTACTIVATE;		// no noise generated
	...
	// core: opening and closing (rotating) doors that are already opening/closing..
	else if ( g_realism.integer & REALISM_EASY_DOORS && !(activator->r.svFlags & SVF_BOT) && ent->classnamehash == FUNC_DOOR_ROTATING_HASH && !Q_stricmp(ent->classname, "func_door_rotating") ) {
		// don't do this on locked doors or teamdoors..
...
void Reached_Train( gentity_t *ent ) {
	...
	ent->s.pos.trDuration = length * SECONDS_1 / speed;
	...
	if ( next->wait ) {
		ent->nextthink = level.time + next->wait * SECONDS_1;
...
void Think_SetupTrainTargets( gentity_t *ent ) {
	gentity_t		*path, *next, *start = NULL;
...
void Reached_Train_rotating( gentity_t *ent ) {
	...
	if (next->duration) {
		ent->s.pos.trDuration = (next->duration * SECONDS_1);
	}
	else {
		ent->s.pos.trDuration = length * SECONDS_1 / speed;
	}
	...
	// Rotate the train
	// IRATA: FIX this? trDuration is not float!
	// -> frames = floor( (float)ent->s.pos.trDuration / 100.f);
	frames = floor(ent->s.pos.trDuration / 100); // @sv_fps
...
void Static_Pain( gentity_t *ent, gentity_t *attacker, int damage, vec3_t point) {
	if (ent->spawnflags & 4) {
		if (level.time > ent->wait + ent->delay + rand()%SECONDS_1 + SECOND_HALF) {
			ent->wait = level.time;
		}
		else {
			return;
...
void SP_func_bobbing (gentity_t *ent) {
	...
	ent->s.pos.trDuration = ent->speed * SECONDS_1;
...
void SP_func_pendulum(gentity_t *ent) {
	...
	ent->s.pos.trDuration = ( SECONDS_1 / freq );
	...
	ent->s.apos.trDuration = SECONDS_1 / freq;
...
void SP_func_door_rotating(gentity_t *ent) {
	...
	if(VectorLengthSquared(ent->rotate) > 1) { // check that rotation is only set for one axis
...
ThrowDebris
==============
IRATA: not used and empty anyway
...
void func_explosive_explode(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod) {
	...
	if(self->target) {
		...
		int hash = BG_StringHashValue( self->target );
		while ( (tent = G_FindByTargetnameFast(tent, self->target, hash)) != NULL ) {
			if( tent->classnamehash == INFO_NOTNULL_HASH /*&& !Q_stricmp(tent->classname, "info_notnull")*/ ) {
				// found an info_notnull
	...
	if(!tent) {
		if(self->s.angles[1]) {
			...
			// yawed
				...
				RotatePointAroundVector(dir, dir, point, self->s.angles[1]);
			}
		}
	}
	...
	if  ( self->constructibleStats.constructxpbonus == 5 ) {
		G_Script_ScriptEvent( self, SE_EXPLODED, "" );
	}
#endif
...
void func_explosive_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	G_Script_ScriptEvent( self, SE_DEATH, "" ); // JPW NERVE used to trigger script stuff for MP
	...
	if(self->parent && Q_stricmp(self->scriptName, self->parent->scriptName))
		G_Script_ScriptEvent(self->parent, SE_DEATH, "");
	func_explosive_explode(self, self, other, self->damage, 0);
}
...
void SP_target_explosion (gentity_t *ent) {
	...
	if(G_SpawnString("type", "wood", &type)) {
		...
		else if (!Q_stricmp(type,"fabric"))	ent->key = FXTYPE_FABRIC;
	}
	else {
		ent->key = 0;
	}
...
void use_invisible_user(gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if(other->client) {
		G_Script_ScriptEvent( ent, SE_ACTIVATE, other->client->sess.sessionTeam == TEAM_AXIS ? "axis" : "allies" );
	}
...
void SP_func_invisible_user (gentity_t *ent) {
	...
	ent->delay *= SECONDS_1;	// convert to ms
	...
	if(!(ent->spawnflags & 4)) {	// !NO_OFF_NOISE
		...
		if ( G_SpawnString( "offnoise", "0", &sound ) ) {
			...
			ent->soundPos1 = GAMESOUND_MOOVERS_LOCK;
		}
	}
}
...
void func_constructible_explode( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod ) {
	// core: try to determine the used weapon, WP_NONE if unknown..
	// int weapon = ( !inflictor || inflictor==NULL )? 0 : ( inflictor < 0 || inflictor >= MAX_CLIENTS )? inflictor->s.weapon : modWeaponTableMP[mod][0];
	int	weapon = ( !inflictor || inflictor==NULL )? WP_NONE : ( inflictor->client )? WP_NONE : inflictor->s.weapon;
	...
	if( self->desstages ) {
		if( self->grenadeFired > 1 ) {
			...
			G_AnnounceMessageForDestruction( attacker, self->parent, 0, weapon ); // damaged
			// core: award stats
			if (attacker && attacker->client) {
				attacker->client->sess.missionair++;

				// credits
				attacker->client->sess.credits += CREDITS_OBJBONUS;
			}

			// run the script
			if( self->grenadeFired == self->count2 ) {
				G_Script_ScriptEvent( self, SE_DESTROYED, "final" );
			}
			else {
				switch( self->grenadeFired ) {
					//case 1: G_Script_ScriptEvent( self, SE_DESTROYED, "stage1" ); break;
					case 2: G_Script_ScriptEvent( self, SE_DESTROYED, "stage2" ); break;
					case 3: G_Script_ScriptEvent( self, SE_DESTROYED, "stage3" ); break;
				}
			}
			...
			for( e = 0; e < listedEntities; e++ ) {
				...
				if( check->client || check->s.eType == ET_CORPSE ) {
					...
					// jet Pilot - Darwin Award for getting buried under rubble
					check->client->sess.darwin++;
				}
					...
					if ( check->s.eType == ET_MISSILE && check->methodOfDeath == MOD_LANDMINE ) {
						G_RemoveTeamMapData( check, qfalse, qtrue, qtrue );
					}
			...
			// core: display a message on who did it..
			{
				...
				if ( self->parent->spawnflags & 8 ) msgType = 0; //damaged (tank)
				G_AnnounceMessageForDestruction( attacker, self->parent, msgType, weapon );
			}
			// core: award stats
			if (attacker && attacker->client) {
				attacker->client->sess.missionair++;
...
void func_constructible_underconstructionthink( gentity_t *ent ) {
	if( level.time - ent->lastHintCheckTime >= CONSTRUCT_PREDECAY_TIME ) {
		//ent->s.angles2[0] -= 0.5f*(255.f/(ent->wait/(float)SERVER_FRAMETIME_DELTA));
		...
		if( ent->s.angles2[0] < 5 ) {
			...
			if( ent->count2 ) {
				...
				if( ent->grenadeFired == ent->count2 )
					G_Script_ScriptEvent( ent, SE_DECAYED, "final" );
				else {
					switch( ent->grenadeFired ) {
						case 1: G_Script_ScriptEvent( ent, SE_DECAYED, "stage1" ); break;
						case 2: G_Script_ScriptEvent( ent, SE_DECAYED, "stage2" ); break;
						case 3: G_Script_ScriptEvent( ent, SE_DECAYED, "stage3" ); break;
					}
				}
			...
			// call script
			G_Script_ScriptEvent( ent, SE_FAILED, "" );
...
//			ent->s.solid = CONTENTS_SOLID;	// FIXME: allow other contents?
		...
		if( !(ent->spawnflags & CONSTRUCTIBLE_INVULNERABLE) ) {
			...
			// if dynamite-able, create a 'destructable' marker for the other team
			{
				...
				{
					int hash = BG_StringHashValue( ent->targetname );
					int i;
					gentity_t* tent = NULL;
					...
					for ( i=0; i<g_triggers.count; i++ ) {
						...
						if ( Q_stricmp(tent->target, ent->targetname) ) continue;
...
void SP_func_constructible( gentity_t *ent ) {
	...
	ent->nextthink = level.time + (2*SERVER_FRAMETIME_DELTA);
}
