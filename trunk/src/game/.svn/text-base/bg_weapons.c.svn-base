...
static qboolean	BG_ParseWeaponConfig( const char *filename, weaponStatsInfo_t *wi, weapontable_t *ammoTable ) {
	...
	if ( len >= sizeof( text ) - 1 ) {
		trap_FS_FCloseFile( f );
		Com_Printf( "File %s too long\n", filename ); // fixme all, repair these warnings
		return qfalse;
	}
	...
	for ( i = WEAP_IDLE1 ; i < MAX_WP_ANIMATIONS  ; ++i ) {
		...
		wi->animationTime[i] = (int)(SECONDS_1_F/fps * (float)frames);
	...
	if ( i != MAX_WP_ANIMATIONS ) {
		Com_Printf( "Error parsing weapon animation file: %s\n", filename );
		return qfalse;
	}
...
static qboolean BG_RW_ParseDefinition( int handle, weaponStatsInfo_t *weaponInfo, weapontable_t *ammoTable/*, qboolean noparse*/ )		// For now will get needed weapon animations , but l8er it will read all stuff and has a lot of ifdefs
{
	...
	while( 1 ) {
		...
		else if( !Q_stricmp( token.string, "skillclip" )  ) {
			if( !PC_Int_Parse( handle, &ammoTable->skillclip ) ) {
				return BG_RW_ParseError( handle, "skillclip needs int" );
			}
		}
...
void BG_RegisterWeapon( int weaponNum, qboolean force ) {
	...
	switch( weaponNum ) {
		...
		case WP_JOHNSON_SCOPE:			altWeap  = qtrue;
		case WP_JOHNSON:				filename = "johnson_m1941_lmg.weap"; break;
		...
		case WP_FOOTKICK:				filename = "foot.weap"; break;
		default:						 return;
	}

	// TODO:
	// check case g_weaponscriptsDir is set, server has got custom files, client not ... (no custom weap files delivered in pk3)
#ifdef CGAMEDLL
	if (cgs.weaponScriptsDir[0] != 0) {
		handle = trap_PC_LoadSource( va( "%s/%s", cgs.weaponScriptsDir, filename ) );
	}
#else
	if (level.weaponScriptsDir[0] != 0) {
	...
	if( !handle || handle == -1) {
		...
		// final check - should never happen!
		if (!handle) {
			Com_Printf( S_COLOR_RED "BG_RegisterWeapon failed. There is no weapon configuration %s for %s \n", filename , BG_Weapons[weaponNum].name );
			// quit ?
		}
	}
