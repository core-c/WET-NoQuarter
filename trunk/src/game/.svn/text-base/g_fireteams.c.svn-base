...
void G_UpdateFireteamConfigString( fireteamData_t* ft ) {
	...
	if(!ft->inuse) {
		...
		// id -> k
		Com_sprintf(buffer, 128, "\\k\\%i\\l\\%i\\c\\%.8x%.8x", ft->ident - 1, ft->joinOrder[0], clnts[1], clnts[0]);
	}
...
void G_RegisterFireteam(/*const char* name,*/ int entityNum) {
	...
	if(entityNum < 0 || entityNum >= g_maxclients.integer) {
...
void G_AddClientToFireteam( int entityNum, int leaderNum ) {
	...
	if((leaderNum < 0 || leaderNum >= g_maxclients.integer) || !g_entities[leaderNum].client) {
		G_Error("G_AddClientToFireteam: invalid leader");
	}
	...
	for( i = 0; i < g_maxclients.integer; i++ ) {
		if( i >= MAX_FIRETEAM_MEMBERS ) {
			...
			// return; // G_ClientPrintAndReturn does it
		}
...
void G_InviteToFireTeam( int entityNum, int otherEntityNum ) {
	...
	if((otherEntityNum < 0 || otherEntityNum >= g_maxclients.integer) || !g_entities[otherEntityNum].client) {
		G_Error("G_InviteToFireTeam: other invalid client");
	}
...
void G_KickFireTeamPlayer( int entityNum, int otherEntityNum ) {
	...
	if((otherEntityNum < 0 || otherEntityNum >= g_maxclients.integer) || !g_entities[otherEntityNum].client) {
		G_Error("G_KickFireTeamPlayer: invalid target client");
	}
...
void G_ApplyToFireTeam( int entityNum, int fireteamNum ) {
	...
	if(ft->joinOrder[0] < 0 || ft->joinOrder[0] >= g_maxclients.integer) {
...
//	G_AddClientToFireteam( entityNum, ft->joinOrder[0] );
	...
	leader->client->pers.applicationEndTime =	level.time + SECONDS_20;
}
...
void G_ProposeFireTeamPlayer( int entityNum, int otherEntityNum ) {
	...
	leader->client->pers.propositionEndTime =	level.time + SECONDS_20;
#ifdef OMNIBOTS
...
void Cmd_FireTeam_MP_f( gentity_t* ent ) {
	...
	if(!Q_stricmp(command, "create")) {
		...
		if(clientnum <= 0) {
			...
			if((clientnum <= 0 || clientnum > g_maxclients.integer) || !g_entities[clientnum-1].inuse || !g_entities[clientnum-1].client) {
