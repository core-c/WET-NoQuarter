...
// NULL for everyone
// TODO: Linux: static __attribute__ ((format (printf, 2, 3))) void QDECL PrintMsg( gentity_t *ent, const char *fmt, ... ) {
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker) {
	...
	if (OtherTeam(targ->client->sess.sessionTeam) < 0) {
	...
	if (targ->client->ps.powerups[enemy_flag_pw]) {
	    // IRATA: carrier stuff isn't used see playerTeamState_s
		// gentity_t *ent;
	...
	if (flag) { // JPW NERVE -- added some more stuff after this fn
		...
		if ((VectorLengthSquared(v1) < CTF_TARGET_PROTECT_RADIUS_SQR /*SQR(CTF_TARGET_PROTECT_RADIUS)*/ ||
		  VectorLengthSquared(v2) < CTF_TARGET_PROTECT_RADIUS_SQR /*SQR(CTF_TARGET_PROTECT_RADIUS)*/ ||
		  ...
		  attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
			...
			// see IRATA: see playerTeamState_s
...
gentity_t *SelectRandomTeamSpawnPoint( int teamstate, team_t team, int spawnObjective, qboolean findClosest ) {
	gentity_t	*spot = NULL;
	gentity_t	*spots[MAX_TEAM_SPAWN_POINTS];
	gentity_t	*validspots[MAX_TEAM_SPAWN_POINTS];
	gentity_t	*enabledspots[MAX_TEAM_SPAWN_POINTS];
	gentity_t	*anyspots[MAX_TEAM_SPAWN_POINTS];
	int			count = 0, countvalid = 0, countenabled = 0, countany = 0;
	int			i = 0;
	char		*classname;
	int			classnamehash;
	vec3_t		target, farthest;
	int			closest = 0;
	float		shortest = FLT_MAX, tmp;
	...
	for ( i=0; i<g_spawns.count; i++ ) {
		...
		if (countany < MAX_TEAM_SPAWN_POINTS) anyspots[countany++] = spot;
		...
		if (countenabled < MAX_TEAM_SPAWN_POINTS) enabledspots[countenabled++] = spot;
		...
		if (countvalid < MAX_TEAM_SPAWN_POINTS) validspots[countvalid++] = spot;

		// cannot spawn inside another player
	...
	if ( !count ) {
		if ( countvalid ) return validspots[ rand() % countvalid ];
		if ( countenabled ) return enabledspots[ rand() % countenabled ];
		if ( countany ) return anyspots[ rand() % countany ];
	...
	if ( !level.numspawntargets ) {
		...
		if ( findClosest ) {
			...
			// choose a somewhat random spawn location from the available ones..
			// .. just check if it closer than X units to the selected spawnpoint,
			// so people do not spawn at the other end of the map in case multiple spawnpoints are available.
			int index;

			closest = rand() % count;

			for(i = 0; i < count; i++) {
				index = (closest + i) % count;
				VectorSubtract(farthest, spots[index]->s.origin, target);
				tmp = VectorLength(target);
...
============
*/
gentity_t *SelectCTFSpawnPoint ( team_t team, int teamstate, vec3_t origin, vec3_t angles, int spawnObjective, qboolean findClosest ) {
	gentity_t	*spot = SelectRandomTeamSpawnPoint( teamstate, team, spawnObjective, findClosest );
...
Format:
	clientNum location health powerups
...
void TeamplayInfoMessage( team_t team ) {
	...
	for (i = 0, cnt = 0; i < level.numConnectedClients; i++) {
		...
		if (player->inuse && player->client->sess.sessionTeam == team) {
			...
			*(unsigned char *)&string[ofs]	= level.sortedClients[i];
			*(int *)&string[ofs + 1]		= player->client->pers.teamState.location[0];
			*(int *)&string[ofs + 5]		= player->client->pers.teamState.location[1];
			*(int *)&string[ofs + 9]		= player->client->pers.teamState.location[2];
			*(short *)&string[ofs + 13]		= h;
			*(int *)&string[ofs + 15]		= player->s.powerups;
	...
	*(unsigned char *)&string[0]	= PACKET_S_TEAMPLAYINFOMESSAGE;	// packet type..
	*(int *)&string[1]				= cnt;							// number of entries in the packet..

	bufferedData = team == TEAM_AXIS ? level.tinfoAxis : level.tinfoAllies;
	if ( !memcmp(bufferedData, string, ofs) ) {
		// G_Printf("Warning: TeamplayInfoMessage not sent: %s\n", string); // IRATA: FIXME! enable debug and it's shown on listen server like popcorn (no one in team so far .. test with bots!)
		return;
	}
	memcpy( bufferedData, string, ofs );
...
/* IRATA ... TEST: let's have a look before we send a message ...
			messageStatus_t ms = trap_MessageStatus(level.sortedClients[i]);
			switch( ms ) {
				case MESSAGE_EMPTY:
					// we should only send here ...
					break;
				case MESSAGE_WAITING_OVERFLOW:
					// if the packet is too big, just return. It will be too big for every client..
					return;
				case MESSAGE_WAITING:
					// data is not sent yet; Wait longer..
				default:
			}
*/

			trap_SendMessage( level.sortedClients[i], (char *)&string, ofs );
		}
	}
/*
	// now check the status of the transmission..
	//!!!!! this doesn't work if this is done in the same frame !!!!!
	//!!!!! You must wait for a next frame to check the status  !!!!!
	//if( cnt > 0 ) {
		messageStatus_t	status;
		qboolean		statusOK;
		for(i = 0; i < level.numConnectedClients; i++) {
			player = g_entities + level.sortedClients[i];
			if ( player->inuse && player->client->sess.sessionTeam == team && player->client->pers.connected == CON_CONNECTED ) {	//&& !(player->r.svFlags & SVF_BOT)
				statusOK = qfalse;
				while( !statusOK ) {
					status = trap_MessageStatus( level.sortedClients[i] );
					switch( status ) {
						case MESSAGE_EMPTY:
							// data is sent..
							statusOK = qtrue;
...
void CheckTeamStatus(void) {
	...
	if (level.time - level.lastTeamLocationTime > SECONDS_1/*TEAM_LOCATION_UPDATE_TIME*/) {
		int i;
		gentity_t *ent;
...
void objective_Register(gentity_t *self) {
	...
	Info_SetValueForKey( cs, "n", numspawntargets );	//numspawntargets
...
void checkpoint_hold_think (gentity_t *self) {
	...
	self->nextthink = level.time + SECONDS_5;
}
// jpw
...
// jpw
	...
	self->nextthink = level.time + SECONDS_1;
}
...
qboolean G_readyMatchState(void) {
	if( (g_doWarmup.integer ||
		 ...
		 level.warmupTime > (level.time + SECONDS_10) ) && // 10*1000
...
void G_verifyMatchState(int nTeam) {
	if((level.lastRestartTime + SECONDS_1) < level.time && (nTeam == TEAM_ALLIES || nTeam == TEAM_AXIS) &&
...
qboolean G_allowFollow(gentity_t *ent, int nTeam) {
	...
	if(level.time - level.startTime > SECONDS_2_5) {
