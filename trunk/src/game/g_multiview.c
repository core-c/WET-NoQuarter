...
void G_smvAddView(gentity_t *ent, int pID)
{
	...
	// jaquboss - allow server to turn it off and save traffic
...
qboolean G_smvRunCamera(gentity_t *ent)
{
	...
	sprintTime = (ent->target_ent->client->pmext.sprintTime >= SECONDS_20) ? 0.0f : (1 + floor(7.0f * (float)ent->target_ent->client->pmext.sprintTime / 20000.0f));
	...
	if(tps->pm_flags & PMF_LIMBO) {
		...
		ps->ammo[id - 1]  = tps->ammo[WeaponTable[tps->weapon].ammoindex] & 0x3FF;	// 11 bits needed to cover 1500 Venom ammo
...
//	G_Printf("tps->hint: %d, dr: %d, collapse: %d\n", tps->serverCursorHint, HINT_DOOR_ROTATING, G_simpleHintsCollapse(tps->serverCursorHint, hintTime));
		...
		ps->ammoclip[id - 1]  = tps->ammoclip[WeaponTable[tps->weapon].clipindex] & 0x1FF;	// 9 bits to cover 500 Venom ammo clip
