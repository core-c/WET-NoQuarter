...
static const weap_ws_convert_t aWeapID[WP_NUM_WEAPONS] = {
	...
	{ WP_FOOTKICK,				WS_FOOTKICK			},		// 61
	{ WP_JOHNSON_SCOPE,			WS_JOHNSON			},		// 62
};
...
{
	if (ps != NULL) {
		return (( ps->persistant[PERS_AWARDS] & AWARDMASK_DEATHS ) >> AWARDSHIFT_DEATHS);
	}
	return 0;
}
...
{
	value &= 255;
	ps->persistant[PERS_AWARDS] &= ~AWARDMASK_DEATHS;
	ps->persistant[PERS_AWARDS] |= (value << AWARDSHIFT_DEATHS);
	return 0;
}
...
{
	BG_SetStatDeaths(ps, (BG_GetStatDeaths(ps) + value) );
	return 0;
}
