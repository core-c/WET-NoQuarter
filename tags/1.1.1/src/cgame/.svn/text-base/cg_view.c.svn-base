...
static void CG_OffsetFirstPersonView( void ) {
	...
	if( cg.snap->ps.weapon == WP_MOBILE_MG42_SET || cg.snap->ps.weapon == WP_MOBILE_BROWNING_SET || cg.snap->ps.weapon == WP_BAR_SET ) 
	{
		...
		origin[2] = oldZ;
	} else if( cg.snap->ps.weapon == WP_MORTAR_SET || cg.snap->ps.weapon == WP_MORTAR2_SET) {
		float yawDiff = cg.refdefViewAngles[YAW] - cg.pmext.mountedWeaponAngles[YAW];
		vec3_t forward, point;
		float oldZ = origin[2];
...
// need to slower raising speed, it was same as for crouch before..
		...
		if ( timeDelta < cg.pmext.proneRaiseTime) {
			cg.refdef_current->vieworg[2] -= cg.duckChange 
				* (cg.pmext.proneRaiseTime - timeDelta) / cg.pmext.proneRaiseTime;
		}
...
void CG_Zoom( void )
{
	...
	if ( (cg.snap->ps.pm_flags & PMF_FOLLOW) || cg.demoPlayback )
	{
		...
		if(cg.predictedPlayerState.weapon == WP_FG42_SCOPE)
	...
	else 
	{
		if (cg.zoomedBinoc)
		{
			...
			if( cg.weaponSelect == WP_FG42_SCOPE ) 
			{
...
Get a projection matrix for LOD calculation
====================
*/
// Meyer - Temp remove
/*
void CG_SetupProjection( void ) {
	...
	// well we can get znear but it is cheat protected anyway
	// we can't get zFar however at ALL! ( it is based on VIS data )
	zNear	= 3;
	zFar	= 2048;
	
	ymax = zNear * tan( cg.refdef_current->fov_y * M_PI / 360.0f );
	ymin = -ymax;

	xmax = zNear * tan( cg.refdef_current->fov_x * M_PI / 360.0f );
	xmin = -xmax;
	...
	cg.projectionMatrix[0] = 2 * zNear / width;
	cg.projectionMatrix[4] = 0;
	cg.projectionMatrix[8] = ( xmax + xmin ) / width;
	cg.projectionMatrix[12] = 0;

	cg.projectionMatrix[1] = 0;
	cg.projectionMatrix[5] = 2 * zNear / height;
	cg.projectionMatrix[9] = ( ymax + ymin ) / height;	
	cg.projectionMatrix[13] = 0;

	cg.projectionMatrix[2] = 0;
	cg.projectionMatrix[6] = 0;
	cg.projectionMatrix[10] = -( zFar + zNear ) / depth;
	cg.projectionMatrix[14] = -2 * zFar * zNear / depth;

	cg.projectionMatrix[3] = 0;
	cg.projectionMatrix[7] = 0;
	cg.projectionMatrix[11] = -1;
	cg.projectionMatrix[15] = 0;
}
*/
...
int CG_CalcViewValues( void ) {
	...
	if( !cg.showGameView ) {
		...
		if( cg_errorDecay.value > 0 ) {
			...
			f = ( cg_errorDecay.value - t ) / errorDecay;
			if ( f > 0 && f < 1 ) {
...
static void CG_RenderLocations( void )
{
	...
	for( i = 0; i < cgs.numLocations; i++ ) {
		...
		VectorSubtract(cg.refdef_current->vieworg, location->origin, dir);
		...
		if ( !CG_WorldCoordToScreenCoordFloat( location->origin, &x, &y ) )
			continue;
		...
		re.radius = 8+strlen(location->message);
		...
		CG_AddOnScreenText( va("%i:%s",location->index, location->message), location->origin);
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	for ( i=0; i< cg.svCvarCount; i++)
	{
		...
		cval = atoi( currentVal );
		val1 = atoi(cg.svCvars[i].Val1);
		val2 = atoi(cg.svCvars[i].Val2);
		...
		switch ( cg.svCvars[i].mode )
		{
			...
			case SVC_GREATER:
				if ( val1 >= cval )
					trap_Cvar_Set(cg.svCvars[i].cvarName, va("%i", val1+1));
				break;
			...
			case SVC_LOWER:
				...
				break;
			case SVC_LOWEREQUAL:
				if ( atoi(cg.svCvars[i].Val1) && atoi(cg.svCvars[i].Val1) < atoi(currentVal) )
					trap_Cvar_Set(cg.svCvars[i].cvarName, cg.svCvars[i].Val1);
				break;
			case SVC_INSIDE:
				if ( val1 ) {
			...
			case SVC_OUTSIDE:
				if ( val1 ) {
					if ( cval >= val1 ) {
						if ( !val2 || cval < val2 )
							trap_Cvar_Set(cg.svCvars[i].cvarName, va("%i", atoi(cg.svCvars[i].Val1)-1));
					}
				}
				if ( val2 ) {
					if ( cval <= val2 ) {
						if ( cval > val1 )
							trap_Cvar_Set(cg.svCvars[i].cvarName, va("%i", atoi(cg.svCvars[i].Val2)+1));
					}
				}	
				break;
			case SVC_INCLUDE:
	...
	if(cg.mvCurrentMainview != NULL && cg.snap->ps.pm_type != PM_INTERMISSION && cgs.mvAllowed ) {
		...
		// jaquboss do a point or directional light
		/*if ( cgs.spectator & SPEC_DLIGHT && cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR )
			trap_R_AddLightToScene( cg.refdef_current->vieworg, 1000, 1, 1, 1, 1, 0,  0 );
		if ( cgs.spectator & SPEC_DLIGHT_GLOBAL && cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR )
			trap_R_AddLightToScene( vec3_origin, 100, 1, 1, 1, 1, 0, REF_DIRECTED_DLIGHT );
*/
		...
		if( (cg_insanity.integer & JP_INSANITY_MORTARCAM || cg_insanity.integer & JP_INSANITY_MISSILECAM)  && cg.latestMissile && !cg.showGameView )
		{
			rectDef_t rect;
		...
		//	CG_DrawBigString( 320 - 9 * 8, 440, CG_TranslateString( "MISSILE CAM" ), 1.f );
			if ( cg.latestMissile->currentState.weapon == WP_MORTAR ||  cg.latestMissile->currentState.weapon == WP_MORTAR2 ||
				cg.latestMissile->currentState.weapon == WP_MORTAR_SET ||  cg.latestMissile->currentState.weapon == WP_MORTAR2_SET){
