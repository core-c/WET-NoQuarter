...
void CG_NewClientInfo( int clientNum ) {
	...
	while(1)
	{
		...
		case TOK_dc:
			...
			newInfo.disguiseClient = atoi(cs_value);
			break;
		case TOK_dr:
			// disguiseRank
	...
	if( clientNum == cg.clientNum ) {
		...
		for( i = 0; i < SK_NUM_SKILLS; i++ ) {
			if( newInfo.skill[i] > cgs.clientinfo[ cg.clientNum ].skill[i] ) {
				...
				int oldBits = cgs.clientinfo[cg.clientNum].skillBits[i];
...
void CG_NewPCInfo( int clientNum ) {
	...
	while(1)
	{
		...
		if ( cs_key[0] == 0 )
			break;
		g_StringToken_t token		= G_GetTokenForString(cs_key);
		switch(token)
		{
		case TOK_bd:
			// body model
			newPC->bodyIndex = atoi(cs_value);
			break;
		case TOK_hd:
			// head model
			newPC->headIndex = atoi(cs_value);
			break;
		case TOK_f:
			// head skin
			newPC->faceIndex = atoi(cs_value);
			break;
		case TOK_t:
			// torso skin
			newPC->torsoIndex = atoi(cs_value);
			break;
		case TOK_l:
			// legs skin
			newPC->legIndex = atoi(cs_value);
			break;
		case TOK_bc:
			// backpack model
			newPC->backPackIndex = atoi(cs_value);
			break;
		case TOK_bcs:
			// backpack skin
			newPC->backPackSkinIndex = atoi(cs_value);
			break;
		case TOK_bl:
			// left belt item model
			newPC->bLeftIndex = atoi(cs_value);
			break;
		case TOK_bls:
			// left belt item skin
			newPC->bLeftSkinIndex = atoi(cs_value);
			break;
		case TOK_br:
			// right belt item model
			newPC->bRightIndex = atoi(cs_value);
			break;
		case TOK_brs:
			// right belt item skin
			newPC->bRightSkinIndex = atoi(cs_value);
			break;
		case TOK_he:
			// helmet model
			newPC->helmetIndex = atoi(cs_value);
			break;
		case TOK_hs:
			// helmet skin
			newPC->helmetSkinIndex = atoi(cs_value);
			break;
		case TOK_g:
			// glasses model
			newPC->glassIndex = atoi(cs_value);
			break;
		case TOK_gs:
			// glasses skin
			newPC->glassSkinIndex = atoi(cs_value);
			break;
		default:
			continue;
		}
...
static void CG_PlayerAnimation( centity_t *cent, refEntity_t *body ) {
	...
	if( !(cent->currentState.eFlags & EF_DEAD) && !(cent->currentState.eFlags & EF_MOUNTEDTANK) && cent->pe.legs.yawing ) {
...
static void CG_PlayerSprites( centity_t *cent ) 
{
	...
	if( (cent->currentState.eFlags & EF_DEAD)
		...
		else if ( cgs.clientinfo[cg.clientNum].skillBits[SK_MEDIC] & (1<<9) )
...
static float ProjectRadius( float r, vec3_t location )
{
	...
	c = DotProduct( cg.refdef_current->viewaxis[0], cg.refdef_current->vieworg );
	dist = DotProduct( cg.refdef_current->viewaxis[0], location ) - c;
	...
	projected[0] = p[0] * cg.projectionMatrix[0] + 
		           p[1] * cg.projectionMatrix[4] +
				   p[2] * cg.projectionMatrix[8] +
				   cg.projectionMatrix[12];

	projected[1] = p[0] * cg.projectionMatrix[1] + 
		           p[1] * cg.projectionMatrix[5] +
				   p[2] * cg.projectionMatrix[9] +
				   cg.projectionMatrix[13];

	projected[2] = p[0] * cg.projectionMatrix[2] + 
		           p[1] * cg.projectionMatrix[6] +
				   p[2] * cg.projectionMatrix[10] +
				   cg.projectionMatrix[14];

	projected[3] = p[0] * cg.projectionMatrix[3] + 
		           p[1] * cg.projectionMatrix[7] +
				   p[2] * cg.projectionMatrix[11] +
				   cg.projectionMatrix[15];
...
qboolean CG_IsBestLOD( refEntity_t *ent )
{
	float radius, flod, scale, projectedRadius;
	int lod, bias;
	vec3_t mins, maxs;
	char	var[MAX_TOKEN_CHARS];

	trap_Cvar_VariableStringBuffer( "r_lodbias", var, sizeof( var ) );
	bias	= atoi( var );

	trap_Cvar_VariableStringBuffer( "r_lodscale", var, sizeof( var ) );
	scale	= atof( var );
	...
	if ( ( projectedRadius = ProjectRadius( radius, ent->origin ) ) != 0 )
	{
		if (scale > 20) scale = 20;
			flod = 1.0f - projectedRadius * scale;
	}
	else
	{
	...
	flod *= 2; // currently check out with 2 LOD models
	lod = myftol( flod );
	...
	// ok assume 2 as MAX, well this is flawed as renderer also know that model exists
	if ( lod >= 3)
		lod = 2;
...
void CG_Player( centity_t *cent )
{
	...
	vec3_t			playerOrigin;
	vec3_t			lightorigin;
	int				clientNum,i,renderfx,rank,team;
	qboolean		shadow;
	float			shadowPlane;
	...
	if ( cg_modelHilights.integer ){
		...
		if( distSquared > Square(384.f) )
		{
			...
			{
				body.hilightIntensity = 1.f * (distSquared / Square(768.f));
			}
		}
	}
...
*/
// Meyer - Temp change
	if( cent->currentState.eType != ET_CORPSE && !(cent->currentState.eFlags & EF_PLAYDEAD) /*&& CG_IsBestLOD( &head )*/ ) {
...
static void CG_SwingAngles_Limbo( float destination, float swingTolerance, float clampTolerance, float speed, float *angle, int *swinging )
{
	float	swing;
	float	move;
	float	scale;
