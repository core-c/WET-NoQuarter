...
// jet Pilot - new cvar declarations
...
vmCvar_t		r_dynamicTextures;
vmCvar_t		cg_drawHUDHead;
vmCvar_t		cg_drawObjIcons;
/*
vmCvar_t		r_dynamicX;
vmCvar_t		r_dynamicY;
vmCvar_t		r_dynamicSize;*/
vmCvar_t		cg_charSizeLarge;
vmCvar_t		cg_charSizeSmall;
vmCvar_t		cg_charSizeTiny;
...
vmCvar_t		hud_powerupX;
vmCvar_t		hud_powerupY;
vmCvar_t		hud_powerupRows;
...
vmCvar_t		cg_markDistance;	// unused
vmCvar_t		cg_insanity;		// jaquboss, predict insanity on client properly
// end of jP cvar additions
#ifdef USELOCATIONS
...
cvarTable_t		cvarTable[] = 
{
	...
	//{ &cg_gun_leftHanded,		"cg_gun_leftHanded",			"0",		CVAR_ARCHIVE	},
	
	{ &cg_test1, 				"cg_test1", 					"0", 		CVAR_CHEAT 		},
	{ &cg_test2, 				"cg_test2", 					"0", 		CVAR_CHEAT 		},
	{ &cg_test3, 				"cg_test3", 					"0", 		CVAR_CHEAT 		},
	...
	{ &pmove_fixed,				"pmove_fixed",					"0",		CVAR_USERINFO	}, // jaquboss tell server we have that on
	...
	// jet Pilot - new cgame cvars applied to cvar table

/*	
	{ &cg_drawDist,				"cg_drawDist",					"1",		CVAR_ARCHIVE	},
	{ &cg_drawDistMod,			"cg_drawDistMod",				"50",		CVAR_ARCHIVE	},
	{ &cg_drawDistUnit,			"cg_drawDistUnit",				"yards",	CVAR_ARCHIVE	},
	
	{ &cg_drawPowerups,			"cg_drawPowerups",				"1",		CVAR_ARCHIVE	},
	{ &cg_mortarMarkTime,		"cg_mortarMarkTime",			"25000",	CVAR_ARCHIVE	},
*/
	...
	// jaquboss - fixme we need to read these cvars from server
	{ &cg_insanity,				"",			"0",		0	},
	...
	{ &r_dynamicTextures,		"r_dynamicTextures",			"0",		CVAR_ARCHIVE	},
	{ &cg_drawHUDHead,			"cg_drawHUDHead",				"0",		CVAR_ARCHIVE	},
	{ &cg_drawObjIcons,			"cg_drawObjIcons",				"0",		CVAR_ARCHIVE	},

	{ &cg_charSizeLarge,        "cg_charSizeLarge",				"14",		CVAR_ARCHIVE	},
	{ &cg_charSizeSmall,        "cg_charSizeSmall",				"14",		CVAR_ARCHIVE	},
	{ &cg_charSizeTiny,			"cg_charSizeTiny",				"10",		CVAR_ARCHIVE	},


	{ &cg_markDistance,			"cg_markDistance",				"384",		CVAR_ARCHIVE	},	
	{ &cg_testVar,				"cg_testVar",					"0",		0 },
	{ &cg_drawHitbox,			"cg_drawHitbox",				"0",		CVAR_CHEAT },

	{ &hud_powerupSize,			"hud_powerupSize",				"18",		CVAR_ARCHIVE	},
	{ &hud_powerupX,			"hud_PowerupX",					"80",		CVAR_ARCHIVE	},
	{ &hud_powerupY,			"hud_PowerupY",					"388",		CVAR_ARCHIVE	},
	{ &hud_powerupRows,			"hud_PowerupRows",				"4",		CVAR_ARCHIVE	},
	...
	{ &hud_flashTime,			"hud_flashTime",				"200",		CVAR_ARCHIVE	},
	...
	{ &cg_atmosphereRenderDistance,		"cg_atmosphereRenderDistance",					"1",		CVAR_ARCHIVE }, 	
	...
	{ &cg_locations,			"cg_locations",					 "0",		 CVAR_ARCHIVE }, 	
	{ &cg_fixedFTeamSize,		"cg_fixedFTeamSize",			 "1",		 CVAR_ARCHIVE }, 	
	

	//{ &cg_classAutoexec,		"cg_classAutoexec",				 "0",		 CVAR_ARCHIVE }, 	
	//{ &cg_teamAutoexec,			"cg_teamAutoexec",				 "0",		 CVAR_ARCHIVE }, 	
	...
	{ &cg_weapAltReload,		"cg_weapAltReload",				 "1",		 CVAR_ARCHIVE },
...
qboolean CG_BackupProfile(void)
{
	...
	Com_sprintf(bakPath, sizeof(bakPath),
		"profiles/%s/backup_%s", profile, CONFIG_NAME);
	...
	if(len <= 0) {
		...
		CG_Printf("forcecvar: profile backuped to %s\n",
			bakPath);
		...
		CG_Printf("forcecvar: profile backup %s already exist\n",
				bakPath);
	}
...
void CG_RestoreProfile(void) 
{
	...
	if(len > 0) {
		...
		CG_Printf("RestoreProfile: profile restored from %s\n",
			cfgPath);
...
static void CG_RegisterSounds( void ) {
	...
	// jaquboss
	//cgs.media.jetFlySound = trap_S_RegisterSound( "sound/jet/fly.wav", qfalse );

	// OSP 
	// jaquboss reactivated this, g_fightsound actualy blows all the way
...
/*	cgs.media.spreeSound[0]			= trap_S_RegisterSound( "sound/jclient/spree.wav",						qfalse );
	cgs.media.spreeSound[1]			= trap_S_RegisterSound( "sound/jclient/rampage.wav",					qfalse );
	cgs.media.spreeSound[2]			= trap_S_RegisterSound( "sound/jclient/dominating.wav",					qfalse );
	cgs.media.spreeSound[3]			= trap_S_RegisterSound( "sound/jclient/unstoppable.wav",				qfalse );
	cgs.media.spreeSound[4]			= trap_S_RegisterSound( "sound/jclient/godlike.wav",					qfalse );
	cgs.media.spreeSound[5]			= trap_S_RegisterSound( "sound/jclient/potter.wav", 					qfalse );
	cgs.media.spreeSound[6]			= trap_S_RegisterSound( "sound/jclient/wicked.wav", 					qfalse );
	cgs.media.spreeSound[7]			= trap_S_RegisterSound( "sound/jclient/wicked.wav", 					qfalse );
	cgs.media.spreeSound[8]			= trap_S_RegisterSound( "sound/jclient/wicked.wav", 					qfalse );
	cgs.media.spreeSound[9]			= trap_S_RegisterSound( "sound/jclient/wicked.wav", 					qfalse );
*/
	...
	for ( i=0 ; i < bg_numGameSound ;  i++ )
	{
		cgs.cachedSounds[i] = trap_S_RegisterSound( gameSounds[i] , qfalse );
	}

	// FIXME: redo to do not need to play this
	cgs.media.blankSound = trap_S_RegisterSound ("sound/player/default/blank.wav", qfalse );
...
//	cgs.media.armorHintShader		= trap_R_RegisterShader( "gfx/2d/armorHint" );	
	...
	// TODO: jaquboss : actually use or scrap!
...
/*	cgs.media.waypointAttackShader	= trap_R_RegisterShaderNoMip( "sprites/waypoint_attack" );
	...
	cgs.media.waypointModelAttackSkin	= trap_R_RegisterSkin( "models/multiplayer/flagpole/waypoint_attack.skin" );
	cgs.media.waypointModelDefendSkin	= trap_R_RegisterSkin( "models/multiplayer/flagpole/waypoint_defend.skin" );
	cgs.media.waypointModelRegroupSkin	= trap_R_RegisterSkin( "models/multiplayer/flagpole/waypoint_regroup.skin" );
...
//	cgs.media.noammoShader =	trap_R_RegisterShader( "icons/noammo" );
	...
	for ( i=0 ; i < bg_numGameModels ;  i++ )
	{
		cgs.cachedModels[i] = trap_R_RegisterModel( gameModels[i] );
	}

	cgs.media.machinegunBrassModel	= trap_R_RegisterModel( "models/weapons2/shells/m_shell.md3" );
	cgs.media.panzerfaustBrassModel = trap_R_RegisterModel( "models/weapons2/shells/pf_shell.md3" );

	// jet Pilot - TODO : put shotgun shell ejecting brass here
//	cgs.media.shotgunBrassModel		= trap_R_RegisterModel( "models/weapons2/shells/sg_shell.md3" );
...
//	cgs.media.bloodExplosionShader	= trap_R_RegisterShader( "bloodExplosion" );
	//cgs.media.disguiseShader		= trap_R_RegisterShader( "sprites/disguised" ); 
...
//	cgs.media.onScreenColorShader =  trap_R_RegisterShader( "onScreenColor"); // jaquboss to blend a screen with color
...
/*	cgs.media.jetModel[0] =  trap_R_RegisterModel( "models/jets/axisJet.md3"); 
	cgs.media.jetModel[1] =  trap_R_RegisterModel( "models/jets/alliedJet.md3"); 
	cgs.media.jetSkin[0] =  trap_R_RegisterSkin( "models/jets/axisJet.skin"); 
	cgs.media.jetSkin[1] =  trap_R_RegisterSkin( "models/jets/alliedJet.skin"); 
*/
...
//	cgs.media.footModel = 	trap_R_RegisterModel( "models/weapons2/foot/v_foot.md3" );
//	cgs.media.footSkin[0] = trap_R_RegisterSkin( "models/weapons2/foot/v_foot_axis.skin" );
//	cgs.media.footSkin[1] = trap_R_RegisterSkin( "models/weapons2/foot/v_foot_allied.skin" );
...
//	cgs.media.shardCeramic2 = trap_R_RegisterModel( "models/shards/ceramic2.md3" );
	...
	// jet Pilot - register gib models
/*
	cgs.media.gibModels[0]   = trap_R_RegisterModel   ( "models/gibs/foot.mdc"		);
	cgs.media.gibModels[1]   = trap_R_RegisterModel   ( "models/gibs/foot.mdc"		);
	cgs.media.gibModels[2]   = trap_R_RegisterModel   ( "models/gibs/leg.mdc"		);
	cgs.media.gibModels[3]   = trap_R_RegisterModel   ( "models/gibs/leg.mdc"		);
	cgs.media.gibModels[4]   = trap_R_RegisterModel   ( "models/gibs/abdomen.mdc"   );
	cgs.media.gibModels[5]   = trap_R_RegisterModel   ( "models/gibs/intestine.mdc" );
	cgs.media.gibModels[6]   = trap_R_RegisterModel   ( "models/gibs/forearm.mdc"   );
	cgs.media.gibModels[7]   = trap_R_RegisterModel   ( "models/gibs/forearm.mdc"   );
	cgs.media.gibModels[8]   = trap_R_RegisterModel   ( "models/gibs/skull.mdc"     );
	cgs.media.gibModels[9]   = trap_R_RegisterModel   ( "models/gibs/abdomen.mdc"   ); 
*/
	// end of jP additions
...
// TODO: FIXME:  REMOVE REGISTRATION OF EACH MODEL FOR EVERY LEVEL LOAD
	...
	cgs.numWeaponScriptsDir = atoi(Info_ValueForKey(CG_ConfigString( CS_NOQUARTERINFO ), "NW" ) );

	if ( cgs.numWeaponScriptsDir < 0 )
		cgs.numWeaponScriptsDir = 0;
	if ( cgs.numWeaponScriptsDir > 16 )
		cgs.numWeaponScriptsDir = 16;

	for ( i = 0 ; i < cgs.numWeaponScriptsDir; i ++ ){
		Q_strncpyz(cgs.weaponScriptsDir[i], Info_ValueForKey(CG_ConfigString( CS_NOQUARTERINFO ), va("W%i",i) ), MAX_CVAR_VALUE_STRING );
	}

	for( i = /*WP_KNIFE*/ WP_NONE; i < WP_NUM_WEAPONS; i++ ) {
		// DHM - Nerve :: Only register weapons we use in WolfMP
		if ( BG_WeaponInWolfMP(i) /*|| i == WP_MAPMORTAR*/ || i == WP_NONE){
...
// jet Pilot - register new shader graphics
	
	// jP - new skill levels in limbo menu
/*
	cgs.media.limboSkillLevel1 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_1"			);
	cgs.media.limboSkillLevel2 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_2"			);
	cgs.media.limboSkillLevel3 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_3"			);
	cgs.media.limboSkillLevel4 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_4"			);
	cgs.media.limboSkillLevel5 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_5"			);
	cgs.media.limboSkillLevel6 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_6"			);
	cgs.media.limboSkillLevel7 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_7"			);
	cgs.media.limboSkillLevel8 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_8"			);
	cgs.media.limboSkillLevel9 			= trap_R_RegisterShaderNoMip( "gfx/limbo/skill_9"			);

	

	cgs.media.limboSkillLevel[0]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/zero_64b"		);
	cgs.media.limboSkillLevel[1]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/one_64b"		);
	cgs.media.limboSkillLevel[2]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/two_64b"		);
	cgs.media.limboSkillLevel[3]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/three_64b"	);
	cgs.media.limboSkillLevel[4]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/four_64b"		);
	cgs.media.limboSkillLevel[5]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/five_64b"		);
	cgs.media.limboSkillLevel[6]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/six_64b"		);
	cgs.media.limboSkillLevel[7]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/seven_64b"	);
	cgs.media.limboSkillLevel[8]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/eight_64b"	);
	cgs.media.limboSkillLevel[9]		= trap_R_RegisterShaderNoMip( "gfx/2d/numbers/nine_64b"		);	
*/
	// jP - new hint shaders
//	cgs.media.hintDanger				= trap_R_RegisterShaderNoMip( "gfx/2d/dangerhint"			);
...
//	int startat = trap_Milliseconds();
	...
	CG_initStrings();
	CG_windowInit();
// jaquboss - @fixme, crandom is always the same here
...
#ifdef __SSYSTEM__
	for ( i = 0 ; i < NUM_PLAYER_CLASSES ; i++ ){
		CG_SavePcInfo(i,TEAM_AXIS);
		CG_SavePcInfo(i,TEAM_ALLIES);
	}
#endif

	CG_RestoreProfile();

	// tjw: just in case we're still connected notify the  
	//      server that we're not just lagged out so it can  
	//      do ClientDisconnect()  
...
qhandle_t CG_GetGameModel ( int index )
{
	...
	if ( index >= bg_numGameModels )
	{
		return (cgs.gameModels[index-bg_numGameModels] ? cgs.gameModels[index-bg_numGameModels] : 0);
	} else if ( cgs.cachedModels[index] ) {
...
{
	if ( index >= bg_numGameSound )
	{
		return (cgs.gameSounds[index-bg_numGameSound] ? cgs.gameSounds[index-bg_numGameSound] : 0);
	} else if ( cgs.cachedSounds[index] ) {
