...
void ClientUserinfoChanged( int clientNum ) {
	...
	if ( ent->r.svFlags & SVF_BOT ) {
		...
		s = va( "n\\%s\\t\\%i\\skill\\%s\\c\\%i\\r\\%i\\m\\%s\\s\\%s%s\\dn\\%s\\dr\\%i\\dc\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\rn\\%i\\lc\\%i\\xp\\%s",
			client->pers.netname,
...
#endif
		s = va( "n\\%s\\t\\%i\\c\\%i\\r\\%i\\m\\%s\\s\\%s\\dn\\%s\\dr\\%i\\dc\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\ref\\%i\\rn\\%i\\lc\\%i\\xp\\%s",
			client->pers.netname, 
			client->sess.sessionTeam, 
			client->sess.playerType, 
			client->sess.rank, 
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	if( isBot ) {
		...
		if (cs_scriptname[0]) {
			Q_strncpyz( client->pers.botScriptName, cs_scriptname, sizeof( client->pers.botScriptName ) );
		...
		if (!firstTime) {
			if (cs_respawn[0] && (!Q_stricmp(cs_respawn, "NO") || !Q_stricmp(cs_respawn, "DISCONNECT"))) {
