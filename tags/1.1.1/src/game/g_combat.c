...
#define SPREELEVEL(ENT) ENT->client->sess.spreeLevel
...
void jP_UpdateSpree(gentity_t *ent, gentity_t *att, qboolean death, qboolean mapEnd)
{
	...
	if (spree <= -10 && ( !death || mapEnd ) )
	{
		SPREELEVEL(ent) = 0;

		if ( g_announcer.integer & ANNOUNCE_DEATHSPREES )
	...
	if (spree >= 5)
	{
		...
		{			
			...
			if ( g_announcer.integer & ANNOUNCE_KILLINGSPREES ){
				AP(va("chat \"^1KILLING SPREE! ^*%s ^*%s ^d(^3%d ^dkills in a row!)\"", ent->client->pers.netname, g_spreeMessage[ SPREELEVEL(ent) <= 9 ? SPREELEVEL(ent) : 9 ].string, spree ));
				te = G_TempEntity( ent->s.pos.trBase, EV_GLOBAL_SOUND );
				te->s.eventParm = G_SoundIndex(g_spreeSound[SPREELEVEL(ent) < 10 ? SPREELEVEL(ent) : 9].string);
				te->r.svFlags |= SVF_BROADCAST;
			}

			SPREELEVEL(ent)++;		// increment the current 'level' of the killing spree
		}
	}
	else if (spree <= -10)
	{
		if ((-spree % 10) == 0)		// -spree is a multiple of 10
		{			
			if ( g_announcer.integer & ANNOUNCE_DEATHSPREES ){
				AP(va("chat \"^1DEATH SPREE! ^*%s ^*%s ^d(^3%d ^ddeaths without a kill!)\"", ent->client->pers.netname,  g_humiliationMessage[ -SPREELEVEL(ent) <= 5 ? -SPREELEVEL(ent) : 5 ].string, -spree));
				te = G_TempEntity( ent->s.pos.trBase, EV_GLOBAL_SOUND );
				te->s.eventParm = G_SoundIndex(g_humiliationSound[-SPREELEVEL(ent) <= 5 ? -SPREELEVEL(ent) : 5].string);
				te->r.svFlags |= SVF_BROADCAST;
			}

			SPREELEVEL(ent)--;		// decrement the current 'level' of the killing spree
		}	
	}
	...
	if (SPREELEVEL(ent) >= 0)
		BG_SetStatBounty(&ent->client->ps, SPREELEVEL(ent));
}
