...
//	int				i, count, tagIndex, gibIndex;
...
#ifndef __SSYSTEM__
	if( !character->accModels[ACC_HAT] && !character->accModels[ACC_MOUTH2])
		return;
#endif
...

	{
		...
		if (ci->skill[SK_HEAVY_WEAPONS] >= 9 && ci->cls == PC_SOLDIER) hatIndex = ACC_MOUTH2;	// drop the shield instead
		
#ifdef __SSYSTEM__
		re->hModel = character->acc[hatIndex][0].model;
		re->customSkin = character->acc[hatIndex][0].skin[0];
#else
		re->hModel = character->accModels[hatIndex];
		re->customSkin = character->accSkins[hatIndex];
#endif
		re->fadeStartTime		= le->endTime - 1000;
		re->fadeEndTime			= le->endTime;
