...
void AddLean(vec3_t viewAngle, vec3_t point,float ammount)
{
	if(ammount)
	{
		...
		//point[2] -= fabs(ammount/2); // jaquboss to match client's view
...
// jet Pilot - server side custom smoke colors for Meyer
		...
		{
			case TEAM_ALLIES:
				Vector4Copy(cgs.smokeColor_Allies,	vc);
				Vector4Copy(cgs.smokeMod_Allies,	vm);
				break;
			case TEAM_AXIS:
				Vector4Copy(cgs.smokeColor_Axis,	vc);
				Vector4Copy(cgs.smokeMod_Axis,		vm);
				break;
			default:			
				Vector4Copy(cgs.smokeColor_Neutral,	vc);
				Vector4Copy(cgs.smokeMod_Neutral,	vm);			
		}		

		le = CG_SmokePuff( origin,		// origin
			dir,						// direction
			25+rnd*110,					// radius
...
/*		if( team == TEAM_ALLIES ) { // allied team, generate blue smoke
			le = CG_SmokePuff( origin, dir, 
				  25+rnd*110,	// width
				  rnd*0.1, rnd*0.1, rnd*0.6+0.1, rnd*0.25+.4,		//rnd*0.5+0.5, rnd*0.5+0.5, 1, 0.5,			  
	//			  rnd, rnd, rnd, 0.5,
			...
			le = CG_SmokePuff( origin, dir, 
				  25+rnd*110,	// width
				  rnd*0.6+0.1, rnd*0.1, rnd*0.1, rnd*0.25+.4,	//1.0, rnd*0.5+0.5, rnd*0.5+0.5, 0.5,
				  4800+(rand()%2800), // duration was 2800+
				  t,
				  0, 
				  0,
...
/*		le = CG_SmokePuff( origin,								// origin
			dir,												// direction
			25+rnd*110,											// radius
			rnd*cgs.smokeColor[team][0]+cgs.smokeVar[team][0],	// red scale + red base 
			rnd*cgs.smokeColor[team][1]+cgs.smokeVar[team][1],	// green scale + green base
			rnd*cgs.smokeColor[team][2]+cgs.smokeVar[team][2],	// blue scale + blue base
			rnd*cgs.smokeColor[team][3]+cgs.smokeVar[team][3],  // alpha scale + alpha base
			4800+(rand()%2800),									// duration
			t,													// start time
			0,													// fade in time
			0,													// flags
...
void CG_FireboltTrail( centity_t *ent,  int ammount, vec3_t particle_origin, qboolean attached ) {
	...
	step = 5000/ammount; // 10

	es = &ent->currentState;

	if ( cg.time - ent->trailTime > 1500 ){	// fixing instant bigexplosion bug
		ent->trailTime = cg.time;
		return;
	}
	...
	for ( ; t <= ent->trailTime ; t += step ) {
		...
		{
				if ( attached ){
			CG_ParticleExplosionTrail( "twiltb2",		particle_origin, vec3_origin, ammount+(int)(rnd*50),	3, 7+(int)(rnd*4),	qfalse	);	// fire
			CG_ParticleExplosionTrail( "blacksmokeanim", particle_origin, vec3_origin, ammount+(int)(rnd*ammount*3),	4, 6+(int)(rnd*15), qfalse );	// smoke
				} else {
			CG_ParticleExplosionTrail( "twiltb2",		lastPos, vec3_origin, ammount+(int)(rnd*100),	4, 14+(int)(rnd*8),	 qfalse	);	// fire with dlight
			CG_ParticleExplosionTrail( "blacksmokeanim", lastPos, vec3_origin, ammount+300+(int)(rnd*ammount*3),	5, 12+(int)(rnd*30), qfalse );	// smoke
				}
		}
	}
}
...
==========================
*/
void CG_RailTrail2( vec3_t color, vec3_t start, vec3_t end, qboolean local) {
	localEntity_t	*le;
	refEntity_t		*re;
	...
	le->startTime = cg.time;
	if ( local )
		le->endTime = cg.time + cg.frametime;
	else
...
==============
*/
void CG_RailTrail( vec3_t color, vec3_t start, vec3_t end, int type, qboolean local) {	//----(SA)	added 'type'
	vec3_t	diff, v1, v2, v3, v4, v5, v6;

	if(!type) {	// just a line
		CG_RailTrail2( color, start, end, local);
		return;
	}
	...
	v2[1] -= diff[1];
	v3[2] -= diff[2];
	CG_RailTrail2( color, start, v1, local);
	CG_RailTrail2( color, start, v2, local);
	CG_RailTrail2( color, start, v3, local);
	...
	v5[1] += diff[1];
	v6[2] += diff[2];
	CG_RailTrail2( color, end, v4, local);
	CG_RailTrail2( color, end, v5, local);
	CG_RailTrail2( color, end, v6, local);

	CG_RailTrail2( color, v2, v6, local);
	CG_RailTrail2( color, v6, v1, local);
	CG_RailTrail2( color, v1, v5, local);

	CG_RailTrail2( color, v2, v4, local);
	CG_RailTrail2( color, v4, v3, local);
	CG_RailTrail2( color, v3, v5, local);
...
static qboolean CG_RW_ParseWeaponLinkPart( int handle, weaponInfo_t *weaponInfo, modelViewType_t viewType )
{
	...
	/*	} else if( !Q_stricmp( token.string, "changeSkin" ) ) {
				partModel->changeSkin = qtrue;*/
		} else {
...
/*		} else if( !Q_stricmp( token.string, "changeSkin" ) ) {
				weaponInfo->weaponModel[viewType].changeSkin = qtrue;*/
	...
	while( 1 ) {
		...
		if( !Q_stricmp( token.string, "standModel" ) ) {
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				weaponInfo->weaponModel[W_PU_MODEL].model[0] = trap_R_RegisterModel( filename );
			}
		} else if( !Q_stricmp( token.string, "axisPickupModel" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected pickupModel filename" );
			} else {
				weaponInfo->weaponModel[W_PU_MODEL].model[TEAM_AXIS] = trap_R_RegisterModel( filename );
			}
		} else if( !Q_stricmp( token.string, "alliedPickupModel" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected pickupModel filename" );
			} else {
				weaponInfo->weaponModel[W_PU_MODEL].model[TEAM_ALLIES] = trap_R_RegisterModel( filename );
			}
		} else if( !Q_stricmp( token.string, "pickupSkin" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected pickupSkin filename" );
			} else {
				weaponInfo->weaponModel[W_PU_MODEL].skin[0] = trap_R_RegisterSkin( filename );
			}
		} else if( !Q_stricmp( token.string, "axisPickupSkin" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected pickupSkin filename" );
			} else {
				weaponInfo->weaponModel[W_PU_MODEL].skin[TEAM_AXIS] = trap_R_RegisterSkin( filename );
			}
		} else if( !Q_stricmp( token.string, "alliedPickupSkin" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected pickupSkin filename" );
			} else {
				weaponInfo->weaponModel[W_PU_MODEL].skin[TEAM_ALLIES] = trap_R_RegisterSkin( filename );
			}
...
//					CG_Error( "Couldn't register weapon %i (failed to parse %s)", weaponNum, filename );
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				for( i = 0; i < 4; i++ ) {
					if( !weaponInfo->deathBySound[i] ) {
						weaponInfo->deathBySound[i] = trap_S_RegisterSound( filename, qfalse );
						weaponInfo->deathBySoundNum = i+1;
						break;
					}
				}
				if( i == 4 )
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				for( i = 0; i < 4; i++ ) {
					if( !weaponInfo->deathBySoundFar[i] ) {
						weaponInfo->deathBySoundFar[i] = trap_S_RegisterSound( filename, qfalse );
						weaponInfo->deathBySoundFarNum = i+1;
						break;
					}
				}
				if( i == 4 )
		...
		/*} else if( !Q_stricmp( token.string, "ammoIcon" ) ) {
			...
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				...
				weaponInfo->missileModel[0] = trap_R_RegisterModel( filename );
			}
		} else if( !Q_stricmp( token.string, "AxisMissileModel" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected missileModel filename" );
			} else {
				weaponInfo->missileModel[TEAM_AXIS] = trap_R_RegisterModel( filename );
				weaponInfo->axiesMissileModel = qtrue;
			}
		} else if( !Q_stricmp( token.string, "AlliedMissileModel" ) ) {
			if( !PC_String_ParseNoAlloc( handle, filename, sizeof(filename) ) ) {
				return CG_RW_ParseError( handle, "expected missileModel filename" );
			} else {
				weaponInfo->missileModel[TEAM_ALLIES] = trap_R_RegisterModel( filename );
				weaponInfo->alliedMissileModel = qtrue;
			}
...
// jaquboss, make sure we have correct format but do not read keys
static qboolean CG_RW_ParseDefinition( int handle)		// For now will get needed weapon animations , but l8er it will read all stuff and has a lot of ifdefs
{
	pc_token_t	token;
...
static qboolean CG_RegisterWeaponFromWeaponFile( int handle, weaponInfo_t *weaponInfo )
{
	pc_token_t token;
	...
	while( 1 ) {
		...
		} else	if ( !Q_stricmp( token.string, "both" ) ) {  // jaquboss, do not care much about it here
			if( !CG_RW_ParseDefinition( handle ) ) {
				return qfalse;
			}
		} else	if ( !Q_stricmp( token.string, "both_altweap" ) ) {  // jaquboss, do not care much about it here
			if( !CG_RW_ParseDefinition( handle ) ) {
				return qfalse;
			}
		} else {
...
void CG_RegisterWeapon( int weaponNum, qboolean force )
{
	...
	switch( weaponNum )
	{
		...
		case WP_BAR:
		case WP_BAR_SET:				filename = "bar.weap"; break;
		case WP_BAZOOKA:				filename = "bazooka.weap"; break;		
		case WP_KNIFE_KABAR:			filename = "knife_kbar.weap"; break;	
		case WP_MOBILE_BROWNING:
		case WP_MOBILE_BROWNING_SET:	filename = "browning.weap"; break;		
		...
		case WP_FG42:
		case WP_FG42_SCOPE:				filename = "fg42.weap"; break;
...
//		case WP_MAPMORTAR:				filename = "mapmortar.weap"; break;	// do we really need this?
	...
	CG_RegisterWeaponFromWeaponFile( handle, weaponInfo );
...
static void CG_FootAnimation( int *footOld, int *foot, float *footBackLerp, playerState_t *ps ) {
	...
	CG_RunWeapLerpFrame( &cgs.clientinfo[ps->clientNum], &cg_weapons[ WP_NONE ], &cent->pe.foot, ps->ammoclip[WP_NONE], 1 );
...
static void CG_CalculateWeaponPosition( vec3_t origin, vec3_t angles ) {
	...
	//	if (!((cg.snap->ps.pm_flags & PMF_FOLLOW) && !cg.demoPlayback)){
			leanf = cg.predictedPlayerState.leanf;
	/*	} else {
		//	CG_PredictLean( &cg_entities[cg.snap->ps.clientNum], NULL, NULL,  cg.snap->ps.viewheight  );
			leanf = cg_entities[cg.snap->ps.clientNum].pe.leanDirection;
		}*/
...
//		if ( cgs.lean & LEAN_VISIBLE && cgs.lean & LEAN_SHOOT )
//		if ( cgs.clientinfo[cg.clientNum].skill[SK_LIGHT_WEAPONS] >= 9)
//			leanf *= 0.5f;
...
//		if ( cgs.clientinfo[cg.clientNum].skill[SK_LIGHT_WEAPONS] < 9)
		...
		VectorMA(origin, -leanf/4.0f, right, origin);
	}
...
static void CG_AddWeaponWithPowerups( refEntity_t *gun, int powerups, playerState_t *ps, centity_t *cent ) {
	...
	if ( cgs.clientinfo[cent->currentState.clientNum].infoValid && ((cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR && cgs.spectator & SPEC_PLAYERGLOW && !ps) || cent->currentState.eFlags & EF_GLOW ))
	{
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
	...
	// If first person view and looking through a camera
	if(ps && cg.cameraMode) 
	{
		return;
	}
	...
	// Third Person - player is on a ladder, so dont draw the weapon
	...
	// jaquboss do it totaly diffirently, we want to always see weapon on ladder if we don't have realism flag set
...
#ifdef __SELFDEFENSE__ // delete this ugly hack
		if ( cent->currentState.eFlags & EF_DEFENDING )
			CG_PositionEntityOnTag( &gun, parent, "tag_weapon2", 0, NULL);
		else
#endif
	...
	if( drawpart ) 
	{
		if( weaponNum == WP_AMMO ) 
		{
			if( ps )	// First Person
			{
				if( cgs.clientinfo[ ps->clientNum ].skill[ SK_SIGNALS ] >= 8 ) 
				{
					gun.customShader = weapon->modModels[1];
				} else if( cgs.clientinfo[ ps->clientNum ].skill[ SK_SIGNALS ] >= 1 ) 
				{
			...
			{
				if( cgs.clientinfo[ cent->currentState.clientNum ].skill[ SK_SIGNALS ] >= 8 ) 
				{
					gun.customShader = weapon->modModels[1];
				} else if( cgs.clientinfo[ cent->currentState.clientNum ].skill[ SK_SIGNALS ] >= 1 ) 
				{
					gun.customShader = weapon->modModels[0];
				}
			}
		...
		for( i = W_PART_1; i < W_MAX_PARTS; i++ ) 
		{
			...
			if( barrel.hModel ) 
			{
				...
				if( drawpart )
				{
					...
					if( weaponNum == WP_KNIFE || weaponNum == WP_KNIFE_KABAR )
					{
						if ( cg.knifeBloodVal )
						{
					...
					if( weaponNum == WP_SATCHEL_DET && i == W_PART_1 ) 
					{
						...
						if ( cgs.clientinfo[ cg.clientNum ].skill[SK_COVERTOPS] >= 7 )
							maxrange = 3000;
					...
					else if( (weaponNum == WP_MORTAR_SET || weaponNum == WP_MORTAR2_SET) && i == W_PART_3 ) 
					{
						if( ps && !cg.renderingThirdPerson && cg.predictedPlayerState.weaponstate != WEAPON_RAISING ) 
						{
...
//			}
	...
	else
	{
		...
		else if( weaponNum == WP_MOBILE_MG42 || weaponNum == WP_MOBILE_BROWNING || weaponNum == WP_BAR ) 
		{
			barrel.hModel = weapon->modModels[0];
			barrel.frame  = 1;
		...
		else if( weaponNum == WP_MOBILE_MG42_SET || weaponNum == WP_MOBILE_BROWNING_SET || weaponNum == WP_BAR_SET )
		{
			barrel.hModel = weapon->modModels[0];
			barrel.frame  = 0;
			CG_PositionEntityOnTag( &barrel, &gun, "tag_bipod", 0, NULL);
	...
	if( weaponNum == WP_FIREBOLT && ( ps || cg.renderingThirdPerson || !isPlayer ) )
	{
		CG_FireboltTrail( cent, 100, flash.origin, qtrue );
	}
	...
	else 
	{
		if ( ps || cg.renderingThirdPerson || !isPlayer)
		{
			...
			if( weaponNum != WP_PANZERFAUST && weaponNum != WP_BAZOOKA ) 	
			{	// hot smoking gun
			...
			else if( weaponNum == WP_PANZERFAUST || weaponNum == WP_BAZOOKA ) 
			{
...
void CG_AddPlayerFoot ( refEntity_t *parent, centity_t *cent, float fovOffset, playerState_t *ps )
{
	...
	weapon = &cg_weapons[WP_NONE];

	/*if ( cg.time - cent->pe.kickTime > 15000 ){
		return;
	}*/
	...
	VectorMA( foot.origin, (-8+fovOffset), cg.refdef_current->viewaxis[2], foot.origin );
	...
	/*if ( (foot.frame == 0 && foot.backlerp == 0) || (foot.oldframe == foot.frame && foot.frame >= weapon->weapAnimations[WEAP_ATTACK1].numFrames) )
		return;
	else*/
...
static qboolean CG_WeaponHasAmmo( int i )
{
	...
	if( i == WP_KNIFE || i == WP_KNIFE_KABAR || i == WP_PLIERS || i == WP_FIREBOLT)
		return qtrue;
...
int getEquivWeapon( int weapnum ) {
	...
	switch(weapnum) {
		...
		case WP_STG44:				num = WP_STEN_MKII;			break;
		case WP_FG42:				num = WP_BAR;				break;
		...
		case WP_STEN_MKII:			num = WP_STG44;				break;
		case WP_BAR:				num = WP_FG42;				break;
	}
	return num;
}
...
void CG_AltWeapon_f(void)
{
	...
	// like etpro
	if(cg.weaponSelect == WP_GRENADE_LAUNCHER || cg.weaponSelect == WP_GRENADE_PINEAPPLE  || cg.weaponSelect == WP_SMOKE_MARKER)
	{
		trap_SendConsoleCommand( "vsay_team fireinthehole\n" );
		return;
	}
	...
	{
		trap_SendConsoleCommand( "vsay_team coverme\n" );
		return;
	}

	// jet Pilot - lvl 7 light weapons give offhand grenades for some guns

/*	if (IS_OFFHAND_NADE_WEAPON(cg.weaponSelect) && cgs.clientinfo[cg.clientNum].skill[SK_LIGHT_WEAPONS] >= 7)// && cg.predictedPlayerState.powerups//pm->skill[SK_LIGHT_WEAPONS] >= 7)
	{
		trap_SendConsoleCommand( "vsay_team fireinthehole\n" );
	}
*/
	...
	if( cg.weaponSelect == WP_MORTAR || cg.weaponSelect == WP_MORTAR2 )
	{
		...
		if( !cg.predictedPlayerState.ammoclip[WP_MORTAR] && (cg.weaponSelect == WP_MORTAR ||  cg.weaponSelect == WP_MORTAR2) )
		{
			return;
		}
	...
	else if( IS_BIPOD_WEAPON(cg.weaponSelect) )
	{
		if( !(cg.predictedPlayerState.eFlags & EF_PRONE) ) 
		{
			return;
	...
	if ( cg_weapAltReload.integer ){
		if ( weapAlts[cg.weaponSelect] <= WP_NONE || !COM_BitCheck( cg.snap->ps.weapons, weapAlts[cg.weaponSelect] ))
			trap_SendConsoleCommand( "+reload;wait 10;-reload\n" );
	}
...
void CG_NextWeap(qboolean switchBanks) {
	...
	{
		num = GET_ALT_WEAPON(curweap); //getAltWeapon(curweap);	// base any further changes on the parent
		if(CG_WeaponSelectable(num)) 	
...
void CG_LastWeaponUsed_f( void ) {
	...
	if( cg.weaponSelect == WP_MORTAR2_SET ||  cg.weaponSelect == WP_MORTAR_SET || cg.weaponSelect == WP_MOBILE_MG42_SET || cg.weaponSelect == WP_MOBILE_BROWNING_SET || cg.weaponSelect == WP_BAR_SET)
	{
		return;
	}
...
//	static qboolean fallback = qtrue;
...
void CG_OutOfAmmoChange( qboolean allowforceswitch ) {
	...
	if( allowforceswitch ) {
		if( cg.weaponSelect == WP_SMOKE_BOMB ) {
			...
			return;
		} else if( cg.weaponSelect == WP_BAR_SET ) {
		...
		if (cg.weaponSelect == WP_PANZERFAUST || cg.weaponSelect == WP_BAZOOKA || cg.weaponSelect == WP_SATCHEL_DET) 
		{
			// CHRUKER: b021 - Actually use a SMG first if we got it 
			for( i = 0; i < MAX_WEAPS_IN_BANK_MP; i++ )  // CHRUKER: b021 - Make sure we don't reselect the panzer. (That took me a while to figure that out :-) 
			{	
				if (weapBanksMultiPlayer[3][i] != WP_PANZERFAUST && weapBanksMultiPlayer[3][i] != WP_BAZOOKA && CG_WeaponSelectable(weapBanksMultiPlayer[3][i])) // find a smg 
				{ 
		...
		{
			cg.weaponSelect = equiv = GET_ALT_WEAPON(cg.weaponSelect); //getAltWeapon(cg.weaponSelect);	// base any further changes on the parent
...
//			trap_S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, cgs.media.hflakWeaponSnd );
	...
	if( ent->weapon == WP_MORTAR_SET ||  ent->weapon == WP_MORTAR2_SET  )
	{
	...
	else if (	ent->weapon == WP_GRENADE_LAUNCHER	|| ent->weapon == WP_GRENADE_PINEAPPLE	||
				ent->weapon == WP_DYNAMITE			|| ent->weapon == WP_SMOKE_MARKER		||
				ent->weapon == WP_LANDMINE			|| ent->weapon == WP_SATCHEL			||
				ent->weapon == WP_TRIPMINE			|| ent->weapon == WP_SMOKE_BOMB			) 
	{ // JPW NERVE
...
/*	if ( cg_impactparticles.integer & 2 ){
		for (i=0; i<count/3; i++) {
			le = CG_AllocLocalEntity();
			re = &le->refEntity;
...
// (SA) small modification.  only do this for non-rifles (so you can see your shots hitting when you're zooming with a rifle scope)
		if (weapon == WP_FG42_SCOPE || weapon == WP_GARAND_SCOPE || weapon == WP_K43_SCOPE || (Distance( cg.refdef_current->vieworg, origin ) < cg_markDistance.integer)) {
			if(clientNum) {
...
// check for water/dirt spurt
	...
	case WP_FIREBOLT: 
		...
		tmpv2[2] -= 20;
		trap_CM_BoxTrace(&trace,tmpv,tmpv2,NULL,NULL,0,MASK_SHOT);
		CG_ParticleExplosion( "explode1", sprOrg, sprVel, 700, 40, 120, qtrue );
		CG_AddDebris( origin, dir, 280,	1400, 7 + rand()%2 );
