...
void CG_OffsetThirdPersonView( void ) 
{
	...
	if( cg.predictedPlayerState.stats[STAT_HEALTH] <= 0  
		...
		&& !(cg.predictedPlayerState.eFlags & EF_DEFENDING )
#endif
		) 
	{
...
#ifdef __JETPILOT__
	if (cg_insanity.integer & JP_INSANITY_FLYING && cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR)
		forwardScale *= 2;
#endif
...
#ifdef __SELFDEFENSE__
		&& !(cg.snap->ps.eFlags & EF_DEFENDING )
#endif
		) {
		angles[ROLL] = 40;
		angles[PITCH] = -15;
...
// need to slower raising speed, it was same as for crouch before..
	} else if( cg.time + cg.pmext.proneTime < 1100 && cgs.realism & REALISM_EXTENDED_PRONE) {
		if (timeDelta < 0)	// Ridah
			cg.duckTime = cg.time - (PRONE_TIME*1.5f);
		if ( timeDelta < (PRONE_TIME*1.5f)) {
			cg.refdef_current->vieworg[2] -= cg.duckChange 
				* ((PRONE_TIME*1.5f) - timeDelta) / (PRONE_TIME*1.5f);
		}
...
#ifdef __JETPILOT__
	&& !(cg_insanity.integer & JP_INSANITY_FLYING)
#endif
	){
		float leanf;
		vec3_t	right;
	...
	for ( i=0; i< cg.svCvarCount; i++)
	{
		...
		switch ( cg.svCvars[i].mode )
		{
			...
			case SVC_LOWEREQUAL:
				...
				break;
			case SVC_INSIDE:
				if ( atoi(cg.svCvars[i].Val1) ){
					if ( atoi(cg.svCvars[i].Val1) > atoi(currentVal))
						trap_Cvar_Set(cg.svCvars[i].cvarName, cg.svCvars[i].Val1);
				} 
				if ( atoi(cg.svCvars[i].Val2)){
					if ( atoi(cg.svCvars[i].Val2) < atoi(currentVal))
...
#ifdef __JETPILOT__
		cg.renderingThirdPerson = cg_thirdPerson.integer || (cg.snap->ps.stats[STAT_HEALTH] <= 0) || cg.showGameView		|| (cg_insanity.integer & JP_INSANITY_FLYING && cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR);
#else
