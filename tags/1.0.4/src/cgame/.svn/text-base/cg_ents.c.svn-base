...
static void CG_EntityEffects( centity_t *cent ) {
	...
	if ( cent->currentState.eType == ET_PLAYER || cent->currentState.eType == ET_CORPSE 
		...
		||(cent->currentState.eFlags & (EF_DEFENDING|EF_DEAD))
#endif
		)
		return;
...
extern void CG_FireboltTrail( centity_t *ent, int ammount, vec3_t particleOrigin, qboolean attached );
...
static void CG_Missile( centity_t *cent ) {
	...
	} else if( s1->weapon == WP_FIREBOLT ) {
		trace_t		tr;
		qboolean visible=qfalse, toofar=qfalse, behind=qfalse;
		float	 dist, dot;
		vec3_t	 dir;
			...
			if ( !behind && !toofar && cg_coronas.integer) {
			...
			trap_R_AddCoronaToScene( cent->lerpOrigin, 0.9f, 0.7f, 0.1f, 0.8f, cent->currentState.number, visible);
			}

		CG_FireboltTrail(cent, 500, cent->lerpOrigin, qfalse);
...
//----(SA)	whoops, didn't mean to check it in with the missile flare
	...
	//		CG_ScanForCrosshairDynamite(cent);			
			...
			// jet Pilot - draw dynamite's owner
			/*
	...
	if (cent->currentState.eType == ET_FP_PARTS) {
		...
		if( missileTeam == TEAM_ALLIES ) {
			...
			if ( weapon->alliedMissileModel /*missileModel[TEAM_ALLIES]*/ /*&& s1->weapon != WP_SATCHEL*/){
				ent.hModel = weapon->missileModel[TEAM_ALLIES];
			}
		} else if( missileTeam == TEAM_AXIS ) {
			ent.customSkin = weapon->missileAxisSkin;
	...
	if( cent->currentState.weapon == WP_LANDMINE ) {
		if( cgs.clientinfo[ /*cg.clientNum*/ cg.snap->ps.clientNum ].team == TEAM_SPECTATOR ) { // jaquboss, draw a mine for followed clients ( ie draw what the player we follow see )
			return;
		}
	...
	if ( cent->currentState.clientNum == cg.predictedPlayerState.clientNum && ( s1->weapon == WP_PANZERFAUST || s1->weapon == WP_BAZOOKA || s1->weapon == WP_FIREBOLT ) && cg_insanity.integer & JP_INSANITY_MISSILECAM )
		cg.latestMissile = cent;

	if ( cent->currentState.clientNum == cg.predictedPlayerState.clientNum && (s1->weapon == WP_MORTAR2 || s1->weapon == WP_MORTAR2_SET || s1->weapon == WP_MORTAR || s1->weapon == WP_MORTAR_SET   ) && cg_insanity.integer & JP_INSANITY_MORTARCAM )
		cg.latestMissile = cent;
...
static void CG_Mover( centity_t *cent ) {
	...
	if( cent->currentState.powerups == STATE_UNDERCONSTRUCTION ) {
		...
		// jet Pilot - lvl 5 engineer can see contructibles 'health' 
		/*if (cgs.clientinfo->cls == PC_ENGINEER && cgs.clientinfo->skill[SK_ENGINEER] >= 5)
		{
			Com_Printf("Doing it\n");
			trap_R_LoadDynamicShader( 
				"constructionShader_health",
				"constructionShader_health\n"
				"{\n"
			...
			trap_R_RegisterShader( "constructionShader_health" );
			trap_R_RemapShader( "sfx/construction", "constructionShader_health", "0" );
...
static void CG_TargetingPointer( centity_t *cent );
#ifdef USELOCATIONS
static void CG_DrawLocation ( centity_t *cent ); // jaquboss - location debug
#endif
...
#ifdef __JETPILOT__
		if ( cg_insanity.integer & JP_INSANITY_FLYING )
			CG_Jet( cent );
		else
#endif
		CG_Player( cent );
...
/*	case ET_CONSTRUCTIBLE_MARKER:
	...
	case ET_SMOKER:
		CG_Smoker( cent );
		break;
	case ET_TARGETING_POINTER:				// jet Pilot - Targeting pointers
		CG_TargetingPointer( cent );
		break;
	case ET_MISSILECAM:
	/*	if ( cent->currentState.effect1Time && &cg_entities[cent->currentState.effect1Time] )
			cg.latestMissile = &cg_entities[cent->currentState.effect1Time]; */
		break;
	case ET_LOCATION_MARKER:
#ifdef USELOCATIONS
		if (cg_drawLocations.integer)
		{
		//	CG_TargetingPointer( cent );
			CG_DrawLocation ( cent );
		}
#endif
		break;
	}
...
// jaquboss - location draw
#ifdef USELOCATIONS
static void CG_DrawLocation( centity_t *cent )
{
		return;

}
#endif
// jet Pilot - targeting pointers
static void CG_TargetingPointer( centity_t *cent )
{
	refEntity_t			ent;
//	centity_t			*parent;
//	centity_t			*curEnt = NULL;
//	centity_t			*bestEnt = NULL;
...
//	int					i;
#ifdef USELOCATIONS
	// don't bother if  cg_drawLocations is off
	if (!cg_drawLocations.integer && cent->currentState.eType == ET_LOCATION_MARKER)
	{
		return;
	}
...
/*
	for (i = MAX_CLIENTS, curEnt = &cg_entities[MAX_CLIENTS]; i < cg.snap->numEntities; i++, curEnt++)
	{
		if (!curEnt->currentValid) continue;		
	}
*/
//	if (cent->currentState.eventParm == 1)
	{
		...
		ent.shaderRGBA[2]= 255;
		ent.shaderRGBA[3]= 255;
		ent.customShader = cgs.media.ccMortarTargetArrow;
		trap_R_AddRefEntityToScene( &ent );
	//	trap_R_AddLightToScene(ent.origin, 200, 1, 1, 1, 1, 0, 0);
	}
}
