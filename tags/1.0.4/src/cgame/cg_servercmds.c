...
*/
static void CG_ClientCmd() {  
	char cmdName[MAX_CVAR_VALUE_STRING]; 
	char cmdValue[MAX_CVAR_VALUE_STRING]; 
	const char *s;
	//char buf[144];

	s = CG_Argv(1);  
	Q_strncpyz(cmdName, s, sizeof(cmdName)); 

	if(!*cmdName)  
		return;  

	// tjw: security check
	if(!Q_stricmp("cl_profile", cmdName))
		return;

	s = CG_Argv(2);  
	Q_strncpyz(cmdValue, s, sizeof(cmdValue)); 

/*	trap_Cvar_VariableStringBuffer(cmdName, buf, sizeof(buf));

	// cvar, use forcecvar code for it*	
	if (strlen(buf)) {
	...
	// command, just execute
		...
		trap_SendConsoleCommand(va("%s %s", cmdName, cmdValue ));
//	}
}  
...
//		cg.scores[i].playerClass = atoi(	CG_Argv( offset + 5 + (j*7)));
		cg.scores[i].ready		 = atoi(	CG_Argv( offset + 5 + (j*7)));
...
// jet Pilot - CG_Parse Locations (obsolete)
#if 0
static void CG_ParseLocations( void ) 
{
	int		i;
//	int		client;
	int		numLocations;

	numLocations = atoi( CG_Argv( 1 ) );

	for ( i = 0; i < numLocations; i++ )
	{
		cgs.location[i].index = atoi(CG_Argv(i * 2 + 2));
		
		Q_strncpyz(cgs.location[i].message, CG_Argv(i * 2 + 3), strlen(cgs.location[i].message));
	}
}
#endif
...
#ifdef USELOCATIONS
		cgs.clientinfo[ client ].locationIndex[0] =		atoi( CG_Argv( i * NUMARGS + 8 ) );
		cgs.clientinfo[ client ].locationIndex[1] =		atoi( CG_Argv( i * NUMARGS + 9 ) );		
	//	cgs.clientinfo[ client ].leanAmount =			atof( CG_Argv( i * NUMARGS + 9 ) );
		strcpy(cgs.clientinfo[ client ].locationName, CG_Argv( i * NUMARGS + 10 ) );
#endif
	}
}
...
void CG_UpdateForceCvars( void ){
	...
	info = CG_ConfigString( CS_FORCECVAR );
...
void CG_ParseNQinfo( void ) {
	...
	info = CG_ConfigString( CS_NOQUARTERINFO );

	// Jaybird
	cgs.jp_keepAwards = atoi(Info_ValueForKey(info, "JPKA"));

	/*cgs.doubleJump = atoi(Info_ValueForKey(info, "g_doubleJump"));
	cgs.doubleJumpHeight = atof(Info_ValueForKey(info, "g_doubleJumpHeight"));
	cgs.medics = atoi(Info_ValueForKey(info, "g_medics"));*/ // jaquboss - WTF is this for?
	cgs.doubleJump = atoi(Info_ValueForKey(info, "DJ")); // use shortcuts instead
	cgs.doubleJumpHeight = atof(Info_ValueForKey(info, "DJH"));

	// jaquboss - predict hitboxes

	cgs.hitboxes = atoi(Info_ValueForKey(info, "HB"));

	// jaquboss - weapon limits

	cgs.maxPanzers = atoi(Info_ValueForKey(info, "TMP"));
	cgs.maxMG42s = atoi(Info_ValueForKey(info, "TMMG"));
	cgs.maxFlamers = atoi(Info_ValueForKey(info, "TMF"));
	cgs.maxMortars = atoi(Info_ValueForKey(info, "TMM"));
	cgs.maxGrenLaunchers = atoi(Info_ValueForKey(info, "TMGL"));
	cgs.maxVenoms = atoi(Info_ValueForKey(info, "TMV"));
	cgs.maxShotguns = atoi(Info_ValueForKey(info, "TMSG"));
...
//	cgs.lean = atoi(Info_ValueForKey(info, "L"));
...

	{
		...
		int oldCount;
		char	oldDir[16][MAX_CVAR_VALUE_STRING];

		oldCount = cgs.numWeaponScriptsDir;

		cgs.numWeaponScriptsDir = atoi(Info_ValueForKey(info, "NW" ));

		for ( i = 0 ; i < oldCount ; i++ )
			Q_strncpyz(oldDir[i], cgs.weaponScriptsDir[i], MAX_CVAR_VALUE_STRING );
 
		for ( i = 0 ; i < cgs.numWeaponScriptsDir ; i++ )
			Q_strncpyz(cgs.weaponScriptsDir[i], Info_ValueForKey(info, va("W%i", i ) ), MAX_CVAR_VALUE_STRING );

		if ( cgs.numWeaponScriptsDir != oldCount )
			modifed = qtrue;

		if ( !modifed ){
			for ( i = 0 ; i < cgs.numWeaponScriptsDir ; i++ ){
				if ( Q_stricmp(cgs.weaponScriptsDir[i], oldDir[i]) != 0 ){
					modifed = qtrue;
					break;
				}
			}
		...
		if ( modifed ){
			for( i = WP_NONE; i < WP_NUM_WEAPONS; i++ ) {
				if( BG_WeaponInWolfMP( i ) || i == WP_NONE ){
	...
	// jet Pilot - read configstring for smoke colors
	// axis color
	buffer = Info_ValueForKey(info, "RS");
	token = strtok(buffer, " ");
	...
	Vector4Copy(col, cgs.smokeColor_Axis);
	//axis mod
	buffer = Info_ValueForKey(info, "RM");
	token = strtok(buffer, " ");
	...
	Vector4Copy(col, cgs.smokeMod_Axis);
	// allied color
	buffer = Info_ValueForKey(info, "BS");
	token = strtok(buffer, " ");
	...
	Vector4Copy(col, cgs.smokeColor_Allies);
	// allied mod
	buffer = Info_ValueForKey(info, "BM");
	token = strtok(buffer, " ");
	...
	Vector4Copy(col, cgs.smokeMod_Allies);
	// neutral color
	buffer = Info_ValueForKey(info, "BS");
	token = strtok(buffer, " ");
	...
	Vector4Copy(col, cgs.smokeColor_Neutral);
	// neutral mod
	buffer = Info_ValueForKey(info, "BM");
	token = strtok(buffer, " ");
	...
	Vector4Copy(col, cgs.smokeMod_Neutral);

	
/*
	CG_Printf("Red %.2f+%.2f Green %.2f+%.2f Blue %.2f+%.2f Alpha %.2f+%.2f",
		cgs.smokeColor_Allies[0],	cgs.smokeMod_Allies[0],
		cgs.smokeColor_Allies[1],	cgs.smokeMod_Allies[1],
		cgs.smokeColor_Allies[2],	cgs.smokeMod_Allies[2],
		cgs.smokeColor_Allies[3],	cgs.smokeMod_Allies[3]); */
...
void CG_ParseWolfinfo( void ) {
	...
	cg_weaponsCvar.integer = atoi( Info_ValueForKey( info, "g_weapons" ) );
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
		...
		CG_ParseOIDInfo( num );
	} else if(num == CS_NOQUARTERINFO) {  
		CG_ParseNQinfo();  
	} else if ( num == CS_SKILLLEVELS ) {
		CG_UpdateSkills();  
	} else if ( num == CS_FORCECVAR ) {
		CG_UpdateForceCvars();  
	} else if ( num == CS_SVCVAR ) {
		CG_UpdateSvCvars();  
	} else if ( num == CS_CONFIGNAME ) {
		//Q_strncpyz( cgs.configName, str, sizeof( cgs.configName ) );
	}
#ifdef __SSYSTEM__
	else if ( num >= CS_PCINFO && num < CS_PCINFO+MAX_CLIENTS) {
		CG_NewPCInfo( num - CS_PCINFO );  
	}
#endif
}
...
const char *CG_GetLocationName(char *dest, int index)
{
...
//	int				j		 = 0;
	static int		count	 = 0;
//	int				total	 = 0;
	static char		curMap[32] = " ";

	if (Q_stricmp(curMap, cgs.rawmapname) != 0)
	{
		first = qtrue;
		count = 0;
		Com_Printf("^9New map loaded [^2%s^9], rebuilding location names (old map was ^2%s^9)\n", cgs.rawmapname, curMap);
	}
	
	// if it's the first call, or if the map name has changed
	if (first)  
	{
		strcpy(curMap, cgs.rawmapname);

		cString = (char *)CG_ConfigString(CS_LOCATIONS);
		tok = strtok(cString, "/\\");

		if (tok != NULL)
		{			
			Com_Printf("[%d] %s\n", i, tok);
			strcpy(locationName[i++], tok);
			while (tok != NULL)
			{
				tok = strtok(NULL, "/\\");
				if (tok != NULL)
		...
		{	// Nothing in the configstring, return NULL
			return NULL;
		}
	}
	if ( count < 1 )
	...
	if (index > count || index < 0) return NULL;//index = 0; // jaquboss - do not give location zero if we are using bad location, simply use coords, shouldn't happen anyway, maybe we could use it to print unknown

	if (dest != NULL)
	{
		strcpy(dest, locationName[index]);
	} else {
	return NULL;
	}
	...
	if ( CG_GetVoiceChat( voiceChatList, cmd, &snd, &sprite, &chat, randomNum ) ) {
		...
		if ( mode == SAY_TEAM || !cg_teamChatsOnly.integer ) {
			...
			if( mode != SAY_ALL )
			{
				...
				if (cg_locations.integer & LOC_VCHAT){
					...
					if ( cg_locations.integer & LOC_SHOWCOORDS_VCHAT && locValid )
					{
						locStr = va("%s ^3(%s)", locStr, BG_GetLocationString( loc ) );
					}
				} else {
...
//	BG_cleanName(cgs.clientinfo[gs->nClientID].name, strName, sizeof(strName), qfalse);
	...
	if(weaponMask != 0) 
	{
		...
		if(gs->fHasStats)
		{
			...
			int darwin	  = BG_GetStatDarwin(&cg.snap->ps);	//cg.snap->ps.persistant[PERS_DARWINDEATHS];		
			int goombas   = BG_GetStatGoombas(&cg.snap->ps);		//cg.snap->ps.persistant[PERS_GOOMBAS];	
			...
			// jet Pilot - darwin and goombas in stats popup
			Q_strncpyz(gs->strExtra[2], va("Darwin Award: %-6d      Goombas: %d", darwin, goombas), sizeof(gs->strExtra[2]));
		}
	}
...
static void CG_ServerCommand( void ) {
	...
	if ( !Q_stricmp( cmd, "vschat" ) ){
		...
		wav = va("%s%s", cgs.clientinfo[clNum].team == TEAM_AXIS ? "axis" : "allies" , systemMessages[msgNum].voiceScript );

		CG_SoundPlaySoundScript( wav, NULL, -1,  qtrue );
		CG_AddToTeamChat( systemMessages[msgNum].chatString, clNum, qfalse );
		CG_Printf( "%s\n", systemMessages[msgNum].chatString );
		return;
	}
	...
	if ( !Q_stricmp( cmd, "tchat" ) ) {
		...
		origin[0] = loc[0] = atoi( CG_Argv( 4 ) );
		origin[1] = loc[1] = atoi( CG_Argv( 5 ) );
		origin[2] = atoi( CG_Argv( 6 ) );

		//Q_strncpyz( text, s, MAX_SAY_TEXT );


		if (cg_locations.integer & LOC_TCHAT){
			qboolean locValid = qtrue;
			...
			if ( cg_locations.integer & LOC_SHOWCOORDS_TCHAT && locValid )
			{
				locStr = va("%s ^3(%s)", locStr, BG_GetLocationString( loc ) );
			}
		} else {
...
//GS Copied in code from old source for mu_start, mu_play & mu_stop
	...
	// jet Pilot - !clientcmd
	if ( !Q_stricmp(cmd, "clientcmd"))
	{
	/*	char tmp[128];
		Q_strncpyz(tmp, va("%s %s", CG_Argv(1), CG_Argv(2)), 127);
		CG_Printf("Clientcmd recieved from server : %s\n", tmp);
		trap_SendConsoleCommand(tmp);
		return;*/
