...
// we should always pickup those if we have them
	// if(item->giType == IT_WEAPON && (item->giTag == WP_PANZERFAUST || item->giTag == WP_BAZOOKA || item->giTag == WP_VENOM)) return;
...
#ifdef VISIBLE_TRIGGERS
			CG_RailTrail( tv( 0.f, 1.f, 0.f), mins, maxs, 1, qtrue );
#endif // VISIBLE_TRIGGERS
...
// jet Pilot - Team Aura Effects
	...
	// first reset auras
	for (i=0; i<5; i++){
	cg_pmove.auraEffects[i] = 0;
	cg.auraEffects[i] = 0;
	}

	// Set all aura levels to our class level
	for ( i = 0 ; i < MAX_CLIENTS; i++ ) 
	{
		...
		// dont aura yourselfs
		if( cg.snap->ps.clientNum == cg_entities[i].currentState.clientNum )
		{
			continue;
		}

		// dont aura from dead
		if( cg_entities[i].currentState.dl_intensity <= 0)
		{
			continue;
		}

		switch (cgs.clientinfo[i].cls)
		{
			case PC_SOLDIER:				
				nLevel = cgs.clientinfo[i].skill[SK_SOLDIER];				
				break;
			case PC_MEDIC:
				nLevel = cgs.clientinfo[i].skill[SK_MEDIC];
				break;
			case PC_ENGINEER:
				nLevel = cgs.clientinfo[i].skill[SK_ENGINEER];
				break;
			case PC_FIELDOPS:
				nLevel = cgs.clientinfo[i].skill[SK_FIELDOPS];
				break;
			case PC_COVERTOPS:
				nLevel = cgs.clientinfo[i].skill[SK_COVERTOPS];
				break;
			default:
				continue;
		}

		if ((nLevel > cg_pmove.auraEffects[cgs.clientinfo[i].cls] ) && (nLevel >= 5) && trap_R_inPVS(cg.snap->ps.origin, cg_entities[i].lerpOrigin)){
			distance = Distance(cg_entities[i].lerpOrigin, cg.snap->ps.origin);
			
			if (distance <= 512 )
			{				
				cg_pmove.auraEffects[cgs.clientinfo[i].cls] = nLevel;
				cg.auraEffects[cgs.clientinfo[i].cls] = nLevel;
			}
		}
	}
...
//unlagged - optimized prediction 
	...
	for ( cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++ ) {
		...
		/*if ( cg_pmove.pmove_fixed ) { // jaquboss, this will make players view totally messed..
			// rain - added tracemask
		...
		if ( cg.predictedPlayerState.commandTime == oldPlayerState.commandTime ) {
			...
			if( BG_PlayerMounted( cg_pmove.ps->eFlags ) ) {
				...
				if( (cg.predictedPlayerState.eFlags & EF_PRONE) || cg.weaponSelect == WP_MORTAR_SET || cg.weaponSelect == WP_MORTAR2_SET )
...
//		memcpy( &pmext, &cg.pmext, sizeof(pmoveExt_t) );	// grab data, we only want the final result
	...
	else
	{
		...
		if ( cg_rtcwShake.integer ){
			// up/down
			cg.predictedPlayerState.viewangles[0] += sin(M_PI * 8 * x + cg.cameraShakePhase) * x * 4.5f * cg.cameraShakeScale; 
			// left/right
			cg.predictedPlayerState.viewangles[1] +=  sin(M_PI * 15 * x + cg.cameraShakePhase) * x * 4.0f * cg.cameraShakeScale;
		} else {
			// move
