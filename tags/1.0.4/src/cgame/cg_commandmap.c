...
//					CG_DrawStringExt_Shadow( string_pos[0], string_pos[1], ci->name, colorWhite, qfalse, 1, 8, 12, 0 );
				...
				if(cent->voiceChatSpriteTime > cg.time) {
					/*
					// jet Pilot - make chat icon fade over the last 2 seconds
					float fadeTime;
					fadeTime = (float)(cent->voiceChatSpriteTime - cg.time);

					if (fadeTime <= 2000.0f)
					{
						vec4_t chatFade = { 1.0f, 1.0f, 1.0f, 1.0f };
						chatFade[3] = ( fadeTime / 2000.f );
						trap_R_SetColor( chatFade );
						CG_DrawPic( icon_pos[0] + 12, icon_pos[1], icon_extends[0] * 0.5f, icon_extends[1] * 0.5f,  cent->voiceChatSprite);
						trap_R_SetColor( NULL );
					}
					else
