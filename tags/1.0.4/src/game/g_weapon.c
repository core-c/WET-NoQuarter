...
qboolean jP_UnDoChargeCost(gentity_t *ent, int skill, float costArray[10])
{
	int skillLvl	= 0;
	int chargeTime	= 0;
	...
	skillLvl   = ent->client->sess.skill[skill];
	switch (skill)
	{
		case SK_SOLDIER:
...
//	if ((level.time - ent->client->ps.classWeaponTime) >= (costArray[skillLvl] * chargeTime))
//	{
		ent->client->ps.classWeaponTime -= (costArray[skillLvl] * chargeTime);
//		return qtrue;
//	}
...
qboolean jP_DoChargeCost(gentity_t *ent, int skill, float costArray[10])
{
	int skillLvl	= 0;
	int chargeTime	= 0;
...
//		if( DotProduct( eforward, pforward ) > 0.6f )		// from behind(-ish)
		{
			...
			if (
				traceEnt->health > 0 &&
				ent->client->sess.skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 4
...
//	medicLevel = ent->client->sess.skill[SK_FIRST_AID];
...
//	}
	...
	jP_DoChargeCost(ent, SK_MEDIC, chargeCost_Health);
...
void Weapon_MagicAmmo_Ext(gentity_t *ent, vec3_t viewpos, vec3_t tosspos, vec3_t velocity)
{
	...
	jP_DoChargeCost(ent, SK_SIGNALS, chargeCost_Ammo);
/*
...
*/
	if ( ent->client->sess.skill[SK_SIGNALS] >= 8 )
...
qboolean ReviveEntity(gentity_t *ent, gentity_t *traceEnt, qboolean resuscitation)
{
	vec3_t		org;
	trace_t		tr;
	int			healamt, headshot, pantsed, glowing, oldweapon,oldweaponstate,oldclasstime=0;
...
//	gentity_t	*traceEnt,
	...
	if ( ent )
	healamt *= ent->client->sess.skill[SK_FIRST_AID] >= 3 ? 1 : 0.5f;
		...
		for( e = 0; e < blockingEntities; e++ ) {
			...
			if( block->client || block->s.eType == ET_CORPSE ) 
			{
				...
				if (block->client)
				{
					...
					block->client->ps.persistant[PERS_DARWINDEATHS]++;
				}
			} 
...
qboolean Engineer_MG42( gentity_t *ent, gentity_t *traceEnt )
{
	...
	ent->client->ps.classWeaponTime += chargeCost_Repair[ENGINEER_LEVEL] * 150;
...
*/
					...
					// jet Pilot - use charge cost array (Landmine)
					jP_UnDoChargeCost(ent, SK_ENGINEER, chargeCost_Landmine);
			//		ent->client->ps.classWeaponTime -= chargeCost_Landmine[ENGINEER_LEVEL] * 
			//			level.engineerChargeTime[ent->client->sess.sessionTeam-1];
...
qboolean weapon_checkAirStrike( gentity_t *ent ) 
{
	...
	if ((g_friendlyFire.integer & FRIENDLY_FIRE_DISABLED || g_friendlyFire.integer == 0 ) && ent->parent->client && ent->parent->client->sess.sessionTeam == TEAM_SPECTATOR)
	{
	...
	if( ent->s.teamNum == TEAM_AXIS ) {
		if( level.numActiveAirstrikes[0] > 6 || !G_AvailableAirstrikes( ent->parent ) ) {
			...
			if( g_weapons.integer & WPF_TM_AIRSTRIKE_RESTORE_FULL ) {
				jP_UnDoChargeCost( ent->parent, SK_FIELDOPS, chargeCost_Airstrike );
			} else if( g_weapons.integer & WPF_TM_AIRSTRIKE_RESTORE_HALF ) {
				float chargeCost[NUM_SKILL_LEVELS];
				int	i;

				for ( i=0;i<NUM_SKILL_LEVELS;i++ )
				{
					chargeCost[i] = chargeCost_Airstrike[i]/2;
				}
				jP_UnDoChargeCost( ent->parent, SK_FIELDOPS, chargeCost );
			}
...
//	}
	...
	if (!jP_DoChargeCost(ent, SK_SIGNALS, chargeCost_Artillery))
	{
		return;
	}
...
//	{
		if( !G_AvailableArty( ent ) ) 
		{
			...
			if( g_weapons.integer & WPF_TM_AIRSTRIKE_RESTORE_FULL ) {
				jP_UnDoChargeCost( ent, SK_FIELDOPS, chargeCost_Artillery );
			} else if( g_weapons.integer & WPF_TM_AIRSTRIKE_RESTORE_HALF ) {
				float chargeCost[NUM_SKILL_LEVELS];
				int	i;
				...
				for ( i=0;i<NUM_SKILL_LEVELS;i++ )
				{
					chargeCost[i] = chargeCost_Artillery[i]/2;
				}
				jP_UnDoChargeCost( ent, SK_FIELDOPS, chargeCost );
			}
...
// i == 1->4 is regular explosives
	...
	// jet Pilot - use array 
	for( i = 0; i < /*count*/artilleryBombs[ent->client->sess.skill[SK_SIGNALS]]; i++ ) 
	{
		bomb						= G_Spawn();
		bomb->think					= G_AirStrikeExplode;
		bomb->s.eType				= ET_MISSILE;
...
void Bullet_Fire (gentity_t *ent, float spread, int damage, qboolean distance_falloff)
{
	...
	switch( ent->s.weapon ) 
	{
		...
		case WP_STEN_MKII:
		case WP_SHOTGUN:			
			// jet Pilot - Use array (Spread Scale)
			spread *= lightWeapon_Spread[ent->client->sess.skill[SK_LIGHT_WEAPONS]];
			/*
	...
	if ( g_realHead.integer & REALHEAD_SHOOT  && ent->client ) // jaquboss we have tags so we may be much more accurate now...
	Bullet_Fire_Extended(ent, ent, muzzleEffect, end, spread, damage, distance_falloff, 0); // shoot from real gun positon
		else
...
qboolean Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker, vec3_t start, vec3_t end, float spread, int damage, qboolean distance_falloff, int reflectNum) 
{
	...
	if ( traceEnt->takedamage && traceEnt->client )
	{
		...
		{	
			...
			*/
			...
			G_RailBox( traceEnt->r.currentOrigin, traceEnt->r.mins, maxs, tv(0.f,0.f,1.f));
			if (g_antilag.integer) {
				// josh: restore after
	...
	else 
	{
		...
		if(g_debugBullets.integer <= -2) {	// show hit thing bb
			...
			VectorCopy(b2, bboxEnt->s.origin2);
			bboxEnt->s.dmgFlags = 1;	// ("type")*/
			G_RailBox( traceEnt->r.currentOrigin, traceEnt->r.mins, traceEnt->r.maxs, tv(0.f,1.f,0.f));
		}
	...
	if ( traceEnt->takedamage ) 
	{
		...
		G_Damage( traceEnt, attacker, attacker, forward, tr.endpos, damage, ( distance_falloff ? DAMAGE_DISTANCEFALLOFF : 0 ), mod );
...
void G_RealGunPos( gentity_t *ent, weapon_t weapon, vec3_t muzzlePoint )
{
	grefEntity_t refent;
	float		gunUp, gunFwd;
	...
	switch ( weapon ) // guess this doesn't work anyway
	{
	case WP_COLT:
	case WP_LUGER:
	case WP_AKIMBO_COLT:
	...
	if ( BG_AkimboFireSequence( weapon, ent->client->ps.ammoclip[BG_FindClipForWeapon(weapon)], ent->client->ps.ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(weapon))] ) || weapon == WP_MOBILE_MG42 )
	mdx_tag_position( ent, &refent, muzzlePoint, "tag_weapon2", gunUp, gunFwd );
		else
	mdx_tag_position( ent, &refent, muzzlePoint, "tag_weapon", gunUp, gunFwd );
...
{
	if ( g_realHead.integer & REALHEAD_SHOOT  && ent->client && weapon ) { // jaquboss we have tags so we may be much more accurate now...
		G_RealGunPos( ent, weapon, muzzlePoint);
		return;
	}
...
/*	if ( g_realHead.integer & REALHEAD_SHOOT  && ent->client && attack ) { // jaquboss we have tags so we may be much more accurate now...
		G_RealGunPos( ent, ent->s.weapon, muzzlePoint);
		return;
	}*/
	...
	} else // jaquboss revived hack, so revive will work a bit better
	if(!attack && ent->client->ps.viewangles[PITCH] > 30 && (ent->client->ps.eFlags & EF_CROUCHING)) 
	{
			muzzlePoint[2] += 30;
	}
	else 
...
qboolean jP_CheckDisguise( gentity_t *ent )
{
	...
	int		 covertopsLevel = ent->client->sess.skill[SK_COVERTOPS];
	float	 testCost;
...
//			SP("^dDisguise Test : ^9Kept Disguise ^7(Stealthy Weapon)^9.\n");
			return qtrue;			
		...
		case WP_GRENADE_PINEAPPLE:
			...
			break;
		default:	
			//Com_Printf("^9jP_CheckDisguise() ^7:^9Lost Disguise ^3(Unstealthy Weapon)^9.\n");
			//return qfalse;
			stealthy = qfalse;				
	}
	...
	if (!stealthy)
	{		
		testCost = disguiseTestCost[covertopsLevel] * level.covertopsChargeTime[ent->client->sess.sessionTeam-1];

		if (jP_DoChargeCost(ent, SK_COVERTOPS, disguiseTestCost))
		{
#ifdef DEBUG
			SP("^dDisguise Test : ^9Kept Disguise ^7(Passed Test)^9.\n");
#endif
			return qtrue;
		}
		else
		{
#ifdef DEBUG
			SP("^dDisguise Test : ^9Lost Disguise ^7(Failed Test)^9.\n");
#endif
			return qfalse;
		}
	}
#ifdef DEBUG
	SP("^dDisguise Test : ^9Kept Disguise ^7(No Witnesses)^9.\n");
#endif
	return qtrue;
}
...
void FireWeapon( gentity_t *ent ) 
{
	...
	if( ent->client->ps.pm_type == PM_DEAD 
		...
		&& !(ent->client->ps.eFlags & EF_DEFENDING)
#endif
		) 
	{
		return;
	...
	if( (ent->client->ps.eFlags & EF_ZOOMING) && (ent->client->ps.stats[STAT_KEYS] & (1 << INV_BINOCS)) ) 
	{
		...
		{
			// jet Pilot - allow firing while leaning if Light Weapons skill >= 9
//			if( !(ent->client->ps.leanf) || (g_lean.integer & LEAN_VISIBLE && g_lean.integer & LEAN_SHOOT )) 
			if( !(ent->client->ps.leanf) /*|| (ent->client->sess.skill[SK_LIGHT_WEAPONS] >= 9)*/)
			{
...
*/
	...
	switch( ent->s.weapon ) 
	{
		...
		case WP_PLIERS:
			...
			break;
		case WP_SMOKE_MARKER:
			jP_DoChargeCost(ent, SK_SIGNALS, chargeCost_Airstrike);
		...
		case WP_MEDIC_ADRENALINE:
			jP_DoChargeCost(ent, SK_FIRST_AID, chargeCost_Adrenaline);
			//ent->client->ps.classWeaponTime = level.time;
		...
		case WP_VENOM:
			...
			break;
		case WP_BAZOOKA:
			jP_DoChargeCost(ent, SK_SOLDIER, chargeCost_Bazooka);
		...
		case WP_PANZERFAUST:
			jP_DoChargeCost(ent, SK_SOLDIER, chargeCost_Panzer);
		...
		case WP_GPG40:
		case WP_M7:
			jP_DoChargeCost(ent, SK_ENGINEER, chargeCost_Riflenade);			
			pFiredShot = weapon_gpg40_fire( ent, ent->s.weapon );
			break;
		case WP_MORTAR2_SET:
		case WP_MORTAR_SET:
			jP_DoChargeCost(ent, SK_SOLDIER, chargeCost_Mortar);			
		...
		case WP_SATCHEL:
			jP_DoChargeCost(ent, SK_COVERTOPS, chargeCost_Satchel);
			pFiredShot = weapon_grenadelauncher_fire( ent, ent->s.weapon );
			break;

		case WP_DYNAMITE:
			jP_DoChargeCost(ent, SK_ENGINEER, chargeCost_Dynamite);
			pFiredShot = weapon_grenadelauncher_fire( ent, ent->s.weapon );
			break;

		case WP_LANDMINE:
			jP_DoChargeCost(ent, SK_ENGINEER, chargeCost_Landmine);
			pFiredShot = weapon_grenadelauncher_fire( ent, ent->s.weapon );
			break;
		case WP_SMOKE_BOMB:
			jP_DoChargeCost(ent, SK_COVERTOPS, chargeCost_Smoke);				
			pFiredShot = weapon_grenadelauncher_fire( ent, ent->s.weapon );
			break;
...
// Written: 12/26/2002
//
qboolean IsSilencedWeapon (	int weaponType )
{
	// Local Variables ////////////////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////
...
void Targeting_Pointer_Think( gentity_t *self )
{
	...
	CalcMuzzlePoint(self->parent, WP_NONE, forward, right, up, start);
...
void G_CanisterKick( gentity_t *ent )
{
	...
	if(g_debugBullets.integer) {
		...
		tent->s.dmgFlags = 1;*/
		G_RailBox( origin, mins, maxs, tv(1.f,0.f,0.f));
