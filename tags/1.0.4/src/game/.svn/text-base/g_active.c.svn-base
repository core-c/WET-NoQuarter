...
qboolean ClientInactivityTimer( gclient_t *client ) {
	// OSP - modified
	if( ( level.numConnectedClients < level.maxclients ) || ( g_inactivity.integer == 0 && client->sess.sessionTeam != TEAM_SPECTATOR ) || ( g_spectatorInactivity.integer == 0 && client->sess.sessionTeam == TEAM_SPECTATOR ) ) {
...
#define CAN_REGENERATE ( (client->sess.playerType == PC_MEDIC /*|| jp_insanity.integer & JP_INSANITY_REGENERATE*/) && !(client->ps.eFlags & EF_DEAD) && !(client->ps.eFlags & EF_POISONED) /*&& !(jp_insanity.integer & JP_INSANITY_VAMPIRE)*/)
...
//	client->latched_wbuttons |= client->wbuttons & ~client->oldwbuttons;	// FIXME:? (SA) MP method
  ...
  if( client->ps.stats[STAT_HEALTH] <= 0 && !(client->ps.eFlags & EF_PLAYDEAD)) {
	...
	/*	VectorCopy(ent->r.currentOrigin, b1);
		...
		G_RailBox( ent->r.currentOrigin, ent->r.mins, maxs, tv(0.f,0.f,1.f));

		head = G_BuildHead(ent);
		G_RailBox( head->r.currentOrigin, head->r.mins, head->r.maxs, tv(0.f,0.f,1.f));
