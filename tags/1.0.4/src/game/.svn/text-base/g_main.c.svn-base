...
vmCvar_t	jp_killSpreeLevels;		// number of killing spree levels. set to 0 to disable
...
//	{ &g_XPSaveMaxAge_xp, "g_XPSaveMaxAge_xp", "86400", 0 },  
	...
	{ &g_tyranny,				"g_tyranny",				"1",				0 }, //CVAR_ROM|CVAR_SERVERINFO_NOUPDATE },
...
// forty - canister kicking  
	...
	{ NULL,						"mod_version",				"1.0.4",								CVAR_SERVERINFO | CVAR_ROM },
...
//	{ &jp_statFile,				"g_statFile",				"playerStats.cfg",			0 },
	...
	{ &g_spreeMessage[0],		"g_spreeMessage0",			"^dis on a ^2Killing Spree!",			0 },
	{ &g_spreeMessage[1],		"g_spreeMessage1",			"^dis on a ^2Rampage!",				0 },
	{ &g_spreeMessage[2],		"g_spreeMessage2",			"^dis ^2Dominating",					0 },
	{ &g_spreeMessage[3],		"g_spreeMessage3",			"^drevels in his ^2Bloodbath!",		0 },
	{ &g_spreeMessage[4],		"g_spreeMessage4",			"^dis a walking ^2Slaughterhouse!",	0 },
	{ &g_spreeMessage[5],		"g_spreeMessage5",			"^dwreaks ^2Havoc ^dupon his foes!", 	0 },
	{ &g_spreeMessage[6],		"g_spreeMessage6",			"^dcuts through enemies like a ^2God ^2of ^2War!", 		0 },
	{ &g_spreeMessage[7],		"g_spreeMessage7",			"^dis the ^2Prophet of Doom", 		0 },
	{ &g_spreeMessage[8],		"g_spreeMessage8",			"^dunleashes the ^>Shitstorm!", 		0 },
	{ &g_spreeMessage[9],		"g_spreeMessage9",			"^dis still unleashing the ^>Shitstorm!", 		0 },
...
#ifdef USELOCATIONS
	{ &g_autoLoadLocations,		"g_autoLoadLocations",		"0",								0 },
#endif
...
#ifdef USELOCATIONS
void Cmd_LoadLocations_f(void); // jaquboss - load locations on startup
#endif
...
qboolean G_LoadConfig( char forceFilename[MAX_QPATH] )
{
...
#ifdef USELOCATIONS
	if ( g_autoLoadLocations.integer ){ // note this is EVIL, if we don't do that on very map start it WON'T load anything
		Cmd_LoadLocations_f(); // jaquboss - load locations
	} else {
...
//		vec3_t b1, b2;
	...
	G_RailBox( ent->r.currentOrigin, mins, maxs, tv(0.f,1.f,0.f));
...
void ServerOutput ( void )
{
	...
	if ( /*Q_stricmp(versionStr, va("ET 2.60b %s May  8 2006", CPUSTRING)) != 0*/ !strstr( versionStr, "2.60b") )
		trap_SendServerCommand( -1, va("cpm \"^3Wrong ET version Installed on this server, contact server Admin\n\"") );

	trap_Cvar_VariableStringBuffer( "fs_game", versionStr, sizeof(versionStr) );
	if ( Q_stricmp(versionStr, "noquarter" ) != 0 )
		trap_SendServerCommand( -1, va("cpm \"^3Wrong modification directory, should be 'noquarter', contact server Admin\n\"") );

	trap_Cvar_VariableStringBuffer( "sv_pure", versionStr, sizeof(versionStr) );
	if ( atoi(versionStr) < 0 )
		trap_SendServerCommand( -1, va("cpm \"^3Warning, server is not pure!! contact server Admin\n\"") );
	...
	for ( i=0; i<level.config.numSetl; i++ )
	{
		...
		if ( Q_stricmp( versionStr, level.config.setl[i].value ) != 0)
			trap_SendServerCommand( -1, va("cpm \"^3Warning, server Admin changed locked cvar, reload config! ( '%s' should be %s not %s)\n\"", level.config.setl[i].name, level.config.setl[i].value, versionStr) );
	}

	trap_Cvar_VariableStringBuffer( "sv_pakNames", pakNames, sizeof(pakNames) );

	if (!strstr(pakNames, "noquarter_b1.0.2"))
		trap_SendServerCommand( -1, va("cpm \"^3No noquarter_b1.0.2.pk3 found on server, missing essential files, contact server Admin\n\"") );

	if (!strstr(pakNames, "noquarter_b1.0.3"))
		trap_SendServerCommand( -1, va("cpm \"^3No noquarter_b1.0.3.pk3 found on server, missing essential files, contact server Admin\n\"") );

	if (!strstr(pakNames, "noquarter_b1.0.4"))
		trap_SendServerCommand( -1, va("cpm \"^3No noquarter_b1.0.4.pk3 found on server, missing essential files, contact server Admin\n\"") );


	if (!strstr(pakNames, "nq_bin_b1.0.4"))
		trap_SendServerCommand( -1, va("cpm \"^3No nq_bin_b1.0.4.pk3 found on server ( using wrong files ? ) contact server Admin\n\"") );


	if (strstr(pakNames, "nq_bin_b1.0.2") || strstr(pakNames, "nq_bin_b1.0.3") )
		trap_SendServerCommand( -1, va("cpm \"^3Old binary pack found on server, contact server Admin\n\"") );
	

	level.whineTime = level.time + 60000;
