...
#ifdef CGAMEDLL
//#define PM_GARAND_RELOADS		(cg_weaponsCvar.integer & WPF_GARAND_RELOADS) 
//#define	PM_LEAN					cgs.lean

#define PM_GameType cg_gameType.integer
#define PM_DOUBLE_JUMP			(cgs.doubleJump & MISC_DOUBLE_JUMP)
...
#define PM_FIXEDPHYSICSFPS		cgs.fixedphysicsfps.integer
...
#define PM_DOUBLE_JUMP			(g_doubleJump.integer & MISC_DOUBLE_JUMP)
...
//extern	vmCvar_t				g_lean;
//#define PM_LEAN					g_lean.integer
...
qboolean CheckSilent( void ){
	...
	if ( pm->skill[SK_COVERTOPS] < 6 || pm->ps->stats[STAT_PLAYER_CLASS] != PC_COVERTOPS )
	{
		return qfalse;
	}
...
int PM_RaiseAnimForWeapon ( int weapon ) {
	switch( weapon ) {
		...
		default:
			if( pm && pm->skill[SK_LIGHT_WEAPONS] >= 5 && IS_FAST_SWITCH_WEAPON(weapon) )
...
static void PM_Friction( void ) {
	...
	if (speed < 1 && pm->ps->pm_type != PM_SPECTATOR && pm->ps->pm_type != PM_NOCLIP && pm->ps->pm_type != PM_JETPILOT)
	{
		vel[0] = 0;
	...
	// our previous speed
	if (pm->ps->pm_type == PM_SPECTATOR || pm->ps->pm_type == PM_NOCLIP || pm->ps->pm_type == PM_JETPILOT)
	{
...
static float PM_CmdScale( usercmd_t *cmd ) {
	...
	// jaquboss done all we want to..
	if (pm->ps->pm_type == PM_JETPILOT)
		return scale*3;
...
//
	if ( BG_Weapons[pm->ps->weapon].movementSpeedScale )
	...
	{
		if( pm->skill[SK_HEAVY_WEAPONS] >= 7)
		{
			scale *= 0.95;
		}
		else if( pm->skill[SK_HEAVY_WEAPONS] >= 3 ) 
		{
			scale *= 0.75;
		}
		else 
	...
	if (pm->ps->weapon == WP_FLAMETHROWER && pm->skill[SK_HEAVY_WEAPONS] < 8 ) { // trying some different balance for the FT
		if( !(pm->skill[SK_HEAVY_WEAPONS] >= 3) || pm->cmd.buttons & BUTTON_ATTACK )
			scale *= 0.7;
	}

	if (pm->ps->weapon == WP_BAR) { // Meyer - trying some different balance for the BAR
...
//		}
		...
		if( pm->ps->weaponDelay && (pm->ps->weapon == WP_PANZERFAUST || pm->ps->weapon == WP_BAZOOKA) )
		{
			return qfalse;
		}
...
//			pm->cmd.serverTime - pm->pmext->proneGroundTime > 450 ||
				...
				if (PM_REALISM & REALISM_EXTENDED_PRONE) { // freeze player according to animation lenght
					if ( pm->waterlevel <= 1 &&
						...
						!(pm->ps->eFlags & EF_MOUNTEDTANK)  ){
							...
							pm->ps->pm_time = BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_PRONE_TO_CROUCH, qfalse, qtrue ); // PRONE_TO_CROUCH
					}
				}
	...
	if( pm->ps->eFlags & EF_PRONE )
	{
		...
		if( userinput && spd > 40.f && !(pm->ps->eFlags & EF_PRONE_MOVING) ) 
		{
			...
			{
				case WP_FG42_SCOPE: 
					PM_BeginWeaponChange( WP_FG42_SCOPE, WP_FG42, qfalse ); 
					break;
...
/*
static void PM_JetAccelerate( vec3_t wishdir, float wishspeed, float accel ) {
	vec3_t		wishVelocity;
	vec3_t		pushDir;
	float		pushLen;
	float		canPush;
...
*/
/*
static void PM_JetPilotMove( void ) 
{
	vec3_t	forward;
	float	wishspeed;
	float	scale=1;
	...
	pm->ps->speed += pm->cmd.forwardmove*pml.frametime;
	...
	wishspeed = scale * pm->ps->speed; //pm->cmd.forwardmove;
	AngleVectors( pm->ps->viewangles, forward, NULL, NULL );


	PM_JetAccelerate(forward, wishspeed, 35); // very fast acceleration

	// set primitive gravity code
	{
		float xyz,gravityf;

		xyz = (float)pm->ps->speed; // much better to use this i think

		if ( xyz > 200 )
			xyz = 200;

		xyz *= 0.005;
		gravityf = (1.f - xyz)* 800;
		pm->ps->gravity = (int)gravityf;
#ifdef GAMEDLL
		Com_Printf("speed: %i, gravity: %i \n", pm->ps->speed, pm->ps->gravity );
#endif
	}
...
static void PM_AirMove( void ) {
	...
	float		scale;
	usercmd_t	cmd;
	//static int	tapCount	= 0; // jP - count presses to prevent triple jumps	// Jaybird - unused
...
// jaquboss, in case we are cvops and silent
#ifdef GAMEDLL // nothing to predict 
		 else {
			pm->pmext->shoved = qfalse;
		}
...
static void PM_CheckDuck (void)
{
	...
	if( (pm->cmd.upmove < 0 && !pm->pmext->kickDelay && !(pm->ps->eFlags & EF_MOUNTEDTANK) && !(pm->ps->pm_flags & PMF_LADDER) && pm->ps->pm_type != PM_JETPILOT ) || pm->ps->weapon == WP_MORTAR_SET || pm->ps->weapon == WP_MORTAR2_SET || pm->ps->pm_flags & PMF_MEDICUSE )
	{	// duck
		pm->ps->pm_flags |= PMF_DUCKED;
	}
	else
...
static void PM_Footsteps( void ) 
{
	...
	if (pm->ps->eFlags & EF_DEAD) 
	{
		...
		else if(!pm->ps->pm_time && !(pm->ps->pm_flags & PMF_LIMBO) ) // DHM - Nerve :: before going to limbo, play a wounded/fallen animation
		{
			...
			else if ( pm->ps->eFlags & EF_DEFENDING )
			{
				animResult = BG_AnimScriptAnimation( pm->ps, pm->character->animModelInfo, ANIM_MT_DEFENDING, qtrue );
			} 
#endif
			else
			{
...
//			}
	...
	{		
		if ( pm->waterlevel == 0 ) 
		{
			...
			if ( footstep && !pm->noFootsteps  && !CheckSilent())
			{
				PM_AddEventExt( EV_FOOTSTEP, PM_FootstepForSurface() );
			}
		} 
...
static void PM_Shotgun_PumpToLoop( void )
{
	...
	if (  BG_Weapons[pm->ps->weapon].weapAnimations[WEAP_ALTSWITCHTO].time )
		pm->ps->weaponTime += BG_Weapons[pm->ps->weapon].weapAnimations[WEAP_ALTSWITCHTO].time;
		else
	pm->ps->weaponTime += 400;
...
static void PM_StopShotgunReload( void )
{
	...
	if (  BG_Weapons[pm->ps->weapon].weapAnimations[WEAP_RELOAD3].time )
		pm->ps->weaponTime += BG_Weapons[pm->ps->weapon].weapAnimations[WEAP_RELOAD3].time;
		else
...
static void PM_Shotgun_StopFromPump( void )
{
	...
	if (  BG_Weapons[pm->ps->weapon].weapAnimations[WEAP_DROP2].time )
		pm->ps->weaponTime += BG_Weapons[pm->ps->weapon].weapAnimations[WEAP_DROP2].time;
		else
	pm->ps->weaponTime += 500; 
	pm->ps->holdable[0] = SHOTGUN_RELOADING_STOP;
...
static void PM_BeginWeaponReload( int weapon ) {
	//gitem_t* item;
	int		 reloadTime;
	qboolean shotgunReload = qfalse;

	shotgunReload = (weapon == WP_SHOTGUN && pm->ps->holdable[0] != SHOTGUN_NOT_RELOADING);

	// only allow reload if the weapon isn't already occupied (firing is okay)
	// jet Pilot - special case for reloading shotgun
	if(pm->ps->weaponstate != WEAPON_READY && pm->ps->weaponstate != WEAPON_FIRING /*&& !shotgunReload*/) // what is point of allowing to reload weapon again when it is reloading ?!
	{		
			return;
	}

	// jaquboss, don't start reloads when not possible
	if ( pm->ps->ammo[BG_FindAmmoForWeapon(pm->ps->weapon)] == 0 ){
	...
	/*	case WP_CARBINE:
		...
		case WP_GARAND_SCOPE:
			...
			break;*/
		default:
			if ( BG_Weapons[weapon].noMCReload && pm->ps->ammoclip[BG_FindClipForWeapon(weapon)] != 0)
				return;	
			break;
		// jet Pilot - Skill - Level 5+ Soldiers can reload MGs mid clip
		case WP_MOBILE_MG42:
		case WP_MOBILE_MG42_SET:	
			if(pm->ps->ammoclip[WP_MOBILE_MG42]		!= 0 && pm->skill[SK_SOLDIER] < 5)
				return;
			break;
		case WP_MOBILE_BROWNING:
		case WP_MOBILE_BROWNING_SET:
			if(pm->ps->ammoclip[WP_MOBILE_BROWNING] != 0 && pm->skill[SK_SOLDIER] < 5)
				return;
			break;		
	}
...
/*	if(pm->ps->eFlags & EF_MELEE_ACTIVE)
	...
	// jet Pilot - reload while leaning
//	if(pm->ps->leanf)
//		return;
...
//		case WP_TRIPMINE:
	...
	return;
	//	PM_ContinueWeaponAnim(PM_ReloadAnimForShotgun(1));
	}
	...
	if (  BG_Weapons[pm->ps->weapon].weapAnimations[PM_ReloadAnimForWeapon(pm->ps->weapon)].time )
			reloadTime = BG_Weapons[pm->ps->weapon].weapAnimations[PM_ReloadAnimForWeapon(pm->ps->weapon)].time;
...
{	
	...
	// jaquboss check if we have loaded weapon animation cfg and use it to do correct time
	 if (switchtime > 0 && newweapon == weapAlts[oldweapon] && BG_Weapons[oldweapon].weapAnimations[PM_AltSwitchFromForWeapon(oldweapon)].time )
		switchtime =  BG_Weapons[oldweapon].weapAnimations[PM_AltSwitchFromForWeapon(oldweapon)].time;
	else if ( switchtime > 0 && newweapon != weapAlts[oldweapon] && BG_Weapons[oldweapon].weapAnimations[PM_DropAnimForWeapon(oldweapon)].time )
		switchtime =  BG_Weapons[oldweapon].weapAnimations[PM_DropAnimForWeapon(oldweapon)].time;
...
//			break;
	...
	// jaquboss check if we have loaded weapon animation cfg and use it to do correct time		if(weapAlts[oldweapon] == newweapon)
	if(newweapon == weapAlts[oldweapon]){
		if ( switchtime > 0 && BG_Weapons[newweapon].weapAnimations[PM_AltSwitchToForWeapon(newweapon)].time )
			switchtime =  BG_Weapons[newweapon].weapAnimations[PM_AltSwitchToForWeapon(newweapon)].time;
	} else {
		if ( switchtime > 0 && BG_Weapons[newweapon].weapAnimations[PM_RaiseAnimForWeapon(newweapon)].time )
			switchtime =  BG_Weapons[newweapon].weapAnimations[PM_RaiseAnimForWeapon(newweapon)].time;
	}
...
//		case WEAPON_RELAXING:
	...
	switch( weapon ) 
	{
		// jet Pilot - TODO : allow higher level coverts to reload without leaving scoped view
		case WP_FG42_SCOPE:
		case WP_GARAND_SCOPE:
		case WP_K43_SCOPE:
...
void PM_WeaponUseAmmo( int wp, int amount ) {
	...
	if(pm->noWeapClips){
		...
		if ( pm->ps->ammo[ BG_FindAmmoForWeapon(wp)] == 0 && fillEmpty )
			pm->ps->ammo[ BG_FindAmmoForWeapon(wp)] = GetAmmoTableData(BG_FindAmmoForWeapon(wp))->defaultStartingAmmo;
		...
		if ( pm->ps->ammoclip[ takeweapon] == 0 && fillEmpty)
			pm->ps->ammoclip[ takeweapon ] = GetAmmoTableData(takeweapon)->defaultStartingClip;
...
void PM_CoolWeapons( void )
{
	...
	for(wp=0;wp<WP_NUM_WEAPONS;wp++) 
	{
		...
		if( COM_BitCheck( pm->ps->weapons, wp) ) 
		{
			...
			{
				// jet Pilot - Modified cooling based on heavy weapon skill (was  && pm->ps->stats[STAT_PLAYER_CLASS] == PC_SOLDIER ) 
				if( (pm->skill[SK_HEAVY_WEAPONS] >= 2 && IS_HEAVY_WEAPON(wp)) || pm->auraEffects[PC_ENGINEER] >= 5 /*BG_GetAuraLevel(PC_ENGINEER, pm->ps) >= 5*/ ) 
				{
					pm->ps->weapHeat[wp] -= ((float)GetAmmoTableData(wp)->coolRate * 1.5f * pml.frametime);
				}
				else 
				{
...
//	int		increase, decrease, i;
	...
	if (wpnScale ) 
	{
		...
		// jet Pilot - try to make the shotgun spread less unpredictable when moving
	/*	if (pm->ps->weapon == WP_SHOTGUN)
		{
			if (increase > 1.0f) increase = 1.0f;
			if (increase < 0.6f) increase = 0.6f;
		}*/
	}
	else
	{
...
static void PM_Weapon( void ) {
	...
	if ( pm->ps->stats[STAT_HEALTH] <= 0 ) {
		...
		// jaquboss - wounded players defending 
		if ( pm->skill[SK_BATTLE_SENSE] >= 9 )
		{
			if( COM_BitCheck(pm->ps->weapons, WP_COLT) || COM_BitCheck(pm->ps->weapons, WP_AKIMBO_COLT) || COM_BitCheck(pm->ps->weapons, WP_AKIMBO_SILENCEDCOLT) ) {
				pm->cmd.weapon = pm->ps->weapon = WP_COLT;
			} else if ( COM_BitCheck(pm->ps->weapons, WP_LUGER) || COM_BitCheck(pm->ps->weapons, WP_AKIMBO_LUGER) || COM_BitCheck(pm->ps->weapons, WP_AKIMBO_SILENCEDLUGER) ) {
				pm->cmd.weapon = pm->ps->weapon = WP_LUGER;
			} else {
				return;
			}

			// grab angles 
			if ( !(pm->ps->eFlags & EF_DEFENDING) ){
				vec3_t axis[3];
			...
			if ( pm->cmd.buttons & BUTTON_ATTACK && pm->ps->ammoclip[BG_FindClipForWeapon(pm->ps->weapon)] > 0 )
			{
				if ( !(pm->ps->eFlags & EF_DEFENDING) ){
					pm->ps->eFlags |= EF_DEFENDING;
					pm->ps->weaponTime += BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_DEFEND_RAISE, qfalse, qtrue );
				}
					pm->pmext->lastFireTime = pm->cmd.serverTime;

				// jaquboss : - fixme this one doesn't work
			} else if ( pm->cmd.serverTime - pm->pmext->lastFireTime  > 3000 || pm->ps->ammoclip[BG_FindClipForWeapon(pm->ps->weapon)] == 0 ){
			  	if ( pm->ps->eFlags & EF_DEFENDING ){
					BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_DEFEND_LOWER, qfalse, qtrue );
					pm->ps->eFlags &= ~EF_DEFENDING;
				}
				return;
			}
...
// jaquboss shoot with on plane guns
#ifdef __JETPILOT__
	if ( PM_INSANITY & JP_INSANITY_FLYING)
	{
		if( pm->ps->weapHeat[WP_DUMMY_MG42] ) {
			pm->ps->weapHeat[WP_DUMMY_MG42] -= (300.f * pml.frametime);
...
/*			case WEAPON_FIRINGALT:
	...
	if(pm->ps->weapon == WP_GRENADE_LAUNCHER || pm->ps->weapon == WP_GRENADE_PINEAPPLE || pm->ps->weapon == WP_DYNAMITE || pm->ps->weapon == WP_SMOKE_BOMB) {// Is nade?
		if( pm->ps->grenadeTimeLeft > 0 ) { // do we have time?
			qboolean forcethrow = qfalse;

			if( pm->ps->weapon == WP_DYNAMITE ) { // dynamite never run of time
			...
			if ( pm->ps->holdable[1] )
				pm->cmd.buttons &= ~BUTTON_ATTACK; // jaquboss don't allow to catch nade again

			if( !(pm->cmd.buttons & BUTTON_ATTACK)  || forcethrow || pm->ps->eFlags & EF_PRONE_MOVING ) { // released button, or forced to throw
				if( pm->ps->weaponDelay == GetAmmoTableData(pm->ps->weapon)->fireDelayTime || forcethrow) { // forced to throw, or we just released button and returned here ( after setting all stuff )
				...
				// not for dynamite
				if ( pm->ps->weapon == WP_DYNAMITE /*|| !(pm->cmd.buttons & BUTTON_ATTACK)*/ )
					return;
...
// rain - moved releasedFire into pmext instead of ps
	...
	if (pm->ps->weaponstate == WEAPON_RELOADING && pm->ps->weapon == WP_SHOTGUN &&
		(pm->ps->holdable[0] == SHOTGUN_RELOADING_LOOP ||
		pm->ps->holdable[0] == SHOTGUN_RELOADING_AFTER_1ST ||
		pm->ps->holdable[0] == SHOTGUN_RELOADING_BEGIN_1ST ) &&
		pm->ps->weaponTime > 0 && pm->cmd.buttons & BUTTON_ATTACK ){ // jaquboss - requesting interrupt
	...
	if( pm->ps->weaponTime > 0 || pm->ps->weaponDelay > 0 )
		...
		pm->pmext->artyFire = qfalse; // jaquboss certainly not after using binocs now
	...
	if (pm->ps->weaponstate == WEAPON_RELOADING) {
		if (pm->ps->weapon == WP_SHOTGUN) {
			if (pm->ps->holdable[0] == SHOTGUN_RELOADING_BEGIN_1ST ||
				( pm->ps->holdable[0] == SHOTGUN_RELOADING_INTERRUPT && pm->ps->ammoclip[WP_SHOTGUN] == 0 ))
			{
			...
			if( pm->ps->holdable[0] == SHOTGUN_RELOADING_LOOP || pm->ps->holdable[0] == SHOTGUN_RELOADING_INTERRUPT )
			{
				...
				if ( ammo > GetAmmoTableData(WP_SHOTGUN)->maxclip || pm->ps->ammo[BG_FindAmmoForWeapon(WP_SHOTGUN)] == 0 ) {
					PM_StopShotgunReload();
					return;
				} else {
					...
					if ( pm->ps->holdable[0] == SHOTGUN_RELOADING_INTERRUPT ||  ammo == GetAmmoTableData(WP_SHOTGUN)->maxclip || pm->ps->ammo[BG_FindAmmoForWeapon(WP_SHOTGUN)] == 0 )
						PM_StopShotgunReload();
					else 
						PM_ContinueShotgunReload();
...
// holster
	if ( ( pm->ps->pm_flags & PMF_MEDICUSE || ( pm->cmd.serverTime < pm->pmext->kickTime ) || ( pm->ps->pm_flags & PMF_LADDER && PM_REALISM & REALISM_NOLADDERGUN) ) && !delayedFire ){
		...
		pm->ps->weaponTime = BG_Weapons[pm->ps->weapon].weapAnimations[PM_DropAnimForWeapon(pm->ps->weapon)].time;
		return;
		...
		pm->ps->weaponTime = BG_Weapons[pm->ps->weapon].weapAnimations[PM_RaiseAnimForWeapon(pm->ps->weapon)].time;
		return;
...
// done
	...
	if ( !BG_CheckCharge(pm) ){	// jaquboss, i want to use this at client too 
	...
	if( pm->ps->weapon == WP_MORTAR_SET || pm->ps->weapon == WP_MORTAR2_SET || pm->ps->weapon == WP_FIREBOLT ) 
	{
		if( !delayedFire ) 
		{
			pm->ps->weaponstate = WEAPON_READY;
	...
	{ 
		if (pm->cmd.serverTime - pm->ps->classWeaponTime < pm->medicChargeTime || PM_MEDIC_NOSELFADREN) 
			return; 
	} 
...
/*	if( pm->ps->weapon == WP_TRIPMINE ) {
	...
	if((!(pm->cmd.buttons & (BUTTON_ATTACK | WBUTTON_ATTACK2)) && !delayedFire) ||
	  (pm->ps->leanf != 0 && /*(!(PM_LEAN & LEAN_SHOOT) || !(PM_LEAN & LEAN_VISIBLE))*/ /*pm->skill[SK_LIGHT_WEAPONS] < 9 && */pm->ps->weapon != WP_GRENADE_LAUNCHER && pm->ps->weapon != WP_GRENADE_PINEAPPLE && pm->ps->weapon != WP_SMOKE_BOMB))
	{
		pm->ps->weaponTime	= 0;
		pm->ps->weaponDelay	= 0;
		...
		if(pm->ps->weapon == WP_FIREBOLT && pm->ps->weaponstate == WEAPON_READY) {	// time to relax fireboys
			PM_ContinueWeaponAnim(PM_IdleAnimForWeapon(pm->ps->weapon));
		}
...
#ifdef GAMEDLL // only do not predict this, so weaponTime and arty Fire will be predicted ok
	...
	if(pm->waterlevel == 3)
	{
		if(	pm->ps->weapon != WP_KNIFE				&&
			...
			pm->ps->weapon != WP_POISON_SYRINGE		&&
...
//		case WP_LOCKPICK:
		...
		case WP_MORTAR2_SET:
			...
			//	pm->ps->weaponDelay = GetAmmoTableData(pm->ps->weapon)->fireDelayTime;
				pm->ps->weaponDelay = BG_Weapons[pm->ps->weapon].weapAnimations[PM_AttackAnimForWeapon(pm->ps->weapon)].time;
			//	Com_Printf( "%i \n", pm->ps->weaponDelay );
			} 
			else 
			{
...
//	}
	...
	if( pm->ps->weapon ) 
	{
		...
		if(ammoNeeded > ammoAvailable)
		{
			...
			switch(pm->ps->weapon) 
			{
				...
				case WP_GARAND_SCOPE:
				case WP_K43_SCOPE:
					// jet Pilot - TODO allow high lvl coverts to reload while looking through scope
					reloading = qfalse;
					break;
			}
	...
	{
		// jet Pilot - allow for lvl 5 engineer 'aura' to lessen this effect
/*		int heatAmt;
		
		heatAmt = GetAmmoTableData(pm->ps->weapon)->nextShotTime;

		if (pm->auraEffects[PC_ENGINEER] >= 5)
		{	// engineer 'aura' halves the heating rate
			heatAmt *= .75;
		}

		pm->ps->weapHeat[pm->ps->weapon] += heatAmt;
*/
		// jaquboss, only increase cooling rate
	...
	switch(pm->ps->weapon)
	{
		...
		case WP_STG44:
		case WP_THOMPSON:
//		case WP_SHOTGUN: - jaquboss this goes above too
		case WP_STEN:
		case WP_STEN_MKII:		
// Jaquboss - fix non-looping animation bug
//		case WP_MEDKIT:
		case WP_PLIERS:
...
//			PM_ContinueWeaponAnim(weapattackanim);
	...
	if ( (pm->ps->weapon == WP_PANZERFAUST) || (pm->ps->weapon == WP_BAZOOKA) || (pm->ps->weapon == WP_SMOKE_MARKER ) || (pm->ps->weapon == WP_DYNAMITE) || (pm->ps->weapon == WP_SMOKE_BOMB) || (pm->ps->weapon == WP_LANDMINE) || (pm->ps->weapon == WP_SATCHEL))
		PM_AddEvent( EV_NOAMMO );
	// jpw

	// jet Pilot - TODO allow high lvl coverts to have more than one satchel
	...
	if(  (pm->ps->weapon == WP_MORTAR2_SET || pm->ps->weapon == WP_MORTAR_SET) && !pm->ps->ammo[WP_MORTAR] ) {
		PM_AddEvent( EV_NOAMMO );
...
// rain - moved releasedFire into pmext instead of ps
	...
	switch( pm->ps->weapon )
	{
		...
		case WP_MORTAR_SET:
			...
			break;
		case WP_FIREBOLT: // jaquboss
			addTime = BG_Weapons[pm->ps->weapon].weapAnimations[WEAP_ATTACK2].time;
			break;
...
//		case WP_BAR_SET:
		...
		case WP_STG44:
			addTime = GetAmmoTableData(pm->ps->weapon)->nextShotTime;
			aimSpreadScaleAdd = 15+rand()%25;	// (SA) new values for DM
			break;
		...
		//case WP_BAR_SET:
			...
			aimSpreadScaleAdd = 20;
			break;
		//Meyer - moved and split BAR and BAR Bipod
		case WP_BAR:
			if(weapattackanim == WEAP_ATTACK_LASTSHOT) {
				addTime = 2000;
			} 
...
//		case WP_LOCKPICK:
	...
	if ( BG_Weapons[pm->ps->weapon].aimSpreadScaleAdd ) {
		aimSpreadScaleAdd = BG_Weapons[pm->ps->weapon].aimSpreadScaleAdd;

		if ( BG_Weapons[pm->ps->weapon].aimSpreadScaleAddRand ){
			int val= BG_Weapons[pm->ps->weapon].aimSpreadScaleAddRand;

			aimSpreadScaleAdd += rand() % val;
		}
	}
	...
	if ( BG_Weapons[pm->ps->weapon].recoil ){
		...
		pm->pmext->weapRecoilYaw = crandom() * BG_Weapons[pm->ps->weapon].recoilYaw;
		pm->pmext->weapRecoilPitch = random() * BG_Weapons[pm->ps->weapon].recoilPitch;
	} else
	...
	switch( pm->ps->weapon ) 
	{
		...
		case WP_SHOTGUN:
			...
			pm->pmext->weapRecoilYaw = 0.f;
			pm->pmext->weapRecoilPitch = .6f * random();
			break;
		...
		case WP_BAR_SET:
			...
			pm->pmext->weapRecoilYaw = 0.f;
			pm->pmext->weapRecoilPitch = .05f * random();
			break;
		...
		case WP_STG44:
			...
			pm->pmext->weapRecoilYaw = 0.f;
			pm->pmext->weapRecoilPitch = .15f * random();
			break;
		...
		case WP_AKIMBO_COLT:
		case WP_AKIMBO_SILENCEDCOLT:
			// jet Pilot - TODO - make higher level light weapons even lower recoil
			pm->pmext->weapRecoilTime = pm->cmd.serverTime;
		...
		//	pm->pmext->weapRecoilPitch = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? .25f * random() * .15f : .45f * random() * .15f;
			pm->pmext->weapRecoilPitch = pistolRecoil[pm->skill[SK_LIGHT_WEAPONS]] * random() * .15f;
			break;
		default:
			pm->pmext->weapRecoilTime = 0;
			pm->pmext->weapRecoilYaw = 0.f;
...
//			PM_StartWeaponAnim(WEAP_IDLE1);	// removed.  client handles anim in overheat event
			...
			addTime = pm->auraEffects[PC_ENGINEER] ? 1500 : 2000;	
//			addTime = BG_GetAuraLevel(PC_ENGINEER, pm->ps) ? 1500 : 2000;
		}
	}
...
//	if (pm->ps->aimSpreadScale > 255) pm->ps->aimSpreadScale = 255;
	...
	// jet Pilot - Does this decrease spread for *all* weapons?
...
static void PM_Foot( void ) {
	...
	pm->ps->ammoclip[WP_NONE] = ( ( pm->ps->ammoclip[WP_NONE] & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) | WEAP_ATTACK1;	
	pm->pmext->kickTime = pm->cmd.serverTime + pm->ps->pm_time;

	//PM_AddEvent( EV_KICKED );
...
//float PlayerLeanValues[MAX_CLIENTS];
...
void PM_UpdateLean(playerState_t *ps, usercmd_t *cmd, pmove_t *tpm) {
	...
	if( cmd->wbuttons & (WBUTTON_LEANLEFT | WBUTTON_LEANRIGHT))
	{
		...
		if (ps->pm_type == PM_SPECTATOR /*|| PM_LEAN & LEAN_RUN*/)
		{
	...
	if(ps->eFlags & EF_FIRING && /*(!(PM_LEAN & LEAN_VISIBLE) || !(PM_LEAN & LEAN_SHOOT))*/ pm->skill[SK_LIGHT_WEAPONS] < 9)
  ...
  // ATVI Wolfenstein Misc #479 - initial fix to #270 would crash in g_synchronousClients 1 situation
	...
	if(!leaning) {	// go back to center position
		...
		//	BG_AnimScriptEvent( ps, pm->character->animModelInfo, ANIM_ET_LEANR, qtrue, qfalse );			
			...
			//FIXME: play lean anim backwards?
		//	BG_AnimScriptEvent( ps, pm->character->animModelInfo, ANIM_ET_LEANL, qtrue, qfalse );
			leanofs += (((float)pml.msec/(float)LEAN_TIME_FR)*LEAN_MAX);
			if ( leanofs > 0 )
				leanofs = 0;
		}
	...
	if(leaning) 
	{
		if(leaning > 0) // right
		{			
			// jet Pilot - update condition to allow leaning if the model supports it
	...
	{
		// jet Pilot - update condition to disable leaning if the model supports it
	//	BG_UpdateConditionValue( ps->clientNum, ANIM_COND_LEANING, LEANING_UNUSED, qtrue );
	}

	ps->leanf = leanofs;
//	PlayerLeanValues[ps->clientNum] = leanofs;
//	tpm->pmext->leaning = ps->leanf;
	...
	// jet Pilot - Allow Spectators to lean while moving
	if(ps->leanf && ps->pm_type != PM_SPECTATOR /*&& !(PM_LEAN & LEAN_RUN)*/) // jaquboss, was teamNum, but that actualy holds player class
		cmd->rightmove = 0;		// also disallowed in cl_input ~391

	 ps->stats[STAT_LEANING] = leaning;
...
void PM_UpdateViewAnglesJet(playerState_t *ps, pmoveExt_t *pmext, usercmd_t *cmd)
{
	short	temp;
	int		i;
	...
	ps->viewangles[2] -= ps->leanf;

	cmdTime = (float)(pm->cmd.serverTime - pm->oldcmd.serverTime) / 1000.0;
	viewchange =  AngleSubtract(ps->viewangles[YAW], oldViewAngles[YAW])/cmdTime;
	PM_UpdateFlyLean(ps, viewchange);

	if ( cmd->wbuttons & WBUTTON_LEANLEFT )
	ps->leanf -= degsSec * pml.frametime;
...
void PM_UpdateViewAngles( playerState_t *ps, pmoveExt_t *pmext, usercmd_t *cmd, void (trace)( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentMask ), int tracemask ) {	//----(SA)	modified
		...
		) 
	{
		...
		tpm.trace = trace; // jaquboss - reset dead ppl leaning
...
void PM_Sprint( void )
{
	...
	{	
		...
		else 
		{
			...
			rechargebase = (pm->auraEffects[PC_FIELDOPS] >= 5) ? 1000 : 500;
//			rechargebase = BG_GetAuraLevel(PC_FIELDOPS, pm->ps) ? 1000 : 500;
...
void PM_CheckAttack( void )
{
	...
	// check for leaning
//	if (pm->ps->leanf != 0 && (!(PM_LEAN & LEAN_SHOOT) || !(PM_LEAN & LEAN_VISIBLE)))
//		return;

	// jet Pilot - lean-fire is now lvl 9 lw
	if (pm->ps->leanf != 0/* && pm->skill[SK_LIGHT_WEAPONS] < 9*/)
		return;
...
/*	if (first)
	{
		memset(&PlayerLeanValues, 0, sizeof(PlayerLeanValues));
		first = 0;
	}*/
	...
	// add rounding  to gravity error manualy
/*	if ( PM_FIXEDPHYSICS && PM_FIXEDPHYSICSFPS > 0 )
	{
		double f = pm->ps->gravity / PM_FIXEDPHYSICSFPS;
		int ipart = (int)f;
...
// Check zoom command
	if( pm->cmd.wbuttons & WBUTTON_ZOOM && pm->ps->stats[STAT_HEALTH] >= 0 && !( pm->ps->weaponDelay )) 
	{
		...
		{	
			...
			{
				if ( (!pm->ps->weaponTime || pm->pmext->artyFire) && !pm->pmext->kickDelay && ( pm->cmd.serverTime >= pm->pmext->kickTime ) ) // jaquboss do not zoom while reloading and suc, artyFire is set if used binocs
					pm->ps->eFlags |= EF_ZOOMING;
			}
		}
	...
	PM_CheckAttack(); // jaquboss, se we don't have to list million of conditions here..
...
#ifdef __JETPILOT__
			if ( PM_INSANITY & JP_INSANITY_FLYING && pm->ps->pm_type != PM_SPECTATOR )
				PM_UpdateViewAnglesJet( pm->ps, pm->pmext, &pm->cmd );
			else
#endif
...
/*	if ( pm->ps->pm_type == PM_JETPILOT )
	...
	{	
		...
		// Switch from the satchel detonator if we no longer have it (used it?)
