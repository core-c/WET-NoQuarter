...
void props_statue_blocked (gentity_t *ent)
{
	...
	else
	{
		...
		// jet Pilot - Darwin Award for getting crushed by a ... STATUE!?
		//traceEnt->client->ps.persistant[PERS_DARWINDEATHS]++;
	}
}
...
//	drop = Drop_Item (ent, item, 0, qtrue);
	drop = LaunchItem( item, ent->r.currentOrigin, tv(0, 0, 20), ent->s.number, FL_DROPPED_ITEM );
