...
void G_BurnTarget( gentity_t *self, gentity_t *body, qboolean directhit )
{
	...
	if (body->client) 
	{
		...
		if( ( g_friendlyFire.integer & FRIENDLY_FIRE_DISABLED || g_friendlyFire.integer == 0 ) && OnSameTeam( body, self->parent ) )
		{
			return;
		}
	}
...
qboolean G_ExplodeSatchels(gentity_t* ent) {
	...
	if ( ent->client && ent->client->sess.skill[SK_COVERTOPS] >= 7 )
		maxrange = 3000;
...
void G_LandmineThink( gentity_t *self ) {
	...
	if( trigger )
	{
		...
		if(ent->r.svFlags & SVF_BOT)
		{
			...
			bud.udata.m_Entity = self;
			Bot_Interface_SendEvent( ET_EVENT_PRETRIGGER_MINE, ent-g_entities, self-g_entities, 0, &bud );
		}
#endif
...
void LandminePostThink( gentity_t *self ) {
	...
	if(!trigger)
	{
		...
		if(ent->r.svFlags & SVF_BOT)
		{
			...
			bud.DataType = dtEntity;
			bud.udata.m_Entity = self;
			Bot_Interface_SendEvent( ET_EVENT_POSTTRIGGER_MINE, ent-g_entities, self-g_entities, 0, &bud );
		}
#endif
...
// jpw
    ...
    case WP_MORTAR_SET:
			...
			if(self->client && jp_insanity.integer & JP_INSANITY_MORTARCAM) 
			{
				gentity_t *camera;
...
void Guided_Missile_Think (gentity_t *missile)
{
	...
	// if damaged but not destroyed, detabilize the missile and lose control
//	if (g_missileHealth.integer > 0)
//	{
//		if (missile->health < g_missileHealth.integer)
//		{
//			missile->nextthink  = level.time + 10;
//			missile->think		= Destabilize_Missile;
//			return;
...
//	}
	...
	CalcMuzzlePoint (player, 0, forward, right, up, muzzle ); // uch, what was point of missile here?!
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir, int rocketType)
{
	...
	bolt->splashDamage			= G_GetWeaponDamage(rocketType); 	// JPW NERVE
...
//	bolt->clipmask 				= MASK_SHOT;
	...
	if(self->client) 
	{
		...
		if (jp_insanity.integer & JP_INSANITY_MISSILECAM ){ // need this
		gentity_t *camera;
...
//	VectorNormalize (dir);
	...
	bolt->splashDamage 			= G_GetWeaponDamage(WP_MORTAR_SET);//WP_MAPMORTAR); // JPW NERVE
