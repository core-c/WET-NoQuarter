...
int Add_Ammo(gentity_t *ent, int weapon, int count, qboolean fillClip) {
	...
	if( ammoweap == WP_PANZERFAUST || ammoweap == WP_FLAMETHROWER || ammoweap == WP_BAZOOKA ) {
...
// Special case for high level soldiers
	...
	if ( client->sess.playerType == PC_SOLDIER && client->sess.skill[SK_SOLDIER] >= 4)
	{
...
qboolean G_CanPickupWeapon( weapon_t weapon, gentity_t* ent )
{
	...
	{
		case PC_SOLDIER:		
			// jet Pilot - Skill - Level 9 Soldiers can pick up and use any weapon, jaquboss - why we simply dont use good old bg_class info?
			if (ent->client->sess.skill[SK_SOLDIER] >= 9) return qtrue;
			if ( IS_AKIMBO_PISTOL(weapon) && ent->client->sess.skill[SK_LIGHT_WEAPONS] >= 4 ) return qtrue;
		...
		case PC_COVERTOPS:
			if (( IS_AKIMBO_PISTOL(weapon) || IS_AKIMBO_SILENCED_PISTOL(weapon) ) && ent->client->sess.skill[SK_LIGHT_WEAPONS] >= 4 ) return qtrue;
...
/*
	if( ent->client->sess.sessionTeam == TEAM_AXIS ) 
	{
		...
		else if( weapon == WP_BAR && (ent->client->sess.playerType == PC_SOLDIER || ent->client->sess.playerType == PC_SOLDIER ))
		{
			return qtrue;
		}
...
/*	if ( ent->s.eFlags & EF_READY ) {	// can't eat plates...
		return ( 0 );
	}*/
	...
	if ( other->client->auraEffects[PC_MEDIC] >= 5)
	other->health += 10;
...
// jaquboss, leaving remains is not really cool in MP
...
void G_BounceItem( gentity_t *ent, trace_t *trace ) {
	...
	/*	gentity_t *tent = G_TempEntity (ent->r.currentOrigin, EV_BOUNCE_SOUND);	
		tent->s.weapon = ent->item->giTag;
		tent->s.eventParm  = ent->item->giType == IT_WEAPON ? 0 : 1;	*/
