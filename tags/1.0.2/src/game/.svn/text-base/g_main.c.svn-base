...
//	{ &g_podiumDrop, 			"g_podiumDrop", 			"70", 				0, 0, qfalse },
	...
	{ &server_motd2,			"server_motd2", 			"Alpha",			0, 0, qfalse, qfalse },
...
//	{ &g_movespeed, "g_movespeed", "127", CVAR_CHEAT, 0, qfalse },
	...
	{ &g_forceLimboHealth,		"g_forceLimboHealth",		"150", 				0 }, // jaquboss - changed behaviour of this
...
qboolean G_missileCamera(gentity_t *ent)
{
	...
	player = &g_entities[ent->s.clientNum];
	...
	G_SetOrigin(ent, player->s.origin);
	VectorCopy(master->r.currentOrigin, ent->s.origin2);
	VectorCopy(player->r.currentOrigin, ent->s.pos.trBase);

/*	if ( (master->s.weapon == WP_PANZERFAUST || master->s.weapon == WP_BAZOOKA ) && ( jp_insanity.integer & JP_INSANITY_GUIDED || jp_insanity.integer & JP_INSANITY_HOMING ) )
	VectorCopy(master->s.angles, ent->s.angles);	// doesnt work with nonguided missiles
		else 
	vectoangles(master->s.pos.trDelta, ent->s.angles); // grab angles
...
void G_RunEntity( gentity_t* ent, int msec ) {
	...
	if( (ent->s.eType == ET_MISSILECAM  ) && G_missileCamera(ent) ) {
		return;
	}
