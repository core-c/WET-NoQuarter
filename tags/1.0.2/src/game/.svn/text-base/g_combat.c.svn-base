...
//	trap_SendServerCommand( -1, va("print \"%i\n\"\n", targ->health) );
	...
	// if there is no attacker, then the world did it
	if ( !attacker ) 
	{
...
//		}
	...
	// jet Pilot - absorb health from enemies if 'Vampire' insanity flag is enabled
	/*if ( (jp_insanity.integer & JP_INSANITY_VAMPIRE) && attacker->client && (targ != attacker) && !OnSameTeam(targ, attacker) && targ->client && targ->health > 0)
	{
		int gain = take;		
	...
	{	// jP optimized	
		...
		if( !(targ->client->ps.eFlags & EF_HEADSHOT) && ( targ->client->sess.skill[SK_BATTLE_SENSE] < 6 || targ->client->sess.skill[SK_BATTLE_SENSE] >= 8)) 
		{	
			G_AddEvent( targ, EV_LOSE_HAT, DirToByte(dir) );

			if( mod != MOD_K43_SCOPE &&	mod != MOD_GARAND_SCOPE && mod != MOD_STG44) 
			{
				if ( targ->client->sess.skill[SK_BATTLE_SENSE] >= 8  )
				take *= .2f;	// helmet shield gives us a lot of protection
					else
				take *= .8f;	// helmet gives us some protection
			}			
		} else if ( targ->client->sess.skill[SK_BATTLE_SENSE] >= 6 )  {
			if( mod != MOD_K43_SCOPE &&	mod != MOD_GARAND_SCOPE && mod != MOD_STG44) 
			{
				take *= .8f;	// helmet gives us some protection
			}	
		...
		//attacker->client->hitSound = IMPACTPOINT_HEAD;
		
		// moved this to below hitsounds so it can still be checked
	...
	//	BG_SetImpactPoint(&targ->client->ps, IMPACTPOINT_HEAD);
	...
	{
	//	BG_SetImpactPoint(&targ->client->ps, (level.time & 1) + IMPACTPOINT_KNEE_RIGHT);
	...
	{
	//	BG_SetImpactPoint(&targ->client->ps, (level.time & 1) + IMPACTPOINT_SHOULDER_RIGHT);
	...
	{
	//	BG_SetImpactPoint(&targ->client->ps, IMPACTPOINT_CHEST + rand()&1);
...
/*	else if (targ->client)	// jet Pilot - prevent crash when shooting non-players
	{
		if (targ->health > 0 && mod == MOD_FLAMETHROWER)
		{
	//		BG_SetPainType(&targ->client->ps, IMPACTPOINT_FIRE);		
		}
		else
		{
	//		BG_SetImpactPoint(&targ->client->ps, IMPACTPOINT_UNUSED);
		}
	}
	*/
	...
	//		vec3_t	forward, angles;
		...
		if ( (level.time > targ->pain_debounce_time)&&
			...
			targ->health - take > 0 &&
			!(targ->client->ps.eFlags & EF_POISONED)) {	//----(SA)	
			int	time=0;
...
//	Team_CheckHurtCarrier(targ, attacker);	
	...
	if ( attacker->client && targ->client && targ != attacker && targ->health > limbo_health && mod != MOD_SWITCHTEAM && mod != MOD_SWAP_PLACES && mod != MOD_SUICIDE && mod != MOD_FEAR && g_hitsounds.integer ) 
	{		
	...
	if( headShot ) {
		if (  targ->client->sess.skill[SK_BATTLE_SENSE] < 6 || targ->client->sess.skill[SK_BATTLE_SENSE] >= 8)
		targ->client->ps.eFlags |= EF_HEADSHOT; 
...
//		if (targ->client) hitLoc = BG_GetImpactPoint(&targ->client->ps);
		
//		if ( hitLoc == IMPACTPOINT_UNUSED || hitLoc > NUM_ANIM_COND_IMPACTPOINT )         
//		{
			G_Script_ScriptEvent( targ, "pain", va("%d %d", targ->health, targ->health+take) );
//		}
//		else if (targ->client)
//		{
//			BG_UpdateConditionValue(targ->client->ps.clientNum, ANIM_COND_IMPACT_POINT, hitLoc, qtrue);
//			G_Script_ScriptEvent( targ, "pain", va("%d %d", targ->health, targ->health+take) );
//		}
#ifndef NO_BOT_SUPPORT	
