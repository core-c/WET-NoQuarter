...
static animStringItem_t animEventTypesStr[] =
{
	...
	{"PAIN_GUT"					, -1},
	{"PAIN_GROIN"				, -1},
	{"PAIN_SHOULDER_RIGHT"		, -1},
	{"PAIN_SHOULDER_LEFT"		, -1},
	{"PAIN_KNEE_RIGHT"			, -1},
	{"PAIN_KNEE_LEFT"			, -1},				// 46
	{"PAIN_FIRE"				, -1},
	{"PAIN_STUN"				, -1},
	{"DEATH_FROM_BEHIND"		, -1},
	{"DEATH_FROM_ABOVE" 		, -1},				// 50
	{"PLANT_LANDMINE"			, -1},
	{"FIRING_BAYONETTE"			, -1},
	{"STAND_RADIO"				, -1},
	{"IMPLORE_MEDIC"			, -1},
...
int BG_ExecuteCommand( playerState_t *ps, animModelInfo_t *animModelInfo, animScriptCommand_t *scriptCommand, qboolean setTimer, qboolean isContinue, qboolean force )
{
	...
	// jaquboss - anybody going to kill me? I just want to return duration for torso only anims
	// CHECK IF ANYTHING CRASH
	/*if (!playedLegsAnim) {
		return -1;
	}*/
...
int BG_AnimScriptAnimation( playerState_t *ps, animModelInfo_t *animModelInfo, scriptAnimMoveTypes_t movetype, qboolean isContinue )
{
	...
	if( ps->eFlags & EF_DEAD && movetype != ANIM_MT_FALLEN && movetype != ANIM_MT_FLAILING && movetype != ANIM_MT_DEFENDING )
	{
		return -1;
	}
	...
	if(movetype == ANIM_MT_FALLEN && !g_realHead.integer) 
	{
		...
		{
			return( BG_ExecuteCommand(ps, animModelInfo, scriptCommand, setTimer, qfalse, qtrue) != -1 );
		}
	}
#endif
...
int	BG_AnimScriptEvent( playerState_t *ps, animModelInfo_t *animModelInfo, scriptAnimEventTypes_t event, qboolean isContinue, qboolean force )
{
	...
	if( event != ANIM_ET_DEATH && event != ANIM_ET_DEATH_FROM_BEHIND && ps->eFlags & EF_DEAD 
#ifdef __SELFDEFENSE__
		&& event != ANIM_ET_DEFEND_RAISE && event != ANIM_ET_DEFEND_LOWER && event != ANIM_ET_FIREWEAPON
#endif
		) {
		return -1;
	}
...
/*	int				hitLoc = 0;
	...
	hitLoc = BG_GetPainType(ps);

	if (hitLoc)
	{			
		if (hitLoc > IMPACTPOINT_UNUSED && 	hitLoc < NUM_ANIM_COND_IMPACTPOINT )
		{
			BG_UpdateConditionValue( ps->clientNum, ANIM_COND_IMPACT_POINT, hitLoc, qtrue );
		}
		else
		{
			BG_SetPainType(ps, 0);
...
/*	if ( ps->stats[STAT_HEALTH]/ps->stats[STAT_MAX_HEALTH] >= 0.66f )
		BG_UpdateConditionValue( ps->clientNum, ANIM_COND_HEALTH_LEVEL, 3, qtrue );
	else if ( ps->stats[STAT_HEALTH]/ps->stats[STAT_MAX_HEALTH] >= 0.33f )
