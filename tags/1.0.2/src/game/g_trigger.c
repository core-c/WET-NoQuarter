...
{
	int			 cnt, players = 0;			// number of ents in trigger
	int			 entList[MAX_GENTITIES];	// list of entities
//	vec3_t		 mins, maxs;				// trigger's bounding box
	gentity_t	 *tnt;						// temp ent for counting players
	int			 i;							// iterator
//	clipHandle_t chandle;					// used to grab the trigger's bbox

	ent->activator = activator;

	if (ent->numPlayers > 1)
	{
		cnt = trap_EntitiesInBox( ent->r.mins, ent->r.maxs, entList, MAX_GENTITIES);
		for ( i = 0; i < cnt; i++)
		{
		...
		{		
	//		CPx(activator-g_entities, va("%d more players to activate", ent->numPlayers - players));
			return;
		}
	}	
...
void heal_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	for( i = 0; i < clientcount; i++) {
		...
		touchClients[i]->client->ps.eFlags &= ~EF_POISONED;
