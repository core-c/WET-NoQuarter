...
// jaquboss - arty noob, todo autokick?
	...
	for (i = 0; i < level.numConnectedClients; i++)
	{
		...
		if (!best || (cl->sess.aWeaponStats[WS_ARTILLERY].kills+cl->sess.aWeaponStats[WS_AIRSTRIKE].kills) > (best->sess.aWeaponStats[WS_ARTILLERY].kills+best->sess.aWeaponStats[WS_AIRSTRIKE].kills)) 
		{
			best = cl;
		}			
	}
	if( best && (best->sess.aWeaponStats[WS_ARTILLERY].kills+best->sess.aWeaponStats[WS_AIRSTRIKE].kills) > 5) 
	{
		best->hasaward = qtrue; 
		Q_strcat(buffer, 1024, va(";%s ^7(%i); %i ", 
			best->pers.netname, best->sess.aWeaponStats[WS_ARTILLERY].kills+best->sess.aWeaponStats[WS_AIRSTRIKE].kills, best->sess.sessionTeam));	
	}
	else
	{
		Q_strcat(buffer, 1024, va(";%s ;%i ", "", -1));
...
//	CHECKSTAT1( sess.kills );										// most kills
//	CHECKSTAT1( ps.persistant[PERS_SCORE] );						// highest experience
//	CHECKSTAT3( sess.rank, medals, ps.persistant[PERS_SCORE] );		// highest rank
//	CHECKSTATMIN( ps.persistant[PERS_GOOMBAS], 1 );					// jP - most Goombas, minimum of one
//	CHECKSTAT1( ps.persistant[PERS_BESTKILLSPREE] );				// jP - best killing spree
//	CHECKSTATSKILL( SK_EXPLOSIVES_AND_CONSTRUCTION );				// headshots
//	CHECKSTAT1( ps.persistant[PERS_DARWINDEATHS] );					// darwin award
//	CHECKSTAT1( ps.persistant[PERS_DARWINDEATHS] );					// falling to death (isaac newton award)
//	CHECKSTATSKILL( SK_LIGHT_WEAPONS );								// best medic
//	CHECKSTATSKILL( SK_HEAVY_WEAPONS );								// best engineer
//	CHECKSTATSKILL( SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS );	// highest light weapons accuracy
//	CHECKSTAT1( acc );												// highest overall accuracy
//	CHECKSTATMIN( sess.team_kills, 5 );								// No friends award
//	CHECKSTATTIME( ps.persistant[PERS_SCORE], pers.enterTime );		// welcome newbie 
