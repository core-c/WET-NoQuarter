...
void CG_NewClientInfo( int clientNum ) {
	...
	if( clientNum == cg.clientNum ) {
		...
		// jaquboss this is most probably making any selection bugs, shouldn't be used if we 
		//			don't have cgs.clientInfo[cg.clientNum] set, that is done at end of this function
		if ( !cg.loading ) 
		CG_LimboPanel_Setup();
...
void CG_NewPCInfo( int clientNum ) {
	...
	//CG_Printf( "pcinfo for player %i changed \n", clientNum );
...
void CG_AddOnScreenText ( const char *text, vec3_t origin )
{
	...
	if ( cg.specOnScreenNamesNum >= 16 )
		...
		if (CG_WorldCoordToScreenCoordFloat( origin, &x, &y )){
			...
			dist = VectorDistance(origin, cg.refdef.vieworg);
...
static void CG_PlayerSprites( centity_t *cent ) 
{
	...
	{
		CG_PlayerFloatText( cent, ci->name, 64 );
		return;
	}

	if ( cent->currentState.powerups & (1<<PW_INVULNERABLE) ) 
	{
		CG_PlayerFloatSprite( cent, cgs.media.spawnInvincibleShader, 56, numIcons++ );
	//	return;
	}
		...
		&& sameTeam )
	{
		if ( cg.snap->ps.stats[ STAT_PLAYER_CLASS ] == PC_MEDIC)
			CG_PlayerFloatSprite( cent, cgs.media.medicReviveShader, 8, numIcons++ );
		else if ( cgs.clientinfo[cg.clientNum].skill[SK_MEDIC] >= 9)
			CG_PlayerFloatSprite( cent, cgs.media.resuscitateShader, 8, numIcons++ );
	}

	if ( cent->currentState.eFlags & EF_CONNECTION && sameTeam  ) 
	{
		CG_PlayerFloatSprite( cent, cgs.media.disconnectIcon, 48, numIcons++ );
	//	return;
	}

	if ( cent->currentState.eFlags & EF_POISONED && sameTeam  ) 
	{
		CG_PlayerFloatSprite( cent, cgs.media.poisonedIcon, 48, numIcons++ );
	//	return;
	}
	...
	{
		CG_PlayerFloatSprite( cent, cent->voiceChatSprite, 56, numIcons++ );
	//	return;
	}
	...
	{
		CG_PlayerFloatSprite( cent, cgs.media.balloonShader, 48, numIcons++ );	
	}
	...
	{
		CG_PlayerFloatSprite( cent, cgs.media.disguiseShader, 56, numIcons++ );
    //  return;
	} 
...
//	CG_PositionEntityOnTag( &body, &body, "tag_torso", 0, NULL );
//	CG_AddRefEntityWithPowerups( &body,	cent->currentState.powerups, ci->team, &cent->currentState, cent->fireRiseDir );
//	cent->pe.bodyRefEnt = body;
...
#ifdef __SSYSTEM__
	if( !(head.hModel = headCharacter->head[pc->headIndex].model) )
		return;
	head.customSkin = headCharacter->head[pc->headIndex].skin[pc->faceIndex];
#else 
		...
		if( !(cent->currentState.eType == ET_CORPSE && cent->currentState.time2 == 1 )) 
		{
			switch(i)
			{
				...
				case ACC_MOUTH2:		// Officer's Cap
					...
					if (cent->currentState.eFlags & EF_HEADSHOT && ci->skill[SK_BATTLE_SENSE] < 8)
						continue;
					// get a rank icon
					if (i == ACC_RANK) {
						int iconNum;
					...
					// helmet shield
					if (i == ACC_MOUTH2) {
						if ( ci->skill[SK_BATTLE_SENSE] < 8 || cent->currentState.eFlags & EF_HEADSHOT )
							continue;
					}				
...
void CG_RenderPlayer( float x, float y, float w, float h, playerInfo_t *pi, clientInfo_t *ci) {
	refdef_t		refdef;
	refEntity_t		body;
	refEntity_t		head;
//	refEntity_t		gun;
	...
	origin[0] = 50 -70 + (len / tan( DEG2RAD(refdef.fov_x) * 0.5 ));
	origin[1] = 0.5 * ( mins[1] + maxs[1] );
	origin[2] = (-0.5 * ( mins[2] + maxs[2] ))-45;
	...
	// add the body
	//
	body.hModel = character->body[cgs.limboPcInfo[pi->classNum][pi->teamNum].bodyIndex].mesh;
	body.customSkin = character->body[cgs.limboPcInfo[pi->classNum][pi->teamNum].bodyIndex].torsoSkin[cgs.limboPcInfo[pi->classNum][pi->teamNum].torsoIndex];
	body.renderfx = renderfx;
	...
	body.customSkin = character->body[cgs.limboPcInfo[pi->classNum][pi->teamNum].bodyIndex].legsSkin[cgs.limboPcInfo[pi->classNum][pi->teamNum].legIndex];
	trap_R_AddRefEntityToScene( &body );
	...
	// add the head
	//
	head.hModel = character->head[cgs.limboPcInfo[pi->classNum][pi->teamNum].headIndex].model;
	head.customSkin = character->head[cgs.limboPcInfo[pi->classNum][pi->teamNum].headIndex].skin[cgs.limboPcInfo[pi->classNum][pi->teamNum].faceIndex];
	...
	for( i = ACC_BELT_LEFT; i < ACC_MAX; i++ ) {
		int	 index=CG_AccIndex(i,&cgs.limboPcInfo[pi->classNum][pi->teamNum]);
		int	 skinIndex=CG_AccSkinIndex(i,&cgs.limboPcInfo[pi->classNum][pi->teamNum]);
...
void CG_DrawPlayer ( rectDef_t *rect, int playerClass, int playerTeam ){
	...
	pi = &cgs.renderedPlayerInfo[playerClass][playerTeam-1];
	...
	CG_RenderPlayer( rect->x, rect->y, rect->w, rect->h, pi, &cgs.clientinfo[cg.clientNum] );
