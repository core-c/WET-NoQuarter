...
gentity_t *SelectSpawnPoint( vec3_t avoidPoint, vec3_t origin, vec3_t angles )
{
	...
	if (!spot) {
		...
		if ( !spot ) {
			G_Error( "Couldn't find a spawn point. Add an \"info_player_intermission\", \"info_player_deathmatch\" or \"info_player_start\"." );
		}
	}
...
void reinforce(gentity_t *ent) {
	...
	spawnPoint = SelectCTFSpawnPoint( ent->client->sess.sessionTeam, ent->client->pers.teamState.state, spawn_origin, spawn_angles, ent->client->sess.spawnObjectiveIndex, qfalse );
	if ( !spawnPoint ) {
		ent->client->ps.pm_flags |= PMF_LIMBO;
...
void ClientSpawn( gentity_t *ent, qboolean revived, qboolean teamChange, qboolean restoreHealth)
{
	...
	int					savedDeathTime;
	qboolean			doWar = (nq_War.integer & WARMODE_ENABLE)? qtrue : qfalse;
	qboolean			validSpawnpoint = qtrue;	// a valid, non-telefragging, spawn location found?..
	...
	if( revived ) {
		...
		if( client->sess.sessionTeam != TEAM_AXIS && client->sess.sessionTeam != TEAM_ALLIES ) {
			...
			// core: if there was no valid (free & non-telefragging) spawnpoint to be found,
			// the player still needs a "spawnpoint" that will provide the camera-view (only).
			// Also set some marker (validSpawnpoint) so we know when not to link the client into the world (in case he cannot spawn right now). It is important!
			// If there isn't even a valid spectator-spawnpoint, then make them spectator (does this make sense??!).. (This should never happen. (safety)).
			if ( !spawnPoint ) {
				validSpawnpoint = qfalse;
				spawnPoint = SelectSpectatorSpawnPoint( spawn_origin, spawn_angles );
				if ( !spawnPoint ) {
...
// dhm - end
	...
	if( ent->client->sess.sessionTeam != TEAM_SPECTATOR ) {
		// core: No spawn location found? then do not link in the client..
		if ( validSpawnpoint ) {
			trap_LinkEntity (ent);
		}
	}
