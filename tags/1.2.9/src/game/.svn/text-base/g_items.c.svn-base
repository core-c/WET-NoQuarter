...
weapon_t G_GetSecondaryWeaponForClient( gclient_t *client, weapon_t primary )
{
	...
	if ( secondary == WP_NONE && client->sess.playerType == PC_SOLDIER && client->sess.skillBits[SK_SOLDIER] & (1<<4)  ) {
		if ( primary != WP_NONE && primary != WP_THOMPSON && primary != WP_MP40 ) {
			...
			if ( soldier4 ) {
				...
				// picking up SMG..
					...
					if ( soldier8 || (g_unlockWeapons.integer & UNLOCK_WEAPONS_SOLDIER) ) {
						...
						else if ( wantSMG && hasPrimary && hasAkimbo ) G_DropWeapon( other, secondary );
						else if ( wantSMG && hasPrimary && hasSecondary && !hasAkimbo ) G_DropWeapon( other, secondary );
				...
				// picking up some primary weapon..
					...
					if ( wantSMG && hasPrimarySMG && hasSecondary ) G_DropWeapon( other, secondary );			// keep the SMG, it will become a secondary weapon
					else if ( wantSMG && hasSecondarySMG && hasPrimary ) G_DropWeapon( other, primaryWeapon );	// switch the primary weapon
