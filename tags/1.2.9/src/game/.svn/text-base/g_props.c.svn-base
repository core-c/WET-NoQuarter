...
{
	self->s.modelindex = GAMEMODEL_WORLD_BOX_32;
...
{
	self->s.modelindex = GAMEMODEL_WORLD_BOX_48;
...
{
	self->s.modelindex = GAMEMODEL_WORLD_BOX_64;
...
void InitProp ( gentity_t *ent ) {
	...
	if (!Q_stricmp (ent->classname, "props_bench"))
		ent->s.modelindex2 = GAMEMODEL_WORLD_BENCH;
	else if (!Q_stricmp (ent->classname, "props_radio"))
		ent->s.modelindex2 = GAMEMODEL_WORLD_RADIO;
		...
		ent->s.modelindex2 = GAMEMODEL_WORLD_CRATE_32X64;
		...
		ent->s.modelindex2 = GAMEMODEL_WORLD_CASTLEBED;
	else if (!Q_stricmp (ent->classname, "props_radioSEVEN"))
		ent->s.modelindex2 = GAMEMODEL_WORLD_RADIOS;
...
void SP_Props_ChairChat(gentity_t *ent) {
	ent->s.modelindex = GAMEMODEL_WORLD_CHAIR_CHAT;
...
void SP_Props_ChairChatArm(gentity_t *ent) {
	ent->s.modelindex = GAMEMODEL_WORLD_CHAIR_CHATARM;
	SP_Props_ChateauChair(ent);
}
...
void SP_Props_Desklamp (gentity_t *ent)
{
	...
	ent->s.modelindex = GAMEMODEL_WORLD_DESKLAMP;
...
void SP_Props_Flamebarrel (gentity_t *ent)
{
	...
	if (ent->spawnflags & 4)
		ent->s.modelindex = GAMEMODEL_WORLD_BARREL_C;
	else if (ent->spawnflags & 1)
		ent->s.modelindex = GAMEMODEL_WORLD_BARREL_D;
	else
		ent->s.modelindex = GAMEMODEL_WORLD_BARREL_B;
...
{
	self->s.modelindex = GAMEMODEL_WORLD_CRATE_64;
...
{
	self->s.modelindex = GAMEMODEL_WORLD_CRATE_32;
...
//	trap_SetBrushModel (self, self->model);
	...
	self->s.modelindex = GAMEMODEL_WORLD_FOOTLOCKER;
...
void props_flamethrower_use (gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	if (ent->spawnflags & 2) {
 	...
 	if (ent->random) {
 		ent->timestamp = level.time + ( rand()%( (int) ent->random * 1000) );
	}
	else {
		ent->timestamp = level.time;
	}
