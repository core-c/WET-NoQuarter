...
qboolean G_shrubbot_medpack(gentity_t *ent, int skiparg)
{
	...
	AP(va("chat \"^dmedpack: ^7%s ^9was given a medpack^7\"", ent->client->pers.netname));
...
qboolean G_shrubbot_ammopack(gentity_t *ent, int skiparg)
{
	...
	AP(va("chat \"^dammopack: ^7%s ^9was given an ammopack^7\"", ent->client->pers.netname));
...
qboolean G_shrubbot_give(gentity_t *ent, int skiparg)
{
	...
	if ( ExecGive( vic, item, amt?amt:"", amt2?amt2:"") ) {
		if (ent && ent->client && ent->client->ps.clientNum == vic->client->ps.clientNum) {
			...
			AP(va( "chat \"^dgive: ^7%s ^9received a cheat (^7%s^9) from %s^7\"", vic->client->pers.netname, item, (ent?ent->client->pers.netname:"^3SERVER CONSOLE") ));
		}
	}
