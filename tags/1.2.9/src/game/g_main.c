...
cvarTable_t		gameCvarTable[] =
{
	...
	{ &nq_playerspawning,					"nq_playerspawning",					"1",									0,															0,		qfalse,		qfalse },

	{ &nq_scriptMode,						"nq_scriptMode",						"7", 									0,															0,		qfalse,		qfalse },
...
static void ServerCheck ( void ) {
	...
	if (pakNames[0]) {
		...
		if (!strstr(pakNames, "nq_v1.2.9_3")) {
			G_Printf("^1Warning: No nq_v1.2.9_3.pk3 found on server! Missing essential files!\n" );
			msgWhine = qtrue;
		}
		if (!strstr(pakNames, "nq_b_v1.2.9_5")) {
			G_Printf("^1Warning: No nq_b_v1.2.9_5.pk3 found on server! Missing essential files!\n" );
			msgWhine = qtrue;
		}
...
void BeginIntermission( void ) {
	...
	if ( g_gametype.integer == GT_WOLF_MAPVOTE ) {
		...
		// core: Using the MAPVOTE_NO_RANDOMIZE bitflag for a real shuffle..
		...
		if ( !(g_mapVoteFlags.integer & MAPVOTE_NO_RANDOMIZE) ) {
			...
			bspptrTmp = bspptr;
			for (i=0; i<maxMaps; ++i) {
				Q_strncpyz( shuffledNames[i], bspptrTmp, sizeof(shuffledNames[i]) );
				shuffled[i] = qfalse;
		...
		for ( i = 0; i < len; i++ ) {
			...
			// map excluded?..
			if ( strstr(g_excludedMaps.string, va(":%s:",level.mapvoteinfo[i].bspName)) ) {
				level.sortedMaps[i] = -1;
			...
			// played too recently
			else if ( level.mapvoteinfo[i].lastPlayed != -1 && level.mapvoteinfo[i].lastPlayed <= g_minMapAge.integer ) {
				level.sortedMaps[i] = -1;
				level.mapvoteinfo[i].zOrder = 0;
				level.mapVoteNumMaps--;
				level.mapvoteinfo[i].lastPlayed++;
...
// forty - arty/airstrike rate limiting.
	...
	if ( level.framenum % NQ_AURA_FRAME_FREQ == 0 ) {
		for( i = 0; i < level.numConnectedClients; ++i ) {
			...
			// Lucel
			(&g_entities[level.sortedClients[i]])->client->ps.stats[STAT_TEAMAURA] = 0;
