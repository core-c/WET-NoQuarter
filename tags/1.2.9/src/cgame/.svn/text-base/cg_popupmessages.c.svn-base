...
// First Aid
	{ "Additional Ammo",
      ...
      "Improved Poison Needles",						// forces to drop weapons of victim
...
// Heavy Weapons
	{ "Extra Heavy Weapon Ammunition",					// Carry one additional belt/round
      ...
      "Improved Flamethrower",							// Heavy weapons use less charge
...
void CG_InitPMGraphics( void )
{
	...
	cgs.media.pmImages[PM_DESTRUCTION]		= trap_R_RegisterShaderNoMip( "sprites/destroy" );
...
int CG_TimeForPopup( popupMessageType_t type ) {
	...
	return (int)(cg_pmPopupTime.value*SECONDS_1);
}

int CG_TimeForBigPopup( popupMessageBigType_t type ) {

	return (int)(cg_pmBigPopupTime.value*SECONDS_1);
}
...
void CG_UpdatePMLists( void ) {
	...
	if ((listItem = cg_pmWaitingList)) {
		...
		if( cg.time > t ) {
			if( listItem->next ) {
				...
				if( cg.time > t + (cg_pmWaitTime.value*SECONDS_1) + (cg_pmFadeTime.value*SECONDS_1) ) {
	...
	lastItem = NULL;
	while( listItem ) {
		int t = (CG_TimeForPopup( listItem->type ) + listItem->time + (cg_pmWaitTime.value*SECONDS_1) + (cg_pmFadeTime.value*SECONDS_1));
...
void CG_AddPMItem2( popupMessageType_t type, const char* msg1, qhandle_t actionShader, const char* msg2,  int actionShaderWScale, vec3_t color)
{
	...
	// redeye - no need to take care of cg_popupMessageFilter, messages are already filtered in caller
...
void CG_DrawPMItems( void )
{
	...
	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR ) {
		// print above chat lines, but as low on screen as possible..
		float lineHeight = (cg_smallFont.integer & SMALLFONT_CHATS)? 7.5f : 9.0f;
		float numlines = (cg_teamChatHeight.integer < TEAMCHAT_HEIGHT)? cg_teamChatHeight.integer : TEAMCHAT_HEIGHT;
	...
	t = cg_pmWaitingList->time + CG_TimeForPopup( cg_pmWaitingList->type ) + (cg_pmFadeTime.value*SECONDS_1);
	if( cg.time > t ) {
		colourText[3] = colour[3] = 1 - ((cg.time - t) / (cg_pmFadeTime.value*SECONDS_1));
	}
	...
	for( i = 0; i < 4 && listItem; i++, listItem = listItem->next ) {
		...
		size = altSize = ( cg_drawSmallPopupIcons.integer )? PM_ICON_SIZE_SMALL : PM_ICON_SIZE_NORMAL;

		y -= size + 2;

		t = listItem->time + CG_TimeForPopup( listItem->type ) + (cg_pmWaitTime.value*SECONDS_1);
		if( cg.time > t ) {
			colourText[3] = colour[3] = 1 - ((cg.time - t) / (cg_pmFadeTime.value*SECONDS_1));
		}
		else {
...
void CG_DrawPMItemsBig( void ) {
	...
	t = cg_pmWaitingListBig->time + CG_TimeForBigPopup( cg_pmWaitingListBig->type ) + (cg_pmWaitTime.value*SECONDS_1);
	if( cg.time > t ) {
		colourText[3] = colour[3] = 1 - ((cg.time - t) / (cg_pmFadeTime.value*SECONDS_1));
	}
	...
	trap_R_SetColor( colourText );
	// draw
	CG_DrawPic( 640 - 106, y, 48, 48, cg_pmWaitingListBig->shader );
	// decolorize
	for (i=0; i<3; i++ )  {
		colourText[i] = 1.f;
	}
	...
	CG_Text_Paint_Ext( 640 - 4 - w - 50, y + 56, 0.22f, 0.24f, colourText, cg_pmWaitingListBig->message, 0, 0, 0, &cgs.media.limboFont2 );
}
	...
	switch( cent->currentState.effect1Time ) {
		...
		case PM_DESTRUCTION:
			if ( cgs.clientinfo[cent->currentState.clientNum].infoValid == qtrue ) {
				switch( cent->currentState.effect2Time ) {
					...
					case 1:
						return va( "%s has been destroyed.", Q_strCapitalize((char *)CG_ConfigString(CS_OID_TRIGGERS + cent->currentState.effect3Time)) );
				}
			}
			else {
				switch( cent->currentState.effect2Time ) {
		...
		case PM_MINES:
			...
			if ( cg_locations.integer & LOC_LANDMINES ) {
				...
				if( !locStr || !*locStr ) {
					return va( "%sSpotted by ^7%s", TXTCOLOR_OBJ, cgs.clientinfo[cent->currentState.clientNum].name );
				}
				return va( "%sSpotted by ^7%s%s at %s", TXTCOLOR_OBJ, cgs.clientinfo[cent->currentState.clientNum].name, TXTCOLOR_OBJ, locStr );
			}
			else {
			...
			switch( cent->currentState.density ) {
				...
				case 1: // team_WOLF_redflag/team_WOLF_blueflag
					if ( cent->currentState.clientNum != -1 && Q_stricmpn(cgs.clientinfo[cent->currentState.clientNum].name, "", 64) ) {
						...
						return va( "%s%s%s is returned!", TXTCOLOR_OBJ, CG_ConfigString( CS_STRINGS + cent->currentState.effect3Time ), TXTCOLOR_OBJ );
					}
				...
				case 3: // touching a team_WOLF_checkpoint
...
void CG_PlayPMItemSound( centity_t *cent )
{
	...
	switch( cent->currentState.effect1Time ) {
		case PM_DYNAMITE:
			...
			break;
		case PM_MINES:
			// core: Not so spammy please..
			// Say it once, then stay quiet (about mines spotted) for 10 seconds.
			if ( !lastSoundTime || (cg.time > lastSoundTime) ) {
				...
				if( cgs.clientinfo[cg.clientNum].team != cent->currentState.effect2Time ) {
					...
					// core: the next sound is allowed to start after 10 seconds from now..
					lastSoundTime = cg.time + 10000;
				}
			}
			break;
		case PM_OBJECTIVE:
