	...
	case ME_PLAYER:
		...
		if( cent-cg_entities == cg.clientNum ) {
			...
			if( !scissor ) {
				...
				// core: interpolate the player position & yaw..
		...
		if( scissor ) {
			icon_pos[0] = mEnt->automapTransformed[0] - scissor->tl[0] + x - ((float)icon_size * (scissor->zoomFactor / 5.159f));
			icon_pos[1] = mEnt->automapTransformed[1] - scissor->tl[1] + y - ((float)icon_size * (scissor->zoomFactor / 5.159f));
...
void CG_DrawMortarMarker( int px, int py, int pw, int ph, qboolean draw, mapScissor_t *scissor, int expand ) {
	if( (cg.lastFiredWeapon == WP_MORTAR_SET || cg.lastFiredWeapon == WP_MORTAR2_SET ) && cg.mortarImpactTime >= 0 ) {
