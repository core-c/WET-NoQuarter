...
void CG_ParseNQinfo( void ) {
	...
	cgs.panzerRestrictions = atoi( Info_ValueForKey( info, "TPR" ) ) * 0.01f;
...
static void CG_ServerCommand( void ) {
	...
	if ( !Q_stricmp( cmd, "cpm" ) ) {
		...
		if (((cg_popupMessageFilter.integer & POPUP_FILTER_CONNECT) &&
			(strstr(msg, " connected") || strstr(msg, " disconnected"))))
		{
