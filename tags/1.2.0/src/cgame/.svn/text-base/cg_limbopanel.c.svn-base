...
// IRATA panzer perc restriction
qboolean CG_IsPanzerWeapon( weapon_t weap ) {
	if( weap == WP_BAZOOKA || weap == WP_PANZERFAUST) {
			return qtrue;
	}
	return qfalse;
}
...
qboolean CG_LimboPanel_WeaponIsDisabled( int index ) {
	...
	// IRATA: never restricted
	if (index == WP_THOMPSON || index == WP_MP40) {
		return qfalse;
	}
	...
	*/
	if(CG_IsHeavyWeapon( classinfo->classWeapons[index] ) ) {			
	    int count = CG_LimboPanel_TeamCount(-1);
	    
		if( wcount >= ceil( count * cgs.weaponRestrictions ) ) {
			return qtrue;
		}		
	    // IRATA team_panzerrestriction	
		if(CG_IsPanzerWeapon(classinfo->classWeapons[index]) ) {
			if( (count/(cgs.maxclients/2.00f)) <= cgs.panzerRestrictions ) {
				return qtrue;
			}	
		}
	}
...
qboolean CG_LimboPanel_RealWeaponIsDisabled( weapon_t weap ) {
	...
	// IRATA: never restricted 
	if (weap == WP_THOMPSON || weap == WP_MP40) {
		return qfalse;
	}
	...
	if( CG_IsHeavyWeapon( weap ) ) {
		...
		// IRATA team_panzerrestriction
		if( CG_IsPanzerWeapon( weap ) ) {
			if( (count/(cgs.maxclients/2.f)) <= cgs.panzerRestrictions ) {
				return qtrue;
			}
		}
	}	
