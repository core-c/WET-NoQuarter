...
//	ent->s.loopSound = ent->soundLoop;
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		if ( ent->flags & FL_KICKACTIVATE )
			G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundKickendo );
...
//	int		partial;
	...
	if ( isblocked ) {
		...
		{
			if (ent->flags & FL_KICKACTIVATE)
				G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundKickopen );
			else if (ent->flags & FL_SOFTACTIVATE)
...
void G_TryDoor(gentity_t *ent, gentity_t *other, gentity_t *activator){
	...
	qboolean	kicking = qfalse;

	walking = (qboolean)(ent->flags & FL_SOFTACTIVATE);
	kicking = (qboolean)(ent->flags & FL_KICKACTIVATE);
		...
		if( self->grenadeFired > 1 ) {
			...
			for( e = 0; e < listedEntities; e++ ) {
				...
				if( check->client || check->s.eType == ET_CORPSE ) {
					...
					BG_AdjustDarwin( &check->client->ps, 1 );
