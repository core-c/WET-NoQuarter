...
//unlagged - smooth clients #1
	...
	//
	// IRATA:
	// Aura also removed if p is poisened or in teleport
	if ( level.framenum % NQ_AURA_FRAME_FREQ == 0 &&
		 !( ent->client->ps.eFlags & (EF_DEAD|EF_PLAYDEAD|EF_POISONED|EF_TELEPORT_BIT) ) 
		)
	{
		int			skill		= 0;
		int			aurabit		= 0;
...
// first reset auras
/*	if ( level.time > ent->auraCheckTime ){
		...
		for ( i = 0 ; i < level.numConnectedClients; i++ ) 
		{
			...
			if ( cl->sess.skillBits[skill] & (1<<5) &&
			...
			ent->auraCheckTime = level.time + 200;
		}
	}*/
