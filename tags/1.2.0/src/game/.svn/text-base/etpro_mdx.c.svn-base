...
#else
	if (ent->s.eType == ET_PLAYER) 
		refent->hModel = character->body[ent->client->pers.pc.bodyIndex].mesh;
	else 
		refent->hModel = character->body[0].mesh; // FIXME? avoids crash BTW
#endif
...
void mdx_PlayerAngles( gentity_t *ent, vec3_t legsAngles, vec3_t torsoAngles, vec3_t headAngles, qboolean doswing )
{
	...
	if ( BG_GetConditionBitFlag( client->ps.clientNum, ANIM_COND_MOVETYPE, ANIM_MT_REVIVE))
	{
...
void mdx_PlayerAnimation(gentity_t *ent)
{
	...
	{
		ent->shufflePause = level.time + 1750;
	}

	// do the shuffle turn frames locally
	if ( level.time > ent->shufflePause && !(ent->s.eFlags & EF_DEAD) && !(ent->s.eFlags & EF_PLAYDEAD) && !(ent->s.eFlags & EF_MOUNTEDTANK) && ent->legsFrame.yawing ) {
