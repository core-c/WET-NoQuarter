...
int PM_RaiseAnimForWeapon ( int weapon ) {
	switch( weapon ) {
		...
		default:
			if( pm && ( pm->skillBits[SK_LIGHT_WEAPONS] & (1<<5)) && IS_FAST_SWITCH_WEAPON(weapon) )
	...
	if ( pm->waterlevel <= 1 ) {
		if ( pml.walking && !(pml.groundTrace.surfaceFlags & SURF_SLICK) ) {
			...
			if ( ! (pm->ps->pm_flags & PMF_TIME_KNOCKBACK) ) {
				float friction = (pm->ps->pm_flags & PMF_STUNNED) ? 0.5f : 1.f ;
...
//
		...
		(pm->ps->weapon == WP_MORTAR				)) 
	{
		...
		else if( pm->skillBits[SK_HEAVY_WEAPONS] & (1<<3) ) 
		{
			scale *= 0.75;
		}
		else 
	...
	if (pm->ps->weapon == WP_FLAMETHROWER && !( pm->skillBits[SK_HEAVY_WEAPONS] & (1<<8) ) ) { // trying some different balance for the FT
		if( !( pm->skillBits[SK_HEAVY_WEAPONS] & (1<<3) ) || pm->cmd.buttons & BUTTON_ATTACK )
			scale *= 0.7;
	}

	if ((pm->ps->weapon == WP_BAR) ||
		(pm->ps->weapon == WP_BAR_SET)){ // Meyer - trying some different balance for the BAR
		if( pm->skillBits[SK_HEAVY_WEAPONS] & (1<<7) )
		{
			scale *= 0.9;
		}
...
//	wishvel[2] = 0;
	...
	} else if ( pm->ps->pm_flags & PMF_STUNNED ) { 
		accelerate = pm_accelerate/1.5f;
	} else {
...
static void PM_BeginWeaponReload( int weapon ) {
	...
	if(pm->ps->weaponstate != WEAPON_READY && pm->ps->weaponstate != WEAPON_FIRING && pm->ps->weaponstate != WEAPON_FIRINGALT ) // what is point of allowing to reload weapon again when it is reloading ?!
	{		
		return;
	}
...
{	
	...
	{/*
#ifdef DEBUG
		Com_Printf("Not owning target weapon! \n");
#endif*/
		return;
	}
	...
	if ( switchtime > 0 ) {
		if ( newweapon == weapAlts[oldweapon] )
		{
			if ( BG_Weapons[oldweapon].weapAnimations[PM_AltSwitchFromForWeapon(oldweapon)].time )
				switchtime =  BG_Weapons[oldweapon].weapAnimations[PM_AltSwitchFromForWeapon(oldweapon)].time;
		}
		else
		{
			if ( BG_Weapons[oldweapon].weapAnimations[PM_DropAnimForWeapon(oldweapon)].time )
...
//			break;
	...
	if ( switchtime > 0 )
	{
		if(newweapon == weapAlts[oldweapon])
		{
			if ( BG_Weapons[newweapon].weapAnimations[PM_AltSwitchToForWeapon(newweapon)].time )
				switchtime =  BG_Weapons[newweapon].weapAnimations[PM_AltSwitchToForWeapon(newweapon)].time;
		} 
		else
		{
			if ( BG_Weapons[newweapon].weapAnimations[PM_RaiseAnimForWeapon(newweapon)].time )
...
void PM_CheckForReload( int weapon )
{
	...
	if (pm->cmd.wbuttons & WBUTTON_ATTACK2 && pm->pmext->weapAltReload)
	...
	for(wp=0;wp<WP_NUM_WEAPONS;wp++) 
	{
		...
		if( COM_BitCheck( pm->ps->weapons, wp) ) 
		{
			...
			if(pm->ps->weapHeat[wp]) 
			{
				...
				if(( pm->skillBits[SK_HEAVY_WEAPONS] & (1<<2) && IS_HEAVY_WEAPON(wp)) || pm->ps->stats[STAT_TEAMAURA] & AURABIT_ENGINEER ) 
				{
...
//	int		increase, decrease, i;
	...
	float	viewchange, speed, anglechange, cmdTime, wpnScale;
	...
	if ( BG_Weapons[pm->ps->weapon].wpnScale )
	...
	if ( wpnScale ) 
	{
		...
		for(i = 0; i < 2; i++) 
		{
			speed += fabs(pm->ps->velocity[i]);
		}
		// take player view rotation into account
		for( i = 0; i < 2; i++ ) 
		{
			anglechange += fabs( AngleSubtract(  SHORT2ANGLE(AngleNormalizeInt(pm->cmd.angles[i])),
												SHORT2ANGLE(AngleNormalizeInt(pm->oldcmd.angles[i]))));
		}

		if ( BG_IsScopedWeapon( pm->ps->weapon ) || BG_Weapons[pm->ps->weapon].velocity2spread == WEAPON_YES )
		{
			if ( BG_Weapons[pm->ps->weapon].velocity2spread != WEAPON_NO )
				viewchange += speed;
		} 
		if ( !BG_IsScopedWeapon( pm->ps->weapon ) || BG_Weapons[pm->ps->weapon].viewchange2spread == WEAPON_YES )
		{
			if ( BG_Weapons[pm->ps->weapon].viewchange2spread != WEAPON_NO )
...
static qboolean PM_CheckGrenade() {
	...
	if( pm->ps->grenadeTimeLeft > 0 ) { // do we have time?
		...
		{
			if( pm->ps->weaponDelay == GetAmmoTableData(pm->ps->weapon)->fireDelayTime ) { // forced to throw, or we just released button and returned here ( after setting all stuff )
...
static qboolean PM_WeaponHasAmmo( int i )
{
	...
	if( i == WP_PLIERS /*|| i == WP_FIREBOLT*/)
		return qtrue;
	
	if (!(pm->ps->ammo[BG_FindAmmoForWeapon(i)]) &&
		!(pm->ps->ammoclip[BG_FindClipForWeapon(i)]) ) {
		return qfalse;
	}
...
static void PM_AltWeapon( void )
{
...
#ifdef GAMEDLL
	if( pm->ps->weapon == WP_GRENADE_LAUNCHER || pm->ps->weapon == WP_GRENADE_PINEAPPLE  || pm->ps->weapon == WP_SMOKE_MARKER)
	{
		pm->voice(pm->ps->clientNum, "fireinthehole");
		return;
	}
	// jet Pilot - 'cover me!' vsay for engies with pliers out
	if(pm->ps->weapon == WP_PLIERS)
	{
	...
	else if( IS_BIPOD_WEAPON(pm->ps->weapon) )
	{
		if( !(pm->ps->eFlags & EF_PRONE) ) 
		{
			return;
	...
	if( ( pm->ps->weaponstate == WEAPON_RAISING || pm->ps->weaponstate == WEAPON_DROPPING ) &&
		( (original == WP_GPG40					|| num == WP_GPG40		|| original == WP_M7			|| num == WP_M7)			||
		  ...
		  (original == WP_BAR_SET				|| num == WP_BAR_SET) ||
		  (original == WP_MOBILE_BROWNING_SET	|| num == WP_MOBILE_BROWNING_SET) ) )
		return;

	if( PM_WeaponSelectable( num ) && original != num )
	{	// new weapon is valid
...
static void PM_Weapon( void ) {
	...
	if ( pm->ps->weaponTime > 0 ) {
		pm->ps->weaponTime -= pml.msec;
		if( !(pm->cmd.buttons & BUTTON_ATTACK) && !PM_AltFire() && pm->ps->weaponTime < 0 )
			pm->ps->weaponTime = 0;
...
// holster
	if ( ( pm->ps->pm_flags & PMF_MEDICUSE || ( pm->cmd.serverTime < pm->pmext->kickTime ) || ( pm->ps->pm_flags & PMF_STUNNED ) || ( pm->ps->pm_flags & PMF_LADDER && (PM_REALISM & REALISM_NOLADDERGUN) ) ) && !delayedFire ){
...
/*	if( pm->ps->weapon == WP_TRIPMINE ) {
	...
	switch( pm->ps->weapon )
	{
		case WP_KNIFE:
		case WP_KNIFE_KABAR:
			addTime = pm->ps->weaponstate == WEAPON_FIRINGALT ? 750 : GetAmmoTableData(pm->ps->weapon)->nextShotTime;
			break;
		case WP_PANZERFAUST:
		case WP_BAZOOKA:
		case WP_DYNAMITE:
	...
	if ( BG_Weapons[pm->ps->weapon].recoil ){
		...
		pm->pmext->weapRecoilYaw = cos(pm->cmd.serverTime) * BG_Weapons[pm->ps->weapon].recoilYaw;
		pm->pmext->weapRecoilPitch = fabs(cos(pm->cmd.serverTime)) * BG_Weapons[pm->ps->weapon].recoilPitch;
	} else
	...
	switch( pm->ps->weapon ) 
	{
		...
		case WP_K43_SCOPE:
			...
			pm->pmext->weapRecoilYaw = cos(pm->cmd.serverTime) * .5f;
		...
		case WP_MOBILE_MG42:
			...
			if( pm->ps->pm_flags & PMF_DUCKED || pm->ps->eFlags & EF_PRONE ) {
				pm->pmext->weapRecoilYaw = cos(pm->cmd.serverTime) * .5f;
				pm->pmext->weapRecoilPitch = .45f * fabs(cos(pm->cmd.serverTime)) * .15f;
			} else {
				pm->pmext->weapRecoilYaw = cos(pm->cmd.serverTime) * .25f;
				pm->pmext->weapRecoilPitch = .75f * fabs(cos(pm->cmd.serverTime)) * .2f;
			}
			break;
		case WP_FG42SCOPE:
			pm->pmext->weapRecoilTime = pm->cmd.serverTime;
		...
		case WP_SHOTGUN:
			...
			pm->pmext->weapRecoilYaw = 0.f;
			pm->pmext->weapRecoilPitch = .6f * fabs(cos(pm->cmd.serverTime));
			break;
		...
		case WP_BAR:
			...
			pm->pmext->weapRecoilYaw = 0.f;
			pm->pmext->weapRecoilPitch = .1f * fabs(cos(pm->cmd.serverTime));
			break;
		case WP_BAR_SET:
			pm->pmext->weapRecoilTime = pm->cmd.serverTime;
			pm->pmext->weapRecoilDuration = 10;
			pm->pmext->weapRecoilYaw = 0.f;
			pm->pmext->weapRecoilPitch = .05f * fabs(cos(pm->cmd.serverTime));
			break;
		...
		case WP_STG44:
			...
			pm->pmext->weapRecoilYaw = 0.f;
			pm->pmext->weapRecoilPitch = .15f * fabs(cos(pm->cmd.serverTime));
			break;
		...
		case WP_AKIMBO_SILENCEDCOLT:
			...
			pm->pmext->weapRecoilTime = pm->cmd.serverTime;
			pm->pmext->weapRecoilDuration = pm->skillBits[SK_LIGHT_WEAPONS] & (1<<3) ? 70 : 100;
			pm->pmext->weapRecoilYaw = 0.f;//crandom() * .1f;
			pm->pmext->weapRecoilPitch =  pm->skillBits[SK_LIGHT_WEAPONS] & (1<<3) ? 0.25f : 0.45f * random() * .15f;
			break;
		default:
			pm->pmext->weapRecoilTime = 0;
			pm->pmext->weapRecoilYaw = 0.f;
...
static void PM_Foot( void ) {
	...
	if ( pm->ps->pm_flags & PMF_TIME_LOCKPLAYER || pm->ps->pm_flags & PMF_STUNNED )
		return;
...
static void PM_DropTimers( void ) {
	...
	// jaquboss need to handle separatly
	if ( pm->ps->stats[STAT_STUN] ){
		if ( pml.msec >= pm->ps->stats[STAT_STUN] ) {
			pm->ps->pm_flags &= ~PMF_STUNNED;
			pm->ps->stats[STAT_STUN]= 0;
		} else {
			pm->ps->stats[STAT_STUN] -= pml.msec;
		}
	}
...
void PM_Sprint( void )
{
	...
	{
		if( pm->ps->powerups[PW_ADRENALINE] && !(PM_ADRENALINE & ADF_NOSTAMINA) )
		{
		...
		else
		{
			...
			pm->pmext->sprintTime -= drainRate*5000*pml.frametime;
		}
		// jpw
	...
	{	
		...
		else 
		{
			...
			rechargebase = (pm->ps->stats[STAT_TEAMAURA] & AURABIT_FIELDOPS) ? 1500 : 500;
...
// Check zoom command
	...
	if ( pm->ps->pm_flags & PMF_STUNNED )
	{
		pm->cmd.upmove		= 0; // cannot jump, crouch..
	}
