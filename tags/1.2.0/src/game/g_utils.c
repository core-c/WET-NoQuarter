...
int G_ModelIndex( char *name ) {
	...
	for ( i=0 ; i < bg_numGameModels ; i++ ) {
		if ( !strcmp( name, gameModels[i]))
		{
			return i;
		}
	}

	return ( G_FindConfigstringIndex (name, CS_MODELS, MAX_MODELS, qtrue) + bg_numGameModels );
} 
...
int G_SoundIndex( const char *name ) {
	...
	for ( i=0 ; i < bg_numGameSound ; i++ ) {
		if ( !strcmp( name, gameSounds[i]))
		{
			return i;
		}
	}

	return (G_FindConfigstringIndex (name, CS_SOUNDS, MAX_SOUNDS, qtrue) + bg_numGameSound);
}
...
static qboolean G_LoadCampaignsFromFile( const char *filename ) {
	...
	while ( trap_PC_ReadToken( handle, &token ) ) {
		if( *token.string == '}' ) {
			...
			// tjw: populate the campaign_maps serverinfo cvar
			//      so that the crash happens at server start
			//      instead of when this campaign starts
			//      (if the serverinfo string doesn't have
			//      enough space.)
			if(*ptr && (g_serverInfo.integer & SIF_CAMPAIGN_MAPS)) {
				char dummy[MAX_CVAR_VALUE_STRING];
				int i;
				
				// tjw: set up a string full of '.'
				for(i=0; i<(MAX_CVAR_VALUE_STRING-1); i++)
					dummy[i] = '.';
				dummy[MAX_CVAR_VALUE_STRING-1] = '\0';

				// tjw: make it the same length as "maps"
				dummy[strlen(ptr)] = '\0';
				trap_Cvar_Set("campaign_maps", dummy);
			}
