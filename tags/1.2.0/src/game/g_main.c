...
void CheckIntermissionExit( void ) {
	...
	// rain - for #105
	gclient_t *cl;
	int ready = 0, notReady = 0, botCount = 0; // redeye/IRATA: botCount - scoreboard bot ready fix  
	...
	for(i = 0; i < level.numConnectedClients; i++) {
		...
		if (cl->pers.connected != CON_CONNECTED || cl->sess.sessionTeam == TEAM_SPECTATOR) {
			continue;
		// redeye/IRATA - scoreboard bot ready fix - also ignore bots
	...
	if((100.0f * (ready / ((level.numConnectedClients - botCount) * 1.0f))) >=  
