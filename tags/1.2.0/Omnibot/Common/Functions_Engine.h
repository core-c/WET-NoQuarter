...
typedef struct 
{
	...
	int (*pfnAddBot)(const MessageHelper &_data);
	...
	obBool (*pfnIsInPVS)(const float _pos[3], const float _target[3]);
	...
	obResult (*pfnTraceLine)(obTraceResult *_result, const float _start[3], const float _end[3], const AABB *_pBBox , int _mask, int _user, obBool _bUsePVS);
	...
	//		This function should return the bot class of the entity.
	obResult  (*pfnGetEntityCategory)(const GameEntity _ent, BitFlag64 &_flags);
	...
	void (*pfnDebugLine)(const float _start[3], const float _end[3], const obColor &_color, float _time);

	// Function: pfnDebugRadius
	//		Adds a radius indicator to be displayed at a certain position with radius and color
	void (*pfnDebugRadius)(const float _pos[3], const float _radius, const obColor &_color, float _time);
	...
	// Function: pfnGetLogPath
	//		This function should get the path to where the bot should write its log to.
	const char *(*pfnGetLogPath)();
