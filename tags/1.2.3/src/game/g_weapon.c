...
qboolean Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker, vec3_t start, vec3_t end, int damage, qboolean distance_falloff, qboolean createEvents, int reflectNum);
...
/*	if (tr.startsolid) {
	...
	// IlDuca - g_realism 8 : drop weapon if poisoned
	if ( g_realism.integer & REALISM_POISONED_DROPGUN )
	{
		weapon_t	secondaryWeapon   = WP_NONE;
		weapon_t	primaryWeapon = G_GetPrimaryWeaponForClient( traceEnt->client );

		secondaryWeapon = G_GetSecondaryWeaponForClient( traceEnt->client, primaryWeapon );

		// Drop the primary weapon
		if( primaryWeapon > WP_NONE && primaryWeapon < WP_NUM_WEAPONS )
		{
			G_DropWeapon( traceEnt, primaryWeapon );
		}

		// Drop the secondary weapon
		if( secondaryWeapon > WP_NONE && secondaryWeapon < WP_NUM_WEAPONS )
		{
			G_DropWeapon( traceEnt, secondaryWeapon );
		}
	}
...
// jaquboss, specifed in weapon file
	...
	switch (weapon)
	{
		...
		// Submachineguns
		case WP_STEN:
		case WP_MP34:		//Meyer - fixed for 1.0.2 - wtf who put MP34 damage at 16! no wonder people
							//	claimed it was more powerful... because it was! ...all this time O_o
			return 14;
		case WP_STEN_MKII:
		case WP_THOMPSON:
		case WP_MP40:
...
void Bullet_Fire (gentity_t *ent, float spread, int damage, qboolean distance_falloff)
{
	...
	Bullet_Fire_Extended(ent, ent, muzzleTrace, end, damage, distance_falloff, qtrue, 0);
	...
	else
	{
		...
		// Ridah, bullet impact should reflect off surface
			...
			if ( tr.surfaceFlags & SURF_METAL && damage >= 2 )
			{
				...
				return Bullet_Fire_Extended(attacker, attacker, tr.endpos, end2, damage*0.5f, distance_falloff, createEvents, reflectNum);
			}
		}
	}
    if ( createEvents )
	...
	if ( traceEnt->takedamage )
	{
		...
		if( traceEnt->s.eType == ET_EXPLOSIVE )
		{
			if(traceEnt->health <= damage)
			{
				...
				return Bullet_Fire_Extended(traceEnt, attacker, tr.endpos, end, damage, distance_falloff, createEvents, reflectNum);
			}
		}
	}
...
gentity_t *weapon_mortar_fire( gentity_t *ent, int grenType ) {
	...
	level.missileCams[ent->client->ps.clientNum]->activator = m;
...
void FireWeapon( gentity_t *ent )
{
	...
	int		shots			= 1;
	// TODO NON omni-bot build we don't need pFiredShot without bots
...
void Weapon_Shotgun( gentity_t *ent ) {
	...
	G_HistoricalTraceEnd( ent );
	// start event from this position
	tent = G_TempEntity( muzzleTrace, EV_SHOTGUN_FIRED );
	...
	for ( i = 0; i < MAX_SHOTGUN_PELLETS; i++ ) {
		Bullet_Endpos_Pattern( ent, seed, SHOTGUN_SPREAD, &end );
		seed = seed * 1.33;
		if( Bullet_Fire_Extended(ent, ent, muzzleTrace, end, SHOTGUN_DAMAGE, qtrue, qfalse, 0) ) {
			hitClient = qtrue;
			tent->s.effect1Time |= (1<<i);
		}
	}
