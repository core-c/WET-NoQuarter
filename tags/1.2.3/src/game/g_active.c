...
/*
=================
ClientInactivityTimer - old version - drop if new one works
...
void G_SetIdentifyClient( gentity_t *ent ){
	...
	if(other->inuse && other->client &&
		...
		if ( skipAmmo ) {
			...
			ent->client->ps.stats[STAT_IDENTIFYAMMO] = (int)((other->client->ps.ammo[BG_FindAmmoForWeapon(other->s.weapon)] / (float)BG_MaxAmmoForWeapon( other->s.weapon, other->client->sess.skillBits) ) * 255);
...
void ClientThink_checkWeapons( gentity_t *ent ){
	...
	if ( g_knifeonly.integer && !ent->client->weaponsRemoved ){
		G_RemoveWeapons( ent );
		ent->client->ps.weapon = (ent->client->sess.sessionTeam == TEAM_AXIS) ? WP_KNIFE : WP_KNIFE_KABAR;
...
// jaquboss althud skill handling
	{
		...
		client->ps.ammoclip[WP_DUMMY_MG42]		= client->sess.skillpoints[clsSkill];
	}
		...
		if( level.gameManager
		  ...
		  && level.timeCurrent - client->pers.lastHQArtyReportTime > 3000 ) {
			...
			if( client->sess.sessionTeam == TEAM_AXIS ) {
				if( G_SweepForArty( ent->r.currentOrigin, 256.f, TEAM_AXIS ) ) {
					G_AddEvent( ent, EV_ARTY_DETECTED, 0 );
				}
			}
			else if( client->sess.sessionTeam == TEAM_ALLIES ) {
				if( G_SweepForArty( ent->r.currentOrigin, 256.f, TEAM_ALLIES ) ) {
					G_AddEvent( ent, EV_ARTY_DETECTED, 0 );
				}
			}
			*/
...
void G_RunClient( gentity_t *ent ) {
	...
	// If we are using antiwarp and we run this command, this means
	// run twice the same thing ( DoClientThinks calls ClientThink_real in itself! )
...
//unlagged - smooth clients #1
	...
	if ( level.framenum % NQ_AURA_FRAME_FREQ == 0 &&
		 !( ent->client->ps.eFlags & (EF_DEAD|EF_PLAYDEAD|EF_POISONED) )
		)
	{
		int			skill		= 0;
		int			aurabit		= 0;
		...
		if ( auraclass && ent->client->sess.skillBits[skill] & (1<<5) )
		{
			...
			for ( i = 0 ; i < level.numConnectedClients; i++ )
			{
				...
				if ( trap_InPVS(target->r.currentOrigin, ent->r.currentOrigin) &&
					 Distance(target->r.currentOrigin, ent->r.currentOrigin) <= NQ_AURA_RADIUS)
				{
