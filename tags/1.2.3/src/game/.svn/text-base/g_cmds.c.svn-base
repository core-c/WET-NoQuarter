...
// redeye - amount of time within a player is counted as in a fight
#define INFIGHT_TIME 10000
...
qboolean G_IsWeaponDisabled( gentity_t* ent, weapon_t weapon, qboolean quiet )
{
	...
	// IRATA: before we start teamcount and rest of function (which is called quite often)
	// we do simple check for most cases increasing performance
	if (weapon == WP_THOMPSON || weapon == WP_MP40) {
		return qfalse;
	}
		...
		if(weapon == WP_BAZOOKA || weapon == WP_PANZERFAUST) {
			if( ( count/(g_maxclients.integer/2.f) ) <=  (team_panzerRestriction.integer*0.01f) ) {
				if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3PANZERFAUST or BAZOOKA not available, more players needed ^1*^7\" 1");
				return qtrue;
			}
		}
	}

	// IRATA fix for jp unlimited weapon mode - we count opposite weapons as well
	// quiet mode fixed for browning
	switch(weapon)
	{
		case WP_VENOM:
			if(!(jp_insanity.integer & JP_INSANITY_VENOM) ){
				if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3VENOM not available ^1*^7\" 1");
				return qtrue;
			}
		...
		case WP_SHOTGUN:
			...
			if(wcount >= team_maxShotguns.integer) {
				if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3SHOTGUN not available ^1*^7\" 1");
				return qtrue;
			}
			break;
		case WP_PANZERFAUST:
			...
			if( wcount + G_TeamCount( ent, WP_BAZOOKA ) >= team_maxPanzers.integer) {
				if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
					// use extended output, what's in use ?
					// CP(va("cp \"^1PANZERFAUST not available, %i Pussies in use^7\" 1", tmppanzer));
 					CP("cp \"^1* ^3PANZERFAUST not available ^1*^7\" 1");
				}
 				return qtrue;
 			}
 			break;
		case WP_BAZOOKA:
			if(team_maxPanzers.integer == -1) return qfalse;
			// IRATA fix panzer count restriction for jp_insanity 128
			if(wcount + G_TeamCount( ent, WP_PANZERFAUST )  >= team_maxPanzers.integer) {
			 	if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
			 		// use extended output ?
			 		CP("cp \"^1* ^3BAZOOKA not available ^1*^7\" 1");
			 	}
			 	return qtrue;
			}
			break;
		case WP_MOBILE_BROWNING:
			...
			if(wcount + G_TeamCount( ent, WP_MOBILE_MG42 ) >= team_maxMG42s.integer) {
				if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3Browning not available ^1*^7\" 1");
				return qtrue;
			}
			break;
		case WP_MOBILE_MG42:
			if(team_maxMG42s.integer == -1) return qfalse;
			if(wcount + G_TeamCount( ent, WP_MOBILE_BROWNING ) >= team_maxMG42s.integer) {
				if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3MG42 not available ^1*^7\" 1");
				return qtrue;
			}
			break;
		case WP_FLAMETHROWER:
			...
			if(wcount >= team_maxFlamers.integer) {
				if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3FLAMETHROWER not available ^1*^7\" 1");
				return qtrue;
			}
			break;
		case WP_MORTAR:
			if(team_maxMortars.integer == -1) return qfalse;
			if(wcount + G_TeamCount( ent, WP_MORTAR2 ) >= team_maxMortars.integer) {
				if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3MORTAR not available ^1*^7\" 1");
				return qtrue;
			}
			break;
		case WP_MORTAR2:
			if(team_maxMortars.integer == -1) return qfalse;
			if(wcount + G_TeamCount( ent, WP_MORTAR ) >= team_maxMortars.integer) {
				if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3MORTAR not available ^1*^7\" 1");
				return qtrue;
			}
		...
		case WP_GPG40:
			...
			if(wcount + G_TeamCount( ent, WP_M7 )  >= team_maxriflegrenades.integer) {
				if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3GRENADE LAUNCHER not available ^1*^7\" 1");
				return qtrue;
			}
			break;
		case WP_M7: // jaquboss - not the carbine!!
			if(team_maxriflegrenades.integer == -1) return qfalse;
			if(wcount  + G_TeamCount( ent, WP_GPG40 ) >= team_maxriflegrenades.integer) {
				if(!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO))
					CP("cp \"^1* ^3GRENADE LAUNCHER not available ^1*^7\" 1");
				return qtrue;
			}
...
void Cmd_SetWayPoint_f( gentity_t *ent ) {
	...
	if ( trap_Argc() != 2 ) {
		trap_SendServerCommand( ent-g_entities, "print \"Usage: waypoint <type: 1-3>\n\"" );
		return;
	}
