...
//	{"420",			G_shrubbot_420,			'T', 0, "gives a blunt to a player or all players", "^9(^3name|slot#^9)"},
	...
	{"disguise",	G_shrubbot_disguise,	'g', SCMDF_CHEAT, "gives a player a disguise of the specified class", "^9[^3name|slot#^9] (^hclass number^9)"},
...
#define DEFAULT_MUTE_SECONDS g_defaultMute.integer > 10 ? g_defaultMute.integer : 10
...
qboolean G_shrubbot_mute(gentity_t *ent, int skiparg) 
{
	...
	if ( G_shrubbot_mute_check(ip, guid)) {
...
qboolean G_shrubbot_unmute(gentity_t *ent, int skiparg) 
{
	...
	if(ClientNumbersFromString(ms, pids) == 1 &&
	  ((gentity_t*)&g_entities[pids[0]])->client->sess.muted)
	{
		((gentity_t*)&g_entities[pids[0]])->client->sess.muted = qfalse;
		((gentity_t*)&g_entities[pids[0]])->client->sess.auto_mute_time = -1;
		AP(va("chat \"^dunmute: ^*%s ^9has been unmuted\"",
			((gentity_t*)&g_entities[pids[0]])->client->pers.netname));
		return qtrue;
	}
	...
	AP(va("chat \"^dunmute: ^*%s ^9has been unmuted\"", g_shrubbot_mutes[pids[0]]->name ));
...
// IlDuca - Modified for g_realism 8 : drop weapon if poisoned
qboolean G_shrubbot_poison(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		for( it = 0; it < level.numConnectedClients; it++ ) {
			...
			if ( g_realism.integer & REALISM_POISONED_DROPGUN )
			{
				primaryWeapon = G_GetPrimaryWeaponForClient( vic->client );
				secondaryWeapon = G_GetSecondaryWeaponForClient( vic->client, primaryWeapon );

				// Drop the primary weapon
				if( primaryWeapon > WP_NONE && primaryWeapon < WP_NUM_WEAPONS )
				{
					G_DropWeapon( vic, primaryWeapon );
				}
		
				// Drop the secondary weapon
				if( secondaryWeapon > WP_NONE && secondaryWeapon < WP_NUM_WEAPONS )
				{
					G_DropWeapon( vic, secondaryWeapon );
				}
			}
