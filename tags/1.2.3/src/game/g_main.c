...
void G_UpdateNQinfo(void)
	...
	Info_SetValueForKey(cs, "DJ", va("%i", g_doubleJump.integer));
	Info_SetValueForKey(cs, "DJH", va("%.3f", g_doubleJumpHeight.value));
#ifdef MV_SUPPORT
	Info_SetValueForKey(cs, "MV", va("%i", g_multiview.integer));
#endif
	// weapon limits
	// IRATA
	Info_SetValueForKey(cs, "TPR",
		va("%i", team_panzerRestriction.integer));
...
void ServerCheck ( void ) {
	...
	if ( (!strstr( versionStr, "2.60b") && !strstr( versionStr, "2.60d")) ) {
		G_Printf( "^1Warning: Wrong ET server application version %s, not recommended for NQ! Use 2.60b/d\n", versionStr);
		msgWhine = qtrue;
	}

	trap_Cvar_VariableStringBuffer( "fs_game", versionStr, sizeof(versionStr) );
	if ( !Q_stricmp(versionStr, "etmain" ) ) {
		G_Printf( "^1Warning: Server installed in etmain directory, features may break!\n" );
		msgWhine = qtrue;
	}
	else if ( Q_stricmp(versionStr, "noquarter" ) != 0 ) {
		G_Printf("^1Warning: Wrong mod directory, should be 'noquarter'\n" );
		msgWhine = qtrue;
	}

	trap_Cvar_VariableStringBuffer( "sv_pure", versionStr, sizeof(versionStr) );
	if ( atoi(versionStr) < 0 ) {
		G_Printf("^1Warning: Server is not pure!! No Quarter will not work properly!\n" );
		msgWhine = qtrue;
	}
	...
	if (pakNames[0]) {
		...
		if (!strstr(pakNames, "noquarter_v1.2.3")) {
			G_Printf("^1Warning: No noquarter_v1.2.3.pk3 found on server! Missing essential files!\n" );
			msgWhine = qtrue;
		}
		if (!strstr(pakNames, "nq_bin_b1.2.3")) {
			G_Printf("^1Warning: No nq_bin_b1.2.3.pk3 found on server! Missing essential files!\n" );
			msgWhine = qtrue;
		}
	...
	if (msgWhine) {
		...
		trap_Cvar_Set( "g_msg1", "^1Warning:^3 Invalid No Quarter installation! Read log & installation instructions."  );
		trap_Cvar_Set( "g_msg2", "^1Warning:^3 Invalid No Quarter installation! Read log & installation instructions."  );
		trap_Cvar_Set( "g_msg3", "^1Warning:^3 Invalid No Quarter installation! Read log & installation instructions."  );
...
void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	BG_ProfilerBlockSetName(PROFILE_ENTITY_TYPE_ET_EF_SPOTLIGHT, 			"RF_ET_EF_SPOTLIGHT");
	...
	// BG_ProfilerBlockSetName(PROFILE_ENTITY_TYPE_ET_WAYPOINT, 				"RF_ET_WAYPOINT");
	...
	// BG_ProfilerBlockSetName(PROFILE_ENTITY_TYPE_ET_BOTGOAL_INDICATOR, 		"RF_ET_BOTGOAL_INDICATOR");
	...
	BG_ProfilerBlockSetName(PROFILE_ENTITY_TYPE_ET_AAGUN, 					"RF_ET_AAGUN");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_JUMP, "PROFILE_EV_JUMP");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_EMPTYCLIP, "PROFILE_EV_EMPTYCLIP");
	BG_ProfilerBlockSetName(PROFILE_EV_FILL_CLIP, "PROFILE_EV_FILL_CLIP");
	BG_ProfilerBlockSetName(PROFILE_EV_MG42_FIXED, "PROFILE_EV_MG42_FIXED");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_PLAYER_TELEPORT_IN, "PROFILE_EV_PLAYER_TELEPORT_IN");
	BG_ProfilerBlockSetName(PROFILE_EV_PLAYER_TELEPORT_OUT, "PROFILE_EV_PLAYER_TELEPORT_OUT");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_VENOM, "PROFILE_EV_VENOM");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_TAUNT, "PROFILE_EV_TAUNT");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_SPIT_HIT, "PROFILE_EV_SPIT_HIT");
	BG_ProfilerBlockSetName(PROFILE_EV_SPIT_MISS, "PROFILE_EV_SPIT_MISS");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_CONCUSSIVE, "PROFILE_EV_CONCUSSIVE");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_FIRE_WEAPON_AAGUN, "PROFILE_EV_FIRE_WEAPON_AAGUN");
	...
	BG_ProfilerBlockSetName(PROFILE_EV_FIREWORKS, "PROFILE_EV_FIREWORKS");
	...
	//BG_ProfilerBlockSetName(PROFILE_EV_BOT_DEBUG_LINE, "PROFILE_EV_BOT_DEBUG_LINE");
	//BG_ProfilerBlockSetName(PROFILE_EV_BOT_DEBUG_RADIUS, "PROFILE_EV_BOT_DEBUG_RADIUS");
...
void CheckIntermissionExit( void ) {
	...
	if(level.numConnectedClients) {
		...
		if((100.0f * (ready / (readyVoters * 1.0f))) >=
...
void ServerOutput ( void )
{
	...
	if ( (!strstr( versionStr, "2.60b") && !strstr( versionStr, "2.60d")) && ( whine < oldWhine ) )
	...
	if (!strstr(pakNames, "noquarter_b1.1.1"))
		trap_SendServerCommand( -1, va("cpm \"^3|^1Warning^d: No ^2noquarter_b1.1.1.pk3^d found on server! Missing essential files! Contact Server Admin^3|\n\"") );

	if (!strstr(pakNames, "noquarter_b1.2.3"))
		trap_SendServerCommand( -1, va("cpm \"^3|^1Warning^d: No ^2noquarter_b1.2.0.pk3^d found on server! Missing essential files! Contact Server Admin^3|\n\"") );

	if (!strstr(pakNames, "nq_bin_b1.2.3"))
		trap_SendServerCommand( -1, va("cpm \"^3|^1Warning^d: No ^2nq_bin_b1.2.0.pk3^d found on server! Missing essential files! Contact Server Admin^3|\n\"") );
...
// forty - arty/airstrike rate limiting.
	...
	// jaquboss - naggy messages for wrong installed servers // IRATA - moved to INIT
