...
//void CG_DrawWaypointInfo( int x, int y, int w, int h, mapScissor_t *scissor, int expand  );
...
//	vec4_t c_clr = {1.f, 1.f, 1.f, 1.f};
	...
	//CG_DrawWaypointInfo( x, y, w, h, scissor, exspawn );
	CG_DrawMortarMarker( x, y, w, h, qtrue, scissor, exspawn );

	for(i = 0, mEnt = &mapEntities[0]; i < mapEntityCount; i++, mEnt++ ) {
		if( mEnt->team != CG_LimboPanel_GetRealTeam() && !CG_DisguiseMapCheck(mEnt) ) {
			continue;
		}
...
void CG_DrawWaypointInfo( int x, int y, int w, int h, mapScissor_t *scissor, int expand  ) {
	...
	for ( i = 0; i < snap->numEntities; i++ ) {
		...
		if( scissor ) {
			pos[0] = ((ent->pos.trBase[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0]) * w * scissor->zoomFactor;
			pos[1] = ((ent->pos.trBase[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1]) * h * scissor->zoomFactor;
		} else {
		...
		if( scissor && CG_ScissorPointIsCulled( pos, scissor, qtrue ) ) {
			continue;
		}

		if( scissor ) {
			pos[0] += x - scissor->tl[0];
			pos[1] += y - scissor->tl[1];
		}
