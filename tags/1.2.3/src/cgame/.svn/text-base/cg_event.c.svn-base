...
static qboolean CG_CheckFilterObituary(int attacker, clientInfo_t *ca, int target)
{
	...
	if (attacker != target)
	{
		if ((cg_popupMessageFilter.integer & POPUP_FILTER_ENEMYKILLS) &&
			 ca && (ca->team != cg.snap->ps.persistant[PERS_TEAM] || ca->team == TEAM_SPECTATOR))
		{
			filter = qtrue;
		}
	...
	if ((attacker == cg.clientNum || target == cg.clientNum) &&
		(cg_popupMessageFilter.integer & POPUP_FILTER_NOTOWNKILLS))
	{
		filter = qfalse; // don't filter own deaths/kills if requested
	}
	return filter;
}
...
// may happen in 2.55 when crouching and shooting at ground, or in all ET versions when just shooting metal surface
	...
	if( ca ) {
		...
		//	message = "^7was ^1TEAMKILLED ^7by";
			...
			// jP - keep the secondary death message
			// message2= "";
	...
	case EV_SHOTGUN_FIRED:
		{
			...
			int			randSeed;
			vec3_t		end,right,up,forward;

			AngleVectors (es->angles, forward, right, up);

			DEBUGNAME("EV_SHOTGUN_FIRED");
			for ( i = 0; i < MAX_SHOTGUN_PELLETS; i++ ) {
				randSeed = seed;
				r = Q_crandom(&randSeed)*SHOTGUN_SPREAD;
				u = Q_crandom(&randSeed)*SHOTGUN_SPREAD;
				VectorMA (es->origin, dist, forward, end);
				VectorMA (end, r, right, end);
				VectorMA (end, u, up, end);
				// this pellet hit a player
				if (es->effect1Time & (1<<i))
				{
					trace_t tr;

					cg.bulletTrace = qtrue;
					CG_Trace( &tr, es->origin, NULL, NULL, end, es->otherEntityNum, MASK_SHOT );
					cg.bulletTrace = qfalse;
					if ( tr.entityNum < MAX_CLIENTS )
						CG_Bullet( end, es->otherEntityNum, es->angles, qtrue, tr.entityNum, -1, 0, seed, qfalse, NULL );
				}
				else
					CG_Bullet( end, es->otherEntityNum, es->angles, qfalse, ENTITYNUM_WORLD, -1, 0, seed, qfalse, NULL );
				seed = seed * 1.4;
			}
		}
		break;
...
/*	case EV_POPUPBOOK:
	...
	case EV_GENERAL_SOUND:
		...
		if ( es->eventParm >= bg_numGameSound ){
			s = CG_ConfigString( CS_SOUNDS + (es->eventParm-bg_numGameSound) );
			if( !strstr( s, ".wav" ) ) {
	...
	case EV_GENERAL_SOUND_VOLUME:
		{
			...
			if ( sound >= bg_numGameSound ){
				s = CG_ConfigString( CS_SOUNDS + (sound-bg_numGameSound) );
				if( !strstr( s, ".wav" ) ) {
					if( CG_SoundPlaySoundScript( s, NULL, es->number, qfalse ) ) {
						break;
					}
