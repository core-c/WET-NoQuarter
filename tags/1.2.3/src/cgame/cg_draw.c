...
static void CG_DrawDanger( void ){
	...
	if ( !(cgs.clientinfo[cg.snap->ps.clientNum].skillBits[SK_BATTLE_SENSE] & (1<<8)) && cg_skillOptions.integer & 16 )
		return;
...
static void CG_DrawDanger( void ){
	...
	for ( i = 0; i < snap->numEntities; i++ ) {
		...
		if (( cent->currentState.weapon == WP_GRENADE_PINEAPPLE ||
			...
			// IlDuca - Changed from 16 to 8
			cg_skillOptions.integer & 8 )
		{
			maxDist = 320.f;
		...
		// it's the first arty missile and skillviewoptions has bit 16 set ...
				  ...
				  /* cent->currentState.teamNum == cgs.clientinfo[cg.snap->ps.clientNum].team && // team only */
...
//	CG_Trace( &trace, start, vec3_origin, vec3_origin, end, cg.snap->ps.clientNum, MASK_ALL &~CONTENTS_MONSTERCLIP);
	...
	if(trace.entityNum == ENTITYNUM_WORLD) {
		if (/*(trace.contents & CONTENTS_WATER)*/ (CG_PointContents( trace.endpos, -1 ) & CONTENTS_WATER) && !(CG_PointContents( cg.refdef.vieworg, -1 ) & CONTENTS_WATER)) {	// jaquboss - was only on servercode
			if(dist <= CH_WATER_DIST) {
				cg.cursorHintIcon = HINT_WATER;
				cg.cursorHintTime = cg.time;
				cg.cursorHintFade = 500;
...
static void CG_DrawCrosshairNames( void ) {
	...
	if ( cg.crosshairClientNum > MAX_CLIENTS ) {
		...
		if( (cg_entities[cg.crosshairClientNum].currentState.powerups & (1 << PW_OPS_DISGUISED)) && cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR) {
			...
			if((cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR &&
				cgs.clientinfo[cg.snap->ps.clientNum].skillBits[SK_SIGNALS] & (1<<4) &&
