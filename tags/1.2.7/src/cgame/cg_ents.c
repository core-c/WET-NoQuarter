...
//----(SA)	whoops, didn't mean to check it in with the missile flare
	...
	if ( cent->currentState.clientNum == cg.predictedPlayerState.clientNum && ( s1->weapon == WP_PANZERFAUST || s1->weapon == WP_BAZOOKA ) && cg_insanity.integer & JP_INSANITY_MISSILECAM ) {
