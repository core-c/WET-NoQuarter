...
vmCvar_t	cg_whizzSounds;

// get next weapon after using cretain weapon (like panzer,airstrike,satchel etc)
vmCvar_t	cg_favWeaponBank; //lastUsedWeaponBank
...
{
	{ &cpu_Affinity,			"cpu_Affinity",					"0",			CVAR_ARCHIVE | CVAR_NORESTART },
			...
			if(cv->modificationCount != cv->vmCvar->modificationCount) {
				   ...
				   cv->vmCvar == &cg_weapAltReloads   )
				{
					...
					if ( !C_SetAffinityCPU(cpu_Affinity.integer, &msg) ) {
						//CG_Printf( "CPU Affinity not set..\n" );
					}
					CG_Printf( msg );
				}
