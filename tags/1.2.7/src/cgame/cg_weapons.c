...
void CG_AddPlayerFoot ( refEntity_t *parent, centity_t *cent, float fovOffset, playerState_t *ps )
{
	refEntity_t		foot;
	vec3_t			kickangle;
	weaponInfo_t	*weapon = &cg_weapons[WP_NONE];
...
int getEquivWeapon( int weapnum ) {
	...
	switch(weapnum) {
		...
		// TODO: What's this ? WP_STEN_MKII ? instead of the BAR ?
...
qboolean ChooseWeapon( int bank, qboolean includePanzer ) {
	...
	for( i=0; i<MAX_WEAPS_IN_BANK_MP; ++i )	{
		...
		if ( bank == WEAPONBANK_SMG && !includePanzer && (weapBanksMultiPlayer[bank][i] == WP_PANZERFAUST || weapBanksMultiPlayer[bank][i] == WP_BAZOOKA) ) {
			continue;
		}
...
void CG_MissileHitWall( int weapon, int clientNum, vec3_t origin, vec3_t dir, int surfFlags ) {	//	(SA) modified to send missilehitwall surface parameters
	...
	localEntity_t	*le;
	qboolean		isSprite = qfalse, alphaFade = qfalse;
