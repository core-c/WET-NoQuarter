...
static void CG_DrawSpectatorMessage( void )
{
	...
	x = (cg_drawObjIcons.integer)? 34 : INFOTEXT_STARTX;
	...
	CG_DrawStringExt( x, y+height+2, str, colorWhite, qtrue, qtrue, width, height, 0 );
...
void CG_DrawObjectiveIcons() {
	...
	const char *s2;
	const char *buf = Info_ValueForKey( s, "numobjectives" );
...
static void CG_DrawWeapRecharge( rectDef_t *rect ) {
	...
	if( cg.snap->ps.stats[ STAT_PLAYER_CLASS ] == PC_FIELDOPS ) {
		...
		if ( !(cg.snap->ps.ammo[WP_ARTY] & NO_ARTY) ) {
			CG_DrawPic( rect->x + 2, rect->y + rect->h + 4, rect->w, rect->w, cg_weapons[WP_BINOCULARS].weaponIcon[1] );
		}
	}
}
