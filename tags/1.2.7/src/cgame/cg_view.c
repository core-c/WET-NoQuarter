...
void CG_KickAngles(void) {
	// TODO isn't 'static' better choice?
...
static void CG_OffsetFirstPersonView( void ) {
	...
	if( cg.snap->ps.weapon == WP_MOBILE_MG42_SET || cg.snap->ps.weapon == WP_MOBILE_BROWNING_SET || cg.snap->ps.weapon == WP_BAR_SET ) {
		...
		VectorMA( origin, 31.0f, forward, point );
		AngleVectors( cg.refdefViewAngles, forward, NULL, NULL );
		VectorMA( point, -32.0f, forward, origin );
...
static int CG_CalcFov( void ) {
	...
	float	x, phase = 0, v = 0, fov_x, fov_y, zoomFov, f;
	...
	if (  cg.predictedPlayerState.eFlags & EF_POISONED ) {
		phase = cg.time / 1000.0 * 0.3 * M_PI * 2;
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	// TODO: move var init out of loops ...
	...
	if(cg.mvCurrentMainview != NULL && cg.snap->ps.pm_type != PM_INTERMISSION && cgs.mvAllowed ) {
		...
		if( (cg_insanity.integer & JP_INSANITY_MORTARCAM
				|| cg_insanity.integer & JP_INSANITY_MISSILECAM)
				&& cg.latestMissile && !cg.showGameView ) {
