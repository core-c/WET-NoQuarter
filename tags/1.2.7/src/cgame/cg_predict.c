 ...
 *	cg_predict.c
 *
 * This file generates cg.predictedPlayerState by either
...
// modifed for cgame
float CG_ClientHitboxMaxZ(entityState_t *hitEnt)
{
	if(!hitEnt) return 0;

	if (hitEnt->eFlags & EF_DEAD)	return 4;
	if (hitEnt->eFlags & EF_PLAYDEAD) return 4;
	if (hitEnt->eFlags & EF_PRONE) return 4;
	if (hitEnt->eFlags & EF_CROUCHING)	return 24;
...
static void CG_ClipMoveToEntities ( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
	...
	for ( i = 0 ; i < cg_numSolidEntities ; i++ ) {
		...
		if ( ent->solid == SOLID_BMODEL ) {
			...
			bmaxs[2] = CG_ClientHitboxMaxZ(ent);
				else
			bmaxs[2] = zu;
...
static void CG_TouchItem( centity_t *cent ) {
	...
	if ( !BG_CanItemBeGrabbed( &cent->currentState, &cg.predictedPlayerState, cgs.clientinfo[cg.snap->ps.clientNum].skillBits, cgs.clientinfo[cg.snap->ps.clientNum].team ) ) {
	...
	if ( item->giType == IT_WEAPON ) {
		...
		if ( !cg.predictedPlayerState.ammo[ WeaponTable[item->giTag].ammoindex ] ) {
			cg.predictedPlayerState.ammo[ WeaponTable[item->giTag].ammoindex ] = 1;
		}
	}
}
...
#ifdef VISIBLE_TRIGGERS
			CG_RailTrail( tv( 0.f, 1.f, 0.f), mins, maxs, 1, -1 );
#endif // VISIBLE_TRIGGERS
...
void CG_PredictPlayerState( void ) {
	...
	for ( i=0; i < 7 ; i++){
		cg_pmove.skillBits[i] = cgs.clientinfo[cg.snap->ps.clientNum].skillBits[i];
	}
