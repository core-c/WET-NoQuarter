...
void PM_CheckForReload( int weapon )
{
    ...
    if (pm->cmd.wbuttons & WBUTTON_ATTACK2 && pm->pmext->weapAltReload && !BG_IsScopedWeapon(pm->ps->weapon)) {
	    ...
	    switch( weapon ) {
		    ...
		    case WP_K43_SCOPE:
				if( reloadRequested && pm->ps->ammo[ammoWeap] && pm->ps->ammoclip[clipWeap] < GetWeaponTableData(weapon)->maxclip) {
					// N!trox : higher level coverts to reload without leaving scoped view
...
void PM_AdjustAimSpreadScale( void )
{
	...
	if ( wpnScale ) {
        ...
        if( BG_IsScopedWeapon( pm->ps->weapon ) && pm->skillBits[SK_COVERTOPS] & (1<<3) ) {
            wpnScale *= 0.5;
        }
...
#ifdef GAMEDLL
	// IRATA: added some more ...
	// TODO: maybe add some more and set option to disable
	if( pm->ps->weapon == WP_DYNAMITE ) {
		pm->voice(pm->ps->clientNum, "FTExploreArea"); // check
		return;
	}

	if( pm->ps->weapon == WP_SMOKE_BOMB ) {
		pm->voice(pm->ps->clientNum, "FTGoUndercover"); // check
		return;
	}

	if( pm->ps->weapon == WP_SMOKE_MARKER || pm->ps->weapon == WP_GRENADE_LAUNCHER || pm->ps->weapon == WP_GRENADE_PINEAPPLE ) {
...
static void PM_Weapon( void ) {
	...
	if ( pm->ps->weapon ) {
		...
		if(ammoNeeded > ammoAvailable && ammoAvailable != -1) {
			...
			reloading = (qboolean)(ammoNeeded <= pm->ps->ammo[ WeaponTable[pm->ps->weapon].ammoindex ]);
...
void PM_UpdateLean(playerState_t *ps, usercmd_t *cmd, pmove_t *tpm) {
	...
	if(leaning) {
		if(leaning > 0) { // right
			...
			if(leanofs < -LEAN_MAX)
				leanofs = -LEAN_MAX;
            ps->stats[STAT_PS_FLAGS] |= 0x1;
		}
    }
	else {
        ps->stats[STAT_PS_FLAGS] &= ~0x3;
    }
...
void PM_LadderMove (void) {
	...
	if( pm->cmd.rightmove ) {
		...
		s = 0.5f * scale * (float)pm->cmd.rightmove;
		VectorMA( wishvel, s, ladder_right, wishvel );
	}
