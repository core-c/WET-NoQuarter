...
qboolean jP_DoChargeCost(gentity_t *ent, int skill, float charge )
{
	...
	if ( ent->client->ps.weapon == WP_MEDIC_ADRENALINE ) {
...
void Weapon_Medic_Ext(gentity_t *ent, vec3_t viewpos, vec3_t tosspos, vec3_t velocity)
{
	...
	item = &bg_itemlist[IL_HEALTH_HEALTH];
...
qboolean ReviveEntity(gentity_t *ent, gentity_t *traceEnt, qboolean resuscitation)
{
	...
	if ( poisoned && poisonAmnt){
		...
		gentity_t *attacker;
		attacker = &g_entities[traceEnt->client->pers.lastkiller_client];
		if ( traceEnt->client->pers.lastkiller_client != -1 &&
...
void artilleryThink_real( gentity_t *ent )
{
	...
	switch( sfx ) {
		case 0:
			G_AddEvent( ent, EV_GENERAL_SOUND, GAMESOUND_WPN_ARTILLERY_FLY_1);
			break;
		case 1:
			G_AddEvent( ent, EV_GENERAL_SOUND, GAMESOUND_WPN_ARTILLERY_FLY_2);
			break;
		case 2:
			G_AddEvent( ent, EV_GENERAL_SOUND, GAMESOUND_WPN_ARTILLERY_FLY_3);
			break;
	}
...
// build arty falling sound effect in front of bomb drop
		...
		bomb2->classnamehash	= AIR_STRIKE_HASH;
...
void Bullet_Endpos_Pattern(gentity_t *ent, int seed, float spread, vec3_t *end) {
	...
	float	tracedist = (BG_IsScopedWeapon(ent->s.weapon))? 2*MAX_TRACE : MAX_TRACE;
...
void Bullet_Fire (gentity_t *ent, float spread, int damage, qboolean distance_falloff)
{
	...
	if( ent->client->ps.groundEntityNum == ENTITYNUM_NONE ) {
		spread = BG_Weapons[ent->s.weapon].spread * 2.0;
	}
	else {
		switch( ent->s.weapon ) {
			// light weapons
	...
	Bullet_Endpos_Pattern( ent, seed, spread, &end );
...
void FireWeapon( gentity_t *ent ) {
	...
	switch( ent->s.weapon ) {
		...
		case WP_MOBILE_MG42:
		case WP_MOBILE_MG42_SET:
			Bullet_Fire( ent, BG_Weapons[ent->s.weapon].spread*aimSpreadScale, BG_Weapons[ent->s.weapon].damage, BG_Weapons[ent->s.weapon].falloff );
			break;
...
void Weapon_Shotgun( gentity_t *ent )
{
	...
	G_AddEvent( ent, EV_SHOTGUN_FIRED, seed );
