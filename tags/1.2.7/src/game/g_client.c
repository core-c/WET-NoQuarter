...
void SetWolfSpawnWeapons( gclient_t *client )
{
	...
	// jet Pilot - Add Knife to Inventory (KABAR for allies, dagger for axis)
	{
		...
		AddWeaponToPlayer( client, knife , 0, 0, qtrue );
	}
...
void ClientUserinfoChanged( int clientNum ) {
	...
	if (g_logOptions.integer & LOGOPTS_GUID) { // Log the GUIDs?
		...
		G_LogPrintf( "ClientUserinfoChanged: %i %s %s\n", clientNum, client->pers.client_ip, s );
		G_DPrintf( "ClientUserinfoChanged: %i :: %s %s\n", clientNum, client->pers.client_ip, s );
	}
}
