...
void UI_DrawMapPreview(rectDef_t *rect, float scale, vec4_t color, qboolean net) {
	...
	// jaquboss, draw levelshot instead of campaign location while non campaign
		...
		if (uiInfo.mapList[map].mapLoadName) {
			...
			preview = trap_R_RegisterShaderNoMip( va("levelshots/%s",uiInfo.mapList[map].mapLoadName ));
...
#ifdef JAQU_WIP
	UI_PreparePlayerRender ( rect, PC_SOLDIER, TEAM_AXIS );
#endif
}
...
static void UI_BuildServerDisplayList(qboolean force) {
	...
	//qboolean	noquarter=qfalse;	// Jaybird - not used
	...
	for (i = 0; i < count; i++) {
		...
		if (ping >/*=*/ 0 || ui_netSource.integer == AS_FAVORITES) {
...

			{
				...
				if( (  !Q_stricmp( gamename, "noquarter") && ui_browserNQonly.integer == 2 ) ||
					(  Q_stricmp( gamename, "noquarter") && ui_browserNQonly.integer == 1 ) ) {
					trap_LAN_MarkServerVisible(ui_netSource.integer, i, qfalse);
					continue;
				}
			}
				...
				case SORT_FILTERS:
					if( ping <= 0 ) {
						...
						*numhandles = 8;
						...
						antilag = atoi( Info_ValueForKey( info, "g_antilag" ) );
						balancedteams = atoi( Info_ValueForKey( info, "balancedteams" ) );
						noquarter = (!Q_stricmp(Info_ValueForKey( info, "game" ), "noquarter") ? 1 : 0);
...
#ifdef JAQU_WIP
	// jaquboss, we need them for UI system as well
	BG_InitWeaponStrings();
	UI_RegisterPlayerClasses();

	memset ( &renderedPlayerInfo, 0, sizeof(renderedPlayerInfo));
	memset ( &customizedChar,	0, sizeof(customizedChar));
#endif
}
...
#ifdef JAQU_WIP
// to be removed
vmCvar_t	ui_bodyIndex;
vmCvar_t	ui_faceIndex;
vmCvar_t	ui_headIndex;
vmCvar_t	ui_legIndex;
vmCvar_t	ui_torsoIndex;
#endif
...
//	{ &ui_actualNetGameType, "ui_actualNetGametype", "5", CVAR_ARCHIVE },		// NERVE - SMF - hardwired for now
	...
	{ NULL,	"cg_drawAccuracy",				 "0",		 CVAR_ARCHIVE },
	...
	{ NULL,	"cg_drawHUDHead",				 "0",		 CVAR_ARCHIVE },
	{ NULL,	"cg_drawObjIcons",				 "0",		 CVAR_ARCHIVE },
	{ NULL,	"cg_charSizeLarge",				 "14",		 CVAR_ARCHIVE },
	{ NULL,	"cg_charSizeSmall",				 "14",		 CVAR_ARCHIVE },
	{ NULL,	"cg_charSizeTiny",				 "10",		 CVAR_ARCHIVE },
	{ NULL,	"cg_markDistance",				 "384",		 CVAR_ARCHIVE },	
	{ NULL,	"hud_powerupSize",				 "18",		 CVAR_ARCHIVE },
	{ NULL,	"hud_PowerupX",					 "80",		 CVAR_ARCHIVE },
	{ NULL,	"hud_PowerupY",					 "388",		 CVAR_ARCHIVE },
	{ NULL,	"hud_PowerupRows",				 "4",		 CVAR_ARCHIVE },
	...
	{ NULL,	"hud_flashTime",				 "200",		 CVAR_ARCHIVE },
	...
	{ NULL,	"cg_atmosphereRenderDistance",	 "1",		 CVAR_ARCHIVE }, 	
	{ NULL,	"cg_optimizePrediction",		 "1",		 CVAR_ARCHIVE }, 
	{ NULL,	"cg_locations",					 "0",		 CVAR_ARCHIVE }, 	
	...
	{ NULL,	"cg_weapAltReload",				 "1",		 CVAR_ARCHIVE },
