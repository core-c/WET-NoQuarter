...
qboolean G_CanPickupWeapon( weapon_t weapon, gentity_t* ent )
{
	...
	switch (playerClass)
	{
		...
		case PC_MEDIC:
			if ( IS_AKIMBO_PISTOL(weapon) && ent->client->sess.skill[SK_LIGHT_WEAPONS] >= 4 && !(g_medics.integer & MEDIC_NOAKIMBO) ) return qtrue;
			if ( IS_PISTOL(weapon) ) return qtrue;
...
qboolean G_CheckForCabinetResupply( gentity_t *ent, gentity_t *traceEnt )
{
	...
	switch ( type ){
		case TYPE_HEALTH:
			if ( traceEnt->s.eType != ET_CABINET_H || ent->parent->client->sess.skill[SK_FIRST_AID] < 7 )
				return qfalse;
			else
				break;
		case TYPE_AMMO:
			if ( traceEnt->s.eType != ET_CABINET_A || ent->parent->client->sess.skill[SK_SIGNALS] < 7  )
				return qfalse;
			else
				break;
		default:
