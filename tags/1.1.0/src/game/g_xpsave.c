...
qboolean G_xpsave_load(gentity_t *ent)
{
	...
	{
		G_Printf("G_xpsave_load: invalid guid.\n");
		return qfalse;
	}
...
static void G_xpsave_convert_old_config()
{
	...
	// Likewise they need to have a configured xpsave file variable...
	...
	if ( len < 0 ) 
	{
		G_Printf("G_xpsave_convert_old_config: could not open xpsave file %s. If it was "
			"already converted then it will have been renamed to <g_XPSaveFile>.converted. You "
	...
	*(xpsave_data + len) = '\0';
	...
	trap_FS_Rename(g_XPSaveFile.string, va("%s.converted", g_XPSaveFile.string));
}
...
void G_xpsave_init_game()
{
	...
	// If the g_XPSaveFile variable is set then we can attempt to perform a conversion
