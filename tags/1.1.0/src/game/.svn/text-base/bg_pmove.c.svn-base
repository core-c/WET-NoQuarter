...
//
	...
	if ((pm->ps->weapon == WP_BAR) ||
		...
		if( pm->skill[SK_HEAVY_WEAPONS] < 7 && pm->skill[SK_HEAVY_WEAPONS] >= 3)
		{
			scale *= 0.9;
		}
		else if( pm->skill[SK_HEAVY_WEAPONS] < 3)
	...
	for(wp=0;wp<WP_NUM_WEAPONS;wp++) 
	{
		...
		if( COM_BitCheck( pm->ps->weapons, wp) ) 
		{
			...
			if(pm->ps->weapHeat[wp]) 
			{
				...
				if( (pm->skill[SK_HEAVY_WEAPONS] >= 2 && IS_HEAVY_WEAPON(wp)) || pm->ps->stats[STAT_TEAMAURA] & AURABIT_ENGINEER ) 
				{
...
qboolean PM_AltFire ( void )
{
	if ( pm->cmd.wbuttons & WBUTTON_ATTACK2 )
	{
		...
		{
			if ( pm->skill[SK_LIGHT_WEAPONS] >= 8 )
				return qtrue;
		}
	}
	return qfalse;
...
/*	if( pm->ps->weapon == WP_TRIPMINE ) {
	...
	switch(pm->ps->weapon)
	{
		...
		case WP_KNIFE_KABAR:
			...
			{
				// throw
				if ( pm->cmd.wbuttons & WBUTTON_ATTACK2 && pm->skill[SK_LIGHT_WEAPONS] >= 8 && PM_WeaponAmmoAvailable(pm->ps->weapon) )
				{
					if( pm->ps->eFlags & EF_PRONE )
