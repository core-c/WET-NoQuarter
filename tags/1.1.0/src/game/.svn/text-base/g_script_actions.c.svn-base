	...
	if (!params || ent->scriptStatus.scriptStackChangeTime < level.time) {			// we are waiting for it to reach destination
		...
		if (ent->s.eType == ET_MOVER) {
			...
			// Send a trigger to omni-bot
			{
				char buffer[512] = {0};
				TriggerInfo ti;
				ti.m_Action = "mover_goto";
				ti.m_Activator = NULL;
				ti.m_TagName = ent->scriptName;
				ti.m_Entity = ent;
				ti.m_UserData.DataType = dt3_Strings;
				...
				ti.m_UserData.udata.m_CharPtrs[0] = buffer;
				ti.m_UserData.udata.m_CharPtrs[1] = 0;
