...
static void AddExtraSpawnAmmo( gclient_t *client, weapon_t weaponNum)
{
	switch( weaponNum ) 
	{
		...
		//case WP_FLAMETHROWER:
			if( client->sess.skill[SK_HEAVY_WEAPONS] >= 6 )
				client->ps.ammo[BG_FindAmmoForWeapon(weaponNum)] += GetAmmoTableData(weaponNum)->maxclip;
			break;
		case WP_MORTAR2:
		case WP_MORTAR2_SET:
...
void SetWolfSpawnWeapons( gclient_t *client ) 
{
	...
	// jet Pilot - Add Knife to Inventory (KABAR for allies, dagger for axis)
	{
		...
		AddWeaponToPlayer( client, knife , 0, client->sess.skill[SK_LIGHT_WEAPONS] >= 8 ? GetAmmoTableData(knife)->defaultStartingClip : 1, qtrue );
	}

	client->ps.weaponstate = WEAPON_READY;

	// jet Pilot - Give Adrenaline if the player has earned it
	if (client->sess.skill[SK_MEDIC] >= 4 && ((pc == PC_MEDIC && !(g_medics.integer & MEDIC_NOSELFADREN) ) || jp_keepAwards.integer & JP_KEEP_ADRENALINE))
	{	
		int needles = 10;
...
void ClientUserinfoChanged( int clientNum ) {
	...
	if ( ent->r.svFlags & SVF_BOT ) {
		...
		s = va( "n\\%s\\t\\%i\\skill\\%s\\c\\%i\\r\\%i\\m\\%s\\s\\%s%s\\dn\\%s\\dr\\%i\\dc\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\rn\\%i\\lc\\%i",
			client->pers.netname,
...
#endif
		s = va( "n\\%s\\t\\%i\\c\\%i\\r\\%i\\m\\%s\\s\\%s\\dn\\%s\\dr\\%i\\dc\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\ref\\%i\\rn\\%i\\lc\\%i",
			client->pers.netname, 
			client->sess.sessionTeam, 
			client->sess.playerType, 
			client->sess.rank, 
...
char* IsFakepConnection(int clientNum, char const* ip, char const* userinfo)
{
	...
	for ( i = 0; i < maxClients; ++i )
	{
		...
		// Get the IP address for this client number 
		trap_GetUserinfo(i, cluserinfo, sizeof(cluserinfo));
		if ( cluserinfo[0] == 0 )
			continue;

		theirIP 		= Info_ValueForKey(cluserinfo, "ip");
		...
		if ( ent->client && ent->client->pers.connected > 0 ) 
		{
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	value = IsFakepConnection(clientNum, value, userinfo);
	if ( value != NULL )
	{
...
//unlagged
	...
	if ( client->sess.skill[SK_BATTLE_SENSE] >= 6 )
