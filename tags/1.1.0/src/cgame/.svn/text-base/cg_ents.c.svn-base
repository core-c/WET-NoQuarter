...
//----(SA)	whoops, didn't mean to check it in with the missile flare
	...
	if ( cent->currentState.weapon == WP_DYNAMITE )
	{
		...
		if ( cent->currentState.teamNum % 4 == cgs.clientinfo[cg.snap->ps.clientNum].team ||
			cgs.clientinfo[cg.snap->ps.clientNum].skill[SK_BATTLE_SENSE] >= 7 )
...
//  ..	firstFrame numFrames loopFrames frameLerp initialLerp
static animation_t flagpoleAnim = { 0, "", 1, 40, 40, 1000/15, 1000/15 };
...
static void CG_Waypoint( centity_t *cent ) {
	...
	if(!CG_IsOnSameFireteam( cg.clientNum, cent->currentState.clientNum )) {
		return;
	}
	...
	// initial setup
	if(!cent->lerpFrame.oldFrame) {
		cent->lerpFrame.frameTime		=
		cent->lerpFrame.oldFrameTime	= cg.time;

		cent->lerpFrame.animationNumber = 0;
		cent->lerpFrame.animation		= &flagpoleAnim;
		cent->lerpFrame.animationTime	= cent->lerpFrame.frameTime + cent->lerpFrame.animation->initialLerp;

		cent->lerpFrame.frame			=
		cent->lerpFrame.oldFrame		= cent->lerpFrame.animation->firstFrame;
	}

	CG_RunLerpFrame(cent, NULL, &cent->lerpFrame, 0, 1);	
	...
	switch( cent->currentState.frame ) {
		case WAYP_ATTACK: ent.customSkin = cgs.media.waypointModelAttackSkin; break;
		case WAYP_DEFEND: ent.customSkin = cgs.media.waypointModelDefendSkin; break;
		case WAYP_REGROUP: ent.customSkin = cgs.media.waypointModelRegroupSkin; break;
		default: return;
	}
	...
	ent.hModel = cgs.media.waypointMarker;
