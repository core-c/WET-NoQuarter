...
static void CG_DrawDanger( void ){
	...
	if ( cgs.clientinfo[snap->ps.clientNum].skill[SK_BATTLE_SENSE] < 8 )
		return;
...
static void CG_DrawCrosshairNames( void ) {
	...
	if ( cg.crosshairClientNum > MAX_CLIENTS ) {
		...
		if( cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR ) {
			if( cg_entities[cg.crosshairClientNum].currentState.eType == ET_MOVER && cg_entities[cg.crosshairClientNum].currentState.effect1Time ) {
				...
				if ( cg_entities[cg.crosshairClientNum].currentState.teamNum >= 4 || cgs.clientinfo[cg.snap->ps.clientNum].skill[SK_BATTLE_SENSE] < 7)
					return;
		...
		if( (cg_entities[cg.crosshairClientNum].currentState.powerups & (1 << PW_OPS_DISGUISED)) && cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR) {
			...
			if((cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR &&
				...
				(cgs.jp_keepAwards & JP_KEEP_RECOGNITION))) && cgs.clientinfo[ cg.crosshairClientNum ].skill[SK_COVERTOPS] < 7){
	...
	if ( !isTank && cg.snap->ps.stats[STAT_IDENTIFYAMMO] > -1 &&
		cgs.clientinfo[cg.snap->ps.clientNum].cls == PC_FIELDOPS &&
		cgs.clientinfo[cg.snap->ps.clientNum].skill[SK_SIGNALS] >= 6 ){
...
static int CG_PlayerAmmoValue( int *ammo, int *clips, int *akimboammo ) {
	...
	*ammo = *clips = *akimboammo = -1;
	...
	switch(weap) {		// some weapons don't draw ammo count text
		...
		case WP_KNIFE:
		case WP_KNIFE_KABAR:
			if ( cgs.clientinfo[cg.snap->ps.clientNum].skill[SK_LIGHT_WEAPONS] < 8 )
				return weap;
		case WP_LANDMINE:
...
static void CG_DrawPlayerStatusHead( void ) 
{
	...
	CG_DrawPlayerHead( &headRect, character, headcharacter, 180, 0, cg.snap->ps.eFlags & EF_HEADSHOT ? qfalse : qtrue,
		anim, painshader, &cgs.clientinfo[ cg.snap->ps.clientNum ], adren, cgs.clientinfo[ cg.snap->ps.clientNum].team,
		cls, cg.snap->ps.powerups[PW_OPS_DISGUISED]);
...
// Equipment Icons
	...
	if (COM_BitCheck( cg.snap->ps.weapons, WP_MEDIC_ADRENALINE ) || ps->powerups[PW_ADRENALINE])
	{
		...
		if ((cg.time - ps->classWeaponTime) < (cg.medicChargeTime[ps->persistant[PERS_TEAM]-1] * BG_ChargeUsage(WP_MEDIC_ADRENALINE, cgs.clientinfo[ps->clientNum].skill)) ) 
		{
