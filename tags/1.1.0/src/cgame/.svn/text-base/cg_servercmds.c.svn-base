...
#include "cg_local.h"
// Lucel test code: #include "../game/g_strparse.h"
...
void CG_ParseWolfinfo( void ) {
	...
	/* Lucel test code - commented out for now
	...
	*/
	...
	value = Info_ValueForKey( info, "g_currentCampaign" );
	// Copy the current campaign name to ensure it doesn't get overwritten - Lucel
	...
	CG_Printf("CG_ParseWolfinfo: info = %s\n", info);
	...
	while(1)
	{
		...
		if ( key[0] == 0 )
			break;
		g_StringToken_t token		= G_GetTokenForString(key);
		switch(token)
		{
		case TOK_g_currentRound:
			cgs.currentRound		= atoi(value);
			CG_Printf("CG_ParseWolfinfo: retrieved currentRound = %d\n", cgs.currentRound);
			break;
		case TOK_g_nextTimeLimit:
			cgs.nextTimeLimit 		= atof(value);
			CG_Printf("CG_ParseWolfinfo: retrieved nextTimeLimit = %d\n", cgs.nextTimeLimit);
			break;
		case TOK_gamestate:
			cgs.gamestate  			= atoi(value);
			CG_Printf("CG_ParseWolfinfo: retrieved gamestate = %d\n", cgs.gamestate);
			break;
		case TOK_g_currentCampaign:
			Q_strncpyz(cgs.currentCampaign, value, sizeof(cgs.currentCampaign));
			CG_Printf("CG_ParseWolfinfo: retrieved currentCampaign = %d\n", cgs.currentCampaign);
			break;
		case TOK_g_currentCampaignMap:
			cgs.currentCampaignMap 	= atoi(value);
			CG_Printf("CG_ParseWolfinfo: retrieved currentCampaignMap = %d\n", cgs.currentCampaignMap);
			break;
		case TOK_UNKNOWN:
		default:
			CG_Printf("CG_ParseWolfinfo: unknown token in config string: %s=>%s\n", key, value);
			break;
		}
	}*/
...
//	BG_cleanName(cgs.clientinfo[gs->nClientID].name, strName, sizeof(strName), qfalse);
	...
	if(weaponMask != 0) 
	{
		...
		if(gs->fHasStats)
		{
			...
			int darwin	  = BG_GetStatDarwin(&cg.snap->ps);			
			int goombas   = BG_GetStatGoombas(&cg.snap->ps);	
