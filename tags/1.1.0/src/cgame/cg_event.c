...
#if 0 // jaquboss not used
static void CG_GraphicObituary( entityState_t *ent)
{
	int				mod;
	int				target, attacker;
	int				curWeap;
	...
	if( attacker == target ) 
	{
		switch (mod) 
		{
			...
			case MOD_MORTAR: // rain
				...
				break;
			case MOD_FIREBOLT: 
				message = "tried to roast his own pants";
				break;
...
// may happen in 2.55 when crouching and shooting at ground, or in all ET versions when just shooting metal surface
	...
	if ( BG_Weapons[weapon].selfKillMessage[0] && message)
		message = va("%s",BG_Weapons[weapon].selfKillMessage );

	// if we've got a message assigned, deal with it and we're done
	if (cg_graphicObituaries.integer && message) {
		int scale=1;
		...
		CG_AddPMItem( PM_DEATH, va( "%s %s.", targetName, message ), deathShader, OB_YELLOW );
		return;
	}
	...
	if( ca ) {
		...
		case MOD_KNIFE:
			...
			message2 = "'s knife";
			// OSP - goat luvin
			if( (attacker == cg.snap->ps.clientNum || target == cg.snap->ps.clientNum) && cg_goatSound.integer & 1) {
		...
		case MOD_BACKSTAB:
			...
			message2 = "'s knife";
			// OSP - goat luvin
			if( (attacker == cg.snap->ps.clientNum || target == cg.snap->ps.clientNum) && cg_goatSound.integer & 2 ) {
				trap_S_StartSound( cg.snap->ps.origin, cg.snap->ps.clientNum, CHAN_AUTO, cgs.media.knifeKill );
			}
			break;
		...
		case MOD_FIREBOLT:
			...
			message2 = "'s searing plasma globe of destruction";
			break;
		...
		case MOD_FG42SCOPE:					// Axis FG42 Paratrooper Rifle, through scope
		...
		case MOD_POISON:					// josh
			overrideIcon = qtrue;
			icon =	cgs.media.poisonObituary;
			message	 = "was lethally injected with";
			message2 = "'s poison";
			break;
...
// JET PILOT MODE OBITUARIES
		case MOD_EXPLOSIVE:
			overrideIcon = qtrue;
			message = "died in explosion of";
			message2 = "'s jet";
			break;
		...
		case MOD_JETGUN:
			overrideIcon = qtrue;
			message  = "was perforated by";
			message2 = "'s jet-mounted machinegun";
			break;

		case MOD_COLISION:
			overrideIcon = qtrue;
			message  = "was knocked out of the skies by";
			message2 = "'s jet";
			break;
// END
		...
		if ( BG_Weapons[weapon].KillMessage[0] )
		message = va("%s",BG_Weapons[weapon].KillMessage);

		if ( BG_Weapons[weapon].KillMessage2[0] )
		message2 = va("%s",BG_Weapons[weapon].KillMessage2);


		if ( flags & OBIT_EXECUTED )
			message  =  "was executed by";

		if( ci->team == ca->team && mod != MOD_SWAP_PLACES) { // jaquboss - not for sapping places
		//	message = "^7was ^1TEAMKILLED ^7by";
			tkString = "^1TEAM KILL:^7 ";
			// jP - keep the scondary death message
			// message2= ""; 
			VectorCopy( OB_RED, color);
		}

	/*	if ( flags & OBIT_EXECUTED )
			message  = ci->team != ca->team ? "was executed by" : "^7was executed by ^1TEAMMATE^7";
	*/
		if (cg_graphicObituaries.integer && message) {
			int scale=1;
			...
			if ( message2 ) 
			{
				...
				CG_AddPMItem( PM_DEATH, va( "%s%s %s %s%s", tkString, targetName, message, attackerName, message2 ), deathShader, color );
			}
			return;
		}
	}
	...
	switch( mod ) {
		default:
			CG_AddPMItem( PM_DEATH, va( "%s died.", targetName ), deathShader, OB_YELLOW );
			break;
	}
}
//==========================================================================
...
static void CG_ItemPickup( int itemNum ) {
	...
	case IT_AMMO:
		shader = cgs.media.pmImages[PM_AMMOPICKUP];
		break;
	case IT_HEALTH:
		shader = cgs.media.pmImages[PM_HEALTHPICKUP];
		break;
	default:
		shader = 0;
		break;
	...
	CG_AddPMItem( PM_AMMOPICKUP, va( "Picked up %s", CG_PickupItemText( itemNum ) ), shader, NULL );
...
//	cg.itemPickupBlendTime	= cg.time;
	...
	if ( bg_itemlist[itemNum].giType == IT_WEAPON ) {
		...
		//	4 - "New or Better"
			...
			if( itemid != WP_FG42_SCOPE && itemid != WP_GARAND_SCOPE && itemid != WP_K43_SCOPE && itemid != WP_AMMO) 
			{	//----(SA)	modified
...
*/
	...
	case EV_WATER_UNDER:
		...
		if( cg.clientNum == es->number ) {
			if ( cgs.clientinfo[cg.clientNum].skill[SK_BATTLE_SENSE] >= 6 )
...
// jpw
	...
	case EV_WEAPONSWITCHED:
		...
		if( es->number == cg.snap->ps.clientNum		&& 
			...
			es->weapon == WP_MORTAR2_SET		 	|| es->weapon == WP_MOBILE_MG42_SET	 		||
			es->weapon == WP_MOBILE_BROWNING_SET 	|| es->weapon == WP_BAR_SET			 		||
			es->weapon == WP_GRENADE_LAUNCHER	 	|| es->weapon == WP_GRENADE_PINEAPPLE	 	||
			es->weapon == WP_DYNAMITE				|| es->weapon == WP_SMOKE_MARKER			||
			es->weapon == WP_PANZERFAUST			|| es->weapon == WP_BAZOOKA					||
			es->weapon == WP_ARTY					||
	...
	case EV_KICK:
		if ( es->clientNum == cg.snap->ps.clientNum )
			CG_WeaponFireRecoil( WP_NONE );
		break;
	/*case EV_KICKED:
		cent->pe.kickTime = cg.time;
		cent->pe.kickAnim  = ( ( cent->pe.kickAnim & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) | WEAP_ATTACK1;	
		break;*/
	case EV_WOLFKICK_HIT_WALL:
		DEBUGNAME( "EV_WOLFKICK_HIT_WALL" );
		ByteToDir( es->eventParm, dir );
		CG_MissileHitWall( WP_NONE, 0, position, dir, 0 );	// (SA) modified to send missilehitwall surface parameters
		break;
...
/*	case EV_FLAMEBARREL_BOUNCE:
	...
	case EV_RAILTRAIL:
		//DEBUGNAME("EV_RAILTRAIL");
		{
			...
			color[0] = (float)(es->angles[0]/255);
			color[1] = (float)(es->angles[1]/255);
			color[2] = (float)(es->angles[2]/255);
	...
	case EV_MG42BULLET_HIT_FLESH:
		...
		CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, qtrue, es->eventParm, es->otherEntityNum2, 0, es->effect1Time, qfalse, NULL);
		break;
...
/*	case EV_POPUPBOOK:
	...
	case EV_GENERAL_SOUND:
		...
		if ( es->eventParm > bg_numGameSound ){
...
//		trap_S_StopStreamingSound( es->number );
	...
	case EV_LOSE_HAT:
		...
		ByteToDir( es->eventParm, dir );
		if ( ci->cls == PC_SOLDIER ) {
			if ( ci->skill[SK_SOLDIER] >= 9 ) 
				CG_LoseACC( cent, tv(0, 0, 1), ACC_MOUTH2, "tag_mouth", qtrue );

			CG_LoseACC( cent, dir, ACC_HAT, "tag_mouth", qtrue );
		} else {
			CG_LoseACC( cent, dir, ci->rank >= NUM_EXPERIENCE_LEVELS-1 ? ACC_MOUTH2 : ACC_HAT, "tag_mouth", qtrue );
		}
		break;
	...
	case EV_MEDIC_CALL:
		...
		// jaquboss - moved here, because it disables autocompletion in VS 03 on 'sound = ( ...'
	case EV_FX_SOUND:
		{
			sfxHandle_t sound;
