...
void CG_Class_f( void ) {
	...
	{
		weaponType_t* wt = WM_FindWeaponTypeForWeapon( weaponNum );

		weaponNum = classInfo->classWeapons[0];
		CG_LimboPanel_SetSelectedWeaponNumForSlot(0, 0);
		CG_Printf( "%s is disabled\n", wt ? wt->desc : va("Weapon number %i", weaponNum ) );
	}

	trap_SendClientCommand( va( "team %s %i %i \n",  cgs.clientinfo[ cg.clientNum ].team == TEAM_AXIS ? "r" : "b", selectClass, weaponNum ) );

#ifdef __SSYSTEM__
	CG_SendPCInfo( selectClass, cgs.clientinfo[ cg.clientNum ].team );
#endif
...
//	{ "setspawnpt",			CG_Obj_f },
	...
	// jet Pilot - mouse pointer mode
//	{ "+mouseMode",			CG_MouseModeDown_f },
//	{ "-mouseMode",			CG_MouseModeUp_f },
};
...
//	trap_AddCommand ("listbotgoals");
	...
	trap_AddCommand ("waypoint");
	trap_AddCommand ("clearwaypoint");
