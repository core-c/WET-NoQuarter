...
panel_button_t playerLimboHead = {
	...
	NULL,	/* font		*/
#ifdef __SSYSTEM__
	CG_LimboPanel_CharacterEdit_KeyDown, // @TODO! MOVE!!!
#else
	NULL,	/* keyDown	*/
#endif
	NULL,	/* keyUp	*/
...
#ifdef __SSYSTEM__
qboolean CG_LimboPanel_CharacterEdit_KeyDown( panel_button_t* button, int key ) {
	if( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
		return qfalse;

	if( key == K_MOUSE1 ) {
		SOUND_SELECT;
		cgs.limboDrawCharacter = cgs.limboDrawCharacter ? qfalse : qtrue;
		return qtrue;
	}
...
#ifdef __SSYSTEM__
void CG_SendPCInfo( int cls, int team) {

	if ( team != TEAM_AXIS && team != TEAM_ALLIES )
		return;

	if ( cls < PC_SOLDIER || cls > PC_COVERTOPS )
		return;

	trap_SendClientCommand( va( "pcinfo %i %i %i %i %i %i %i %i %i %i %i %i %i %i %i\n", 
		cgs.limboPcInfo[cls][team-1].headIndex,
		cgs.limboPcInfo[cls][team-1].faceIndex,
		cgs.limboPcInfo[cls][team-1].bodyIndex,
		cgs.limboPcInfo[cls][team-1].torsoIndex,
		cgs.limboPcInfo[cls][team-1].legIndex,
		cgs.limboPcInfo[cls][team-1].helmetIndex,
		cgs.limboPcInfo[cls][team-1].helmetSkinIndex,
		cgs.limboPcInfo[cls][team-1].backPackIndex,
		cgs.limboPcInfo[cls][team-1].backPackSkinIndex,
		cgs.limboPcInfo[cls][team-1].bLeftIndex,
		cgs.limboPcInfo[cls][team-1].bLeftSkinIndex,
		cgs.limboPcInfo[cls][team-1].bRightIndex,
		cgs.limboPcInfo[cls][team-1].bRightSkinIndex,
		cgs.limboPcInfo[cls][team-1].glassIndex,
		cgs.limboPcInfo[cls][team-1].glassSkinIndex
		) );
}
#endif
...
#ifdef __SSYSTEM__
	CG_SendPCInfo( CG_LimboPanel_GetClass(), team );
#endif
...
// jet Pilot - CG_LimboPanel_RenderClassButton - 
// change the way class buttons are rendered from the pie wedges to a numerical format
/*void CG_LimboPanel_RenderClassButton( panel_button_t* button )
{
//	vec4_t		clr  = { 1.f, 1.f, 1.f, 0.4f };		// 40% opacity
	vec4_t		clr2 = { 1.f, 1.f, 1.f, 0.75f };	// 25
	vec4_t		clr3 = { 1.f, 1.f, 1.f, 0.6f };		// 40

	int			skillimage;							// a placeholder for skill level graphics
	int			buttonskill;						// the clients skill level for the class this button represents
//	qboolean	faded;								// is the button faded?
	float		bwidth;								// button width
	float		bheight;							// button height

	// Set up the skill level image
	buttonskill = cgs.clientinfo[cg.clientNum].skill[BG_ClassSkillForClass(button->data[1])];
	skillimage	= cgs.media.limboSkillLevel[buttonskill];
	bwidth		= button->rect.w - button->rect.x;
	bheight		= button->rect.h - button->rect.y;
//	faded		= (BG_CursorInRect( &button->rect ) && (button->data[1] != CG_LimboPanel_GetClass()));

	// Draw the blank button
	...
	if( CG_LimboPanel_GetTeam() != TEAM_SPECTATOR  )
	{
		if( button->data[1] == CG_LimboPanel_GetClass()) 
		{
			...
			CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, skillimage );

			trap_R_SetColor( NULL );	// Reset opacity
		}
		else if( BG_CursorInRect( &button->rect ) ) 
		{				
			trap_R_SetColor( clr3 );	// Set opacity to 40%

			// Draw lit button and skill level at reduced opacity
			CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, cgs.media.limboClassButton2Back_on );			
			CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, skillimage );
...
#ifdef __SSYSTEM__
	if ( !cgs.limboDrawCharacter || CG_LimboPanel_GetTeam() == TEAM_SPECTATOR){
#endif
...
#ifdef __SSYSTEM__
	// ..draw character instead..
	} else {
		rectDef_t rect = { 80, 25, 450, 400 };

		CG_DrawPlayer( &rect, CG_LimboPanel_GetClass(), CG_LimboPanel_GetTeam() );
	}
#endif
}
...
// jet Pilot - modified CG_DrawPlayerHead to accept a clientinfo pointer instead of rank.
void CG_DrawPlayerHead( rectDef_t *rect, bg_character_t* character, bg_character_t* headcharacter, float yaw, float pitch, qboolean drawHat, hudHeadAnimNumber_t animation, qhandle_t painSkin, clientInfo_t *ci, qboolean adrenaline, int team, int cls, int disguise ) 
{
	float			len;
	vec3_t			origin;
	...
	float			x, y, w, h;
	refdef_t		refdef;
	refEntity_t		head, hat, armor, mrank;
	...
	if ( rank >= NUM_EXPERIENCE_LEVELS-1 && cls != PC_SOLDIER )
		hatIndex = ACC_MOUTH2;
...
#ifdef __SSYSTEM__
	trap_R_ModelBounds( character->head[0].model, mins, maxs );
#else
...
#ifdef __SSYSTEM__
	head.hModel = headcharacter->head[ci->pc.headIndex].model;
	head.customSkin = headcharacter->head[ci->pc.headIndex].skin[ci->pc.faceIndex];
#else 
	...
	CG_HudHeadAnimation( headcharacter, &cg.predictedPlayerEntity.pe.hudhead, &head.oldframe, &head.frame, &head.backlerp, animation, ci );

	if( drawHat ) 
	{
		memset( &mrank, 0, sizeof( mrank ) ); // jaquboss - protection from client error while rank is > 4
		memset( &hat, 0, sizeof( hat ) );
#ifndef __SSYSTEM__
		hat.hModel = character->accModels[hatIndex];
		hat.customSkin = character->accSkins[hatIndex];
#else
		hat.hModel = character->acc[ACC_HAT][ci->pc.helmetIndex].model;
		hat.customSkin = character->acc[ACC_HAT][ci->pc.helmetIndex].skin[ci->pc.helmetSkinIndex];
#endif
		...
		if ( cls == PC_SOLDIER && ci && ci->skill[SK_SOLDIER] >= 9 )
		{
			memset( &armor, 0, sizeof( armor ) );
#ifndef __SSYSTEM__
			armor.hModel = character->accModels[ACC_MOUTH2];
			armor.customSkin = character->accSkins[ACC_MOUTH2];
#else
			armor.hModel = character->acc[ACC_MOUTH2][ci->pc.helmetIndex].model;
			armor.customSkin = character->acc[ACC_MOUTH2][ci->pc.helmetIndex].skin[ci->pc.helmetSkinIndex];
#endif
			armor.renderfx = hat.renderfx;		// no stencil shadows
			CG_PositionEntityOnTag( &armor, &head, "tag_mouth", 0, NULL);

		}

		if( rank && hatIndex == ACC_HAT )
		{
#ifndef __SSYSTEM__
			mrank.hModel = character->accModels[ ACC_RANK ];
#else
			mrank.hModel = character->acc[ACC_RANK][ci->pc.helmetIndex].model;
#endif
			mrank.customShader = rankicons[ rank ][  team == TEAM_AXIS ? 1 : 0 ][1].shader; // jaquboss, team support
	...
	if( drawHat ) {
		...
		if ( cls == PC_SOLDIER && ci && ci->skill[SK_SOLDIER] >= 9 ) {
			trap_R_AddRefEntityToScene( &armor );
		}
	}
...
void CG_LimboPanel_RenderHead( panel_button_t* button ) {
	...
	if( CG_LimboPanel_GetTeam() != TEAM_SPECTATOR ) {
		...
		CG_DrawPlayerHead( &button->rect, CG_LimboPanel_GetCharacter(), CG_LimboPanel_GetCharacter(), 180, 0, qtrue, HD_IDLE4, 0, &cgs.clientinfo[ cg.clientNum ], qfalse, CG_LimboPanel_GetTeam(), CG_LimboPanel_GetClass(), 0 );
	} else {
...
void CG_LimboPanel_WeaponPanel( panel_button_t* button ) {
	...
	if( BG_PanelButtons_GetFocusButton() == button && cnt > 1 ) {
		...
		for( i = 0, x = 1; i < cnt; i++ ) {
			...
			if( cycleWeap != weap && (cycleWeap != WP_VENOM || cg_insanity.integer & JP_INSANITY_VENOM)) {
...
int CG_LimboPanel_WeaponCount_ForSlot( int number ) {
	if( number == 1 ) {
		...
		for(i = 0; i < MAX_WEAPS_PER_CLASS; i++) {
			...
			if ( classInfo->classWeapons[i] == WP_VENOM && !(cg_insanity.integer & JP_INSANITY_VENOM ))
				continue;
...
int CG_LimboPanel_ClassCount( int cls ) {
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		if( cls != -1 ) {
			...
			if( cgs.clientinfo[i].cls != cls && cgs.clientinfo[i].latchedCls != cls ) {
				continue;
			}
		}
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		if( weap != -1 ) {
			...
			if ( weap == WP_M7 || weap == WP_GPG40 ){
				if ( !cgs.clientinfo[i].rifleNade || ( cgs.clientinfo[i].weapon != weapAlts[weap] && cgs.clientinfo[i].latchedweapon != weapAlts[weap] ) )
					continue; // jaquboss added case for non rifle owners
			} else if( cgs.clientinfo[i].weapon != weap && cgs.clientinfo[i].latchedweapon != weap ) {
				if ( cgs.clientinfo[i].weapon != weapTeamConversion[weap] && cgs.clientinfo[i].latchedweapon != weapTeamConversion[weap] )
					continue;
			}
		}
...
qboolean CG_LimboPanel_ClassIsDisabled( int index ) {
	...
	{
		case PC_SOLDIER:
			if(cgs.maxSoldiers == -1) return qfalse;
			if(wcount >= cgs.maxSoldiers) {
				return qtrue;
			}
			break;
		case PC_MEDIC:
			if(cgs.maxMedics == -1) return qfalse;
			if(wcount >= cgs.maxMedics) {
				return qtrue;
			}
			break;
		case PC_ENGINEER:
			if(cgs.maxEngineers == -1) return qfalse;
			if(wcount >= cgs.maxEngineers) {
				return qtrue;
			}
			break;
		case PC_FIELDOPS:
			if(cgs.maxFieldops == -1) return qfalse;
			if(wcount >= cgs.maxFieldops) {
				return qtrue;
			}
			break;
		case PC_COVERTOPS:
			if(cgs.maxCovertops == -1) return qfalse;
			if(wcount >= cgs.maxCovertops) {
				return qtrue;
			}
			break;
		default:
...
qboolean CG_LimboPanel_WeaponIsDisabled( int index ) {
	...
	} else if( wcount >= ceil( count * cgs.weaponRestrictions ) ) {
		return qtrue;
	}

	switch(classinfo->classWeapons[index]) 
	{
		case WP_VENOM:
			if(!(cg_insanity.integer & JP_INSANITY_VENOM) ){ 
				return qtrue;
			}
			if(cgs.maxVenoms == -1) return qfalse;
			if(wcount >= cgs.maxVenoms) {
				return qtrue;
			}
			break;
		case WP_SHOTGUN:
			if(cgs.maxShotguns == -1) return qfalse;
			if(wcount >= cgs.maxShotguns) {
				return qtrue;
			}
			break;
		case WP_PANZERFAUST:
		case WP_BAZOOKA:
			if(cgs.maxPanzers == -1) return qfalse;
			if(wcount >= cgs.maxPanzers) {
				return qtrue;
			}
			break;
		case WP_MOBILE_BROWNING:
		case WP_MOBILE_MG42:
			if(cgs.maxMG42s == -1) return qfalse;
			if(wcount >= cgs.maxMG42s) {
				return qtrue;
			}
			break;
		case WP_FLAMETHROWER:
			if(cgs.maxFlamers == -1) return qfalse;
			if(wcount >= cgs.maxFlamers) {
				return qtrue;
			}
			break;
		case WP_MORTAR:
		case WP_MORTAR2:
			if(cgs.maxMortars == -1) return qfalse;
			if(wcount >= cgs.maxMortars) {
				return qtrue;
			}
			break;
		case WP_GPG40:
		case WP_M7: // jaquboss - not the carbine!!
			if(cgs.maxGrenLaunchers == -1) return qfalse;
			if(wcount >= cgs.maxGrenLaunchers) {
				return qtrue;
			}
			break;
		default:	// Jaybird
