...
static void CG_PlayerAura( centity_t *cent, float shadowPlane, clientInfo_t *ci )
{
	vec4_t			projection;
	qhandle_t		shader;
	vec3_t			end;
	...
	{
		case PC_MEDIC:
			if ( ci->skill[SK_MEDIC] < 5 )
				return;
			shader = cgs.media.auraCaduceus;
			break;
		case PC_ENGINEER:
			if ( ci->skill[SK_ENGINEER] < 5 )
				return;
			shader = cgs.media.auraEngineer;
			break;
		case PC_FIELDOPS:
			if ( ci->skill[SK_SIGNALS] < 5 )
				return;
	...
	projection[ 3 ] = radius;
	VectorMA( trace.endpos, -16.0f, projection, trace.endpos );

	CG_ImpactMark( shader, trace.endpos, projection, radius, 0, 1.f, 1.f, 1.f, 1.0f-trace.fraction, -1 );
...
//	float			gumsflappin = 0;	// talking amplitude
	...
	CG_PlayerAura( cent, shadowPlane, ci );
...
//	head.renderfx = renderfx;
	...
	if( cent->currentState.eType != ET_CORPSE && !(cent->currentState.eFlags & EF_PLAYDEAD) ) {
			...
			switch(i)
			{
				...
				case ACC_MOUTH2:		// Officer's Cap
					...
					if (i == ACC_RANK) {
						...
						if( ci->rank <= 0 || ci->rank >= NUM_EXPERIENCE_LEVELS )
							continue;
		
						if ( ci->cls != PC_SOLDIER && ci->rank >= NUM_EXPERIENCE_LEVELS-1 )
							continue;

						if ( cent->currentState.powerups & (1 << PW_OPS_DISGUISED) ){
							iconNum = ci->team == TEAM_AXIS ? 0 : 1;
							acc.customShader = rankicons[ ci->disguiseRank ][ iconNum ][1].shader;
						} else {
							iconNum = ci->team == TEAM_AXIS ? 1 : 0;
							acc.customShader = rankicons[ ci->rank ][ iconNum ][1].shader;	
						}				
					}
					// generals use cap not hat
					if ( i == ACC_HAT )
					{
						if ( cent->currentState.powerups & (1 << PW_OPS_DISGUISED) ){
							if ( ci->disguiseRank >= NUM_EXPERIENCE_LEVELS-1 && (( cent->currentState.powerups >> PW_OPS_CLASS_1 ) & 7) != PC_SOLDIER )
								continue;
						} else if ( ci->cls != PC_SOLDIER && ci->rank >= NUM_EXPERIENCE_LEVELS-1 ){
							continue;
						}
					}

					// helmet shield or GoA cap
					if (i == ACC_MOUTH2) {
						if ( cent->currentState.powerups & (1 << PW_OPS_DISGUISED) ){
							if ( ci->disguiseRank < NUM_EXPERIENCE_LEVELS-1 || (( cent->currentState.powerups >> PW_OPS_CLASS_1 ) & 7) == PC_SOLDIER ) // never do a shield for disguised covert ops
								continue;
						} else if ( ( ci->skill[SK_HEAVY_WEAPONS] < 9 || ci->cls != PC_SOLDIER ) && ci->rank < NUM_EXPERIENCE_LEVELS-1 ){
							continue;
						}
					}			
