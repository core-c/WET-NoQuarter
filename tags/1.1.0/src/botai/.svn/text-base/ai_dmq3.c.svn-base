...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	if ( g_entities[bs->client].s.eFlags & EF_MG42_ACTIVE ) {
		...
		if (bs->weaponnum == WP_PANZERFAUST || bs->weaponnum == WP_MORTAR_SET  || bs->weaponnum == WP_MORTAR2_SET) {
...
void BotDropFlag( bot_state_t *bs ) {
	...
	if (item) {
		...
		flag = LaunchItem(item,ent->r.currentOrigin,launchvel,ent->s.number, FL_DROPPED_ITEM);
...
void BotCalculateMg42Spots(void) {
	...
	// Jaybird - clean up compiler warnings
...
qboolean BotGetReachableEntityArea( bot_state_t *bs, int entityNum, bot_goal_t *goal )
{
	...
	memset(&brushPos, 0, sizeof(brushPos));
...
int BotCanSnipe( bot_state_t *bs, qboolean checkAmmo ) {
	int sniperWeapons[] = {WP_GARAND_SCOPE, WP_K43_SCOPE, WP_FG42_SCOPE, -1};
...
qboolean BotEntityWithinView( bot_state_t *bs, int viewEnt )
{
	...
	} else if( bs->cur_ps.weapon == WP_MORTAR_SET || bs->cur_ps.weapon == WP_MORTAR2_SET ) {
		// yaw
