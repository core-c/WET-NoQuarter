...
void P_WorldEffects( gentity_t *ent ) {
	...
	if ( ent->client->noclip ) {
		if ( ent->client->sess.skill[SK_BATTLE_SENSE] >= 7 )
...
void ClientImpacts( gentity_t *ent, pmove_t *pm ) {
	...
	for (i=0 ; i<pm->numtouch ; i++) {
		...
		if ( ent->client && other && other->client ){
#ifdef __JETPILOT__
			if ( jp_insanity.integer & JP_INSANITY_FLYING ){
			G_Damage( other, ent, ent, NULL, NULL, 50, 0, MOD_COLISION );
				continue;
			}
#endif
		}
...
#define CAN_REGENERATE ( (client->sess.playerType == PC_MEDIC /*|| jp_insanity.integer & JP_INSANITY_REGENERATE*/) && !(client->ps.eFlags & EF_DEAD) /*&& !(jp_insanity.integer & JP_INSANITY_VAMPIRE)*/)
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while( client->timeResidual >= 1000 ) 
	{
		...
		else 
		{
			...
			// count down health when over max or if 'Vampire' insanity flag is on // jet Pilot
			/*if ( jp_insanity.integer & JP_INSANITY_VAMPIRE )
			{
				if (ent->health > 1) 
					ent->health--;
				else 
					G_Damage( ent, ent, ent, NULL, NULL, 5, 0, MOD_BLEEDING);
			}*/
		}
		
		// jP Note - if a player is poisoned (and poison is enabled) do the poison
		if(client->ps.eFlags & EF_POISONED && ent->health > 0 && g_poison.integer) 
		{
			attacker = g_entities + client->pmext.poisonerEnt;

			if(g_poisonSound.string[0]) 
			{
				G_AddEvent(ent, EV_GENERAL_SOUND, G_SoundIndex(g_poisonSound.string));
			}

			G_Damage(ent, attacker, attacker, NULL, NULL, g_poison.integer, 0, MOD_POISON);
		}

		// jP Note - clear poison flag if the player is dead or wounded
		if(client->ps.eFlags & EF_POISONED && (ent->health <= 0 || client->ps.eFlags & EF_DEAD)) 
		{
			client->ps.eFlags &= ~EF_POISONED;
		}
	}
...
//	vec3_t	temp;	// rain - unused
	...
	if (self->client->ps.eFlags & EF_DEFENDING)
		return;
#endif
...
void ClientThink_real( gentity_t *ent ) {
	...
	if ( jp_insanity.integer & JP_INSANITY_FLYING )
	{
		if ( !client->jetBox ){
			trap_UnlinkEntity(ent);
			VectorCopy(jetMins,ent->r.mins);
			VectorCopy(jetMins,client->ps.mins);
			VectorCopy(jetMaxs,ent->r.maxs);
			VectorCopy(jetMaxs,client->ps.maxs);
			client->jetBox = qtrue;
			trap_LinkEntity(ent);
		}
	} else {
...
// jaquboss, be a bit hacky, add scope to fg42 if we dont have any we need to do so because for some reason other code fail
	if(COM_BitCheck(client->ps.weapons, WP_FG42) && !COM_BitCheck(client->ps.weapons, WP_FG42_SCOPE)) 
		COM_BitSet(client->ps.weapons, WP_FG42_SCOPE); 

	if ( (level.time - ent->client->respawnTime < sv_fps.integer +1) && ent->client->ps.stats[STAT_HEALTH] > 0 ) // note this might happen more times but it doesnt really matter
...
// note this is because sending command from server does REALLY weird things
...
/*lse if ( jp_insanity.integer & JP_INSANITY_FLYING ){
		client->ps.pm_type = PM_JETPILOT;
	}*/
	...
	// set speed
#ifdef __JETPILOT__
	if ( !(jp_insanity.integer & JP_INSANITY_FLYING)){ // jaquboss -> jets are storing your acceleration there
#endif
...
// jet Pilot - Team Aura Effects
	...
	for ( i = 0 ; i < level.numConnectedClients; i++ ) 
	{
		...
		{
			case PC_SOLDIER:				
				nLevel = cl->sess.skill[SK_SOLDIER];				
				break;
			case PC_MEDIC:
			...
			case PC_FIELDOPS:
				...
				break;
			case PC_COVERTOPS:
				nLevel = cl->sess.skill[SK_COVERTOPS];
				break;
			default:
				continue;
		}

		if ((nLevel > pm.auraEffects[cl->sess.playerType] /*BG_GetAuraLevel(cl->sess.playerType, &pm.ps)*/) && (nLevel >= 5) &&
			trap_InPVS( g_entities[level.sortedClients[i]].r.currentOrigin, ent->r.currentOrigin ) )
		{
			...
			if (distance <= 512)
			{				
				pm.auraEffects[cl->sess.playerType] = nLevel;
				ent->client->auraEffects[cl->sess.playerType] = nLevel;
			//	BG_SetAuraLevel(&pm.ps, cl->sess.playerType, nLevel);			
			}
		}
	}
...
//	// use the snapped origin for linking so it matches client predicted versions
	...
	// events are called so if we had a crash we already recieved damage
