...
void G_Script_ScriptChange( gentity_t *ent, int newScriptNum ) 
{
	...
	if ( G_Script_ScriptRun( ent ) && ( ent->scriptStatus.scriptId == scriptStatusBackup.scriptId + 1 ) ) // make sure we didnt change our script via a third party
	{	
...
jet_fire
jaquboss -
fires 4 mg42 guns, with 1/4 damage...
*/
void jet_fire( gentity_t *ent ) {
	...
	for ( i=0;i<4;i++ ){
		...
		{
			case 0:
				VectorMA( muzzle, -32, right, origin );
				break;
			case 1:
				VectorMA( muzzle, -16, right, origin );
				break;
			case 2:
				VectorMA( muzzle, 16, right, origin );
				break;
		...
		Fire_Lead_Ext( ent, ent, MG42_SPREAD_MP, MG42_DAMAGE_MP/4, origin, forward, right, up, MOD_JETGUN );
	}
}
...
void script_mover_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod) 
{
	...
	// jet Pilot - store the number of the killer
	//self->botLastAttackedEnt = attacker ? attacker : ( inflictor ? inflictor : -1 );
	// Jaybird - cleaned this up.  A pointer was being stored here instead of an int
	if (attacker)
		self->botLastAttackedEnt = attacker - g_entities;
	else if (inflictor)
		self->botLastAttackedEnt = inflictor - g_entities;
	else
		self->botLastAttackedEnt = -1;	
...
void script_mover_spawn(gentity_t *ent) 
{
	...
	// jet Pilot - clear killer list
	ent->botLastAttackedEnt = -1;
}
...
void script_mover_blocked( gentity_t *ent, gentity_t *other ) {
	...
	if (other->client)
		other->client->ps.persistant[PERS_DARWINDEATHS]++;
}
