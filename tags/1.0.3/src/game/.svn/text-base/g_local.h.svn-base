...
#else 
#define BODY_TIME(t) (10000)	// jaquboss, old ET time, since there is 'unlimited' number of bodies now
#endif
...
typedef struct playerTeamState_s
{
	...
	int						location[3];
#ifdef USELOCATIONS
	int						locationIndex[2];		// ETPro/RTCW style locations
...
#ifdef USELOCATIONS
	char				locationName[64];		// jP - location name
#endif
...
//	int			accuracy_hits;		// total number of hits
	...
	gentity_t	*targetingPointer;	// jet Pilot - targeting Pointer
...
// jaquboss
	int			lastSkillsCheckTime;
	qboolean	weaponsRemoved;

	// copy it here to use on server
...
//	char				forceCvars[MAX_FORCECVARS][2][MAX_CVAR_VALUE_STRING];
...
#ifdef USELOCATIONS
	// jet Pilot - reimplementation of target_location	
	qboolean			locationLinked;			// locations have been loaded
	gentity_t			*locationHead;			// head of the location list
	int					numLocations;			// total number of locations on map
	qboolean			noloc;					// jaquboss, no locations found
	// ~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~
#endif
	...
	int		whineTime; // to whine about wrong server installation
	int		pk3whine; // with the thing bellow to do not open pk3 all the time
...
void ExecGive( gentity_t *ent, char *name, char *amt, char *amt2); // jaquboss
void Cmd_GiveDisguise_f(gentity_t *ent);
...
void jet_fire( gentity_t *ent ); // jaquboss
...
extern vmCvar_t		g_shotgunPellets;

extern vmCvar_t		g_maxLandminesRed;
extern vmCvar_t		g_maxLandminesBlue;
extern vmCvar_t		g_allowPersonalMines;

extern vmCvar_t		jp_maxLevelBattleSense;
extern vmCvar_t		jp_maxLevelLightWeapons;
extern vmCvar_t		jp_maxLevelSoldier;
extern vmCvar_t		jp_maxLevelMedic;		
extern vmCvar_t		jp_maxLevelEngineer;
extern vmCvar_t		jp_maxLevelFieldOp;
extern vmCvar_t		jp_maxLevelCovertOp;

extern vmCvar_t		jp_falloff;
extern vmCvar_t		jp_keepAwards;
extern vmCvar_t		jp_drawBB;
extern vmCvar_t		jp_insanity;
extern vmCvar_t		jp_killSpreeLevels;
extern vmCvar_t		g_weaponBounce;
extern vmCvar_t		g_throwDistance;
extern vmCvar_t		g_realHead;

//extern vmCvar_t		g_missileFlags;
...
extern vmCvar_t		g_painAnims;
extern vmCvar_t		g_painChance;
#ifdef USELOCATIONS
extern vmCvar_t		g_autoLoadLocations;
#endif
...
// tjw: number of players allowed on one fireteam (moved to bg_public.h, for use in cgame)
...
//Josh: g_skills flags
/*
//Josh: g_killRating flags  (if any flag is set, killrating is calculated)
...
#define KILL_RATING_SHUFFLE			4	// shuffle uses kill rating
*/
...
// jet Pilot - Throw your weapon to the ground - from code3arena
void ThrowWeapon( gentity_t *ent );
void jP_GetDurationString(int secs, char *dur, int len);
gentity_t *dropWeapon( gentity_t *ent, gitem_t *item, float angle, int flags );  // XRAY FMJ
