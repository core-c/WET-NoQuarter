...
void TeamplayInfoMessage( team_t team ) {
	...
	for (i = 0, cnt = 0; i < level.numConnectedClients; i++)
	{
		...
		if (player->inuse && player->client->sess.sessionTeam == team)
		{
			...
			// jet Pilot - send leanAmount to clients, (last on list)
#ifdef USELOCATIONS
			Com_sprintf( entry, sizeof(entry), " %i %i %i %i %i %i %i %i %s", 
				...
				player->client->pers.teamState.location[2],		// location grid z
				h,												// player's health
				player->s.powerups, 							// player's powerups
				player->client->pers.teamState.locationIndex[0],// player's location Index
				player->client->pers.teamState.locationIndex[1],
				player->client->pers.teamState.locationName);	
#else 
...
#ifdef USELOCATIONS
				// jet Pilot - ETPro/RTCW style location names
				// if a target_location exists for this postition, store it in location[0], and set location[1] to -999
				loc = Team_GetLocation( ent );
				if (loc != NULL && !level.noloc)
				{
					strcpy(ent->client->pers.teamState.locationName, loc->message ? loc->message : "Unknown");
					ent->client->pers.teamState.locationIndex[0] = loc->health;
					ent->client->pers.teamState.locationIndex[1] = loc->count2;
					//ent->client->pers.teamState.location[0] = loc->health;
					//ent->client->pers.teamState.location[1] = -999;
				}
				else
				{
					strcpy(ent->client->pers.teamState.locationName, "Unknown");
					ent->client->pers.teamState.locationIndex[0] = 0;
					ent->client->pers.teamState.locationIndex[1] = 0;
				}
				// end of jP additions
#endif
			}
...
{	
   	...
   	bestLoc = Team_GetLocation(ent);	
   	   	
	Com_sprintf(loc, loclen, /*"[lon]%s[lof]"*/"%s", bestLoc != NULL ? bestLoc->message : "Unknown");
...
void G_ActiveTeamBalance() 
{
	...
	if(!g_activeTeamBalance.integer)
		return;
	...
	if(percent < (g_activeTeamBalance.value * 1.0f))
		return;
	...
	if(g_ATB_rating.integer == ATBR_KILLRATING &&
		(g_killRating.integer & KILL_RATING_ENABLE)) {
	...
	if(l33t > -1) {
		...
		if(SetTeam(ent, loseteam, qtrue, -1, -1, qfalse)) {
			...
			if(ent->client->sess.sessionTeam == TEAM_ALLIES) {
				...
				// don't swap if losing team is down in players
				if((bcount + 1) <= (rcount - 1)) 
					return;
			}
			else {
