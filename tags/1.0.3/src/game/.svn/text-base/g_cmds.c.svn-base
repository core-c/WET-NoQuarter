...
void Cmd_PcInfo_f( gentity_t *ent ) {
	...
	if ( trap_Argc() < 2 ) {
		CP(va("print \"this command shouldn't be used manualy\n\""));
		return;
	}
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	//		{
	//			*location = BG_GetLocationString( ent->r.currentOrigin );
	//		}
	//		Com_sprintf (name, sizeof(name), "[lof](%s%c%c) (%s): ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, location);

#ifdef USELOCATIONS	
			if (Team_GetLocationMsg(ent, location, sizeof(location)) && !level.noloc)
			{
				Com_sprintf (name, sizeof(name), "[lof](%s%c%c) (%s): ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE, location);
			}
			else
