...
//
	...
	{
		if( pm->skill[SK_HEAVY_WEAPONS] >= 8)
		{
			scale *= 0.95;
		}
...
static void PM_CheckDuck (void)
{
	...
	if( (pm->cmd.upmove < 0 && !(pm->ps->eFlags & EF_MOUNTEDTANK) && !(pm->ps->pm_flags & PMF_LADDER) && pm->ps->pm_type != PM_JETPILOT ) || pm->ps->weapon == WP_MORTAR_SET || pm->ps->weapon == WP_MORTAR2_SET || pm->ps->pm_flags & PMF_MEDICUSE )
	{	// duck
		pm->ps->pm_flags |= PMF_DUCKED;
	}
	else
...
// holster
	if ( ( pm->ps->pm_flags & PMF_MEDICUSE || ( pm->ps->pm_flags & PMF_LADDER && PM_REALISM & REALISM_NOLADDERGUN) ) && !delayedFire ){
...
// Check zoom command
	if( pm->cmd.wbuttons & WBUTTON_ZOOM && pm->ps->stats[STAT_HEALTH] >= 0 && !( pm->ps->weaponDelay )) 
	{
		...
		{	
			...
			{
				if ( !pm->ps->weaponTime || pm->pmext->artyFire ) // jaquboss do not zoom while reloading and suc, artyFire is set if used binocs
					pm->ps->eFlags |= EF_ZOOMING;
			}
		}
