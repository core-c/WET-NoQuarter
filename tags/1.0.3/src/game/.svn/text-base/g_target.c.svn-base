...
#ifdef USELOCATIONS
// jet Pilot - un obsolete-ized target_location code
...
//	char		cString[1024] = "";
   	...
   	{
   	//	Com_Printf("^9target_location_linkup ^3WARNING: linkup already completed.\n", total);
   		return;
   	}
	...
	strcpy(locationName[0], "Unknown");
   	...
   	for (ent = g_entities, i = 0, n = 1, total = 0; i < level.num_entities; ent++, i++)
   	{
		if (n >= MAX_LOCATIONS)
		{
			Com_Printf("^3Warning : ^9Number of ^7target_location^9s (%d) exceeds ^7MAX_LOCATIONS (%d)\n",
				n, MAX_LOCATIONS);
			return;
		}
   		...
   		{   			
			...
			for (j = 0, k = 0; j < strlen(cString); j++)
			{
				if (cString[j] == '\\') k++;
			}
			if (k >= 5) 
			...
			strcat(cString, va("\\%s", ent->message));
   	     	
			trap_SetConfigstring( CS_LOCATIONS + curTot, cString );
#else	
			//trap_SetConfigstring( CS_LOCATIONS + n, ent->message );
			strcpy(locationName[n], ent->message);
#endif
      		ent->health			= n++;				
