...
/*		case HINT_ARMOR:
		...
		return; // jaquboss dont bother with these
			break;
...
*/
	...
	if (cg.cursorHintIcon == HINT_WEAPON || cg.cursorHintIcon == HINT_AMMO ){
		...
		CG_DrawPic( rect->x - (weaponIconScale(cg.cursorHintValue) * halfscale), rect->y - halfscale,  weaponIconScale(cg.cursorHintValue) * (rect->w + scale), rect->h + scale,  cg_weapons[ cg.cursorHintValue ].weaponIcon[1] );
...
//				trap_S_FadeBackgroundTrack( 0.0f, 500, 0 ); // jaquboss, limbo music was removed from game even before release, this will just make bugs bugs and will stop custom music
