...
void CG_Respawn( qboolean revived ) {
	...
	// jaquboss
	if ( cg_classAutoexec.integer && ( cgs.clientinfo[cg.clientNum].team == TEAM_AXIS || cgs.clientinfo[cg.clientNum].team == TEAM_ALLIES ) )
	{
		trap_SendConsoleCommand(va("exec autoexec_%s.cfg\n", BG_ClassnameForNumber_Filename(cgs.clientinfo[cg.clientNum].cls)));
	}
	if ( cg_teamAutoexec.integer )
	{
		trap_SendConsoleCommand(va("exec autoexec_%s.cfg\n", CG_TeamnameForNumber(cgs.clientinfo[cg.clientNum].team)));
	}
}
...
void CG_CheckLocalSounds( playerState_t *ps, playerState_t *ops ) {
	...
	// jaquboss - hitsounds
		if ( ops->persistant[PERS_HITS] != ps->persistant[PERS_HITS] && cg_hitSounds.integer & 1) {
		/*	const char			*s; // old code, was configurable, but printed a lot of crap
			char				tempStr[MAX_QPATH];

			s = CG_ConfigString( CS_SOUNDS + ps->persistant[PERS_HITSOUND] );
			if ( cgs.gameSounds[ ps->persistant[PERS_HITSOUND] ] ) {
				trap_S_StartSound (NULL, ps->clientNum, CHAN_AUTO, cgs.gameSounds[ ps->persistant[PERS_HITSOUND] ] );
			} else {
				s = CG_ConfigString( CS_SOUNDS + ps->persistant[PERS_HITSOUND] );
				trap_S_StartSound (NULL, ps->clientNum, CHAN_AUTO, CG_CustomSound( ps->clientNum, s ) );
			}
		*/
			// easy and nice way
