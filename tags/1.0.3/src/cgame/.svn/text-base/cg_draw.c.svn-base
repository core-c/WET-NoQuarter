...
/*
extern pmove_t *pm;
static float CG_DrawAuraLevels( float y, qboolean draw  )
{
	char		 	*s;
	int			 	w;
	int			 	aura[5];
//	int				size;
	...
	if (draw) // jaquboss - check for pm
	{
		
	//	size = BG_GetAuraLevels(aura, &cg.snap->ps); 

		for (i = 0; i < 5; i++)
		{
		//	aura[i] = BG_GetAuraLevel(i, &cg.snap->ps);
			aura[i] = cg.auraEffects[i];
		}
				
		s = va( "S%d M%d E%d F%d C%d", 
			aura[0], 
			aura[1],
			aura[2], 
			aura[3], 
...
*/
// jet Pilot - test func for smoke colors
/*static float CG_DrawSmokeColor( float y, qboolean draw  )
{
	char		 	*s;
	int			 	w;
	...
	if (draw) // jaquboss - check for pm
	{							
		s = va( "R%f G%f B%f A%f", 
			cgs.smokeColor_Allies[0], 
			cgs.smokeColor_Allies[1],
			cgs.smokeColor_Allies[2], 
			cgs.smokeColor_Allies[3]);	
...
static float CG_DrawKillSpree( float y, qboolean draw  )
{
	...
	if (draw)
	{
		kills = BG_GetKillSpree(&cg.snap->ps); //.persistant[PERS_KILLSPREE];
		totalKills = cg.snap->ps.persistant[PERS_KILLS];
		bounty = BG_GetStatBounty(&cg.snap->ps); //.persistant[PERS_SPREELEVEL];		
...
void CG_ScanForCrosshairDynamite(centity_t *cent) 
{
	...
	vec3_t      start, end;
   	...
   	if	(Square(trace.endpos[0] - cent->currentState.pos.trBase[0]) < 256 &&
   		 ...
   		 Square(trace.endpos[2] - cent->currentState.pos.trBase[2]) < 256)
	{
   		cg.crosshairDynamite	 = cent->currentState.otherEntityNum;
...
//	CG_Trace( &trace, start, vec3_origin, vec3_origin, end, cg.snap->ps.clientNum, MASK_ALL &~CONTENTS_MONSTERCLIP);
	...
	if(trace.entityNum == ENTITYNUM_WORLD) {
		...
		// knife
		if(trace.entityNum < MAX_CLIENTS &&	(cg.snap->ps.weapon == WP_KNIFE ||	cg.snap->ps.weapon == WP_KNIFE_KABAR || (IS_PISTOL_ALL(cg.snap->ps.weapon) && cgs.clientinfo[cg.clientNum].skill[SK_LIGHT_WEAPONS] >= 9 ) ) ) 
		{
...
static void CG_DrawCrosshairNames( void ) {
	...
	if ( cg.crosshairMine > -1 ) 
	{
		...
		//CG_DrawTinyStringColor( 320 - w / 2, 170, s, color );
		CG_DrawStringExt(320 - w / 2, 170, s, colorWhite, qfalse, qfalse, SMALLCHAR_WIDTH - 2, SMALLCHAR_HEIGHT - 2, 0);
		cg.crosshairMine = -1;
		return;
	} 
	// jet Pilot - crosshair dynamite
	if (cg.crosshairDynamite > -1 )
	{
	...
	if ( cg.crosshairClientNum > MAX_CLIENTS ) {
		...
		if( (cg_entities[cg.crosshairClientNum].currentState.powerups & (1 << PW_OPS_DISGUISED)) && cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR) {
				...
				(cgs.clientinfo[cg.snap->ps.clientNum].cls == PC_FIELDOPS || (cgs.jp_keepAwards & JP_KEEP_RECOGNITION)) ) 
			{
				...
				return;
			} else if( dist > 512 || cgs.clientinfo[ cg.crosshairClientNum ].skill[SK_COVERTOPS] >= 9) {
				if ( !cg_drawCrosshairNames.integer ) {
					return;
				}
				...
				//CG_DrawSmallStringColor( 320 - w / 2, 170, s, color );
				CG_DrawStringExt(320 - w / 2, 170, s, colorWhite, qfalse, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0);
	...
	if ( !isTank &&	!( cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR && cgs.clientinfo[ cg.crosshairClientNum ].team != cgs.clientinfo[cg.snap->ps.clientNum].team ) ) 
	{
		...
		// jet Pilot - draw colored names when looking at a player
		CG_DrawStringExt(320 - w / 2, 170, s, colorWhite, qfalse, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0);
...
static void CG_DrawNewCompass( void ) {
	...
	// jet Pilot - bugfix : still draw automap if cg_drawCompass is 0
...
//	rectDef_t headHintRect =	{ 40, 480 - 22, 20, 20 };
	...
	for ( i = 0 ; i < MAX_HD_ANIMATIONS ; i++ ) {
		animations[i] = character->head[0].animations[i];
	}
#else
	for ( i = 0 ; i < MAX_HD_ANIMATIONS ; i++ ) {
	...
	CG_DrawPlayerHead( &headRect, character, headcharacter, 180, 0, cg.snap->ps.eFlags & EF_HEADSHOT ? qfalse : qtrue, anim, painshader, &cgs.clientinfo[ cg.snap->ps.clientNum ], qfalse, cgs.clientinfo[ cg.snap->ps.clientNum].team );
...
{ 
   ...
   float frac;
   	...
   	if ( cgs.clientinfo[cg.clientNum].skill[SK_BATTLE_SENSE] >= 7 )
...
/*	if ( pm ){
	qboolean alt=qfalse;
		...
		if ( pm->ps->weapon == WP_MORTAR2 || pm->ps->weapon == WP_MORTAR || pm->ps->weapon == WP_CARBINE ||pm->ps->weapon == WP_KAR98 ) // display alt charge instead
		{
			pm->ps->weapon = weapAlts[pm->ps->weapon];
			alt = qtrue;
		}
		...
		{
			color[0] = 1;
			color[1] = color[2] = 0; // red like a soviet
			color[3] = 1; // red like a soviet
		}

		if ( alt ) // set weapon back
		{
			pm->ps->weapon = weapAlts[pm->ps->weapon];
		}
...
// jaquboss -  we can't use pm, that's sad.. 
	{
	 ...
	 qboolean charge=qtrue;
		...
		if ((cg.time - cg.snap->ps.classWeaponTime) < (chargeTime * BG_ChargeUsage(weapon, cgs.clientinfo[cg.snap->ps.clientNum].skill)) ) 
		{ // red like a soviet
			color[0] = 1;
			color[1] = color[2] = 0; 
			color[3] = 1;
...
// ==
	...
	rect.w = 12;
	rect.h = 72;
//	if (pm && pm->waterlevel == 3 )  // jaquboss - fixed crash on map starts because pm might not be set
	if ( BG_GetConditionValue( cg.clientNum, ANIM_COND_UNDERWATER, qtrue ) ) // jaquboss - looks weird but it is only place we know for waterlevel
	{
...
{	
	...
	int 			icons		= 0;
	int				cls;						// clas a covert is disguised as
	int				i, tmp		= 0;
	int				x, y, blockHeight;
	static int		first		= 1;
...
// Visibility Icon - ( Carrying Objective or Wearing Disguise )
	...
	{	// cannot have a disguise while carrying an objective
		...
		cls = ( ci->powerups >> PW_OPS_CLASS_1 ) & 7;
...
// Teamplay Auras - effect icons
#if 0
	if ( /*BG_GetAuraLevel(PC_SOLDIER, &cg.snap->ps) >= 5*/  cg.auraEffects[PC_SOLDIER] >= 5 ) 
	{
	...
	if ( /*BG_GetAuraLevel(PC_MEDIC, &cg.snap->ps) >= 5*/  cg.auraEffects[PC_MEDIC] >= 5) // jaquboss - checking for pm
	{
		CG_DrawPic( offsetX[icons], offsetY[icons], iconSize, iconSize, cgs.media.hudCaduceus );
		icons++;
	}
	...
	// TODO!!
	if ( /*BG_GetAuraLevel(PC_ENGINEER, &cg.snap->ps) >= 5*/ cg.auraEffects[PC_ENGINEER] >= 5)
	{
		CG_DrawPic( offsetX[icons], offsetY[icons], iconSize, iconSize, cgs.media.hudEngineer );
		icons++;
	}

	if ( /*BG_GetAuraLevel(PC_FIELDOPS, &cg.snap->ps) >= 5*/ cg.auraEffects[PC_FIELDOPS] >= 5)
	{
...
#if 0
	if ( /*BG_GetAuraLevel(PC_COVERTOPS, &cg.snap->ps)*/ >= 5 cg.auraEffects[PC_COVERTOPS] >= 5)
	{
		CG_DrawPic( offsetX[icons], offsetY[icons], iconSize, iconSize, cgs.media.hudCaduceus );
		icons++;
	}
...
// Equipment Icons
	...
	// Flak Jacket
	if ( /*(ci->skill[SK_ENGINEER] > 3 && ci->cls == PC_ENGINEER) ||*/ ps->powerups[PW_FLAKJACKET] )	
	{
		CG_DrawPic( offsetX[icons], offsetY[icons], iconSize, iconSize, cgs.media.hudFlakJacket[ps->persistant[PERS_TEAM]-1] );	
		icons++;		
	}
	// Adrenaline
	if (COM_BitCheck( cg.snap->ps.weapons, WP_MEDIC_ADRENALINE ) )
	{
		//if (pm && pm->cmd.serverTime - pm->ps->classWeaponTime < pm->medicChargeTime) // jaquboss - checking for pm
		if ((cg.time - cg.snap->ps.classWeaponTime) < (cg.medicChargeTime[cg.snap->ps.persistant[PERS_TEAM]-1] * BG_ChargeUsage(WP_MEDIC_ADRENALINE, cgs.clientinfo[cg.snap->ps.clientNum].skill)) ) 
		{
...
#ifdef USELOCATIONS
	// jet Pilot - in game location editor
	if ( cg.editingLocations )
	{
	//	CG_LocationEditorDraw();
		return;
	}
#endif
...
// jaquboss - tick with hud off
	...
	if( !cg.cameraMode ) {
		...
		if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR ) {
			...
			CG_DrawCrosshair();
			CG_DrawCrosshairNames();
			trap_R_SetColor( NULL ); // safety check
	...
	if ( !CG_DrawScoreboard() ) 
	{
		if( cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR ) {
			...
			{				
				...
				if (cg_drawHUDHead.integer && !hud_drawAltHUD.integer) // jaquboss temporary disabled for althud since it is covered by it
				{
