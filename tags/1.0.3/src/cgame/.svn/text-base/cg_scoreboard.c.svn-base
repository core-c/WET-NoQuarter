...
static void WM_DrawClientScore( int x, int y, score_t *score, float *color, float fade ) {
	...
	if ( ci->team != TEAM_SPECTATOR ) {
		...
		if ( score->ready == 1 && cgs.gamestate != GS_PLAYING ) {
	...
	// draw name
	CG_DrawStringExt( tempx, y, /*ci->name*/va("%s^7",ci->name), hcolor, qfalse, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, maxchars );
...
static void WM_DrawClientScore_Small( int x, int y, score_t *score, float *color, float fade ) {
	...
	if ( ci->team != TEAM_SPECTATOR ) {
		...
		if ( score->ready == 1&& ( cgs.gamestate == GS_WARMUP || cgs.gamestate == GS_INTERMISSION ) ) {
	...
	// draw name
	CG_DrawStringExt( tempx, y, va("%s^7",ci->name), hcolor, qfalse, qfalse, MINICHAR_WIDTH, MINICHAR_HEIGHT, maxchars );
