...
=================
*/
// #define DEBUG_GSPAWN // TODO: remove me cs: temp, there are reports of an infinite loop here in some cases
	...
	for ( force = 0 ; force < 2 ; force++ ) {
		...
		if ( !e ) {
			...
			while ( e ) {
				...
				loopc++;
				if (loopc > 2048) {
					G_LogPrintf( "G_Spawn: Endless loop" );
					G_Error( "G_Spawn: Endless loop" );
				}
#endif
...
#ifdef DEBUG_GSPAWN // TODO: remove me
					if ( e && i == e-g_entities ) {
						// cs: current = next (and inuse). seems to me that if this case is reached, it could result in infinite loop
						G_LogPrintf( "G_Spawn: inuse next free entity is the same as current" );

						// optional to not stop the server 
						i = level.num_entities;
						e = &g_entities[i];
						break;

						// stop the server, comment above lines 
						// G_Error( "G_Spawn: inuse next free entity is the same as current" );
					}
#endif
					if ( !e ) {
