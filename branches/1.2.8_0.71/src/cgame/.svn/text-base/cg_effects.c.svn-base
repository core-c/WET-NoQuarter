...
#define	BLOOD_SPURT_COUNT	4
	...
	if ( entityNum != cg.snap->ps.clientNum ) {
...

		{
			...
			if( VectorLength(vec) > 8 ) {
				VectorMA( bOrigin, 8.0f, dir, bOrigin );
			}
		}
...
void CG_ProjectedSpotLight( vec3_t start, vec3_t dir )
{
	...
	VectorMA( start, 1000.0f, dir, end );
...
void CG_Spotlight(centity_t *cent, float *color, vec3_t realstart, vec3_t lightDir, int segs, float range, int startWidth, float coneAngle, int flags) {
	...
	if(colorNorm[3] != 1) {		// it needs to be boosted
		s = 1.0/colorNorm[3];
		VectorMA(color, s, color, colorNorm);	// FIXME: div by 0
	}
	else {
	...
	if(cent->currentState.modelindex) {
		...
		VectorMA(start, 14.0f, lightDir, start);
	}
...
#define CORESCALE 0.6f
	...
	if(!(flags & SL_NOCORE)) {
		...
		s = startWidth*0.5f;
		VectorMA(start, s, coreright, coreverts[0].xyz);
		s = -startWidth*0.5f;
		VectorMA(start, s, coreright, coreverts[1].xyz);
		s = -coreEndRadius * CORESCALE;
		VectorMA(endCenter, s, coreright, coreverts[2].xyz);
		VectorAdd(start, coreverts[2].xyz, coreverts[2].xyz);
		s = coreEndRadius * CORESCALE;
		VectorMA(endCenter, s, coreright, coreverts[3].xyz);
	...
	if(!(flags & SL_NODLIGHT)) {
		vec3_t dlightLoc;
		VectorMA(tr.endpos, 0.0f, lightDir, dlightLoc);	// back away from the hit
