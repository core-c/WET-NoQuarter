...
int CG_CountPlayersSF( void ) {
	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		...
		if( cgs.clientinfo[clientNum].team != cgs.clientinfo[cg.clientNum].team ) {
			continue;
		}

		if( CG_IsOnFireteam(clientNum) != CG_IsOnFireteam(cg.clientNum) ) {
			continue;
		}
