	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		...
		if( weap != -1 ) {
			if ( weap == WP_M7 || weap == WP_GPG40 ){
				if ( !cgs.clientinfo[clientNum].rifleNade || ( cgs.clientinfo[clientNum].weapon != weapAlts[weap] && cgs.clientinfo[clientNum].latchedweapon != weapAlts[weap] ) )
					continue; // jaquboss added case for non rifle owners
			} else if( cgs.clientinfo[clientNum].weapon != weap && cgs.clientinfo[clientNum].latchedweapon != weap ) {
				if ( cgs.clientinfo[clientNum].weapon != weapTeamConversion[weap] && cgs.clientinfo[clientNum].latchedweapon != weapTeamConversion[weap] )
					continue;
			}
		}
...
qboolean CG_LimboPanel_ClassIsDisabled( int index ) {
	...
	if ( (!(cgs.maxIsPercentage & TEAM_MAXCLASSES_PERCENT) && wcount >= value) ||					// compare absolute teamtotal
		((cgs.maxIsPercentage & TEAM_MAXCLASSES_PERCENT) && wcount >= value * count / 100.0f) )	// compare percentage of team
	{
		return qtrue;
	}
...
// Always syncronize with G_IsWeaponDisabled g_cmds
qboolean CG_LimboPanel_RealWeaponIsDisabled( weapon_t weap ) {
	...
	// IRATA: never restricted -before we start teamcount and rest of function
	// we do a simple check for most cases which are never restricted increasing performance ...
	if ( !weaponsCanDisable[weap] ) {
		return qfalse;
	}
	...
	// if (CG_IsHeavyWeapon( weap ) && restriction check) return qtrue ...
	if( !IS_HEAVY_WEAPON( weap ) ) {
	} else if( wcount >= ceil( count * cgs.weaponRestrictions ) ) {
		return qtrue;
	}

	// IRATA team_panzerrestriction
	if( IS_PANZER_WEAPON(weap) ) {
	...
	switch(weap)
	{
		...
		case WP_SHOTGUN:
			value = cgs.maxShotguns;
			if (value == -1) return qfalse;
			break;

		case WP_PANZERFAUST:
			value = cgs.maxPanzers;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_BAZOOKA);
			break;
		case WP_BAZOOKA:
			value = cgs.maxPanzers;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_PANZERFAUST);
			break;

		case WP_MOBILE_BROWNING:
			value = cgs.maxMG42s;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_MOBILE_MG42) + CG_LimboPanel_TeamCount(WP_MOBILE_MG42_SET) + CG_LimboPanel_TeamCount(WP_MOBILE_BROWNING_SET);
			break;
		case WP_MOBILE_MG42:
			value = cgs.maxMG42s;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_MOBILE_BROWNING) + CG_LimboPanel_TeamCount(WP_MOBILE_MG42_SET) + CG_LimboPanel_TeamCount(WP_MOBILE_BROWNING_SET);
			break;

		case WP_FLAMETHROWER:
			value = cgs.maxFlamers;
			if (value == -1) return qfalse;
			break;

		case WP_MORTAR:
			value = cgs.maxMortars;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_MORTAR2);
			break;
		case WP_MORTAR2:
			value = cgs.maxMortars;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_MORTAR);
			break;

		case WP_GPG40:
			value = cgs.maxGrenLaunchers;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_M7);
			break;
		case WP_M7: // jaquboss - not the carbine!!
			value = cgs.maxGrenLaunchers;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_GPG40);
			break;

		case WP_STG44:
			value = cgs.maxLMG;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_BAR) + CG_LimboPanel_TeamCount(WP_BAR_SET);
			break;
		case WP_BAR:
			value = cgs.maxLMG;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_STG44) + CG_LimboPanel_TeamCount(WP_BAR_SET);
			break;

		case WP_GARAND:
			value = cgs.maxScopeguns;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_FG42) + CG_LimboPanel_TeamCount(WP_FG42SCOPE) + CG_LimboPanel_TeamCount(WP_K43) + CG_LimboPanel_TeamCount(WP_K43_SCOPE) + CG_LimboPanel_TeamCount(WP_GARAND_SCOPE);
			break;
		case WP_FG42:
			value = cgs.maxScopeguns;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_GARAND) + CG_LimboPanel_TeamCount(WP_GARAND_SCOPE) + CG_LimboPanel_TeamCount(WP_K43) + CG_LimboPanel_TeamCount(WP_K43_SCOPE) + CG_LimboPanel_TeamCount(WP_FG42SCOPE);
			break;
		case WP_K43:
			value = cgs.maxScopeguns;
			if (value == -1) return qfalse;
			wcount += CG_LimboPanel_TeamCount(WP_GARAND) + CG_LimboPanel_TeamCount(WP_GARAND_SCOPE) + CG_LimboPanel_TeamCount(WP_FG42) + CG_LimboPanel_TeamCount(WP_FG42SCOPE) + CG_LimboPanel_TeamCount(WP_K43_SCOPE);
			break;
	...
	if ( (!(cgs.maxIsPercentage & TEAM_MAXWEAPONS_PERCENT) && wcount >= value) ||					// compare absolute teamtotal
		((cgs.maxIsPercentage & TEAM_MAXWEAPONS_PERCENT) && wcount >= value * count / 100.0f) )	// compare percentage of team
	{
		return qtrue;
	}
