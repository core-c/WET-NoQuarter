...
//===================================
	...
	if ( cg.predictedPlayerState.eFlags & EF_POISONED ) {
		float phase = cg.time / 1000.0 * 0.3 * M_PI; // cg.time / 1000.0 * 0.5 * M_PI * 2;
...
static int CG_CalcFov( void ) {
	...
	float	x, phase = 0, v = 0, fov_x, fov_y;
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	if (cg.weaponSelect == WP_FG42SCOPE) {
		if (VectorLength(cg.snap->ps.velocity) > 180.0f) // spd
	...
	if(cg.mvCurrentMainview != NULL && cg.snap->ps.pm_type != PM_INTERMISSION && cgs.mvAllowed ) {
		...
		if ( !cg.showGameView && !cg.intermissionStarted && !cgs.dbShowing ) {
			...
			if ( cg.latestMissile && (cg_insanity.integer & (JP_INSANITY_MORTARCAM | JP_INSANITY_MISSILECAM | JP_INSANITY_RIFLENADECAM)) ) {
				if ( IS_PANZER_WEAPON(cg.latestMissile->currentState.weapon) ) {
					CG_DrawSmallStringColor( 48, 146, "MISSILE CAM", colorShade);
					CG_DrawSmallStringColor( 49, 145, "MISSILE CAM", color);
				}
				else if ( IS_RIFLENADE_WEAPON(cg.latestMissile->currentState.weapon) ) {
			...
			// core: The killer Cam..
				...
				CG_DrawSmallStringColor( 48, 146, "KILLER CAM", colorShade);
				CG_DrawSmallStringColor( 49, 145, "KILLER CAM", color);
