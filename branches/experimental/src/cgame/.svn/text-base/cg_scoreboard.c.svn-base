...
int WM_DrawObjectives( int x, int y, int width, float fade )
{
	...
	if ( cg.snap->ps.pm_type == PM_INTERMISSION ) {
		const char *s = CG_ConfigString( CS_MULTI_MAPWINNER ), *buf = Info_ValueForKey( s, "winner" ), *flagshader = NULL, *nameshader = NULL;
		int x_right, x_offset=0;
		...
		if ( flagshader ) {
			CG_DrawPic( 302, 2, 36, 24, trap_R_RegisterShaderNoMip( flagshader ) );
		}

		if ( nameshader ) {
			CG_DrawPic( x+INFO_TOTAL_WIDTH-52+x_offset, 1, 52, 26, trap_R_RegisterShaderNoMip( nameshader ) );
			CG_DrawPic( x_right-5, 1, 52, 26, trap_R_RegisterShaderNoMip( "ui/assets/portraits/text_win.tga" ) );
		}
		return 30;
	}
...
static int WM_TeamScoreboard( int x, int y, team_t team, float fade, int maxrows, int absmaxrows )
{
	...
	const char *str = Info_ValueForKey(info, "g_maxlives");
...
{
	int		x = 20, y = 20, x_right = 640 - x - (INFO_TOTAL_WIDTH - 5);
	float	fade;
	float	*fadeColor;
