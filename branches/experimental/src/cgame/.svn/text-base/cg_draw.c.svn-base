...
static void CG_DrawSpectatorMessage( void )
{
	...
	// draw a spectator flag for floating spectators..
	if ( cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR && !(cg.snap->ps.pm_flags & PMF_FOLLOW) ) {
		...
		y = ( cg_smallFont.integer & SMALLFONT_LIMBO )? y+2+height : y+4+height;
		trap_R_SetColor( clr );
...
static qboolean CG_DrawFollow(void)
{
	...
	if(cg.snap->ps.pm_flags & PMF_LIMBO) {
		...
		// draw a spectator flag for following spectators..
		{
			static vec4_t clr = { 1.f, 1.f, 1.f, 0.6f };
			wr = x + 9*width - 9;
			hr = (12.0f / 18.0f) * wr;
...

	{
		...
		for (i = 0; i < 3; ++i) {
			...
			if (ci->skill[skill] < NUM_SKILL_LEVELS - 1 && !cg.xpOrder ) {
				int need = skillLevels[skill][ci->skill[skill]+1] - skillLevels[skill][ci->skill[skill]];//nextLvl[ci->skill[skill]] - curLvl[ci->skill[skill]];
				int have = skillpoints - skillLevels[skill][ci->skill[skill]];//curLvl[ci->skill[skill]];
				float prog = (float)have / need; // progress toward next level
