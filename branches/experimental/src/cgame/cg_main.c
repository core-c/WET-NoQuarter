...
#pragma export off
	...
	case CG_MESSAGERECEIVED:
		if ( *(unsigned char*)arg0 == PACKET_TEAMPLAYINFOMESSAGE ) {
...
vmCvar_t	cg_speedinterval; 	// forty - speedometer
vmCvar_t	cg_speedunit; 		// forty - speedometer
vmCvar_t	cg_optimizePrediction;
vmCvar_t	cg_locations;
vmCvar_t	cg_fixedFTeamSize;
...
vmCvar_t	cg_favWeaponBank; // lastUsedWeaponBank - get next weapon after using cretain weapon (like panzer,airstrike,satchel etc)
vmCvar_t        cg_spawnTimer_set;      // spawntimer from etpub
vmCvar_t        cg_spawnTimer_period;   // spawntimer from etpub


#ifdef USELOCATIONS
char locationName[256][64];		// jet Pilot - Storage for location names
#endif
...
cvarTable_t		cvarTable[] =
{
	...
	{ &cg_gun_foreshorten, 		"cg_gun_foreshorten", 			"0", 		CVAR_CHEAT 		},
	...
	{ &cg_killAnnouncer,		"cg_killAnnouncer",				"1",		CVAR_ARCHIVE	},
	...
	{ &cg_drawAccuracy,			"cg_drawAccuracy",				"0",		CVAR_ARCHIVE	},
	...
	{ &cg_drawHUDHead,			"cg_drawHUDHead",				"0",		CVAR_ARCHIVE	},
	...
	{ &cg_favWeaponBank,		"cg_favWeaponBank",				"0",		CVAR_ARCHIVE	},	// 0=previously used weapon.  banks: 1=knife, 2=pistol, 3=SMG, 4=grenade
...
static void CG_RegisterItemSounds( int itemNum )
{
	...
	if( developer.integer ) {
		CG_Printf("...register item sound %s\n", item->classname );
	}
...
static void CG_RegisterGraphics( void )
{
	...
	// jaquboss load for rage pro only, but i doubt that sush ancient hardware is still used
	...
	cgs.cachedModels[GAMEMODEL_WORLD_SEARCHLIGHT]		= trap_R_RegisterModel("models/mapobjects/light/searchlight_pivot.md3");
...
=================
*/
static void CG_RegisterStrings( void )
{
	cgs.cachedStrings[GAMESTRING_ERROR]								= "";
	// print
	cgs.cachedStrings[GAMESTRING_TIMELIMIT_HIT]						= "Timelimit hit.\n";
	cgs.cachedStrings[GAMESTRING_AXIS_FIRST_BLOOD]					= "Axis team wins by drawing First Blood.\n";
	cgs.cachedStrings[GAMESTRING_ALLIES_FIRST_BLOOD]				= "Allied team wins by drawing First Blood.\n";
	cgs.cachedStrings[GAMESTRING_AXIS_SURVIVORS]					= "Axis team has the most survivors.\n";
	cgs.cachedStrings[GAMESTRING_ALLIES_SURVIVORS]					= "Allied team has the most survivors.\n";
	cgs.cachedStrings[GAMESTRING_HEAD_SHOT]							= "Head Shot\n";
	cgs.cachedStrings[GAMESTRING_LEG_SHOT]							= "Leg Shot\n";
	cgs.cachedStrings[GAMESTRING_ARM_SHOT]							= "Arm Shot\n";
	cgs.cachedStrings[GAMESTRING_BODY_SHOT]							= "Body Shot\n";
	cgs.cachedStrings[GAMESTRING_NO_CHEATS_ENABLED]					= "Cheats are not enabled on this server.\n";
	cgs.cachedStrings[GAMESTRING_ALIVE_FOR_USE]						= "You must be alive to use this command.\n";
	cgs.cachedStrings[GAMESTRING_GIVEDISGUISE_HELP_1]				= "^9usage : ^7givedisguise ^2<class>\n";
	cgs.cachedStrings[GAMESTRING_GIVEDISGUISE_HELP_2]				= "^9        available choices are ^2s, m, e, f, ^9and ^2c^9, or the numbers ^20 ^9through ^24\n";
	cgs.cachedStrings[GAMESTRING_GIVEDISGUISE_HELP_3]				= "^2        -1 ^9removes any current disguise.\n";
	cgs.cachedStrings[GAMESTRING_GIVEDISGUISE_HELP_4]				= "^9givedisguise : ^7Removing disguise\n";
	cgs.cachedStrings[GAMESTRING_NO_VOTE_IN_PROGRESS]				= "No vote in progress.\n";
	cgs.cachedStrings[GAMESTRING_VOTE_ALREADY_CAST]					= "Vote already cast.\n";
	cgs.cachedStrings[GAMESTRING_NO_VOTE_SPEC]						= "Not allowed to vote as spectator.\n";
	cgs.cachedStrings[GAMESTRING_NO_VOTE_MUTED]						= "Not allowed to vote when muted.\n";
	cgs.cachedStrings[GAMESTRING_NO_VOTEKICK_ENEMY]					= "Cannot vote to kick player on opposing team.\n";
	cgs.cachedStrings[GAMESTRING_VOTE_CAST]							= "Vote cast.\n";
	cgs.cachedStrings[GAMESTRING_SETVIEWPOS_USAGE]					= "usage: setviewpos x y z yaw\n";
	cgs.cachedStrings[GAMESTRING_BOT_CANNOT_SWAP_LIMBO]				= "Bot is in limbo mode, cannot swap places.\n";
	cgs.cachedStrings[GAMESTRING_BOT_CANNOT_SWAP_TEAM]				= "Bot is on different team, cannot swap places.\n";
	cgs.cachedStrings[GAMESTRING_NO_CHAT_AS_SPEC]					= "Can't team chat as spectator\n";
	cgs.cachedStrings[GAMESTRING_AUTO_UNMUTE_LANGUAGE]				= "^5You've been auto-unmuted. Language penalty lifted.\n";
	cgs.cachedStrings[GAMESTRING_AUTO_UNMUTE_SHRUB]					= "^5You've been auto-unmuted. Shrub penalty lifted.\n";
	cgs.cachedStrings[GAMESTRING_AUTO_UNMUTE_LUA]					= "^5You've been auto-unmuted. Lua penalty lifted.\n";
	cgs.cachedStrings[GAMESTRING_CANT_FOLLOW_NOT_LIMBO]				= "Can't follow while not in limbo if on a team!\n";
	cgs.cachedStrings[GAMESTRING_PART_NAME_MORE_MATCHES]			= "Partial Name Matches more than 1 Player.\n";
	cgs.cachedStrings[GAMESTRING_CANT_FOLLOW_ENEMIES]				= "Can't follow a player on an enemy team!\n";
	cgs.cachedStrings[GAMESTRING_IGNORE_USAGE]						= "usage: ignore [name|slot#]\n";
	cgs.cachedStrings[GAMESTRING_CANT_IGNORE_SELF]					= "ignore: you can't ignore yourself\n";
	cgs.cachedStrings[GAMESTRING_UNIGNORE_USAGE]					= "usage: unignore [name|slot#]\n";
	cgs.cachedStrings[GAMESTRING_CURRENTLY_IGNORING]				= "currently ignoring:\n";
	cgs.cachedStrings[GAMESTRING_UNIGNORE_NO_MATCH]					= "unignore: no match found.  Run /unignore with no parameter for a list\n";
	cgs.cachedStrings[GAMESTRING_OSP_COMMANDS]						= "^5\nAvailable OSP Game-Commands:\n----------------------------\n";
	cgs.cachedStrings[GAMESTRING_OSP_INFO]							= "\nType: ^3\\command_name ?^7 for more information\n";
	cgs.cachedStrings[GAMESTRING_MATCH_UNPAUSED]					= "\n^3Match is ^5UNPAUSED^3 ... resuming in 10 seconds!\n";
	cgs.cachedStrings[GAMESTRING_PLAYERS_HEADER1]					= "\n^3 ID^1 : ^3Player                    Nudge  Rate  MaxPkts  Snaps\n";
	cgs.cachedStrings[GAMESTRING_PLAYERS_HEADER2]					= "^1-----------------------------------------------------------^7\n";
	cgs.cachedStrings[GAMESTRING_PLAYERS_HEADER3]					= "\n^3Status^1   : ^3ID^1 : ^3Player                    Nudge  Rate  MaxPkts  Snaps\n";
	cgs.cachedStrings[GAMESTRING_PLAYERS_HEADER4]					= "^1---------------------------------------------------------------------^7\n";
	cgs.cachedStrings[GAMESTRING_MUTED_BY_LUA]						= "^9You've been muted by Lua.\n";
	cgs.cachedStrings[GAMESTRING_INVALID_GAMETYPE]					= "Invalid gametype was specified, Restarting\n";
	cgs.cachedStrings[GAMESTRING_MATCH_RESUMING]					= "^3Match resuming in 10 seconds!\n";
	cgs.cachedStrings[GAMESTRING_REFEREE_COMMANDS_HELP1]			= "\n^3Referee commands:^7\n";
	cgs.cachedStrings[GAMESTRING_REFEREE_COMMANDS_HELP2]			= "------------------------------------------\n";
	cgs.cachedStrings[GAMESTRING_REFEREE_COMMANDS_HELP3]			= "\n^5allready         putallies^7 <pid>  ^5speclock          warmup\n";
	cgs.cachedStrings[GAMESTRING_REFEREE_COMMANDS_HELP4]			= "^5lock             putaxis^7 <pid>    ^5specunlock        warn ^7<pid>\n";
	cgs.cachedStrings[GAMESTRING_REFEREE_COMMANDS_HELP5]			= "^5help             remove^7 <pid>     ^5unlock            mute ^7<pid>\n";
	cgs.cachedStrings[GAMESTRING_REFEREE_COMMANDS_HELP6]			= "^5pause            unpause           unmute ^7<pid>\n";
	cgs.cachedStrings[GAMESTRING_REFEREE_COMMANDS_HELP7]			= "Usage: ^3\\ref <cmd> [params]\n\n";
	cgs.cachedStrings[GAMESTRING_REF_STATUS_DISABLED]				= "Sorry, referee status disabled on this server.\n";
	cgs.cachedStrings[GAMESTRING_REF_PASSWORD_USAGE]				= "Usage: ref [password]\n";
	cgs.cachedStrings[GAMESTRING_REF_PASSWORD_INVALID]				= "Invalid referee password!\n";
	cgs.cachedStrings[GAMESTRING_REF_YOUVE_BEEN_MUTED]				= "^5You've been muted\n";
	cgs.cachedStrings[GAMESTRING_REF_YOUVE_BEEN_UNMUTED]			= "^5You've been unmuted\n";
	cgs.cachedStrings[GAMESTRING_VOTE_HELP1]						= "\nValid ^3callvote^7 commands are:\n^3----------------------------\n";
	cgs.cachedStrings[GAMESTRING_VOTE_HELP2]						= "\nUsage: ^3\\callvote <command> <params>\n^7For current settings/help, use: ^3\\callvote <command> ?\n\n";
	cgs.cachedStrings[GAMESTRING_PLAYER_LEFT_BEFORE_REF]			= "Player left before becoming referee\n";
	cgs.cachedStrings[GAMESTRING_MAPVOTING_DISABLED]				= "^3Map voting not enabled!\n";
	cgs.cachedStrings[GAMESTRING_MAPVOTING_INTERMISSION]			= "^3You can only vote during intermission\n";
	cgs.cachedStrings[GAMESTRING_MAPVOTING_NOT_TWICE]				= "^3Can't vote for the same map twice\n";
	cgs.cachedStrings[GAMESTRING_XP_EXPIRED]						= "^3server: your saved xp has expired\n";
	cgs.cachedStrings[GAMESTRING_INACTIVITY_SECONDS_TO_SPEC]		= "^3%s seconds until moving to spectator for inactivity!\n";				// 1 param
	cgs.cachedStrings[GAMESTRING_INACTIVITY_SECONDS_DROP]			= "^3%s seconds until inactivity drop!\n";									// 1 param
	cgs.cachedStrings[GAMESTRING_ENTERED_THE_GAME]					= "[lof]%s^7 [lon]entered the game\n";										// 1 param
	cgs.cachedStrings[GAMESTRING_MUTED_SECONDS_LANGUAGE]			= "^9You've been auto-muted for %s seconds for foul language.\n";			// 1 param
	cgs.cachedStrings[GAMESTRING_MUTED_MORESECONDS_LANGUAGE]		= "^9You've been auto-muted for another %s seconds for foul language.\n";	// 1 param
	cgs.cachedStrings[GAMESTRING_TEAMDAMAGESTATS]					= "Team Hits: %s Total Hits: %s Pct: %s Limit: %s\n";						// 4 params
	cgs.cachedStrings[GAMESTRING_BAD_CLIENT_SLOT]					= "Bad client slot: [lof]%s\n";												// 1 param
	cgs.cachedStrings[GAMESTRING_CLIENT_NOT_ACTIVE]					= "Client[lof] %s [lon]is not active\n";									// 1 param
	cgs.cachedStrings[GAMESTRING_USER_NOT_ON_SERVER]				= "User [lof]%s [lon]is not on the server\n";								// 1 param
	cgs.cachedStrings[GAMESTRING_TEAM_EMPTY_FOLLOW_IGNORED]			= "The %s team %s empty!  Follow command ignored.\n";						// 2 params
	cgs.cachedStrings[GAMESTRING_TEAM_LOCKED_FOR_SPECS]				= "Sorry, the %s team is locked from spectators.\n";						// 1 param
	cgs.cachedStrings[GAMESTRING_SPEC_FOLLOW_LOCKED_ON_TEAM]		= "Spectator follow is now locked on the %s team.\n";						// 1 param
	cgs.cachedStrings[GAMESTRING_TEAM_SPECTATING_DISABLED]			= "%s team spectating is now disabled.\n";									// 1 param
	cgs.cachedStrings[GAMESTRING_UNKNOWN_VOTE_COMMAND]				= "\n^3>>> Unknown vote command: ^7%s %s\n";								// 2 params
	cgs.cachedStrings[GAMESTRING_CALLED_A_VOTE_VOTED_FOR]			= "[lof]%s^7 [lon]called a vote.[lof]  Voting for: %s\n";					// 2 params
	cgs.cachedStrings[GAMESTRING_ALREADY_IGNORING]					= "ignore: you are already ignoring %s\n";									// 1 param
	cgs.cachedStrings[GAMESTRING_ADDED_TO_IGNORE_LIST]				= "ignore: %s^7 added to your ignore list\n";								// 1 param
	cgs.cachedStrings[GAMESTRING_REMOVED_FROM_IGNORE_LIST]			= "unignore: %s^7 removed from your ignore list\n";							// 1 param
	cgs.cachedStrings[GAMESTRING_CMD_NOT_ALLOWED_INTERMISSION]		= "^3%s^7 not allowed during intermission.\n";								// 1 param
	cgs.cachedStrings[GAMESTRING_UNKNOWN_CMD]						= "unknown cmd[lof] %s\n";													// 1 param
	cgs.cachedStrings[GAMESTRING_WAIT_SECONDS_TO_ISSUE_CMD]			= "Wait another %ss to issue ^3%s\n";										// 2 params
	cgs.cachedStrings[GAMESTRING_TEAM_ALREADY_LOCKEDSTATE]			= "^3Your team is already %sed!\n";											// 1 param
	cgs.cachedStrings[GAMESTRING_TEAM_NOW_LOCKEDSTATE]				= "The %s team is now %sed!\n";												// 2 params
	cgs.cachedStrings[GAMESTRING_SPEC_CANT_LOCKSTATE_TEAM]			= "Spectators can't %s a team!\n";											// 1 param
	cgs.cachedStrings[GAMESTRING_MATCH_ALREADY_PAUSESTATE]			= "The match is already %sPAUSED^7!\n";										// 1 param
	cgs.cachedStrings[GAMESTRING_MATCH_PAUSED_TIMEOUTS_LEFT]		= "^3Match is ^1PAUSED^3!\n^7[%s^7: - %s Timeouts Remaining]\n";			// 1 param
	cgs.cachedStrings[GAMESTRING_TOTAL_PLAYERS]						= "\n^3%s^7 total players\n\n";												// 1 param
	cgs.cachedStrings[GAMESTRING_TEAM_IS_SPECLOCKED]				= "** %s team is speclocked.\n";											// 1 param
	cgs.cachedStrings[GAMESTRING_ALREADY_READYSTATE]				= "You are already%s ready!\n";												// 1 param
	cgs.cachedStrings[GAMESTRING_PLAYER_IS_READY]					= "%s^7 is%s ready!\n";														// 2 params
	cgs.cachedStrings[GAMESTRING_BEEN_SENT_SPEC_INVITATION]			= "%s^7 has been sent a spectator invitation.\n";							// 1 param
	cgs.cachedStrings[GAMESTRING_TEAM_ALREADY_SPECLOCKSTATE]		= "\n^3Your team is already %sed from spectators!\n\n";						// 1 param
	cgs.cachedStrings[GAMESTRING_SPEC_CANT_SPECLOCKASTATE]			= "Spectators can't %s a team from spectators!\n";							// 1 param
	cgs.cachedStrings[GAMESTRING_MUTED_SECONDS_LUA]					= "^9You've been muted for %s seconds by Lua.\n";							// 1 param
	cgs.cachedStrings[GAMESTRING_SERVER_CVAR_CHANGED]				= "Server:[lof] %s [lon]changed to[lof] %s\n";								// 2 params
	cgs.cachedStrings[GAMESTRING_SW_CLOCK_SET_TO]					= ">>> ^3Clock set to: %s:%s\n\n\n";										// 2 params
	cgs.cachedStrings[GAMESTRING_SW_OBJ_REACHED_AT]					= ">>> ^3Objective reached at %s:%s (original: %s:%s)\n\n\n";				// 4 params
	cgs.cachedStrings[GAMESTRING_SW_OBJ_NOT_REACHED_IN_TIME]		= ">>> ^3Objective NOT reached in time (%s:%s)\n\n\n";						// 2 params
	cgs.cachedStrings[GAMESTRING_REF_PAUSED_MATCH]					= "^3%s ^1PAUSED^3 the match^3!\n";											// 1 param
	cgs.cachedStrings[GAMESTRING_REF_UNPAUSES_MATCH]				= "\n^3%s ^5UNPAUSES^3 the match ... resuming in 10 seconds!\n\n";			// 1 param
	cgs.cachedStrings[GAMESTRING_REF_REMOVED_FROM_TEAM]				= "^5You've been removed from the %s team\n";								// 1 param
	cgs.cachedStrings[GAMESTRING_VOTESTRING_SET_TO]					= "^3%s set to: ^5%s\n";													// 2 params
	cgs.cachedStrings[GAMESTRING_XPSAVE_STATE_RETRIEVED]			= "^3server: retrieved xp save state from %s ago\n";						// 1 param
	// CP
	cgs.cachedStrings[GAMESTRING_WEAPON_RESTRICTION]				= "^1Weapon Restriction^7: Dropping restricted weapons";
	cgs.cachedStrings[GAMESTRING_MINES_IN_AREA]						= "Mines have been reported in this area.";
	cgs.cachedStrings[GAMESTRING_DYNAMITE_NOT_ARMED]				= "Dynamite is set, but NOT armed!";
	cgs.cachedStrings[GAMESTRING_PLAYER_FROZEN]						= "^1You are frozen!";
	cgs.cachedStrings[GAMESTRING_NO_SWITCH_DURING_MATCH]			= "1^You cannot switch during a match, please wait until the round ends.";
	cgs.cachedStrings[GAMESTRING_SPAWN_NEXT_ROUND]					= "Will spawn next round, please wait.";
	cgs.cachedStrings[GAMESTRING_SP_VOICE_IGNORED]					= "^1Spam Protection^7: VoiceChat ignored";
	cgs.cachedStrings[GAMESTRING_WARNING_CONSTRUCTION_AREA]			= "Warning, leave the construction area...";
	cgs.cachedStrings[GAMESTRING_REPAIRED_MG]						= "You have repaired the MG!";
	cgs.cachedStrings[GAMESTRING_LANDMINE_INVALID]					= "Landmine cannot be armed here...";
	cgs.cachedStrings[GAMESTRING_LANDMINE_LIMIT]					= "Your team has too many landmines placed...";
	cgs.cachedStrings[GAMESTRING_LANDMINE_ARMED]					= "Landmine armed...";
	cgs.cachedStrings[GAMESTRING_LANDMINE_DEFUSED]					= "Landmine defused...";
	cgs.cachedStrings[GAMESTRING_DYNAMITE_INVALID]					= "You cannot arm dynamite near a friendly objective!";
	cgs.cachedStrings[GAMESTRING_DYNAMITE_30_SECS]					= "Dynamite is now armed with a 30 second timer!";
	cgs.cachedStrings[GAMESTRING_LANDMINE_REVEALED]					= "Landmine Revealed";
	cgs.cachedStrings[GAMESTRING_MOVED_TO_SPEC_INACTIVITY]			= "^3moved to spectator for inactivity!";
	cgs.cachedStrings[GAMESTRING_DROPPED_FOR_INACTIVITY]			= "^3dropped for inactivity!";
	cgs.cachedStrings[GAMESTRING_AXIS_TOO_MANY_PLAYERS]				= "^1The Axis has too many players.";
	cgs.cachedStrings[GAMESTRING_ALLIES_TOO_MANY_PLAYERS]			= "^1The Allies have too many players.";
	cgs.cachedStrings[GAMESTRING_CANT_SWITCH_NO_MORE_LIVES]			= "^1You can't switch teams because you are out of lives.";
	cgs.cachedStrings[GAMESTRING_HEAVYWEAP_NOT_AVAILABLE]			= "^1* ^3Heavy weapon not available ^1*^7";
	cgs.cachedStrings[GAMESTRING_CANNOT_VOTE_WHILE_MUTED]			= "You cannot call a vote while muted.";
	cgs.cachedStrings[GAMESTRING_VOTE_IN_PROGRESS]					= "A vote is already in progress.";
	cgs.cachedStrings[GAMESTRING_NO_VOTE_INTERMISSION]				= "Cannot callvote during intermission.";
	cgs.cachedStrings[GAMESTRING_VOTING_NOT_ENABLED]				= "Voting not enabled on this server.";
	cgs.cachedStrings[GAMESTRING_NO_CALLVOTE_SPEC]					= "Not allowed to call a vote as a spectator.";
	cgs.cachedStrings[GAMESTRING_NO_CALLVOTE_ADMINLEVEL]			= "Your adminlevel isn't allowed to call votes.";
	cgs.cachedStrings[GAMESTRING_REFEREE_SETTING_CHANGE]			= "^1** Referee Server Setting Change **";
	cgs.cachedStrings[GAMESTRING_NO_VOTE_ENEMY_SURRENDER]			= "You cannot vote on the other team's surrender";
	cgs.cachedStrings[GAMESTRING_SWITCH_KNIFE_DROP_OBJ]				= "You must switch to knife to drop objective";
	cgs.cachedStrings[GAMESTRING_LOAD_NEXT_CAMPAIGN_MAP]			= "^3*** Loading next map in campaign ***";
	cgs.cachedStrings[GAMESTRING_LOAD_NEXT_CAMPAIGN]				= "^3*** Loading Next Campaign ***";
	cgs.cachedStrings[GAMESTRING_LOAD_NEXT_MAP]						= "^3*** Loading Next Map ***";
	cgs.cachedStrings[GAMESTRING_REACHED_MAX_XP]					= "The good news is you've reached g_maxXP. The bad news is now you lose it all.";
	cgs.cachedStrings[GAMESTRING_TEAMS_SWAPPED]						= "^1Teams have been swapped!";
	cgs.cachedStrings[GAMESTRING_TEAMS_SHUFFLED]					= "^1Teams have been shuffled!";
	cgs.cachedStrings[GAMESTRING_COUNTDOWN_STOPPED]					= "^1COUNTDOWN STOPPED!^7  Back to warmup...";
	cgs.cachedStrings[GAMESTRING_LEVEL_RESTARTED]					= "^1*** Level Restarted! ***";
	cgs.cachedStrings[GAMESTRING_MATCH_RESET]						= "^1*** Match Reset! ***";
	// CPM
	cgs.cachedStrings[GAMESTRING_FTC]								= "You are now the leader of your fireteam";
	cgs.cachedStrings[GAMESTRING_FT_DISBANDED]						= "The Fireteam you are on has been disbanded";
	cgs.cachedStrings[GAMESTRING_FTC_WARNING]						= "You have been warned by your Fireteam Commander";
	cgs.cachedStrings[GAMESTRING_MUTED_PLAYER]						= "^3You have been muted";
	cgs.cachedStrings[GAMESTRING_UNMUTED_PLAYER]					= "^3You have been un-muted";
	cgs.cachedStrings[GAMESTRING_NO_COMPLAINT_FILED]				= "No complaint filed against you.";
	cgs.cachedStrings[GAMESTRING_NO_TEAMCOMMANDS_ENABLED]			= "Team commands not enabled on this server.";
	cgs.cachedStrings[GAMESTRING_NO_TIMEOUTS_REMAINING]				= "^3Your team has no more timeouts remaining!";
	cgs.cachedStrings[GAMESTRING_DIDNT_CALL_TIMEOUT]				= "^3Your team didn't call the timeout!";
	cgs.cachedStrings[GAMESTRING_MATCH_IN_PROGRESS]					= "Match is already in progress!";
	cgs.cachedStrings[GAMESTRING_COUNTDOWN_STARTED]					= "Countdown started.... ^3notready^7 ignored!";
	cgs.cachedStrings[GAMESTRING_MUST_BE_INGAME]					= "You must be in the game to be ^3ready^7!";
	cgs.cachedStrings[GAMESTRING_NOT_ENOUGH_PLAYERS]				= "Not enough players to start match!";
	cgs.cachedStrings[GAMESTRING_NOT_LOCKED_FOR_SPECS]				= "Your team isn't locked from spectators!";
	cgs.cachedStrings[GAMESTRING_CANT_SPECINVITE_SELF]				= "You can't specinvite yourself!";
	cgs.cachedStrings[GAMESTRING_CANT_SPECINVITE_NONSPEC]			= "You can't specinvite a non-spectator!";
	cgs.cachedStrings[GAMESTRING_CANT_SPECINVITE_PLAYERS]			= "Spectators can't specinvite players!";
	cgs.cachedStrings[GAMESTRING_USE_SPECINVITE]					= "Use ^3specinvite^7 to invite people to spectate.";
	cgs.cachedStrings[GAMESTRING_SPECS_CANT_READY_TEAM]				= "Spectators can't ready a team!";
	cgs.cachedStrings[GAMESTRING_VOTE_PASSED]						= "^5Vote passed!";
	cgs.cachedStrings[GAMESTRING_SEE_STATS]							= "Type ^3\\stats <player_id>^7 to see stats on an active player.";
	cgs.cachedStrings[GAMESTRING_COMPETITION_SETTINGS]				= "Competition Settings Loaded!";
	cgs.cachedStrings[GAMESTRING_PUBLIC_SETTINGS]					= "Public Settings Loaded!";
	// chat 0 params
	cgs.cachedStrings[GAMESTRING_PLAYER_NOT_FOUND]					= "Player not found";
	cgs.cachedStrings[GAMESTRING_SHRUB_TEAMS_SWAPPED]				= "^dswap: ^9Teams were swapped^7";
	cgs.cachedStrings[GAMESTRING_NEXTMAP_WAS_LOADED]				= "^dnextmap: ^9Next map was loaded";
	cgs.cachedStrings[GAMESTRING_NEXTMAP_VOTE_PASSED]				= "^3*** Nextmap vote passed! Choose a new map! ***";
	// chat 1 param
	cgs.cachedStrings[GAMESTRING_1_AUTO_UNMUTE_LANGUAGE]			= "%s^7 has been auto-unmuted. Language penalty lifted.";
	cgs.cachedStrings[GAMESTRING_1_AUTO_UNMUTE_SHRUB]				= "%s^7 has been auto-unmuted. Shrub penalty lifted.";
	cgs.cachedStrings[GAMESTRING_1_AUTO_UNMUTE_LUA]					= "%s^7 has been auto-unmuted. Lua penalty lifted..";
	cgs.cachedStrings[GAMESTRING_1_POISONED_HIS_DIRTY_MOUTH]		= "^7%s ^9poisoned his dirty mouth.^7";
	cgs.cachedStrings[GAMESTRING_1_BURNED_HIS_TONGUE]				= "^7%s ^9burned his tongue.^7";
	cgs.cachedStrings[GAMESTRING_1_CENSOR_PENALTY_XP]				= "^1Warning^9: Watch your mouth!^* You have lost %s XP.";
	cgs.cachedStrings[GAMESTRING_1_IS_IGNORING_YOU]					= "%s^1 is now ignoring you.";
	cgs.cachedStrings[GAMESTRING_1_STOPPED_IGNORING_YOU]			= "%s^1 has stopped ignoring you.";
	cgs.cachedStrings[GAMESTRING_1_BOUNTY_ENDED_SUICIDE]			= "^9The bounty on ^*%s ^9is gone... he ended it himself!";
	cgs.cachedStrings[GAMESTRING_1_PICKED_UP_X_AMMO]				= "Picked up %s ammo.";
	cgs.cachedStrings[GAMESTRING_1_MUTED_BY_LUA]					= "%s^9 has been muted by Lua.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_UNMUTED]					= "^dunmute: ^*%s ^9has been unmuted.";
	cgs.cachedStrings[GAMESTRING_1_HELP_COMMANDS]					= "^dhelp: ^9%s available commands (open console for the full list).";
	cgs.cachedStrings[GAMESTRING_1_SPEC999_MOVED_TO_SPECS]			= "^dspec999: ^*%s^9 moved to spectators^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_GIBBED]			= "^dgib: ^9%d players gibbed^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_GIBBED]				= "^dgib: ^7%s ^9was gibbed^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_SLAPPED]				= "^dslap: ^*%s ^9was slapped^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_REVIVED]				= "^drevive: ^*%s ^9was revived^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_GIVEN_MEDPACK]			= "^dmedpack: ^7%s ^9was given a medpack^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_GIVEN_AMMOPACK]		= "^dammopack: ^7%s ^9was given an ammopack^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_DISGUISE_ADMIN_CHEATS]		= "^ddisguise: ^7%s ^9This admin cheats!^7";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_POISONED]		= "^dpoison: ^9%s players poisoned^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_POISONED]				= "^dpoison: ^7%s ^9was poisoned^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_DROPPED_WEAP]	= "^ddw: ^9%s players drop their weapons^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_PLAYER_DROPS_WEAPON]		= "^ddw: ^7%s ^9was forced to drop weapons^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_GLOWING]			= "^dglow: ^9%s players are now glowing^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_IS_NOW_GLOWING]			= "^dglow: ^7%s ^9is now glowing^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_STOPPED_GLOWING]			= "^dglow: ^7%s ^9stopped glowing^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_PANTSED]			= "^dpants: ^9%s players pantsed^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_PANTSED]				= "^dpants: ^7%s ^9was pantsed^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_FROZEN]			= "^dfreeze: ^9%s players are frozen^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_IS_FROZEN]					= "^dfreeze: ^7%s ^9is frozen^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_UNFROZEN]		= "^dunfreeze: ^9%s players are unfrozen^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_IS_UNFROZEN]				= "^dunfreeze: ^7%s ^9is unfrozen^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_BURNED]			= "^dburn: ^9%s players burned^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_IS_BURNED]					= "^dburn: ^7%s ^9was set ablaze^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_WARNED]				= "^dwarn: ^7%s ^9was warned^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_NADED]			= "^dnade: ^9%s players naded^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_IS_NADED]					= "^dnade: ^7%s";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_POPPED]			= "^dpop: ^9%s players popped.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_POPPED]				= "^dpop: ^7%s ^9lost his hat^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_X_PLAYERS_PIPPED]			= "^dpip: ^9%s players pipped^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_WAS_PIPPED]				= "^dpip: ^7%s ^9was pipped^7.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_IS_DISORIENTED]			= "^ddisorient: ^7%s ^9is disoriented.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_NOT_DISORIENTED]			= "^dorient: ^7%s ^9is no longer disoriented.";
	cgs.cachedStrings[GAMESTRING_1_SHRUB_XP_RESET_FOR_PLAYER]		= "^dresetxp: ^9XP has been reset for player %s";
	cgs.cachedStrings[GAMESTRING_1_TEAM_HAS_SURRENDERED]			= "%s have surrendered!";
	// chat 2 params
	cgs.cachedStrings[GAMESTRING_2_MUTED_SECONDS_LANGUAGE]			= "%s^9 has been auto-muted for %s seconds due to foul language.";
	cgs.cachedStrings[GAMESTRING_2_MUTED_MORESECONDS_LANGUAGE]		= "%s^9 has been auto-muted for another %s seconds due to foul language.";
	cgs.cachedStrings[GAMESTRING_2_KILLINGSPREE_ENDED_SUICIDE]		= "%s^d's killing spree ended after ^3%s^d kills! ^d(^1SUICIDE!^d).";
	cgs.cachedStrings[GAMESTRING_2_DEATHSPREE_ENDED]				= "%s^d's death spree has ended after ^3%s ^ddeaths!";
	cgs.cachedStrings[GAMESTRING_2_ATTACKER_HAD_HP_LEFT]			= "%s ^7had %s HP left.";
	cgs.cachedStrings[GAMESTRING_2_MUTED_SECONDS_LUA]				= "%s^9 has been muted for %s seconds by Lua.";
	cgs.cachedStrings[GAMESTRING_2_IS_MUTED]						= "^dmute: ^*%s ^9has been muted %s.";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_NOW_LEVEL_X_USER]			= "^dsetlevel: ^9%s^9 is now a level %s user^7.";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_RENAMED_TO]				= "^drename: ^*%s ^9has been renamed to ^7%s";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_WAS_DISGUISED_BY]			= "^ddisguise: ^7%s ^9was disguised by %s^7";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_GIVE_ADMIN_CHEATS]			= "^dgive: ^7%s ^7%s^9 This admin cheats!^7";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_ALL_TEAMS_LOCKSTATE]		= "^d%s: ^9All teams %sed^7.";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_AXIS_TEAM_LOCKSTATE]		= "^d%s: ^9Axis team %sed^7.";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_ALLIES_TEAM_LOCKSTATE]		= "^d%s: ^9Allied team %sed^7.";
	cgs.cachedStrings[GAMESTRING_2_SHRUB_SPECS_LOCKSTATE]			= "^d%s: ^9Spectators %sed^7.";
	// chat 3 params
	cgs.cachedStrings[GAMESTRING_3_DEATHSPREE]						= "^1DEATH SPREE! ^*%s ^*%s ^d(^3%s ^ddeaths without a kill!).";
	cgs.cachedStrings[GAMESTRING_3_ATTACKER_HP_LEFT_DISTANCE]		= "%s ^7had %s HP left. Distance was %s.";
	cgs.cachedStrings[GAMESTRING_3_SHRUB_GIVE_RECEIVED_CHEAT]		= "^dgive: ^7%s ^9received a cheat (^7%s^9) from %s^7";
	cgs.cachedStrings[GAMESTRING_3_SHRUB_X_PLAYERS_FLUNG]			= "^d%s: ^9%s players %s^7.";
	cgs.cachedStrings[GAMESTRING_3_SHRUB_WAS_FLUNG]					= "^d%s: ^7%s ^9was %s^7.";
	// chat 4 params
	cgs.cachedStrings[GAMESTRING_4_PM_X_RECIPIENTS]					= "%s^7 -> %s^7: (%s recipients^7): ^3%s^7";
	cgs.cachedStrings[GAMESTRING_4_KILLINGSPREE_ENDED_BY]			= "%s^d's killing spree ended by ^*%s%s ^dafter ^3%s ^dkills!";
	// chat 5 params
	cgs.cachedStrings[GAMESTRING_5_ADMINTEST_LEVEL_X_USER]			= "^dadmintest: ^7%s^9 is a level %s user %s%s%s";
	// scores
	cgs.cachedStrings[GAMESTRING_SC_HEADER1]						= "^7TEAM   Player          Kll Dth Sui TK Eff  ^3GP^7    ^2DG    ^1DR   ^6TD  ^3Score\n^7---------------------------------------------------------------------\n";
	cgs.cachedStrings[GAMESTRING_SC_HEADER2]						= "^7---------------------------------------------------------------------\n";
}
...
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum, qboolean demoPlayback )
{
	...
	CG_LoadingString( ":caching strings:" );
	CG_RegisterStrings();
...
void jP_GetHUDColorBackground(vec4_t bg)
{
	char	color[MAX_CVAR_VALUE_STRING];
	char	*nextVal;
	float	col[4];

	Q_strncpyz( color, cg_HUDBackgroundColor.string, 		sizeof(cg_HUDBackgroundColor.string) );

	if (!Q_stricmp( color, "default" )) {
		Vector4Set(bg, 0.16f, 0.2f, 0.17f, cg_HUDAlpha.value);
		return;
	}
	else if (!Q_stricmp( color, "black" )) {
		Vector4Set(bg, 0.0f, 0.0f, 0.0f, cg_HUDAlpha.value);
		return;
	}
	else if (!Q_stricmp( color, "blue" )) {
		Vector4Set(bg, 0.0f, 0.0f, 0.75f, cg_HUDAlpha.value);
		return;
	}
	else if (!Q_stricmp( color, "red" )) {
		Vector4Set(bg, 0.75f, 0.0f, 0.0f, cg_HUDAlpha.value);
		return;
	}
	else if (!Q_stricmp( color, "green" )) {
		Vector4Set(bg, 0.0f, 0.75f, 0.0f, cg_HUDAlpha.value);
		return;
	}
	else	{
		int		count;
		...
		for (count = 0; count < 4 && nextVal; ++count) {
			col[count]	= atof(nextVal);
			nextVal		= strtok(NULL, " ,");
		}
	}
	Vector4Set(bg, col[0], col[1], col[2], cg_HUDAlpha.value == 1 ? col[3] : cg_HUDAlpha.value);
}

void jP_GetHUDColorBorder(vec4_t bg)
{
	char	color[MAX_CVAR_VALUE_STRING];
	float	col[4];

	Q_strncpyz( color, cg_HUDBorderColor.string, 		sizeof(cg_HUDBorderColor.string) );
