...
int CG_LimboPanel_TeamCount( weapon_t weap ) {
	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		...
		if( weap != -1 ) {
			pw = cgs.clientinfo[clientNum].weapon;
			lpw = cgs.clientinfo[clientNum].latchedweapon;
			switch( weap ) {
				case WP_GARAND:
...
qboolean CG_LimboPanel_RealWeaponIsDisabled( weapon_t weap ) {
	...
	if( IS_HEAVY_WEAPON(weap) ) {
		if ( wcount >= ceil((float)(count * cgs.weaponRestrictions)) ) {
			return qtrue;
		}
		// IRATA team_panzerrestriction
		if( IS_PANZER_WEAPON(weap) ) {
			if( (float)(count/(cgs.maxclients/2.f)) <= cgs.panzerRestrictions ) {
				return qtrue;
			}
		}
	}
