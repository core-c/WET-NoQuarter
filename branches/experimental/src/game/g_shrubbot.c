...
static const struct g_shrubbot_cmd g_shrubbot_cmds[] =
{
	...
	{"ammopack",	G_shrubbot_ammopack,	'J', SCMDF_CHEAT, 	"an ammo pack will spawn out of the player if they have enough chargebar", "^9[^3name|slot#^9]"},
	{"medpack",		G_shrubbot_medpack,		'J', SCMDF_CHEAT, 	"a health pack will spawn out of the player if they have enough chargebar", "^9[^3name|slot#^9]"},
...
qboolean G_shrubbot_cmd_check(gentity_t *ent)
{
	...
	for (i=0; g_shrubbot_cmds[i].keyword[0]; i++) {
		...
		if((g_shrubbot_cmds[i].cmdFlags & SCMDF_CHEAT) &&
			!(g_tyranny.integer)) {

			SP(va("%s: Shrubbot cheat commands are not enabled\n",
				g_shrubbot_cmds[i].keyword));
			_shrubbot_log(ent, "attempted", skip-1);
		}
	...
	for ( len=0; len < mc; ++len ) {
		...
		if ( n == 1 ) {
			if ( localtime(&expiretime) ) {
				...
				AP( va("chat %i \"%s\" \"%s\"", GAMESTRING_2_IS_MUTED,	pidEnt->client->pers.netname, duration) );
			}
		}
	}
...
qboolean G_shrubbot_setlevel(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\" \"%d\" -1", GAMESTRING_2_SHRUB_NOW_LEVEL_X_USER, vic->client->pers.netname, l) );
...
qboolean G_shrubbot_kick(gentity_t *ent, int skiparg)
{
	...
	if ( g_entities[pids[0]].client->pers.localClient ) {
		SP("^dkick: ^9Cannot kick server host.\n");
		return qfalse;
	}
...
qboolean G_shrubbot_mute(gentity_t *ent, int skiparg)
{
	...
	if ( !vic->client->sess.muted ) {
		...
		vic->client->sess.auto_mute_time = level.time + seconds*1000;	// unmute time
		vic->client->sess.muted_by = MUTED_BY_SHRUB;
		ClientUserinfoChanged( pids[0] );
	}
	else {
		if ( vic->client->sess.auto_mute_time >= 0 ) {
			vic->client->sess.auto_mute_time += seconds*1000;	// increase unmute time
		}
	}
	...
	AP( va("chat %i \"%s\" \"%s\"", GAMESTRING_2_IS_MUTED,	vic->client->pers.netname, duration) );
...
qboolean G_shrubbot_unmute(gentity_t *ent, int skiparg)
{
	...
	if ( n == 1 ) {
		if ( pidEnt->client->sess.muted ) {
			...
			ClientUserinfoChanged( pids[0] );
			AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_UNMUTED, pidEnt->client->pers.netname) );
			if ( recIndex != -1 ) {
	...
	ClientUserinfoChanged( pids[0] );
	if ( recIndex != -1 ) {
		AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_UNMUTED, g_shrubbot_mutes[recIndex]->name) );
		g_shrubbot_mutes[recIndex]->expires = t - SHRUBBOT_BAN_EXPIRE_OFFSET;
	}
...
qboolean G_shrubbot_help(gentity_t *ent, int skiparg)
{
	...
	if(Q_SayArgc() < 2+skiparg) {
		...
		CP( va("chat %i \"%i\" -1", GAMESTRING_1_HELP_COMMANDS, count) );
...
qboolean G_shrubbot_admintest(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\" \"%d\" \"%s\" \"%s\" \"%s\" -1", GAMESTRING_5_ADMINTEST_LEVEL_X_USER,
		ent->client->pers.netname,
		l,
		(lname) ? "^9(^7" : "",
...
qboolean G_shrubbot_spec999(gentity_t *ent, int skiparg)
{
	...
	for(i = 0; i < level.maxclients; i++) {
		...
		if(vic->client->ps.ping >= 500 && vic->client->ps.ping <= 999) {
			...
			AP( va("chat %i \"%s\" -1", GAMESTRING_1_SPEC999_MOVED_TO_SPECS, vic->client->pers.netname) );
		}
	}
	return qtrue;
}
...
qboolean G_shrubbot_rename(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\" \"%s\"", GAMESTRING_2_SHRUB_RENAMED_TO, oldname, newname) );
...
qboolean G_shrubbot_gib(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_GIBBED, count) );
		return qtrue;
	}
	...
	*/
	...
	G_Damage(vic, NULL, NULL, NULL, NULL, 500, 0, MOD_UNKNOWN);
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_GIBBED, vic->client->pers.netname) );
	return qtrue;
}
...
qboolean G_shrubbot_slap(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_SLAPPED, vic->client->pers.netname) );
...
//G_shrubbot_rocket
// Meyer - The concept was to shoot a rocket from the player, but instead it shoots a rocket
// at the exact spot the last fired shot in the game was hit. Very buggy. Never bother to fix as
// no one really cared.
...
qboolean G_shrubbot_revive(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_REVIVED, vic->client->pers.netname) );
...
qboolean G_shrubbot_medpack(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_GIVEN_MEDPACK, ent->client->pers.netname) );
...
qboolean G_shrubbot_ammopack(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_GIVEN_AMMOPACK, ent->client->pers.netname) );
...
qboolean G_shrubbot_disguise(gentity_t *ent, int skiparg)
{
	...
	if (ent && ent->client->pers.netname == vic->client->pers.netname) {
		AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_DISGUISE_ADMIN_CHEATS, vic->client->pers.netname) );
	}
	else {
		AP( va("chat %i \"%s\" \"%s\"", GAMESTRING_2_SHRUB_WAS_DISGUISED_BY, vic->client->pers.netname, (ent?ent->client->pers.netname:"^3SERVER CONSOLE") ));
	}
...
qboolean G_shrubbot_poison(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_POISONED, count) );
		return qtrue;
	}
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_POISONED, vic->client->pers.netname) );
...
qboolean G_shrubbot_dropweapon(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_DROPPED_WEAP, count) );
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_PLAYER_DROPS_WEAPON, vic->client->pers.netname) );
...
qboolean G_shrubbot_glow(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_GLOWING, count) );
		return qtrue;
	}
	...
	if ( vic->client->ps.eFlags & EF_GLOW) {
		AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_IS_NOW_GLOWING, vic->client->pers.netname) );
	}
	else {
		AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_STOPPED_GLOWING, vic->client->pers.netname) );
	}
...
qboolean G_shrubbot_pants(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_PANTSED, count) );
		return qtrue;
	}
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("^dpants: ^9sorry, but your intended victim has a higher admin^ level than you do.^7\n");
		return qfalse;
	}
	if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_PANTSED, vic->client->pers.netname) );
...
qboolean G_shrubbot_freeze(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_FROZEN, count) );
		return qtrue;
	}
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("^dfreeze: ^9sorry, but your intended victim has a higher admin^ level than you do.^7\n");
		return qfalse;
	}
	if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_IS_FROZEN, vic->client->pers.netname) );
...
qboolean G_shrubbot_unfreeze(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_UNFROZEN, count) );
		return qtrue;
	}
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("^dunfreeze: ^9sorry, but your intended victim has a higher admin^ level than you do.^7\n");
		return qfalse;
	}
	if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_IS_UNFROZEN, vic->client->pers.netname) );
...
qboolean G_shrubbot_burn(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_BURNED, count) );
		return qtrue;
	}
	...
	G_BurnMeGood(vic, vic, NULL);
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_IS_BURNED, vic->client->pers.netname) );
...
qboolean G_shrubbot_warn(gentity_t *ent, int skiparg)
{
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("^dwarn: ^9sorry, but your intended victim has a higher admin level than you do.^7\n");
		return qfalse;
	}

	G_AddEvent(vic, EV_GENERAL_SOUND,GAMESOUND_MISC_REFEREE);

	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_WARNED, vic->client->pers.netname) );
...
qboolean G_shrubbot_give(gentity_t *ent, int skiparg)
{
	...
	if (ent && ent->client->pers.netname == vic->client->pers.netname) {
		AP( va("chat %i \"%s\" \"%s\"", GAMESTRING_2_SHRUB_GIVE_ADMIN_CHEATS, vic->client->pers.netname, item) );
	}
	else {
		AP( va("chat %i \"%s\" \"%s\" \"%s\"", GAMESTRING_3_SHRUB_GIVE_RECEIVED_CHEAT, vic->client->pers.netname, item, (ent?ent->client->pers.netname:"^3SERVER CONSOLE")) );
	}
...
qboolean G_shrubbot_lockteams(gentity_t *ent, int skiparg, qboolean toLock)
{
	...
	if ( !Q_stricmp(team, "all") ) {
		...
		AP( va("chat %i \"%s\" \"%s\" -1", GAMESTRING_2_SHRUB_ALL_TEAMS_LOCKSTATE, cmd, cmd) );
	}
	else if ( !Q_stricmp(team, "r") ) {
		teamInfo[TEAM_AXIS].team_lock = (TeamCount(-1, TEAM_AXIS)) ? toLock : qfalse;
		AP( va("chat %i \"%s\" \"%s\" -1", GAMESTRING_2_SHRUB_AXIS_TEAM_LOCKSTATE, cmd, cmd) );
	}
	else if ( !Q_stricmp(team, "b") ) {
		teamInfo[TEAM_ALLIES].team_lock = (TeamCount(-1, TEAM_ALLIES)) ? toLock : qfalse;
		AP( va("chat %i \"%s\" \"%s\" -1", GAMESTRING_2_SHRUB_ALLIES_TEAM_LOCKSTATE, cmd, cmd) );
	}
		...
		AP( va("chat %i \"%s\" \"%s\" -1", GAMESTRING_2_SHRUB_SPECS_LOCKSTATE, cmd, cmd) );
	} else {
		SP(va("^d%s usage: ^2!%s ^9r|b|s|all^7\n",cmd,cmd));
		return qfalse;
	}
...
qboolean G_shrubbot_lol(gentity_t *ent, int skiparg)
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_NADED, count) );
		return qtrue;
	}
	...
	if(pcount > 1) {
		...
		for( it = 0; it < pcount; it++) {
			...
			if(!_shrubbot_admin_higher(ent, vic)) {
				SP(va("^dnade: ^9sorry, but ^7%s^9 has a higher admin level than you do.^7\n", vic->client->pers.netname));
				continue;
			}
			else if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
				SP(va("^dnade: ^7%s^9 must be on a team to be naded^7\n", vic->client->pers.netname));
				continue;
			}
			...
			AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_IS_NADED, vic->client->pers.netname) );
		}
		return qtrue;
	}
	else if( pcount < 1 ) {
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("^dnade: ^9sorry, but your intended victim has a higher admin level than you do.^7\n");
		return qfalse;
	}
...
qboolean G_shrubbot_pop( gentity_t *ent, int skiparg )
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		for( it = 0; it < level.numConnectedClients; ++it ) {
			vic = g_entities + level.sortedClients[it];
			if( !_shrubbot_admin_higher( ent, vic ) || !(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES) || (vic->client->ps.eFlags & EF_HEADSHOT)) {
				continue;
			}
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_POPPED, count) );
		return qtrue;
	}
	pcount = ClientNumbersFromString(name, pids);
	if(pcount > 1) {
		...
		for( it = 0; it < pcount; it++) {
			vic = &g_entities[pids[it]];
			if(!_shrubbot_admin_higher(ent, vic)) {
				SP( va("^dpop: ^9sorry, but ^7%s^9 has a higher admin level than you do.^7\n", vic->client->pers.netname));
				continue;
			}
			if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
				SP(va("^dpop: ^7%s^9 must be on a team to be popped^7\n", vic->client->pers.netname));
				continue;
			}
			G_AddEvent( vic, EV_LOSE_HAT, DirToByte(dir) );
			AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_POPPED, vic->client->pers.netname) );
		}
		return qtrue;
	}
	else if(pcount < 1) {
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, vic)) {
		SP( "^dpop: ^9sorry, but your intended victim has a higher admin level than you do.^7\n" );
		return qfalse;
	}
	if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
...
qboolean G_shrubbot_pip( gentity_t *ent, int skiparg )
{
	...
	if( !Q_stricmp( name, "-1" ) || doAll ) {
		...
		AP( va("chat %i \"%d\"", GAMESTRING_1_SHRUB_X_PLAYERS_PIPPED, count) );
		return qtrue;
	}
	pcount = ClientNumbersFromString(name, pids);
	if(pcount > 1) {
		...
		for( it = 0; it < pcount; it++) {
			vic = &g_entities[pids[it]];
			if(!_shrubbot_admin_higher(ent, vic)) {
				SP( va("^dpip: ^9sorry, but ^7%s^9 has a higher admin level than you do.\n", vic->client->pers.netname));
				continue;
			}
			if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
				SP(va("^dpip: ^7%s^9 must be on a team to be pipped^7\n", vic->client->pers.netname));
				continue;
			}
			G_MakePip( vic );
			AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_WAS_PIPPED, vic->client->pers.netname) );
		}
		return qtrue;
	}
	else if(pcount < 1) {
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("^dpip: ^9sorry, but your intended victim has a higher admin level than you do.^7\n");
		return qfalse;
	}
	if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
...
{
	Svcmd_SwapTeams_f();
	AP( va("chat %i",GAMESTRING_SHRUB_TEAMS_SWAPPED) );
	return qtrue;
}
...
qboolean G_shrubbot_fling(gentity_t *ent, int skiparg)
{
	...
	if( doAll ) {
		...
		AP( va("chat %i \"%s\" \"%d\" \"%s\"", GAMESTRING_3_SHRUB_X_PLAYERS_FLUNG, flingCmd, count, pastTense) );
		return qtrue;
	}
	pcount = ClientNumbersFromString(name, pids);
	if(pcount > 1) {
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP(va("^d%s: ^9sorry, but your intended victim has a higher admin level than you do.^7\n",flingCmd));
		return qfalse;
	}

	if(G_FlingClient( vic, flingType )) {
		AP( va("chat %i \"%s\" \"%s\" \"%s\"", GAMESTRING_3_SHRUB_WAS_FLUNG, flingCmd, vic->client->pers.netname, pastTense) );
	}
	return qtrue;
}
...
qboolean G_shrubbot_disorient(gentity_t *ent, int skiparg)
{
	...
	vic = &g_entities[pids[0]];
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("disorient: sorry, but your intended victim has a higher admin level than you do.\n");
		return qfalse;
	}
	if(!(vic->client->sess.sessionTeam == TEAM_AXIS || vic->client->sess.sessionTeam == TEAM_ALLIES)) {
	...
	AP( va("chat %i \"%s\" -1", GAMESTRING_1_SHRUB_IS_DISORIENTED, vic->client->pers.netname) );
...
qboolean G_shrubbot_orient(gentity_t *ent, int skiparg)
{
	...
	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_NOT_DISORIENTED, vic->client->pers.netname) );

	CPx(pids[0], va("cp \"%s ^9oriented you\"", (ent?ent->client->pers.netname:"^3SERVER CONSOLE")));
	return qtrue;
}
...
qboolean G_shrubbot_resetxp(gentity_t *ent, int skiparg)
{
	...
	if(!_shrubbot_admin_higher(ent, &g_entities[pids[0]])) {
		SP("^dresetxp: ^9sorry, but your intended victim has a higher admin level than you do.\n");
		return qfalse;
	}
	vic = &g_entities[pids[0]];
	G_ResetXP(vic);

	AP( va("chat %i \"%s\"", GAMESTRING_1_SHRUB_XP_RESET_FOR_PLAYER, vic->client->pers.netname) );
...
qboolean G_shrubbot_nextmap(gentity_t *ent, int skiparg)
{
	...
	if( g_gametype.integer == GT_WOLF_CAMPAIGN ) {
		...
		if( campaign->current + 1 < campaign->mapCount ) {
			...
			AP( va("cp %i",GAMESTRING_LOAD_NEXT_CAMPAIGN_MAP) );
		}
		else {
			 // Load in the nextcampaign
			 trap_SendConsoleCommand(EXEC_APPEND, "vstr nextcampaign\n");
			AP( va("cp %i",GAMESTRING_LOAD_NEXT_CAMPAIGN) );
		}
    }
	else {
		// Load in the nextmap
		trap_SendConsoleCommand(EXEC_APPEND, "vstr nextmap\n");
		AP( va("cp %i",GAMESTRING_LOAD_NEXT_MAP) );
	}

	AP( va("chat %i",GAMESTRING_NEXTMAP_WAS_LOADED) );
	return qtrue;
}
