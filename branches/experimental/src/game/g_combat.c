...
static void G_UpdateKillingSpree(gentity_t *ent, gentity_t *att, qboolean death)
{
	...
	if (old_spree >= 5 && death) {
		...
		if (g_announcer.integer & ANNOUNCE_KILLINGSPREES) {
			if (att == NULL || !att->client || ent == att) {
				AP( va("chat %i \"%s\" \"%d\"", GAMESTRING_2_KILLINGSPREE_ENDED_SUICIDE, ent->client->pers.netname, old_spree) );
			}
			else {
				AP( va("chat %i \"%s\" \"%s\" \"%s\" \"%d\"", GAMESTRING_4_KILLINGSPREE_ENDED_BY,
	...
	if (old_spree <= -10 && ! death && (g_announcer.integer & ANNOUNCE_DEATHSPREES)) {
		AP( va("chat %i \"%s\" \"%d\"", GAMESTRING_2_DEATHSPREE_ENDED, ent->client->pers.netname, -old_spree) );
	}
...
*/
	...
	// new death spree (death spree every 10 deaths, % 10)
		...
		AP( va("chat %i \"%s\" \"%s\" \"%d\"", GAMESTRING_3_DEATHSPREE, ent->client->pers.netname,  humiliationMessage[snd_idx < 6 ? snd_idx : 5], -spree) );
	}
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
	...
	if(attacker == self) {
		if(self->client) {
			...
			if (bounty > 0)	{
				AP( va("chat %i \"%s\"", GAMESTRING_1_BOUNTY_ENDED_SUICIDE, self->client->pers.netname) );
			}
		...
		if (!(self->r.svFlags & SVF_BOT) &&
		  ...
		  !onSameTeam ) {
			...
			if (jp_insanity.integer & JP_INSANITY_HPLEFT) {
				CPx( self - g_entities, va("chat %i \"%s\" \"%i\"", GAMESTRING_2_ATTACKER_HAD_HP_LEFT, attacker->client->pers.netname, attacker->health));
			}

			if (jp_insanity.integer & JP_INSANITY_HPLEFT_EXT) {
				float d = Distance (self->r.currentOrigin, attacker->r.currentOrigin);
				CPx( self - g_entities, va("chat %i \"%s\" \"%i\" \"%f\"", GAMESTRING_3_ATTACKER_HP_LEFT_DISTANCE, attacker->client->pers.netname, attacker->health, d));
			}
		}
			  ...
			  vec3_t	point,	int			damage,		int			dflags,		int		mod )
{
	...
	if ( headShot ) {
		...
		if( g_debugBullets.integer ) {
			trap_SendServerCommand( attacker-g_entities, va("print %i",GAMESTRING_HEAD_SHOT) );
		}
		...
		hr = HR_LEGS;
		if( g_debugBullets.integer ) {
			trap_SendServerCommand( attacker-g_entities, va("print %i",GAMESTRING_LEG_SHOT) );
		}
	}
		...
		hr = HR_ARMS;
		if( g_debugBullets.integer ) {
			trap_SendServerCommand( attacker-g_entities, va("print %i",GAMESTRING_ARM_SHOT) );
		}
	}
		...
		hr = HR_BODY;
		if( g_debugBullets.integer ) {
			trap_SendServerCommand( attacker-g_entities, va("print %i",GAMESTRING_BODY_SHOT) );
		}
	}
	...
	if ( playerAttackedByPlayer && targ != attacker &&
			targ->health > limbo_health &&
			mod != MOD_SWITCHTEAM && mod != MOD_SWAP_PLACES && mod != MOD_SUICIDE && mod != MOD_FEAR &&
			mod != MOD_POISON ) {
