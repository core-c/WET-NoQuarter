...
qboolean ClientInactivityTimer( gclient_t *client ) {
	...
	if ( !client->inactivityWarning ) {
			...
			inTeam )
		{
			...
			CPx( client - level.clients, va("print %i \"%i\"", GAMESTRING_INACTIVITY_SECONDS_TO_SPEC, inactivity) );
	  			...
	  			!inTeam )
		{
			...
			CPx(client - level.clients, va("print %i \"%i\"", GAMESTRING_INACTIVITY_SECONDS_DROP, inactivityspec));
		...
		if ( level.time > client->inactivityTime ) {
			...
			if ( showMsg ) {
				...
				if ( g_inactivity.integer && inTeam ) {
					if ( secondsLeft <= 0 ) {
						CPx(client - level.clients, va("cp %i",GAMESTRING_MOVED_TO_SPEC_INACTIVITY) );
					}
					else {
						CPx(client - level.clients, va("cp \"^3%i seconds until moving to spectator for inactivity!\n\"", secondsLeft));
					}
					...
					if ( secondsLeft <= 0 ) {
						CPx(client - level.clients, va("cp %i",GAMESTRING_DROPPED_FOR_INACTIVITY) );
					}
					else {
						CPx(client - level.clients, va("cp \"^3%i seconds until inactivity drop!\n\"", secondsLeft));
					}
...
void KillerCam( gentity_t *self ) {
	...
	self->client->pmext.viewlocked_time = level.time + (duration * 1000);
}
...
void ClientThink_checkWeapons( gentity_t *ent ){
	...
	if ( !ent->client->weaponsRemoved && doWar ) {
			...
			|| (ent->r.svFlags & SVF_BOT) )
		{
			...
			if ( nq_War.integer & WARMODE_RIFLE ) {
				...
				Bot_Event_AddWeapon(ent-g_entities, Bot_WeaponGameToBot(weapon));
#endif
				// marker to indicate we can use a riflenade
				// (it is no marker that we have a nade attached, but we have a weapon that -can- attach it)
	...
	passed:
		...
		if ( drop ) {
			trap_SendServerCommand( ent-g_entities,	va("cp %i",GAMESTRING_WEAPON_RESTRICTION) );
		}
	}
}
...
void ClientThink_real( gentity_t *ent ) {
	...
	if ( g_truePing.integer && !(ent->r.svFlags & SVF_BOT)) {
		...
		// initialize the 'real' ping from samples
	...
	if ( !doAntiWarp && ucmd->serverTime < level.time - 1000 )
	...
	if ( client->sess.muted && client->sess.auto_mute_time != -1 && level.time > client->sess.auto_mute_time ) {
		if ( client->sess.muted_by & MUTED_BY_CENSOR ) {
			CPx(ent - g_entities, va("print %i",GAMESTRING_AUTO_UNMUTE_LANGUAGE) );
			AP( va("chat %i \"%s\"", GAMESTRING_1_AUTO_UNMUTE_LANGUAGE, ent->client->pers.netname) );
		}
		else if ( client->sess.muted_by & MUTED_BY_SHRUB ) {
			CPx(ent - g_entities, va("print %i",GAMESTRING_AUTO_UNMUTE_SHRUB) );
			AP( va("chat %i \"%s\"", GAMESTRING_1_AUTO_UNMUTE_SHRUB, ent->client->pers.netname) );
		}
		else if ( client->sess.muted_by & MUTED_BY_LUA ) {
			CPx(ent - g_entities, va("print %i",GAMESTRING_AUTO_UNMUTE_LUA) );
			AP( va("chat %i \"%s\"", GAMESTRING_1_AUTO_UNMUTE_LUA, ent->client->pers.netname) );
		}
	...
	if( client->ps.stats[STAT_HEALTH] <= 0 && !(client->ps.eFlags & EF_PLAYDEAD)) {
		...
		if( level.timeCurrent > client->respawnTime && !(ent->client->ps.pm_flags & PMF_LIMBO) ) {
			...
			if( client->ps.stats[STAT_HEALTH] <= GIB_HEALTH || (g_forcerespawn.integer > 0 && level.timeCurrent - client->respawnTime > g_forcerespawn.integer * 1000) ) {
				limbo(ent, (client->ps.stats[STAT_HEALTH] > GIB_HEALTH));
			}
		}
		return;
	  ...
	  && level.timeCurrent - client->pers.lastHQMineReportTime > 20000 ) {	// NOTE: check is every 20 secs
		...
		if( level.gameManager->s.modelindex && client->sess.sessionTeam == TEAM_AXIS ) {
			if( G_SweepForLandmines( ent->r.currentOrigin, 256.f, TEAM_AXIS ) ) {
				trap_SendServerCommand(ent-g_entities, va("cp %i 1",GAMESTRING_MINES_IN_AREA) );
			}
		}
