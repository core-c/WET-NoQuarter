...
void G_refHelp_cmd(gentity_t *ent)
{
	...
	if(ent) {
		CP( va("print %i",GAMESTRING_REFEREE_COMMANDS_HELP1) );
		CP( va("print %i",GAMESTRING_REFEREE_COMMANDS_HELP2) );

		G_voteHelp(ent, qfalse);
		// CHRUKER: b038 - Removed non-existing restart command
		// CHRUKER: b039 - Added <pid> parameter to remove command
		CP( va("print %i",GAMESTRING_REFEREE_COMMANDS_HELP3) );
		CP( va("print %i",GAMESTRING_REFEREE_COMMANDS_HELP4) );
		CP( va("print %i",GAMESTRING_REFEREE_COMMANDS_HELP5) );
		CP( va("print %i",GAMESTRING_REFEREE_COMMANDS_HELP6) );
		CP( va("print %i",GAMESTRING_REFEREE_COMMANDS_HELP7) );
...
void G_ref_cmd(gentity_t *ent, unsigned int dwCommand, qboolean fValue)
{
	...
	if(ent) {
		if(!Q_stricmp(refereePassword.string, "none") || !refereePassword.string[0]) {
			...
			CP( va("print %i",GAMESTRING_REF_STATUS_DISABLED) );
			return;
		}

		if(trap_Argc() < 2) {
			// CHRUKER: b046 - Was using the cpm command, but this is really just for the console.
			CP( va("print %i",GAMESTRING_REF_PASSWORD_USAGE) );
			return;
		}
		...
		if(Q_stricmp(arg, refereePassword.string)) {
			// CHRUKER: b046 - Was using the cpm command, but this is really just for the console.
			CP( va("print %i",GAMESTRING_REF_PASSWORD_INVALID) );
			return;
		}
...
void G_refPause_cmd(gentity_t *ent, qboolean fPause)
{
	...
	if(fPause) {
		...
		AP( va("print %i \"%s\"", GAMESTRING_REF_PAUSED_MATCH, referee) );
		// CHRUKER: b047 - Remove unneeded \" and linebreak
		CP(va("cp \"^3Match is ^1PAUSED^3! (^7%s^3)", referee));
		level.server_settings |= CV_SVS_PAUSE;
		trap_SetConfigstring(CS_SERVERTOGGLES, va("%d", level.server_settings));
	} else {
		AP( va("print %i \"%s\"", GAMESTRING_REF_UNPAUSES_MATCH, referee) );
...
void G_refRemove_cmd(gentity_t *ent)
{
	...
	CPx(pid, va("print %i \"%s\"", GAMESTRING_REF_REMOVED_FROM_TEAM, aTeams[player->client->sess.sessionTeam]));
...
void G_refMute_cmd(gentity_t *ent, qboolean mute)
{
	...
	if(player->client->sess.muted == mute) {
		// CHRUKER: b047 - Remove unneeded linebreak
		G_refPrintf(ent, "\"%s^7\" %s", player->client->pers.netname, mute ? "is already muted!" : "is not muted!" ); // ^*
		return;
	}

	if( mute ) {
		CPx(pid, va("print %i",GAMESTRING_REF_YOUVE_BEEN_MUTED) );
		player->client->sess.muted = qtrue;
		player->client->sess.auto_mute_time = -1;
		player->client->sess.muted_by = MUTED_BY_REF;
		ClientUserinfoChanged( pid );
		G_Printf( "\"%s^7\" has been muted\n",  player->client->pers.netname ); // ^*
	}
	else {
		CPx(pid, va("print %i",GAMESTRING_REF_YOUVE_BEEN_UNMUTED) );
		player->client->sess.muted = qfalse;
		player->client->sess.auto_mute_time = -1;
		player->client->sess.muted_by = UNMUTED;
		ClientUserinfoChanged( pid );
...
void G_PlayerBan()
{
	...
	if(bannum != MAX_CLIENTS ) {
		AddIPBan( level.clients[bannum].pers.client_ip );
	}
}
...
void G_MuteClient()
{
	...
	if (cnum != MAX_CLIENTS ) {
		if( level.clients[cnum].sess.referee != RL_RCON ) {
			trap_SendServerCommand( cnum, va("cpm %i",GAMESTRING_MUTED_PLAYER) );
...
void G_UnMuteClient()
{
	...
	if (cnum != MAX_CLIENTS ) {
		if( level.clients[cnum].sess.muted ) {
			trap_SendServerCommand( cnum, va("cpm %i",GAMESTRING_UNMUTED_PLAYER) );
