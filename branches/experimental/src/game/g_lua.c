...
// et.MutePlayer( clientnum, duration )
...
static int _et_MutePlayer(lua_State *L)
{
	...
	if ( duration == -1 ) {
		CPx( clientnum, va("print %i",GAMESTRING_MUTED_BY_LUA) );
		AP( va("chat %i \"%s\"", GAMESTRING_1_MUTED_BY_LUA, ent->client->pers.netname ));
	}
	else {
		CPx( clientnum, va("print %i \"%d\"", GAMESTRING_MUTED_SECONDS_LUA, duration) );
		AP( va("chat %i \"%s\" \"%i\"", GAMESTRING_2_MUTED_SECONDS_LUA, ent->client->pers.netname, duration) );
	}
	return 0;
}
...
static int _et_UnmutePlayer(lua_State *L)
{
	...
	ClientUserinfoChanged( clientnum );
	// g_active.c ClientThink_real() will report the unmute message for Lua..
	return 0;
}
...
static const gentity_field_t gclient_fields[] = {
	...
	_et_gclient_addfield(sess.rifleNade, FIELD_INT, FIELD_FLAG_READONLY),
