...
#define	JITTER_MAX	(check->r.maxs[0]/2.0)
	...
	if (check->client) {
		...
		if ( (g_realism.integer & REALISM_MOVER_ROTATES_PLAYERS) ||
			(!(g_realism.integer & REALISM_MOVER_ROTATES_PLAYERS) && ((check->client->ps.eFlags & EF_PRONE) || check->s.weapon == WP_MORTAR_SET || check->s.weapon == WP_MORTAR2_SET)) )
		{
...
void func_constructible_explode( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
	...
	if( self->desstages ) {
		if( self->grenadeFired > 1 ) {
			...
			// core: award stats
			if (attacker && attacker->client) attacker->client->sess.missionair++;
