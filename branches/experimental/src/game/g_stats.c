...
void G_AddSkillPoints( gentity_t *ent, skillType_t skill, float points )
{
	...
	if(g_maxXP.integer >= 0 && (ent->client->ps.persistant[PERS_SCORE] + points) >= g_maxXP.integer) {
		...
		if(g_maxXP.integer) {
			CP( va("cp %i 1",GAMESTRING_REACHED_MAX_XP) );
		}
		return;
	}
	...
	trap_PbStat ( ent - g_entities, "addskill",	va("%d %d %d %f", ent->client->sess.sessionTeam , ent->client->sess.playerType,	skill , points) );
...
void jP_BuildEndgameStats(void)
{
	...
	*buffer = '\0';
	...
	if( best && best->sess.aWeaponStats[WS_GRENADE].kills > 0) {
		best->hasaward = qtrue;
	...
	if( best && best->sess.aWeaponStats[WS_GRENADELAUNCHER].kills > 0) {
		best->hasaward = qtrue;
	...
	if( best && best->sess.panzernoob > 0) {
		best->hasaward = qtrue;
