...
static const vote_reference_t aVoteInfo[] = {
	...
	{ 0x1ff, "shuffleteamsxp_norestart", G_ShuffleTeams_NoRestart_v, "Shuffle Teams by XP",	" ^7\n  Randomly place players on each team, based on XP" },
...
void G_voteHelp(gentity_t *ent, qboolean fShowVote)
{
	...
	if (fShowVote) {
		CP( va("print %i",GAMESTRING_VOTE_HELP1) );
	}
	...
	if(fShowVote) {
		CP( va("print %i",GAMESTRING_VOTE_HELP2) );
	}
...
{
	AP( va("print %i \"%s\" \"%s\"", GAMESTRING_VOTESTRING_SET_TO, desc, level.voteInfo.vote_value) );
...
int G_Comp_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	// Vote action (vote has passed)
		...
		AP( va("cpm %i",GAMESTRING_COMPETITION_SETTINGS) );
	}
...
int G_Kick_v( gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd )
{
	...
	if( arg ) {
		...
		if(G_shrubbot_permission(&g_entities[pid], SBF_IMMUNITY)) {
			G_refPrintf( ent, "Can't vote to kick immune admins!" );
			return G_INVALID;
		}
...
int G_Mute_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	if(arg) {
		...
		if(G_shrubbot_permission(&g_entities[pid], SBF_IMMUNITY)) {
			G_refPrintf( ent, "Can't vote to mute immune admins!" );
			return G_INVALID;
		}
	...
	// Vote action (vote has passed)
		...
		if( level.clients[pid].sess.referee != RL_RCON ) {
			trap_SendServerCommand( pid, va("cpm %i",GAMESTRING_MUTED_PLAYER) );
...
int G_UnMute_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	// Vote action (vote has passed)
		...
		// Mute a player
		if( level.clients[pid].sess.referee != RL_RCON ) {
			trap_SendServerCommand( pid, va("cpm %i",GAMESTRING_UNMUTED_PLAYER) );
...
//		}
	// Vote action (vote has passed)
		...
		AP( va("cp %i",GAMESTRING_LEVEL_RESTARTED) );
	}
...
//			}
	...
	// Vote action (vote has passed)
		...
		// Restart the map back to warmup
		Svcmd_ResetMatch_f(qtrue, qtrue);
		AP( va("cp %i",GAMESTRING_MATCH_RESET) );
	}
	...
	// Vote action (vote has passed)
		...
		if( g_gametype.integer == GT_WOLF_CAMPAIGN ) {
			...
			if( campaign->current + 1 < campaign->mapCount ) {
				...
				AP( va("chat %i",GAMESTRING_NEXTMAP_VOTE_PASSED) );
		}
		else {
			// Load in the nextmap
...
int G_Pub_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	// Vote action (vote has passed)
		...
		// CHRUKER: b042 - Was using cp, but subsequent cp calls
		//          quickly removed it, so now its using cpm
		AP( va("cpm %i",GAMESTRING_PUBLIC_SETTINGS) );
	}
...
int G_Referee_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd)
{
	...
	// Vote action (vote has passed)
		...
		if(cl->pers.connected == CON_DISCONNECTED) {
			AP( va("print %i",GAMESTRING_PLAYER_LEFT_BEFORE_REF) );
		}
		else {
...
int G_Surrender_v( gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd )
{
	...
	if(arg) {
		...
		Q_strncpyz(arg2, (ent->client->sess.sessionTeam == TEAM_AXIS)? "[AXIS]" : "[ALLIES]", VOTE_MAXSTRING);
	}
	// Vote action (vote has passed)
		...
		Info_SetValueForKey(cs, "winner", (level.voteInfo.voteTeam == TEAM_AXIS) ? "1" : "0");
		trap_SetConfigstring(CS_MULTI_MAPWINNER, cs);
		LogExit(va("%s Surrender\n", (level.voteInfo.voteTeam == TEAM_AXIS)? "Axis" : "Allies"));
		AP( va("chat %i \"%s\"", GAMESTRING_1_TEAM_HAS_SURRENDERED, (level.voteInfo.voteTeam == TEAM_AXIS)? "^1AXIS^7" : "^4ALLIES^7") );
	}
	return(G_OK);
}
...
void G_IntermissionMapVote( gentity_t *ent )
{
	...
	if ( g_gametype.integer != GT_WOLF_MAPVOTE ) {
		CP( va("print %i",GAMESTRING_MAPVOTING_DISABLED) );
		return;
	}

	if( g_gamestate.integer != GS_INTERMISSION ) {
		CP( va("print %i",GAMESTRING_MAPVOTING_INTERMISSION) );
		return;
	}
	...
	if ( trap_Argc() == 2 ) {
		...
		for ( i = 0; i < 3; i++ ) {
			...
			if ( ent->client->sess.mapVotedFor[i] == atoi(arg) ) {
				CP( va("print %i",GAMESTRING_MAPVOTING_NOT_TWICE) );
				return;
			}
		}
	...
	// core: someone has voted. Send the votetally to all..
...
void G_IntermissionVoteTally( gentity_t *ent )
{
	...
	// core: when argument "ent" == NULL, the votetally should be send to all players..
	if ( ent ) {
