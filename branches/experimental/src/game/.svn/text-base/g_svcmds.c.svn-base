...
void Svcmd_RevivePlayer( char *name )
{
	...
	if( !g_cheats.integer ) {
		trap_SendServerCommand( -1, va("print %i",GAMESTRING_NO_CHEATS_ENABLED) );
		return;
	}
...
{
	trap_Cvar_Set( "g_unlockWeapons", va("%d",UNLOCK_WEAPONS_ALL) );
}

void Cmd_Weapons_Off_f( void )
{
	trap_Cvar_Set( "g_unlockWeapons", va("%d",UNLOCK_WEAPONS_NONE) );
}
