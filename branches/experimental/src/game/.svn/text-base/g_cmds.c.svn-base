...
void G_CensorPenalize(gentity_t *ent)
{
	...
	if ( g_censorPenalty.integer & CNSRPNLTY_POISON ) {
			...
			level.match_pause == PAUSE_NONE )
		{
			...
			AP( va("chat %i \"%s\"", GAMESTRING_1_POISONED_HIS_DIRTY_MOUTH, ent->client->pers.netname) );
		}
	}
	if ( g_censorPenalty.integer & CNSRPNLTY_BURN) {
			...
			level.match_pause == PAUSE_NONE )
		{
			...
			AP( va("chat %i \"%s\"", GAMESTRING_1_BURNED_HIS_TONGUE, ent->client->pers.netname) );
		}
	}
	         // forty - #127 - censor xp penalty
	if ( g_censorPenalty.integer & CNSRPNLTY_XP) {
		if(ent->client->sess.sessionTeam != TEAM_SPECTATOR) {
			CPx(ent - g_entities, va("chat %i \"%d\"", GAMESTRING_1_CENSOR_PENALTY_XP, 10 ));
	...
	if ( g_censorPenalty.integer & CNSRPNLTY_TMPMUTE ) {
		if ( !ent->client->sess.muted ) {
			...
			CPx( ent - g_entities, va("print %i \"%d\"", GAMESTRING_MUTED_SECONDS_LANGUAGE, g_censorMuteTime.integer ) );
			AP( va("chat %i \"%s\" \"%d\"", GAMESTRING_2_MUTED_SECONDS_LANGUAGE, ent->client->pers.netname, g_censorMuteTime.integer) );
		} else {
			...
			if ( ent->client->sess.auto_mute_time >= 0 ) {
				...
				CPx( ent - g_entities, va("print %i \"%d\"", GAMESTRING_MUTED_MORESECONDS_LANGUAGE, g_censorMuteTime.integer ) );
				AP( va("chat %i \"%s\" \"%d\"", GAMESTRING_2_MUTED_MORESECONDS_LANGUAGE, ent->client->pers.netname, g_censorMuteTime.integer) );
			}
		}
	}
...
void G_PrivateMessage(gentity_t *ent)
{
	...
	for(i=0; i<pcount; i++) {
		...
		CPx(pids[i], va("chat %i \"%s\" \"%s\" \"%d\" \"%s\"", GAMESTRING_4_PM_X_RECIPIENTS, netname, name, pcount, msg));
		CPx(pids[i], va("cp \"^3private message from ^7%s^7\"", netname));
		/*CPx(ent-g_entities, va("chat \"private message sent to %s: ^3%s^7\"", tmpent->client->pers.netname, msg ));*/
	...
	if ( sent && ent && !self ) {
		CPx(ent-g_entities, va("chat %i \"%s\" \"%s\" \"%d\" \"%s\"", GAMESTRING_4_PM_X_RECIPIENTS, netname, name, pcount, msg));
	}

	if(!sent) {
		CPx(ent-g_entities, va("chat %i",GAMESTRING_PLAYER_NOT_FOUND) );
	}
...
void G_TeamDamageStats(gentity_t *ent)
{
	...
	{
		float teamHitPct = (ent->client->sess.hits)? (ent->client->sess.team_hits / ent->client->sess.hits)*(100): 0;

		CPx( ent-g_entities, va("print %i \"%.2f\" \"%.2f\" \"%.2f\" \"%d\"", GAMESTRING_TEAMDAMAGESTATS,
...
void G_SendScore( gentity_t *ent )
{
	...
	*startbuffer = '\0';
	...
	Q_strncpyz(startbuffer, va("sc0 %i %i", level.teamScores[TEAM_AXIS], level.teamScores[TEAM_ALLIES]), sizeof(startbuffer));
...
// TODO add to project files
	...
	if ( !g_cheats.integer ) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_NO_CHEATS_ENABLED) );
		return qfalse;
	}
	if ( ent->health <= 0 ) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_ALIVE_FOR_USE));
		return qfalse;
	}
	return qtrue;
}
...
int ClientNumberFromString( gentity_t *to, char *s )
{
	...
	if(fIsNumber) {
		...
		if ( idnum < 0 || idnum >= level.maxclients ) {
			CPx( to-g_entities, va("print %i \"%i\"", GAMESTRING_BAD_CLIENT_SLOT, idnum) );
			return -1;
		}

		cl = &level.clients[idnum];
		if ( cl->pers.connected != CON_CONNECTED ) {
			CPx( to-g_entities, va("print %i \"%i\"", GAMESTRING_CLIENT_NOT_ACTIVE, idnum) );
			return -1;
		}
		return(idnum);
	}

	CPx( to-g_entities, va("print %i \"%s\"", GAMESTRING_USER_NOT_ON_SERVER, s) );
...
void Cmd_GiveDisguise_f(gentity_t *ent, int startArg)
{
	...
	if (! *cls) {
		...
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_GIVEDISGUISE_HELP_1) );
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_GIVEDISGUISE_HELP_2) );
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_GIVEDISGUISE_HELP_3) );
		return;
	}
	...
	else if (!Q_stricmpn(cls, "e", 1) || classNum == 2) {
		msg = "^9givedisguise : ^7Engineer\n";
		classNum = 2;
	}
	else if (!Q_stricmpn(cls, "f", 1) || classNum == 3) {
		msg = "^9givedisguise : ^7Field Ops\n";
		classNum = 3;
	}
	else if (!Q_stricmpn(cls, "c", 1) || classNum == 4) {
		msg = "^9givedisguise : ^7Covert Ops\n";
		classNum = 4;
	}
	else if (!Q_stricmpn(cls, "s", 1) || classNum == 0) {
		msg = "^9givedisguise : ^7Soldier\n";
		classNum = 0;
	}
	if (!Q_stricmpn(cls, "-1", 1) || classNum == -1) { // this just mean that there must be '-' so '-x' does the shit
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_GIVEDISGUISE_HELP_4));
...
void ExecGive( gentity_t *ent, char *name, char *amt, char *amt2)
{
	...
	if (give_all || Q_stricmp(name, "weapons") == 0) {
		for( i = WP_KNIFE; i < WP_FOOTKICK; i++ ) {
...
qboolean SetTeam( gentity_t *ent, const char *s, qboolean force, weapon_t w1, weapon_t w2, qboolean setweapons )
{
	...
	if (ent->client->freezed) {
		trap_SendServerCommand( clientNum, va("cp %i",GAMESTRING_PLAYER_FROZEN) );
		return(qfalse);
	}
	...
	if( team != TEAM_SPECTATOR ) {
		...
		if(g_noTeamSwitching.integer && (team != ent->client->sess.sessionTeam && ent->client->sess.sessionTeam != TEAM_SPECTATOR ) && g_gamestate.integer == GS_PLAYING && !force ) {
			trap_SendServerCommand( clientNum, va("cp %i",GAMESTRING_NO_SWITCH_DURING_MATCH) );
		...
		if ( ( (g_gametype.integer == GT_WOLF_LMS && g_lms_teamForceBalance.integer) || g_teamForceBalance.integer ) && !force ) {
			...
			if ( team == TEAM_AXIS && counts[TEAM_AXIS] - counts[TEAM_ALLIES] >= 1 ) {
				CP( va("cp %i",GAMESTRING_AXIS_TOO_MANY_PLAYERS) );
				return qfalse; // ignore the request
			}
			if ( team == TEAM_ALLIES && counts[TEAM_ALLIES] - counts[TEAM_AXIS] >= 1 ) {
				CP( va("cp %i",GAMESTRING_ALLIES_TOO_MANY_PLAYERS) );
				return qfalse; // ignore the request
			}
	...
	if( g_gametype.integer != GT_WOLF_LMS ) {
		if( ( g_maxlives.integer > 0 ||
			...
			CP( va("cp %i",GAMESTRING_CANT_SWITCH_NO_MORE_LIVES) );
			return qfalse;	// ignore the request
		}
	}
	...
	*/
	...
	if( g_gamestate.integer == GS_PLAYING && ( client->sess.sessionTeam == TEAM_AXIS || client->sess.sessionTeam == TEAM_ALLIES ) ) {
		if(g_gametype.integer == GT_WOLF_LMS && level.numTeamClients[0] > 0 && level.numTeamClients[1] > 0) {
			trap_SendServerCommand( clientNum, va("cp %i",GAMESTRING_SPAWN_NEXT_ROUND) );
			limbo(ent, qfalse);
			return(qfalse);
		}
		else {
...
int G_TeamCount( gentity_t* ent, weapon_t weap )
{
	...
	for( i = 0; i < level.numConnectedClients; i++ ) {
		...
		if( weap != -1 ) {
			pw = level.clients[j].sess.playerWeapon;
			lpw = level.clients[j].sess.latchPlayerWeapon;
			switch( weap ) {
				case WP_GARAND:
				case WP_GARAND_SCOPE:
				case WP_K43:
				...
				case WP_FG42:
				case WP_FG42SCOPE:
					if ( !(pw == WP_GARAND			|| lpw == WP_GARAND			||
							pw == WP_GARAND_SCOPE	|| lpw == WP_GARAND_SCOPE	||
							pw == WP_K43			|| lpw == WP_K43			||
							pw == WP_K43_SCOPE		|| lpw == WP_K43_SCOPE		||
							pw == WP_FG42			|| lpw == WP_FG42			||
							pw == WP_FG42SCOPE		|| lpw == WP_FG42SCOPE) )
					{
						continue;
					}
					break;
				...
				default:
					if ( !(pw == weap									|| lpw == weap										||
							pw == weapAlts[weap]						|| lpw == weapAlts[weap]							||
							pw == weapTeamConversion[weap]				|| lpw == weapTeamConversion[weap]					||
							pw == weapTeamConversion[weapAlts[weap]]	|| lpw == weapTeamConversion[weapAlts[weap]]) )
					{
						continue;
					}
					break;
...
qboolean G_IsWeaponDisabled( gentity_t* ent, weapon_t weapon, qboolean quiet )
{
	...
	if( IS_HEAVY_WEAPON(weapon) ) {
		if( wcount >= ceil( (float)(count * g_heavyWeaponRestriction.integer * 0.01f) ) ) {
			if (!quiet && !(ent->client->ps.pm_flags & PMF_LIMBO)) { // do not show when client is in limbo (fade out of text might confuse
				CP( va("cp %i 1",GAMESTRING_HEAVYWEAP_NOT_AVAILABLE) );
			}
			return qtrue;
		}
		// IRATA team_panzerrestriction
		if( IS_PANZER_WEAPON(weapon) ) {
			if( ( (float)(count/(g_maxclients.integer/2.f)) ) <=  (float)(team_panzerRestriction.integer*0.01f) ) {
	...
	if ( (!(team_maxIsPercentage.integer & TEAM_MAXWEAPONS_PERCENT) && wcount >= value) ||					// compare absolute teamtotal
		((team_maxIsPercentage.integer & TEAM_MAXWEAPONS_PERCENT) && wcount >= value * count / 100.0f) )	// compare percentage of team
	{
		wrestrict = qtrue;
	}
...
void Cmd_Follow_f( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
	...
	if ((ent->client->sess.sessionTeam == TEAM_AXIS || ent->client->sess.sessionTeam == TEAM_ALLIES) && !(ent->client->ps.pm_flags & PMF_LIMBO)) {
		CP( va("print %i",GAMESTRING_CANT_FOLLOW_NOT_LIMBO) );
		return;
	}
	...
	if(ClientNumbersFromString(arg, pids) > 1) {
		CP( va("print %i",GAMESTRING_PART_NAME_MORE_MATCHES) );
		return;
	}
	i = pids[0];
	...
	if ( i == -1 ) {
		...
		if ((ent->client->sess.sessionTeam == TEAM_AXIS ||
			...
			CP( va("print %i",GAMESTRING_CANT_FOLLOW_ENEMIES) );
			return;
		}

		if(!TeamCount(ent - g_entities, i)) {
			CP( va("print %i \"%s\" \"%s\"", GAMESTRING_TEAM_EMPTY_FOLLOW_IGNORED, aTeams[i], ((ent->client->sess.sessionTeam != i) ? "is" : "would be")) );
			return;
		}
		...
		if(ent->client->sess.spec_team != i) {
			if(teamInfo[i].spec_lock && !(ent->client->sess.spec_invite & i)) {
				CP( va("print %i \"%s\"", GAMESTRING_TEAM_LOCKED_FOR_SPECS, aTeams[i]) );
			}
			else {
				ent->client->sess.spec_team = i;
				CP( va("print %i \"%s\"", GAMESTRING_SPEC_FOLLOW_LOCKED_ON_TEAM, aTeams[i]) );
				Cmd_FollowCycle_f(ent, 1, qfalse);
			}
		} else {
			ent->client->sess.spec_team = 0;
			CP( va("print %i \"%s\"", GAMESTRING_TEAM_SPECTATING_DISABLED, aTeams[i]) );
		}
	...
	if(!G_allowFollow(ent, level.clients[i].sess.sessionTeam)) {
		CP( va("print %i \"%s\"", GAMESTRING_TEAM_LOCKED_FOR_SPECS, aTeams[level.clients[i].sess.sessionTeam]) );
		return;
	}
...
void G_Voice( gentity_t *ent, gentity_t *target, int mode, const char *id, const char *customChat, qboolean voiceonly )
{
	...
	if ( ent->voiceChatSquelch >= 30000 && !G_shrubbot_permission(ent, SBF_NOCENSORFLOOD) ) {
		// CHRUKER: b066 - Was using the cpm command, but this needs
		//          to be displayed immediately.
		trap_SendServerCommand( ent-g_entities, va("cp %i",GAMESTRING_SP_VOICE_IGNORED) );
		return;
	}
...
qboolean Cmd_CallVote_f( gentity_t *ent, unsigned int dwCommand, qboolean fRefCommand )
{
	...
	if( !fRefCommand ) {
		...
		if (muted) {
			CP( va("cp %i",GAMESTRING_CANNOT_VOTE_WHILE_MUTED) );
			return qfalse;
		}
		else if( level.voteInfo.voteTime ) {
			CP( va("cp %i",GAMESTRING_VOTE_IN_PROGRESS) );
			return qfalse;
		}
		else if( level.intermissiontime ) {
			CP( va("cp %i",GAMESTRING_NO_VOTE_INTERMISSION) );
			return qfalse;
		}
			...
			if( voteFlags.integer == VOTING_DISABLED && !sbfNoVoteLimit) {
				CP( va("cp %i",GAMESTRING_VOTING_NOT_ENABLED) );
				return qfalse;
			}
	...
	if(trap_Argc() > 1 && (i = G_voteCmdCheck(ent, arg1, arg2, fRefCommand)) != G_NOTFOUND) {	//  --OSP
		...
		if(!fRefCommand) {
			CP( va("print %i \"%s\" \"%s\"", GAMESTRING_UNKNOWN_VOTE_COMMAND, arg1, arg2) );
	...
	if( fRefCommand ) {
		...
		if(level.voteInfo.vote_fn != G_Kick_v && level.voteInfo.vote_fn != G_Mute_v) {
			AP( va("cp %i",GAMESTRING_REFEREE_SETTING_CHANGE) );
		}
		...
		AP( va("print %i \"%s\" \"%s\"", GAMESTRING_CALLED_A_VOTE_VOTED_FOR, ent->client->pers.netname, level.voteInfo.voteString) );
...
void Cmd_Vote_f( gentity_t *ent )
{
	...
	if ( ent->client->pers.complaintEndTime > level.time && g_gamestate.integer == GS_PLAYING && g_complaintlimit.integer ) {
		...
		if ( msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1' ) {
			...
			trap_SendServerCommand( ent->client->pers.complaintClient, va("cpm %i",GAMESTRING_NO_COMPLAINT_FILED) );
	...
	if ( !level.voteInfo.voteTime ) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_NO_VOTE_IN_PROGRESS) );
		return;
	}
	if ( ent->client->ps.eFlags & EF_VOTED ) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_VOTE_ALREADY_CAST) );
		return;
	}
	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_NO_VOTE_SPEC) );
		return;
	}

	if ( muted ) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_NO_VOTE_MUTED) );
		return;
	}
	...
	if( level.voteInfo.vote_fn == G_Kick_v ) {
		...
		if( g_entities[ pid ].client->sess.sessionTeam != TEAM_SPECTATOR && ent->client->sess.sessionTeam != g_entities[ pid ].client->sess.sessionTeam ) {
			trap_SendServerCommand( ent - g_entities, va("print %i",GAMESTRING_NO_VOTEKICK_ENEMY) );
			return;
		}
	}
	else if(level.voteInfo.vote_fn == G_Surrender_v) {
		if(ent->client->sess.sessionTeam != level.voteInfo.voteTeam) {
			CP( va("cp %i",GAMESTRING_NO_VOTE_ENEMY_SURRENDER) );
			return;
		}
	}

	trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_VOTE_CAST) );
...
void Cmd_SetViewpos_f( gentity_t *ent )
{
	...
	if ( trap_Argc() != 5 ) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_SETVIEWPOS_USAGE) );
		return;
	}
...
void Cmd_Activate2_f( gentity_t *ent )
{
	...
	if(g_dropObj.integer &&
		...
		if(ent->client->ps.weapon != WP_KNIFE && ent->client->ps.weapon != WP_KNIFE_KABAR) {
			CP( va("cp %i",GAMESTRING_SWITCH_KNIFE_DROP_OBJ) );
		}
		else {
			G_DropItems(ent);
...
void Cmd_Ignore_f (gentity_t* ent )
{
	...
	if(!*cmd) {
		CP( va("print %i",GAMESTRING_IGNORE_USAGE) );
		return;
	}
	...
	if(pids[0] == (ent-g_entities)) {
		CP( va("print %i",GAMESTRING_CANT_IGNORE_SELF) );
		return;
	}
	...
	if(COM_BitCheck(ent->client->sess.ignoreClients, pids[0])) {
		CP( va("print %i \"%s\"", GAMESTRING_ALREADY_IGNORING, victim->client->pers.netname) );
		return;
	}

	COM_BitSet(ent->client->sess.ignoreClients, pids[0]);
	CP( va("print %i \"%s\"", GAMESTRING_ADDED_TO_IGNORE_LIST, victim->client->pers.netname) );
	CPx(pids[0], va("chat %i \"%s\"", GAMESTRING_1_IS_IGNORING_YOU, ent->client->pers.netname));
}
...
void Cmd_UnIgnore_f (gentity_t* ent )
{
	...
	if(!*cmd) {
		CP( va("print %i",GAMESTRING_UNIGNORE_USAGE) );
		if(!ent->client->sess.ignoreClients[0] && !ent->client->sess.ignoreClients[1]) {
			return;
		}
		CP( va("print %i",GAMESTRING_CURRENTLY_IGNORING) );
		for(i=0; i<g_maxclients.integer; i++) {
	...
	if(matches != 1) {
		CP( va("print %i",GAMESTRING_UNIGNORE_NO_MATCH) );
		return;
	}
	victim = &g_entities[clientNum];

	COM_BitClear(ent->client->sess.ignoreClients, clientNum);
	CP( va("print %i \"%s\"", GAMESTRING_REMOVED_FROM_IGNORE_LIST, victim->client->pers.netname) );
	CPx(clientNum, va("chat %i \"%s\"", GAMESTRING_1_STOPPED_IGNORING_YOU, ent->client->pers.netname));
}
...
void Cmd_SwapPlacesWithBot_f( gentity_t *ent, int botNum )
{
	...
	if (botent->health <= 0 && (botent->client->ps.pm_flags & PMF_LIMBO)) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_BOT_CANNOT_SWAP_LIMBO) );
		return;
	}

	if (client->sess.sessionTeam != botent->client->sess.sessionTeam) {
		trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_BOT_CANNOT_SWAP_TEAM) );
		return;
	}
...
void ClientCommand( int clientNum )
{
	...
	// core: received a request for csMethod-1-data..
	...
	// core: received a bindsearch result..
	if ( !Q_stricmp(cmd, "bindsearch_r") ) {
		char	argv[MAX_TOKEN_CHARS];
		char	*result = va("%s","");
		char	*txt = NULL;
		int		i;
	...
	if( Q_stricmp (cmd, "say_team") == 0 ) {
		if( ent->client->sess.sessionTeam == TEAM_SPECTATOR || ent->client->sess.sessionTeam == TEAM_FREE ) {
			trap_SendServerCommand( ent-g_entities, va("print %i",GAMESTRING_NO_CHAT_AS_SPEC) );
			return;
		}
	...
	if (level.intermissiontime) {
		CPx( clientNum, va("print %i \"%s\"", GAMESTRING_CMD_NOT_ALLOWED_INTERMISSION, cmd) );
		return;
	}
...
*/
		...
		trap_SendServerCommand( clientNum, va("print %i \"%s\"", GAMESTRING_UNKNOWN_CMD, cmd) );
	}
}
