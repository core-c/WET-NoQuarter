...
void G_ExplodeMissile( gentity_t *ent )
{
	...
	if( ent->splashDamage ) {
		...
		if( ( ent->s.weapon == WP_DYNAMITE && ( ent->etpro_misc_1 & 1 ) ) || ent->s.weapon == WP_SATCHEL ) {
			...
			G_ResetTempTraceIgnorePlayersAndBodies();
		}
		else {
	...
	if (etype == ET_MISSILE){ //|| etype == ET_BOMB) {
				...
				if ( ent->etpro_misc_1 & 1 ) { // do some scoring
					...
					for( i = 0; i < num; ++i ) {
						...
						if ( ((hit->spawnflags & AXIS_OBJECTIVE) && (ent->s.teamNum == TEAM_ALLIES)) || ((hit->spawnflags & ALLIED_OBJECTIVE) && (ent->s.teamNum == TEAM_AXIS)) ) {
							...
							// core: award stats
							if (ent->parent && ent->parent->client) ent->parent->client->sess.missionair++;
		...
		switch (ent->s.weapon) {
			...
			case WP_SATCHEL:
			case WP_BAZOOKA:
			// IRATA: mortar should do the shaky shaky too
			case WP_MORTAR:
			case WP_MORTAR2:
			case WP_MORTAR_SET:
			case WP_MORTAR2_SET:
    ...
    case WP_MORTAR_SET:
      ...
      break;
		...
		case WP_DYNAMITE:
			...
			trap_SendServerCommand( self-g_entities, va("cp %i",GAMESTRING_DYNAMITE_NOT_ARMED) );
