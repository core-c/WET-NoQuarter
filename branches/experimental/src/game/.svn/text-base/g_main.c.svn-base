...
void G_RegisterCvars( void )
{
	...
	if((g_gametype.integer == GT_WOLF_CAMPAIGN  ||
			(g_gametype.integer == GT_WOLF_MAPVOTE && !(g_XPSave.integer & XPSF_NR_EVER))) &&
		...
		if ( cv->vmCvar ) {
			...
			if(cv->modificationCount != cv->vmCvar->modificationCount) {
				...
				if ( cv->trackChange && !(cv->cvarFlags & CVAR_LATCH) ) {
					trap_SendServerCommand( -1, va("print %i \"%s\" \"%s\"", GAMESTRING_SERVER_CVAR_CHANGED, cv->cvarName, cv->vmCvar->string ) );
				}
				...
				// Moved this check out of the main world think loop
					  ...
					  )
					{
						...
						AP( va("print %i",GAMESTRING_INVALID_GAMETYPE) );
...
static void ServerCheck ( void ) {
	...
	if ( (!strstr( versionStr, "2.60b") && !strstr( versionStr, "2.60d")) ) {
		G_Printf( "^1Error: No Quarter doesn't support ET version %s. Upgrade to 2.60b for Win/Linux or 2.60d for Mac.\n", versionStr);
		shutdown = qtrue;
		msgWhine = qtrue;
	}
	...
	if (pakNames[0]) {
		...
		if (!strstr(pakNames, "nq_v1.3.0")) {
			G_Printf("^1Warning: No nq_v1.3.0.pk3 found on server! Missing essential files!\n" );
			msgWhine = qtrue;
		}
		if (!strstr(pakNames, "nq_b_v1.3.0")) {
			G_Printf("^1Warning: No nq_b_v1.3.0.pk3 found on server! Missing essential files!\n" );
			msgWhine = qtrue;
		}
		...
		if (strstr(pakNames, "noquarter_v1.2.5")) {
			G_Printf("^1Warning: Old assets noquarter_v1.2.5.pk3 found on server! Remove it.\n" );
			msgWhine = qtrue;
		}
...
void SendScoreboardMessageToAllClients( void ) {
	...
	for (i = 0; i < level.numConnectedClients; ++i) {
		if ( level.clients[level.sortedClients[i]].pers.connected == CON_CONNECTED ) {
			level.clients[level.sortedClients[i]].wantsscore = (g_entities[level.sortedClients[i]].r.svFlags & SVF_BOT)? qfalse : qtrue;
		}
	}
}
...
void BeginIntermission( void ) {
	...
	if ( g_gametype.integer == GT_WOLF_MAPVOTE ) {
		...
		for ( i = 0; i < level.mapVoteNumMaps; i++, bspptr += len + 1 ) {
			len = strlen(bspptr);
			Q_strncpyz( level.mapvoteinfo[i].bspName, Q_StrReplace(bspptr,".bsp",""), sizeof(level.mapvoteinfo[0].bspName) );
		...
		for ( i = 0; i < len; i++ ) {
			...
			// played too recently
			if ( level.mapvoteinfo[i].lastPlayed <= g_minMapAge.integer && level.mapvoteinfo[i].lastPlayed != -1 ) {
				level.sortedMaps[i] = -1;
				level.mapvoteinfo[i].zOrder = 0;
				level.mapVoteNumMaps--;
				level.mapvoteinfo[i].lastPlayed++;
			}
			else if ( strstr(g_excludedMaps.string, va(":%s:",level.mapvoteinfo[i].bspName)) ) {
				level.sortedMaps[i] = -1;
				level.mapvoteinfo[i].zOrder = 0;
				level.mapVoteNumMaps--;
...
//MAPVOTE TODO: CHECK - still rotating immediately on map vote and empty servers ??
...
void CheckExitRules( void ) {
	...
	if ( g_timelimit.value && !level.warmupTime ) {
		...
		// OSP
			...
			if( g_gametype.integer == GT_WOLF_LMS ) {
				...
				if( axisSurvivors == alliedSurvivors ) {
					...
					if( level.firstbloodTeam == TEAM_AXIS ) {
						...
						trap_SendServerCommand( -1, va("print %i",GAMESTRING_AXIS_FIRST_BLOOD) );
					}
						...
						trap_SendServerCommand( -1, va("print %i",GAMESTRING_ALLIES_FIRST_BLOOD) );
					}
					else {
					...
					trap_SendServerCommand( -1, va("print %i",GAMESTRING_AXIS_SURVIVORS) );
					return;
				}
				else {
					trap_GetConfigstring( CS_MULTI_MAPWINNER, cs, sizeof(cs) );
					...
					trap_SendServerCommand( -1, va("print %i",GAMESTRING_ALLIES_SURVIVORS) );
					return;
				}
			}
			else {
			...
			trap_SendServerCommand( -1, va("print %i",GAMESTRING_TIMELIMIT_HIT) );
...

	{
		...
		if( level.voteInfo.voteYes > pcnt*total/100 ) {
			...
			if(level.voteInfo.voteYes > total + 1) {
				...
				AP( va("cpm %i",GAMESTRING_VOTE_PASSED) );
...
qboolean G_missileCamera(gentity_t *ent)
{
	...
	if ( !(jp_insanity.integer & (JP_INSANITY_MISSILECAM | JP_INSANITY_MORTARCAM)) ) {
		return qfalse;
	}

	{
		...
		if ( IS_PANZER_WEAPON(ent->activator->s.weapon) ) {// rocket
