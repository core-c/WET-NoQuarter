		...
		case DP_PAUSEINFO:
		{
			if(level.match_pause > PAUSE_UNPAUSING) {
				...
				if(cSeconds > 1000) {
					...
					AP( va("print %i",GAMESTRING_MATCH_RESUMING) );
		...
		if(dwDumpType == EOM_WEAPONSTATS) {
			...
			if(g_gametype.integer == GT_WOLF_STOPWATCH) {
				if(g_currentRound.integer == 1) {	// We've already missed the switch
					CP( va("print %i \"%d\" \"%02d\"", GAMESTRING_SW_CLOCK_SET_TO,
					...
					if(val < g_timelimit.value) {
						CP( va("print %i \"%d\" \"%02d\" \"%d\" \"%02d\"", GAMESTRING_SW_OBJ_REACHED_AT,
												(int)val,
						...
						CP( va("print %i \"%d\" \"%02d\"", GAMESTRING_SW_OBJ_NOT_REACHED_IN_TIME,
												g_timelimit.integer,
												(int)(60.0 * (float)(g_timelimit.value - g_timelimit.integer))));
					}
				}
...
void G_statsPrint(gentity_t *ent, int nType)
{
	...
	if(trap_Argc() < 2) {
		...
		// Specs default to players they are chasing
			...
			CP( va("cpm %i",GAMESTRING_SEE_STATS) );
			return;
		}
	}
	else {
