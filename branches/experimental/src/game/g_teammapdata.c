...
void G_SendSpectatorMapEntityInfo( gentity_t* e ) {
	...
	if ( !memcmp(buffer, lastBuffer, strlen(buffer)) ) {
		return;
	}
	memcpy( lastBuffer, buffer, strlen(buffer) );
...
void G_UpdateTeamMapData( void ) {
...

	{
		...
		for ( i = 0; i < level.numConnectedClients; i++ ) {
			...
			f1 = f2 = qfalse; // Jaybird - clean up warnings
			...
			if( ent->client->sess.playerType == PC_FIELDOPS ) {
				if( ent->client->sess.skillBits[SK_SIGNALS] & (1<<4) && ent->health > 0 ) {
					vec3_t pos[3];
			...
			if ( ent->client->sess.sessionTeam != TEAM_SPECTATOR && !(ent->client->ps.pm_flags & PMF_LIMBO) && ent->health > 0 )
											...
											case TEAM_AXIS:
												if( !ent2->s.modelindex2 ) {
													...
													if(ent->client->landmineSpotted->count2 >= 250) {
														...
														trap_SendServerCommand( ent-g_entities, va("cp %i",GAMESTRING_LANDMINE_REVEALED) );
