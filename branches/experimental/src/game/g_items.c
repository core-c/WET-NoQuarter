			  ...
			  int outOfReserve)	// is the amount to be added out of reserve
{
	...
	if (weapon < WP_LUGER || weapon >= WP_FOOTKICK)	// core: exclude WP_FOOTKICK..
		return qfalse;
...
weapon_t G_GetPrimaryWeaponForClient( gclient_t *client )
{
	...
	// Note: When pickup-all-weapons is enabled, soldier (lvl 4+) can carry 2 SMGs (thompson + mp40).
	// In this case, one is the primary weapon, and the other one is secondary.
	// The own team weapon will always be secondary..
	else if ( client->sess.playerType == PC_SOLDIER && client->sess.skillBits[SK_SOLDIER] & (1<<4)  ) {
		if ( client->sess.sessionTeam == TEAM_ALLIES ) {
			...
			if ( COM_BitCheck(client->ps.weapons, WP_THOMPSON) ) primary = WP_THOMPSON;
		}
	}

	// soldiers that did not reach lvl. 4 skill yet, perform the following checks..
	// They can have an SMG as primary weapon.
...
weapon_t G_GetSecondaryWeaponForClient( gclient_t *client, weapon_t primary )
{
	...
	// core: a soldier lvl. 4+ can have an SMG as secondary weapon,
	// but only if it is one of our own team, otherwise it will always be a primary SMG.
	if ( secondary == WP_NONE && client->sess.playerType == PC_SOLDIER && client->sess.skillBits[SK_SOLDIER] & (1<<4)  ) {
		// own team weapon as secondary?..
		if ( client->sess.sessionTeam == TEAM_ALLIES ) {
			if ( COM_BitCheck(client->ps.weapons, WP_THOMPSON) ) secondary = WP_THOMPSON;
		}
		else if ( client->sess.sessionTeam == TEAM_AXIS ) {
			if ( COM_BitCheck(client->ps.weapons, WP_MP40) ) secondary = WP_MP40;
		}
	}
...
// basically changed *LauchItem, maybe cleaner if we use tjis ...
gentity_t *LaunchDogTag( gitem_t *item, vec3_t origin, vec3_t velocity, int ownerNum )
{
	...
	dropped->nextthink = level.time + 15000;
...
void G_DropDogTag( gentity_t *ent )
{
	...
	gclient_t	*client = ent->client;
	gentity_t	*ent2;
	gitem_t		*item = &bg_itemlist[83];	// no need to search for the item, using BG_FindItemForWeapon(ITEM_DOGTAGS)..
	trace_t		tr;
...
int Pickup_Weapon( gentity_t *ent, gentity_t *other )
{
	...
	if( alreadyHave ) {
		...
		if( G_CanPickupWeapon( ent->item->giTag, other )) {
			...
			qboolean soldier8			= (other->client->sess.playerType == PC_SOLDIER && other->client->sess.skillBits[SK_SOLDIER] & (1<<8))? qtrue : qfalse;
			...
			qboolean hasPrimary			= (primaryWeapon != WP_NONE)? qtrue : qfalse;
			qboolean hasSecondary		= (secondary != WP_NONE)? qtrue : qfalse;
			qboolean hasPrimarySMG		= (IS_SUBMACHINEGUN(primaryWeapon))? qtrue : qfalse;
			qboolean hasSecondarySMG	= (IS_SUBMACHINEGUN(secondary))? qtrue : qfalse;
			qboolean pickupSMG			= (IS_SUBMACHINEGUN(ent->item->giTag))? qtrue : qfalse;
			qboolean pickupPrimarySMG	= (pickupSMG && ((other->client->sess.sessionTeam == TEAM_ALLIES && ent->item->giTag == WP_MP40) || (other->client->sess.sessionTeam == TEAM_AXIS && ent->item->giTag == WP_THOMPSON)) )? qtrue : qfalse;
			qboolean pickupSecondarySMG	= (pickupSMG && ((other->client->sess.sessionTeam == TEAM_ALLIES && ent->item->giTag == WP_THOMPSON) || (other->client->sess.sessionTeam == TEAM_AXIS && ent->item->giTag == WP_MP40)) )? qtrue : qfalse;
			qboolean pickupPistol		= (IS_SECONDARY(ent->item->giTag))? qtrue : qfalse;
			...
			if ( soldier4 ) {
				...
				// picking up SMG..
					...
					else if ( pickupPrimarySMG ) {
						if ( soldier8 || (g_unlockWeapons.integer & UNLOCK_WEAPONS_SOLDIER) ) {
							if ( hasPrimary ) G_DropWeapon( other, primaryWeapon );
						}
						else {
							if ( hasPrimary ) G_DropWeapon( other, primaryWeapon );
							if ( hasSecondarySMG ) G_DropWeapon( other, secondary );
						}
					}
				}
			...
			if( ent->item->giTag == WP_MORTAR || ent->item->giTag == WP_MORTAR2 ) {
				...
				CP( va("chat %i \"%d\"", GAMESTRING_1_PICKED_UP_X_AMMO, quantity) );
			}
		}
		else {
			return 0;
...
void G_BounceItem( gentity_t *ent, trace_t *trace )
{
	...
	if ( trace->plane.normal[2] >= 0.7 || VectorLength( ent->s.pos.trDelta ) < 16 || !ent->physicsSlide ) {
		...
		else if ( ent->item && ( ent->item->giType == IT_WEAPON || ent->item->giType == IT_AMMO ) && ent->item->giTag > WP_NONE && ent->item->giTag < WP_FOOTKICK) {
