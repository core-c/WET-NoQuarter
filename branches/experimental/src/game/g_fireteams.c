				...
				if(i == 0) {
					if(ft->joinOrder[1] == -1) {
						...
						if ( G_OnlyBotsInFireteam(ft,entityNum,&firstHuman) ) {
							...
							if ( firstHuman != -1 ) {
								trap_SendServerCommand( ft->joinOrder[firstHuman], va("cpm %i",GAMESTRING_FTC) );
							}
						}
					}
				}
...
#endif
			trap_SendServerCommand( ft->joinOrder[0], va("cpm %i",GAMESTRING_FT_DISBANDED) );
		}
...
void G_WarnFireTeamPlayer( int entityNum, int otherEntityNum ) {
	...
	trap_SendServerCommand( otherEntityNum, va("cpm %i",GAMESTRING_FTC_WARNING) );
#ifdef OMNIBOTS
